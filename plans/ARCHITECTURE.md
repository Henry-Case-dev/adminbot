...
- `info_text.md` (корень репо): 4455 байт (2753 симв., канон Epic 59), UTF-8, LF, БЕЗ хвостового `\n` (последний байт `.`); формат-прецедент 52.4 (на момент прецедента — 1408 байт).
...
```python
# КАНОН дефолтной справки (Epic 44 R44-1 → Epic 55 T-431 → Epic 56 T-437 → Epic 57 T-442 → Epic 58 T-447 → Epic 59 (правка пользователя, D224), Section 53.3) — байт-в-байт тест
...
**Свойства канона (итог Epic 58, v2.42.0):** 1 пара `<h1>` (интро «Гайд по фичам…»), 6 пар `<h2>` (секции 1–6), 27 пар `<b>` и 27 пар `<i>` (цитаты-триггеры, вложенность b>i — 2 уровня < 16), 0 пар `<u>`, 0 `<blockquote>`, 2 пары `<a href>` (кликабельные ссылки); все теги парные; legacy-лимит 100 сущностей на rich НЕ применяется (лимиты rich: 500 блоков / 16 уровней вложенности); канон 2753 символа / 4455 байт (UTF-8) — < 8000 (кнопка «Show More» не появится) и < 32768 (лимит rich-текста) — chunking НЕ срабатывает; `&`/`<`/`>` вне тегов отсутствуют; кавычки `"` — verbatim (4 символа ТЕКСТА: «не "видит"» в разделе 4 + «"@"» в разделе 6; ещё 4 — внутри атрибутов `<a href="...">`); `...` — текст после `</a>`; суть — ровно backlog R44-1 (проверка тестом 53.7 #24; 2241 симв. strip-теги); последний байт — `.`. Раздел 6 — пользовательская версия «6. Прямое обращение к Богу Машине» (D224; Epic 59 — правка пользователя: интро без «с выходом в сеть internet», ведущие пробелы «Способ 1/2/3» убраны). `/summary` и `/edit_info` в канон НЕ входят.

**Формат `info_text.md` (T-344-B):** содержимое == html-блоку выше БАЙТ-В-БАЙТ; UTF-8; LF; **БЕЗ хвостового `\n`** (прецедент 52.4: на момент прецедента 1408 байт; сейчас — 4455 байт; последний байт `.`; `InfoService._write_default` пишет `DEFAULT_INFO_TEXT` как есть — байт-эталон теста #16 сходится на linux-проде).
...
**Дополнение Epic 58 (T-447, v2.42.0, 2026-08-24, решения D229/D230/D231 — ревизия D227, вопросы 1–6 закрыты):** ресёрч (context7 — Invalid API key; duckduckgo — anomaly; фактура — exa + интроспекция установленного aiogram 3.29.1 в `.venv` — авторитетный источник для нашей версии).

**(1) Точный синтаксис (интроспекция `.venv/Lib/site-packages/aiogram`):** `bot.send_rich_message(chat_id: ChatIdUnion, rich_message: InputRichMessage, business_connection_id=None, message_thread_id=None, direct_messages_topic_id=None, disable_notification=None, protect_content=None, allow_paid_broadcast=None, message_effect_id=None, suggested_post_parameters=None, reply_parameters: ReplyParameters | None = None, reply_markup=None, request_timeout=None) -> Message` (`aiogram/client/bot.py:6332-6383`, `aiogram/methods/send_rich_message.py`). **`InputRichMessage` в aiogram 3.29.1 содержит ТОЛЬКО поля `html`, `markdown`, `is_rtl`, `skip_entity_detection`** (`aiogram/types/input_rich_message.py`) — полей `blocks`/`media` (Bot API 10.2, July 14, 2026) НЕТ: `aiogram/__meta__.py` — `__api_version__ = "10.1"`. **Вердикт: отправка через `InputRichMessage(html=текст)`** (rich-HTML); blocks недоступны до апгрейда aiogram (не нужен, пин `<4.0.0` сохранён). Источники: https://core.telegram.org/bots/api#sendrichmessage, https://core.telegram.org/bots/api#inputrichmessage.

**(2) Теги rich-HTML:** `<h1>`…`<h6>` (блок `RichBlockSectionHeading`, `size` 1–6 — `aiogram/types/rich_block_section_heading.py`; в html-форме просто `<h1>текст</h1>`), `<b>`, `<i>`, `<u>`, `<s>`, `<a href>`, `<p>`, `<br>`, списки и т.д. — глубокая вложенность inline-форматирования (до 16 уровней; «Deeply nested inline formatting» — https://core.telegram.org/bots/features). Наши `<b><i>…</i></b>` и `<a href>` валидны; `&`/`<`/`>` вне тегов — как в HTML parse mode (в каноне вне тегов отсутствуют).

**(3) Лимиты rich:** 32 768 UTF-8 символов текста, 500 блоков, 16 уровней вложенности, 50 медиа, 20 колонок таблицы (https://core.telegram.org/bots/api#rich-message-formatting-options; подтверждено https://blog.cosine.ren/en/post/telegram-rich-messages-integration/ и https://github.com/vdistortion/tg-rich-messages; «Show More» после ~8 000 — https://telegram.org/blog/watch-apps-and-more). Канон Epic 58 (на момент Epic 58) — **2782 символа / 4497 байт** → **chunking НЕ нужен** (send_chunked_reply из /info уходит; утилита остаётся для других команд — не трогать). Legacy-лимит 100 сущностей на rich НЕ распространяется.

**(4) Ошибки rich:** rich-специфичного исключения в aiogram НЕТ (`aiogram/exceptions.py`: AiogramError → TelegramAPIError → TelegramBadRequest/Forbidden/NotFound/RetryAfter/…). HTTP 400 → `TelegramBadRequest(method, message=description)` (`aiogram/client/session/base.py:120-121`), текст ошибки — в `exc.message` (прецедент `_is_reply_target_gone`). Невалидный rich-HTML → **тот же `TelegramBadRequest`** — это и есть триггер фолбека D231.

**(5) reply_to:** у `sendRichMessage` НЕТ `reply_to_message_id` — только `reply_parameters: ReplyParameters` (`ReplyParameters(message_id=...)`). Поведение сохраняем: `reply_to = None if deleted else message.message_id` (53.4).

**(6) /edit_info DM-превью:** превью админу переводится на rich: `bot.send_rich_message(ADMIN_USER_ID, InputRichMessage(html=new_text))`; `TelegramBadRequest` → пул `INFO_BAD_MARKUP_PHRASES` (файл/кэш не трогаем, D163); `save_text` без изменений. Валидация rich — тем же классом ошибки, что и боевой путь /info.

**ВЕРДИКТ (D231 уточнён):** sendRichMessage + rich-HTML; при `TelegramBadRequest` → фолбек `sendMessage` + `parse_mode="HTML"` с **детерминированной on-the-fly трансформацией rich→legacy-HTML** (эмуляция Epic 57 как фолбек-вид, БЕЗ второго байт-в-байт канона — канон-цепочка остаётся 5 мест): (1) `<h1>`→`<b><u>`, `</h1>`→`</u></b>`; (2) `<h2>`→`<b>`, `</h2>`→`</b>`; (3) `<b><i>`→`<b><i><u>`, `</i></b>`→`</u></i></b>` (порядок существенен: шаг 3 после шагов 1–2; roundtrip-проверка: фолбек-текст == канону Epic 57, 2964 симв. < 4096). Фолбек НЕ спасает от «This message is not supported» на старых клиентах — деградация принята product-ом (D229), остаётся в риск-реестре; D231 покрывает ТОЛЬКО API-ошибки rich.

**Счётчики канона Epic 58:** h1=1, h2=6, b=27, i=27, u=0, a=2, blockquote=0; маркер «Гайд по фичам» выживает (текст интро verbatim); «Канон R44-1» (backlog, fenced-блок) — суть без тегов НЕ меняется (правки fenced-блока не требуются; преамбула-пояснение над блоком может быть дополнена пометкой Epic 58 — @Builder в T-448).

**Дополнение Epic 59 (правка пользователя, 2026-08-24, прецедент D224):** канон синхронизирован с правкой пользователя в `info_text.md` (рабочая копия — источник истины, правки пользователя НЕ откатываются): интро-H1 без «с выходом в сеть internet»; ведущие пробелы у строк «Способ 1/2/3» раздела 6 убраны. Теги без изменений (h1=1, h2=6, b=27, i=27, u=0, a=2); канон 2753 символа / 4455 байт (UTF-8); strip-теги 2241 симв.; последний байт — `.`. «Канон R44-1» (backlog) — суть ИЗМЕНИЛАСЬ → fenced-блок обновлён (strip-теги нового канона == содержимому fenced-блока), преамбула дополнена пометкой Epic 59.
