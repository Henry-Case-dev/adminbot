...
- **ConfigCache + hot.get**: чтение параметров `hot.get(key, default)` с кастом по каталогу (кэш → PG → дефолт); `param_catalog` — 274 записи (prompts 10, models 29, keys 12, limits 120, flags 42, reactions 38, content 1, infra 22; группы/описания 84.24, 61 группа после эпика 04.09.2026).
