...
**Свойства канона (итог Epic 56, v2.40.0):** 7 пар `<b>` (интро + секции 1–6), 27 пар `<blockquote>` (бывшие `<code>`: триггеры/примеры; 4 из них — слова-триггеры раздела 6), 2 пары `<a href>` (кликабельные ссылки); все теги парные; `&`/`<`/`>` вне тегов отсутствуют; кавычки `"` — verbatim (4 символа ТЕКСТА: «не "видит"» в разделе 4 + «"@"» в разделе 6; ещё 4 — внутри атрибутов `<a href="...">`); `...` — текст после `</a>`; суть — ровно backlog R44-1 (проверка тестом 53.7 #24); последний байт — `.`. Раздел 6 — пользовательская версия «6. Прямое обращение к Богу Машине» (D224: источник истины — локальная рабочая копия пользователя (раздел 6 «Прямое обращение к Богу Машине»); серверный info_text.md на момент сверки T-438-A == HEAD c7a6da5; после коммита git-канон == источнику истины; строки способов 1–3 начинаются с пробела — verbatim). `/summary` и `/edit_info` в канон НЕ входят (справка — про нативные фичи, как просит пользователь).
