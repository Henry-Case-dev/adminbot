...
<b>6. Что нового (свежие апдейты, о которых ты не спрашивал)</b>
Ниже — только последние обновления: старые фичи живут в разделах 1–5.

<b>v2.37.0 — Алан, Славик и немного дисциплины</b>
...
- direct_chat: бот откликается на <code>бот</code>, <code>ботик</code>, <code>ботяра</code> и <code>ботохуета</code>. Как вызвать: просто напиши одно из этих слов в чат — бот ответит реплаем на твое сообщение. Робот, работа и ботва не в счет: они его не разбудят.

<b>v2.38.0 — Алан версия 2 и предохранитель</b>
...
**Свойства канона:** До Epic 55: 6 пар `<b>` (интро + 5 секций), 23 пары `<code>` (триггеры + примеры), 2 пары `<a href>` (кликабельные ссылки); все теги парные; `&`/`<`/`>` вне тегов отсутствуют; кавычки `"` — verbatim (1 вхождение); `...` — текст после `</a>`; суть — ровно backlog R44-1 (проверка тестом 53.7 #24). `/summary` и `/edit_info` в канон НЕ входят (справка — про нативные фичи, как просит пользователь).
