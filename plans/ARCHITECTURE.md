...
**Свойства канона (итог Epic 58, v2.42.0):** 1 пара `<h1>` (интро «Гайд по фичам…»), 6 пар `<h2>` (секции 1–6), 27 пар `<b>` и 27 пар `<i>` (цитаты-триггеры, вложенность b>i — 2 уровня < 16), 0 пар `<u>`, 0 `<blockquote>`, 2 пары `<a href>` (кликабельные ссылки); все теги парные; legacy-лимит 100 сущностей на rich НЕ применяется (лимиты rich: 500 блоков / 16 уровней вложенности); канон 2782 символа / 4497 байт (UTF-8) — < 8000 (кнопка «Show More» не появится) и < 32768 (лимит rich-текста) — chunking НЕ срабатывает; `&`/`<`/`>` вне тегов отсутствуют; кавычки `"` — verbatim (4 символа ТЕКСТА: «не "видит"» в разделе 4 + «"@"» в разделе 6; ещё 4 — внутри атрибутов `<a href="...">`); `...` — текст после `</a>`; суть — ровно backlog R44-1 (проверка тестом 53.7 #24; 2270 симв. strip-теги); последний байт — `.`. Раздел 6 — пользовательская версия «6. Прямое обращение к Богу Машине» (D224, строки способов 1–3 начинаются с пробела — verbatim). `/summary` и `/edit_info` в канон НЕ входят.
