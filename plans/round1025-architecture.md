...
> **▶️ Указатель Эпика 2 (обновлено @Architect, closing-синк S4, 23.09.2026):** **S3 `summary-l1-clusterizer` — ✅ COMPLETED + MERGED (§74, T-3278) + ARCHIVED (T-3279)** — контракт §95 (fail-closed `L1Result`, ID-пространства DB↔TG, служебные `response_mode`/`cover_prompt`), ровно 2 вызова (L1 заменяет Stage-1, L2 — Stage-2), канон +1 (каталог **468/426/443**; F8 переиздан, delta **57**), env-only слот `SUMMARY_L1_*`, логи `L1_*`; pytest **8685/0** (+116), JS 43/43; **S3 DONE + deploy VERIFIED 2.58.22** (`b31c2a6`/`3ccb1bb`; T-3280 ✅); **врезка L1 — DEFERRED (S5/S6)**. **S4 `summary-fact-package` §96 — ✅ COMPLETED + MERGED (§75, T-3303) + APPROVED (@Reviewer T-3300) + SCANNED (@Scanner T-3301, C0/H0)** — `FactPackage` v1 (description = детерминированная агрегация фактов L1, chronology = ASC из §92, `service{response_mode,cover_prompt}`, `budget` = reuse `limits.summary_max_context_*`), 0 LLM-вызовов (2-вызовность сохранена), fail-closed `ok/truncated/empty/invalid/error/not_built` (§106), ID TG/DB, логи `FACT_PACKAGE_*`; pytest **8737/0** (+52), JS 43/43; Δ каталога=0, Δ DDL=0; **врезка DEFERRED (S5/S6)**; deploy=ДА (**2.58.23**, T-3305 @DevOps), откат `pre-round1026-s4` → `59f5921`; техдолг L-R1026S4-1/-2/-3, I-R1026S4-1/-2 → S5. **Гейты без изменений:** S6/S10 (публикация/hybrid) — 🔒 закрыт (ADR-1025-24 D4); live-гейты (Эпик 1, S1/S2/S3/S4) — ⏳ **PENDING OWNER VERIFICATION**. Детали — `plans/ARCHITECTURE.md` **§75** (S3 — **§74**).
