...
> **Версия:** v2.27.0 DEPLOYED (Epic 29 — UX-полировка: удаление команды, ack-вариации, промпт v4). **Epic 29: T-221…T-226 ALL DONE (Шаг 8). Коммит `7160a33`, 1002 тестов (995 + 7 новых), ревью PASS. Прод v2.27.0, PID 937634 (был 936542), dim=3072, 0 traceback.** **Epics 1–29 ALL DEPLOYED ✅. Цикл воркфлоу (0–8) полностью закрыт.**
> **Дата:** 2026-08-17
...
> **Статус:** Epics 1–29 ALL DEPLOYED ✅ (v2.27.0, `7160a33`, 1002 тестов, PID 937634, dim=3072). **Epic 29 (UX-полировка, v2.27.0) — Шаг 8: DEPLOYED ✅. T-221…T-226 ALL DONE.** Вся цепочка воркфлоу (0–8) завершена. Ручные действия пользователя: живой тест `/summary` в чате (порядок «command deleted → ack sent» проверится на первом реальном вызове — не блокер), Н1 (BotFather `/setprivacy` → Disable), при WARNING удаления — выдать боту админ-права `delete_messages`.
> **Текущий коммит:** `7160a33` (feat(summary): Epic 29 — UX-полировка: удаление команды, ack-вариации, промпт v4 (v2.27.0)) — в origin/master (github.com/Henry-Case-dev/adminbot.git). Прод версия **v2.27.0** (PID 937634). **Epic 29 DEPLOYED (T-221…T-226 Done, 1002 тестов, ревью PASS). Epics 1–29 ALL DEPLOYED.**
> **Сервер:** 198.46.175.136:/var/www/admin_bot, systemctl active (running), PID 937634 (был 936542), 0 traceback после рестарта; git pull ff `ac80ce8..7160a33` (прод был на ac80ce8 — docs-коммит ccfad99 на проде не было, не критично); .env НЕ тронут (SUMMARY_ALIASES 36 пар остались). «Dimension mismatch» НЕТ (штатный WARNING автолечения «EMBEDDING_DIM=768 != actual API dim=3072 — using actual» остаётся), «SUMMARY_ALIASES invalid JSON» НЕТ; sqlite-vec dim=3072. Известные не-блокеры (pre-existing): бот не отвечает на SIGTERM (~95с рестарт), journalctl требует sudo для nik.
...
- **Коммит:** `7160a33` — `feat(summary): Epic 29 — UX-полировка: удаление команды, ack-вариации, промпт v4 (v2.27.0)` — запушен в origin/master. **9 файлов** (handlers/summary.py +39, services/summary_prompts.py +7, tests +82 и др.), +541/−125. **Канон-правка пункта 6 пользователя — ДОСЛОВНО**; Builder восстановил только номера пунктов 4/5/6/7 (зазор D84) — текст канона НЕ тронут.
...
- **Коммит на русском** ✅ `feat(summary): Epic 29 — UX-полировка: удаление команды, ack-вариации, промпт v4 (v2.27.0)` = `7160a33` — **канон пользователя вошёл ДОСЛОВНО** (`M services/summary_prompts.py`); Builder восстановил только номера пунктов 4/5/6/7 (зазор D84) — текст канона НЕ тронут. 9 файлов в коммите, запушен в origin/master.
...
| **Epic 29** | UX-полировка — **DEPLOYED (Шаг 8 @Memory)**, коммит `7160a33`, v2.27.0, 1002 теста (995+7), T-221–T-226 ALL DONE, прод PID 937634, канон пользователя дословно (D83), зазор-нумерация сохранён (D84), 0 traceback |
