...
> **Версия:** v2.26.0 DEPLOYED (Epic 28 — Качество памяти: векторы, репосты, алиасы, очистка). **Epic 28: T-211…T-220 ALL DONE (Шаг 8). Коммит `ac80ce8`, 995 тестов (939 baseline + 56 новых), ревью PASS. Прод v2.26.0, PID 936542, dim=3072 (автолечение 768→3072 сработало штатно на первом старте).**
> **Дата:** 2026-08-16
...
> **Статус:** Epics 1–28 ALL DEPLOYED ✅ (v2.26.0, `ac80ce8`, 995 тестов, PID 936542, dim=3072). **Epic 28 (Качество памяти, v2.26.0) — Шаг 8: DEPLOYED ✅.** Вся цепочка воркфлоу (0–8) завершена. Ручные действия пользователя: живой тест `/summary` в чате, Н1 (BotFather `/setprivacy` → Disable), при WARNING удаления — выдать боту админ-права `delete_messages`.
> **Текущий коммит:** `ac80ce8` (feat(summary): Epic 28 — качество памяти: репосты, алиасы, векторное автолечение и cleanup (v2.26.0)) — в origin/master (github.com/Henry-Case-dev/adminbot.git). Прод версия **v2.26.0**. **Epic 28 DEPLOYED (T-211…T-220 Done, 995 тестов, PID 936542).**
> **Сервер:** 198.46.175.136:/var/www/admin_bot, systemctl active (running), PID 936542 (был 934174), 0 traceback после рестарта; git pull ff `1d7bed4..ac80ce8`; .env НЕ тронут (SUMMARY_ALIASES 36 пар остались, WARNING «invalid JSON» отсутствует). **Автолечение VecAutoHeal отработало штатно на первом старте:** «EMBEDDING_DIM=768 != actual API dim=3072 — using actual» → DROP smart_archive (stored=768 vs actual=3072, facts in smart_archive_facts are kept) → «sqlite-vec loaded (dim=3072)». «Dimension mismatch» отсутствует. Новые сообщения векторизируются в 3072 корректно. Известный не-блокер (pre-existing): бот не отвечает на SIGTERM (~95с рестарт) — рекомендуется отдельный тикет graceful shutdown.
...
- **Хелпер-диапазон `lines[1517:1540]`** — реализован и сверен (v3 байт-в-байт == backlog 1518–1540); в проде v3 активен. При правках backlog выше блока пересчитывать (формула `1538 + N_вставленных`).
- **SYSTEM_PROMPT v3 — байт-в-байт** == backlog 1518–1540 (инвариант D74); перед коммитом `git diff --check` и полный pytest (995 passed) — чисто.
...
| Параметр | Значение |
|----------|----------|
| **Версия в проде** | **v2.26.0** (Epic 28 — Качество памяти: векторы, репосты, алиасы, очистка — DEPLOYED; dim=3072 после автолечения) |
| **Текущий коммит** | `ac80ce8` (feat(summary): Epic 28 — качество памяти: репосты, алиасы, векторное автолечение и cleanup (v2.26.0)); прод HEAD после pull ff `1d7bed4..ac80ce8` |
| **Дата** | 2026-08-16 |
| **Сервер** | 198.46.175.136 |
| **Путь** | /var/www/admin_bot |
| **Статус** | systemctl status adminbot → active (running), PID 936542 (был 934174), 0 traceback после рестарта; автолечение отработало штатно («EMBEDDING_DIM=768 != actual API dim=3072 — using actual» → DROP smart_archive, facts kept → «sqlite-vec loaded (dim=3072)»); «Dimension mismatch» отсутствует; SUMMARY_ALIASES 36 пар OK, WARNING «invalid JSON» отсутствует |
| **Git remote** | origin (github.com/Henry-Case-dev/adminbot.git) — pushed успешно (`7c7c241..1d7bed4`), локальный HEAD = origin/master |
| **Тесты** | 995 PASS (939 baseline + 56 Epic 28; 0 failed, 0 skipped) |
| **Эпики** | 1-28 ALL DEPLOYED ✅ |
