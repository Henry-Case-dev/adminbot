...
Индекс долговременной памяти. Архитектура — `plans/ARCHITECTURE.md` (§1–§48);
...
> **🔄 АКТИВНЫЙ РАУНД 10.23 (Step 3 @Memory, 19.09.2026): `Epic round1023 (Adaptive System 2, Token Analytics, Anti-Cliche Cache & Image Generation)` — SPEC_READY / PLANNED, старт разработки с F1.** Архитектура — `plans/features/round1023-architecture.md` (Part 1/3 — ядро + канон; Part 2/3 — F4–F6); 9 `spec.md` + **ADR-1023-1…9** (Accepted; `plans/features/*-round1023/`). **Baseline:** HEAD **`731a845`**; pytest **6962/0**; каталог **439/409/414/92/90/20**; SQLite **v12**; APP_VERSION **2.57.0**; прод **`a8a6437`**. Задачи **T-2097…T-2186** (PM T-2097…T-2182 + UPD-задачи F3 T-2183…T-2186). **F1 уже реализована — commit `9ff836d`.**
>
...
>
> **Решения владельца (UPD 19.09.2026, `plans/current_task.md` стр. 101–120):** **aiogram>=3.31 обязателен** (жёсткое условие F6/деплоя, не опция); **жёсткое разделение разметки по каналу** — Сценарий А plain: таблицы ЗАПРЕЩЕНЫ (ParseError), обязательны жирный + буллиты `- `; Сценарий Б rich (`sendRichMessage`): разрешено всё (Markdown/HTML/таблицы); `casual`/`serious` — без Markdown; пункты 2,4,5,6,7 подтверждены. **Constraints:** `response-mode-router-stage1-only-round1023`, `anticliche-feeds-detector-not-prompt-round1023`, `analytics-ddl-in-pg-round1023`, `channel-markup-rules-round1023`, `aiogram-min-3-31-round1023`, `target-message-marking-rule-round1023`, `shared-file-staging-round1023`. **Risks:** `risk-image-key-leak-round1023` (Critical), `risk-plain-tables-parseerror-round1023` + `risk-aiogram-version-rich-media-round1023` (High), 4×Medium + 1×Low (`risk-token-analytics-failopen-round1023`) — все mitigated. **ExternalDependency:** `pollinations-ai`, `telegram-sendRichMessage-api`. **Техдолг:** `tech-debt-round10.23` (закрывает S10.22-4b; остаётся CLI `.env`-ключ, vec best-effort, stale-lock 6ч, приёмки T-2069/T-2089). **Metric:** `metric-snapshot-round1023-spec-ready`. R17/R18: `current_task.md` untracked, креды/ключ не цитируются. **Код не начат (кроме F1); этап 10.23 — Step 3 завершён.**
>
