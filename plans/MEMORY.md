...
Индекс долговременной памяти. Архитектура — `plans/ARCHITECTURE.md` (§1–§92);
...
> **🆕 НОВАЯ САМОСТОЯТЕЛЬНАЯ ЗАДАЧА (round 10.27): `memory-context-autonomy` (MCA) — INTAKE / Step 0 @Memory выполнен 26.09.2026.** Источник — `plans/current_task.md` (ТЗ v1.8 от 26.09.2026, ~1865 строк; git-ignored; содержит секреты SSH/ANU — значения не переносить). Redacted-индекс: `plans/archive/requests/memory-context-autonomy.md`. Предыдущая задача `adminbot-master-spec-v6` ЗАКРЫТА (rev 176; прод 2.58.31). Состав — **MCA-01…MCA-21**: транзакции/конкурентность · SafeFetcher+cookies/профиль · сообщения/identity/время/версии · provenance + досье/full rebuild · истории/эпизоды · сон/глубокий сон/парадигмы · retrieval/EvidenceBundle/бюджет · характер/речь · интенты/инициатива/Decision · RandomSource/ANU + применения 14.6–14.11 (игровой stub 14.12 неактивен) · tools/ToolResult/расходы · миниапп (истории на «Статусе», таблица в «Памяти») · журналирование · схема/совместимость/kill switch · достоверная статистика (MetricResult/NumericClaim) · обучение на опыте (Lesson) · прозрачность процессов (реестр/виджеты/trace/heartbeat) · SelfModel/BehaviorFrame · vision · временной фактчек · оба гайда. Приёмки **A01–A95**; §20.2 — всё ON, кроме игрового stub (только код) и денежных лимитов OFF. **Baseline Step 0 (26.09.2026):** HEAD **`7165ff7`** (master), `APP_VERSION` **2.58.31**; каталог подтверждён импортом — **473/430/448/102/100/21**, SQLite **v12**, канон **12**; pytest **9513/0** + JS **47/47** — последние verified Эпика 3 (НЕ новый прогон). Код НЕ менялся (только `plans/**` + KG). Конфликт-карта по 12 крупным блокам MCA↔завершённые эпики/архивы (вердикты REUSE/AMEND/SUPERSEDE/NEW) — в отчёте Step 0; KG: узел `memory-context-autonomy` (+связи follows Эпики 2/3, part-of AdminBot) + 6 Risk. **▶️ Следующий шаг — инвентаризация/декомпозиция @PM (реализации кода ещё нет).**

> **✅ ЭПИК 3 «Agentic Intelligence» (round 10.26) — RELEASED + DEPLOYED + VERIFIED (25.09.2026).** Агрегатный Reviewer release gate **APPROVED** (`plans/reports/round1026_epic3_aggregate_review.md`; binding Reviewed-Commit `e8646af`, WTH `8a00476a…`, spec-manifest `d7d8a926…`). Доставка: коммиты **`e5bd73d`** (код+тесты+bump 2.58.31+README+F8-meta+27 test re-pins) + **`89a1261`** (plans/архивы D-3, 63 файла) + deploy-doc **`7165ff7`**, push без force; **прод `/var/www/admin_bot` FF до `89a1261`, `/healthz` 2.58.31, MainPID 948338**, все health-проверки зелёные, 9 kill-switch'ей ON, `image_reservation` создана (инертна), каталог 473/430/448/102/100/21, канон 12, SQLite v12, теги 14 + stash целы; `epic-deployment.md` = **VERIFIED** (`plans/archive/agentic-verification-round1026/`). Все 11 фич A0–A10 выпущены (merged §82–§92 + archived `plans/archive/*-round1026/`); `APP_VERSION` **2.58.31**. KG: `Эпик 3 (Agentic Intelligence)` → RELEASED/DEPLOYED/VERIFIED + `metric-snapshot-round1026-epic3-closure`. **Watch-items: 6 owner-gates — PENDING OWNER VERIFICATION** (внешние, не блокирующие, не закрывать: §52 сценарии 1/2/4/10/18/22 — живые пробы изображений HY-01/02/04/05/06, live URL, live toggle); doc watch-items Lows / A9 L-01..04 / F/C/L-наборы — исторические (зарегистрированы в архивах + отчёте A10). **Оригинальное ТЗ `plans/current_task.md` §1–§92: Эпик 3 — последний выпущенный раунд в текущей сессии; следующий шаг — оценка остаточного скоупа за Orchestrator/PM (следующий эпик НЕ изобретать).**
>
> **✅ A10 ЭПИКА 3 «Agentic Intelligence» (Wave 5 — ФИНАЛ; Step 10 @Memory, 25.09.2026; Merge §92; deploy A10 NOT_APPLICABLE / эпик DEFERRED_TO_EPIC): `A10 agentic-verification-round1026`** — **COMPLETED + APPROVED (собственный Reviewer gate T-3726, 1-й цикл; R1; Scanner отсутствует) + MERGED (§92, ADR-1026-23 → Accepted) + ARCHIVED (`plans/archive/agentic-verification-round1026/`, 5 файлов: spec, adr-1026-23-verification-gate, tasks, evidence, review-T-3726; SHA-256 до/после идентичны; spec `a50b7cb0…` байт-в-байт; **`threat-failure-analysis.md` отсутствует — `NOT_APPLICABLE` R1**; `deployment.md` отсутствует — деплой не применим) + ACCEPTED INTO PENDING EPIC 3 RELEASE / verified-for-epic-release**. Отчёт приёмки остаётся вне архива — **`plans/reports/round1026_a10_acceptance.md`**. **Deploy: A10 собственный вердикт = `epic deployment NOT_APPLICABLE`** (read-only; footprint только `plans/**` + `plans/reports/**`; @DevOps не вызывался; `APP_VERSION` **2.58.30** без bump); **эпик-деплой = `DEFERRED_TO_EPIC`** (EPIC_ONLY) — bump **2.58.30 → 2.58.31** на границе агрегатного релиза Эпика 3. Binding: Reviewed-Commit `e8646af2…`, WTH `2264399e…` (**stale после merge/архивации — агрегатный gate пересчитывает; superset untracked `96c291c1…`**), Spec-Hash `a50b7cb0…`; **ADR-1026-23 hash re-pin `37d8cef7…`**. Итоги: **§52 23/23 PASS** (multi-source; **6 PENDING OWNER VERIFICATION** — сценарии 1/2/4/18 прод-пробы изображений HY-01/02/04/05/06, 10 live URL, 22 live toggle — никогда не закрывались), **§53 15/15 YES**, **§54 12/12**; harness **pytest 9513/0**, JS **47/47**, F8 **473**, каталог **473/430/448/102/100/21** (Δ=0), канон **12**, SQLite **v12** (Δ DDL=0); watch-register **11 групп** (CLOSE/PASS-THROUGH/PENDING); defect-rule **D5** (4 residual зарегистрированы); **Lows 1–4** (Low-1 закрыт при архивации: авторитетный tasks-hash `a8401a97…` / on-disk `34c91179…`). **A10 — ПОСЛЕДНЯЯ фича Эпика 3.**
>
> **🗂️ (историческая Step-10-часть баннера, 25.09.2026; на момент записи агрегат был PENDING — итог: эпик RELEASED, см. баннер `✅ ЭПИК 3 ... RELEASED` выше): ЭПИК 3 «Agentic Intelligence» — ВСЕ ФИЧИ A0–A10 DONE/APPROVED; следующая стадия = EPIC AGGREGATE RELEASE GATE.** Агрегатный Reviewer release review по **ADR-1026-23 D7/D10**: вход = отчёт A10 + watch-register + чек-листы §52/§53/§54; gate покрывает полный эпик-diff A2–A9+A10, cross-feature, интеграцию/регрессию, миграцию A5 `image_reservation`, config/observability, совместимость, security R17/R18, готовность отката, per-spec консистентность; binding = release-commit + deterministic WTH + aggregate spec-manifest hash incl **ADR re-pins 1026-20 `deb6257d…` / 1026-21 / 1026-22 `3085143B…` / 1026-23 `37d8cef7…`** → **delivery + @DevOps bump 2.58.30 → 2.58.31** + **D-3 staging** (9 untracked архив-папок). Агрегат ещё **PENDING**: bump не выполнен, @DevOps не вызывался (EPIC_ONLY; прод 2.58.30).
>
> **✅ A9 ЭПИКА 3 «Agentic Intelligence» (Wave 5 — продолжение; Step 10 @Memory, 25.09.2026; Merge §91; deploy DEFERRED_TO_EPIC): `A9 agentic-events-graph-round1026`** — **COMPLETED + APPROVED (единый Reviewer gate T-3702, 1-й цикл) + MERGED (§91, ADR-1026-22 → Accepted) + ARCHIVED (`plans/archive/agentic-events-graph-round1026/`, 5 файлов: spec, adr-1026-22, tasks, evidence, review-T-3702; SHA-256 до/после идентичны, spec `3f703584…` байт-в-байт; **`threat-failure-analysis.md` отсутствует — `NOT_APPLICABLE` R2/D1**; `deployment.md` отсутствует — деплой отложен, корректно) + ACCEPTED INTO PENDING EPIC 3 RELEASE / verified-for-epic-release** (deploy `DEFERRED_TO_EPIC`, EPIC_ONLY — **НЕ deployed**, DevOps не вызывался; APP_VERSION **2.58.30** без bump, 2.58.31 — на границе агрегатного релиза Эпика 3; откат: hot `AGENTIC_EVENTS_ENABLED=false`, cold `git revert e8646af` + агрегатный анкер, DDL-откат не нужен). Approved **T-3702 cycle 1** C0/H0, Risk **R2**; binding Reviewed-Commit `e8646af2bcaa79b55cadda756d0e8cc7789fe24f`, WTH `84d6df00b7cc5ce13cc343d01e1ea4cd6474587560c9e92739f23a42bc210f23`, Spec-Hash `3f7035843cdd9fa0f76e7ecab8aa76cfbd9b6a24b68d7cd430b1b559b963cd1e` (отчёт — `plans/archive/agentic-events-graph-round1026/review-T-3702.md`); **⚠️ ADR-1026-22 при Accepted сменил hash на `3085143B31307A8EFDF7B08AEC64A5459DFB8C2D1E439D556314392FBCEA8FF5` → re-pin агрегатного манифеста Эпика 3 (вместе с ADR-1026-20 `deb6257d…` / ADR-1026-21).** Реализация: **REUSE ExecutionGraph** — 9 реальных этапов §51 на существующих `kind` (+`tool`), честные `null`-токены (не `$0`), silent = узел Decision без фиктивного Вербализатора; API **без правок** (аддитивность через существующий `GET /analytics/execution/latest`; `analytics.py` diff пуст) + display-only срез Mini App §50 в существующей «Аналитике» (`execution_graph.js`); точки эмиссии: direct_chat_service (решение/silent/react), tool_loop (план/старт/успех/провал), image-пути, anticliche `_event`; **L-1 закрыт** (`TelegramForbiddenError → REACTION_FORBIDDEN` в `react_moai`); kill-switch env-only **`AGENTIC_EVENTS_ENABLED`** (default ON; OFF → 0 событий, baseline-паритет); канон **12**; Δ DDL=0; Δ каталога=0. Числа (зеркало metrics **10.26-A9**, 25.09.2026): pytest **9513/0** (база 9459 + 54; A9 core `tests/test_agentic_events_round1026.py` = 52 + L-1 2), JS **47/47**, F8 `CHECK OK` (473), каталог **473/430/448/102/100/21** (Δ=0), SQLite **v12** (Δ DDL=0), `git diff --check` 0, APP_VERSION **2.58.30**. Watch-items **4 Low non-blocking → агрегатный gate**: L-A9-3702-01 (evidence-stress-claim), -02 (реально достижимы 7 из 9 этапов через канон-имена), -03 (`started_at=null` верхнего уровня), -04 (смена префикса логов anticliche) + **test-filename discrepancy** (spec `test_agentic_events_graph_round1026.py` vs actual `test_agentic_events_round1026.py`) + флак betterstack (среда). KG: `ADR-1026-22-agentic-events-graph` (Accepted) + `metric-snapshot-round1026-a9-final`; `risk-a8-l1-forbidden-classification` → **closed** (L-1 FIXED). **▶️ Следующая — A10 `agentic-verification` (§52–§54; **P0**; **23 сценария §52**; **ПОСЛЕДНЯЯ фича Эпика 3**) → затем агрегатный Reviewer release gate Эпика 3 (bump 2.58.30 → 2.58.31, DevOps).** Порядок факта: `A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → A7 ✅ → A8 ✅ → A9 ✅ → ▶️ A10`.
>
> **✅ A8 ЭПИКА 3 «Agentic Intelligence» (Wave 5; Step 10 @Memory, 25.09.2026; Merge §90; deploy DEFERRED_TO_EPIC): `A8 telegram-reactions-round1026`** — **COMPLETED + MERGED (§90, ADR-1026-21 → Accepted) + ARCHIVED (`plans/archive/telegram-reactions-round1026/`, 5 файлов: spec, adr-1026-21-reaction-mechanics, tasks, evidence, review-T-3683; SHA-256 до/после переноса идентичны; spec `6e5cfc80…` байт-в-байт; **`threat-failure-analysis.md` отсутствует — `NOT_APPLICABLE` при R2/D12**; `deployment.md` отсутствует — деплой отложен, это корректно) + ACCEPTED INTO PENDING EPIC 3 RELEASE** (deploy `DEFERRED_TO_EPIC`, EPIC_ONLY — **НЕ deployed**, DevOps не вызывался; APP_VERSION **2.58.30** без bump, 2.58.31 — на границе агрегатного релиза Эпика 3; откат: hot `REACTION_MECHANICS_ENABLED=OFF` (legacy 🗿) и/или A7-тумблеры REACTIONS/IMAGE_REACTIONS OFF, cold `git revert e8646af` + агрегатный анкер, DDL-откат не нужен). Approved **единым Reviewer gate T-3683** (обе линзы; Scanner отсутствует) — **Approved на 1-м цикле C0/H0, Risk R2**; binding Reviewed-Commit `e8646af2bcaa79b55cadda756d0e8cc7789fe24f`, WTH `ec57a9518e8f7ea8003701af42d133eeb5103f78a31e6c38748bc4239bd1bc97`, Spec-Hash `6e5cfc80df5c6eaf7e6333a748b51805c6973b9cd5257f23fb81edc56ce0e4ef` (отчёт — `plans/archive/telegram-reactions-round1026/review-T-3683.md`); **⚠️ при merge ADR-1026-21 сменил hash (Status Proposed→Accepted) → re-pin агрегатного манифеста Эпика 3 (вместе с ADR-1026-20 `deb6257d…`).** Реализация: механика §41 в **единственном примитиве** `react_moai` (`services/smartmodule_utils.py`) — **один официальный `setMessageReaction`-call-site**; `react_moai` расширен аддитивно (`*, reaction=None, reason_code=None`); реактивная доступность + детерминированная альтернатива/тихий отказ (**≤2 попытки**, `REACTION_FALLBACK_ORDER=(😂,👍,🔥,🗿)`, без рандома); **закрытый enum 7 исходов** (`REACTION_OK/UNAVAILABLE/FORBIDDEN/MESSAGE_GONE/SERVICE/RATE_LIMITED/UNKNOWN`); **no-text-on-error** (ни один исход не вызывает `send_message`); цель = trigger `target_message_id` (media-group — серверное «первое неудалённое»); 429/RetryAfter — без повтора; контекстная карта **`reason_code→эмодзи`** ({🗿 дефолт/fallback, 😂, 👍, 🔥}) в `direct_chat_service.py` (`_reaction_for_reason`; изменено **только значение эмодзи** — правила §44 остались A7 по делегированию ADR-1026-20 D5); закрыт разрыв **T-3678** (проброс `reaction`/`reason_code` `:1313–1315`); kill-switch **`REACTION_MECHANICS_ENABLED`** env-only default ON (OFF → точный legacy: одиночная 🗿); прочие `react_moai`-сайты не мигрированы (D11); канон **12**; **Δ DDL=0 / Δ каталога=0** (473/430/448/102/100/21; F8 не переиздаётся). Числа (зеркало строки metrics **10.26-A8**, 25.09.2026): pytest **9459/0** (база 9392 + 67; A8 suite 63), JS **47/47**, F8 CHECK OK, `git diff --check` 0, APP_VERSION 2.58.30. Watch-item агрегатного gate: **L-1** (Low, non-blocking) — `TelegramForbiddenError` не подкласс `TelegramBadRequest` → реальный 403 классифицируется `unknown` вместо `forbidden` (поведение идентично, fail-silently; **follow-up явно назначен A9**: `except TelegramForbiddenError → REACTION_FORBIDDEN` + тест) + structural-proof note (нет A7 byte-снапшота — A7 незакоммичен). KG: `ADR-1026-21-reaction-mechanics` (Accepted) + `metric-snapshot-round1026-a8-final`; `risk-a8-l1-forbidden-classification`. **▶️ Следующая — A9 `agentic-events-graph`** (§49/§51; P1; зависит A1–A8), затем **A10 `agentic-verification`** (§52–§54; P0) — до агрегатного Reviewer release gate Эпика 3. Порядок: `A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → A7 ✅ → A8 ✅ → A9 → A10`.
>
> **✅ A7 ЭПИКА 3 «Agentic Intelligence» (Wave 5; Step 10 @Memory, 25.09.2026; Merge §89; deploy DEFERRED_TO_EPIC): `A7 decision-making-round1026`** — **COMPLETED + MERGED (§89, ADR-1026-20 → Accepted) + ARCHIVED (`plans/archive/decision-making-round1026/`, 6 файлов: spec, adr-1026-20-decision-making-policy-and-action-contract, tasks, evidence, threat-failure-analysis, review-T-3663; SHA-256 до/после переноса идентичны 6/6; spec `950c5f80…` байт-в-байт; `deployment.md` отсутствует — деплой отложен, это корректно) + ACCEPTED INTO PENDING EPIC 3 RELEASE** (deploy `DEFERRED_TO_EPIC`, EPIC_ONLY — **НЕ deployed**, DevOps не вызывался; APP_VERSION **2.58.30** без bump, 2.58.31 — на границе агрегатного релиза Эпика 3; откат: hot `DIRECT_DECISION_MAKING_ENABLED=OFF` (A1-baseline) и/или 3 тумблера OFF, cold `git revert e8646af` + откат F8-артефактов + агрегатный анкер, DDL-откат не нужен). Approved **единым Reviewer gate T-3663** (обе линзы; Scanner отсутствует) — **Approved на 2-м цикле C0/H0, Risk R3 сохранён**; binding Reviewed-Commit `e8646af2bcaa79b55cadda756d0e8cc7789fe24f`, WTH `4b3df1402375e2287e67ec25a2d0c3e0f61662b5f6afeec43d9509f2ed8cccf4`, Spec-Hash `950c5f8039791ae4353449ae53bc31e1aa244f585ed3320522de72c364d52ea0` (отчёт — `plans/archive/decision-making-round1026/review-T-3663.md`); **⚠️ на merge ADR-1026-20 сменил hash на `deb6257de0b8d14417c0a0009c30c332b77342f47673b937a63005173c3f332c` → re-pin агрегатного манифеста Эпика 3 (flag для aggregate gate).** Сюжет 2 циклов: c1 **Needs Fixes** (F-1/F-2 High — end-to-end tool-путь и §47-поля не читались политикой; F-3/F-5 Medium — «???»→react и отсутствие `disabled`; Low F-4/F-6/F-7/F-8) → rework **1** @Builder (e2e `handle()`-тесты, §47-поля в политике, вопросительная пунктуация, `REASON_DISABLED`) → c2 **Approved**. Реализация: аддитивное расширение A1-объекта **`CoordinatorDecision`** (`services/direct_chat_service.py`; второй контракт/координатор не вводится) — `action` {reply/react/silent/tool} **ОТДЕЛЬНО** от `style` {casual/serious/deep_research} (не `style=silent`), +`target_message_id`/`reaction`(🗿)/`reason_code`(enum 15)/`needs_tools`; **2-фазный программный расчёт без 3-го LLM-вызова** (Phase P `_decision_pre_action` до LLM, Phase T `build_coordinator_decision`; AMEND `physical-two-call-pipeline` ADR-1022-5; 2 tool / 0 silent); детерминированная политика §42–§45 (вопросы/задачи перевешивают; без «короткое → silent»; без случайности; fail-safe error→reply; сбой инструмента → reply); порядок §46 (`silent`/`react` → Вербализатор не запускается; реакция — reuse **`react_moai`** 🗿, механика §41 = A8; safety-net сохранён); контекст §47 из существующих `bot_replies`/`reply_to`/маркеров (без нового хранилища); ровно **3 настройки §48** в `mod_direct` (группа `flags_decision_making`, Δ каталога **+3/+1**); env-only **`DIRECT_DECISION_MAKING_ENABLED`** default ON (OFF → A1-baseline байт-в-байт, `REASON_DISABLED` на объекте). Числа (зеркало строки metrics **10.26-A7**, 25.09.2026): pytest **9392/0** (база 9303 + 89; A7 suite 89; focused direct+каталог+F8 222/0), JS **47/47**, каталог **473/430/448/102/100/21**, SQLite **v12** (Δ DDL=0), канон **12**, F8 `--check` OK (473), `git diff --check` **0**, APP_VERSION **2.58.30**. Watch-items агрегатного gate: **F-9/D-2** (A1-тест `test_forbidden_paths_out_of_diff` ослаблен под санкционированные A3/A5-diff — сужение сломало бы зелёные A3/A5; независимо пиннится `TestBounds::test_counts_sanctioned` + F8 `--check`), R-1 (устаревший code-указатель threat-анализа, Low), R-2 (`expects_tool_result` грубость, Low), R-3 (алиас `bot_replied_recently`, Low), `REASON_DISABLED` на объекте (не в OFF-строке лога); **D-3** — archive-папки untracked → stage/включить в архив-манифест при эпик-релизе. KG: `A7 decision-making` (APPROVED/verified-for-epic-release), `ADR-1026-20-decision-making` (Accepted, §89; AMENDs ADR-1022-5/ADR-1023-3, note ADR-1026-14), `SpecDecision-a7-decision-making` → RESOLVED, `metric-snapshot-round1026-a7-final`, `react_moai`. **▶️ Следующая — A8 `telegram-reactions`** (§41/§44-механика; backend/Telegram API; P0; зависит A7) → A9 `agentic-events-graph` (§49/§51; P1) → A10 `agentic-verification` (§52–§54; P0) → агрегатный release gate Эпика 3 (bump 2.58.31, DevOps). **Вход A8:** контракт A7 уже поставляет `action=react` (+`target_message_id`/`reaction`/`reason_code`); reuse `react_moai` (`services/smartmodule_utils.py:93–107`, best-effort/fail-silently); границы «A7 = правила / A8 = механика §41»; kill-switches; архив/§89; watch-items.
>
> **✅ A4 ЭПИКА 3 «Agentic Intelligence» (Wave 4; Step 10 @Memory, 25.09.2026; Merge §88; deploy DEFERRED_TO_EPIC): `A4 image-context-memory-round1026`** — **COMPLETED + MERGED (§88, ADR-1026-19 → Accepted) + ARCHIVED (`plans/archive/image-context-memory-round1026/`, 6 файлов: spec, adr-1026-19, tasks, evidence, threat-failure-analysis, review-T-3640; SHA-256 до/после идентичны; spec `63064ccb…` байт-в-байт) + ACCEPTED INTO PENDING EPIC 3 RELEASE** (deploy `DEFERRED_TO_EPIC`, EPIC_ONLY — **НЕ deployed**, DevOps не вызывался; APP_VERSION **2.58.30** без bump, 2.58.31 — на границе агрегатного релиза Эпика 3; откат: hot `IMAGE_CONTEXT_MEMORY_ENABLED=OFF` (режим A3) и/или `UNIFIED_IMAGE_REQUEST_ENABLED=OFF` (legacy), cold `git revert e8646af`, DDL-откат не нужен). Approved **единым Reviewer gate T-3640** (обе линзы; Scanner отсутствует) — **Approved C0/H0, Risk R3 на 4-м цикле**; binding Reviewed-Commit `e8646af2bcaa79b55cadda756d0e8cc7789fe24f`, WTH `5f9ac819d7ce3f15d5d44d0628738d09c67c6088961ca38eedd4288e4af60dfa`, Spec-Hash `63064ccbd5891bde4082d365e7c52def0e0640529a06f3eaa0fe632e33f840c7` (отчёт — `plans/archive/image-context-memory-round1026/review-T-3640.md`). Сюжет 4 циклов: c1 F1 High alias-коллизия + F2 Med exact-likeness → c2 F1 class-level (6 repro) → **D13-амендмент @Architect** (класс-закрывающий корроборационный шлюз G0–G3) → c3 C3-H1 High (prefix→exact-set) + C3-M1 Med («похож») → c4 точечный фикс G1 → **Approved** (rework 3 @Builder + 1 root-cause @Architect). Реализация: детерминированный helper **`services/image_context_memory.py`** (`build_image_memory_context`; не второй pipeline — одна реализация, две точки вызова direct `maybe_handle_keyword` + tool `_generate_image`) + хук `image_generation.build_final_prompt`; заполняет A3-заглушки `context_required/context_sources/resolved_subjects`; **D13/G0–G3**: alias-матч = только кандидат, `resolved` — G1 person-intent (closed-set, вкл. «похож») + G2 persona-рекорд `db.get_persona_names` (имя+счётчик, факты не читаются) + G3 image-object cross-check; ≥2 → ambiguous (0 фактов), 1 → resolved; класс-инвариант «нарисуй <существительное>» не персонализируется; **5-частная сборка** с §37-обёрткой «данные ≠ инструкции» и байт-в-байт A3-parity при `context_required=False`; визуальный срез (psychological-denylist; ≤8/160, ≤3/200, ≤1200); exact-likeness reply-заметка «арт ≠ портрет»; env-only kill-switch `IMAGE_CONTEXT_MEMORY_ENABLED` (default ON). Числа (зеркало строки metrics **10.26-A4**, 25.09.2026): pytest **9303/0** (база 9213 + 90; A4 suite 90), §104 AST-гейт **7/0**, JS **47/47**, каталог **470/427/445/101/99/21** (Δ каталога = 0), SQLite **v12** (Δ DDL=0), `git diff --check` **0**, канон **12**. Watch-item агрегатного gate Эпика 3 — **C4-N1** (Medium, spec-санкционированный residual: alias = слово person-маркера G1 → персонализация на голом generic-запросе; полное закрытие требует reply/sender-плумбинга — вне scope A4) + C4-N2 (Low) + N2 (Low); полный список — C4-N1/C4-N2/N2 + F1–F7 (A5) + L-A6-01…-06 (A6) + owner-gate A3 (PENDING OWNER VERIFICATION). KG: `ADR-1026-19-image-context-memory` (Accepted, §88) + `metric-snapshot-round1026-a4-final`; отдельный `SpecDecision-a4` не заводился (решение оформлено ADR-1026-19 D1–D13). **▶️ Следующая — A7 `decision-making`** (§38–§40/§42–§48; depends A1/A2) → затем A8 → A9 → A10 → **агрегатный релиз Эпика 3 (bump 2.58.31, DevOps) — PENDING**.
>
> **✅ A6 ЭПИКА 3 «Agentic Intelligence» (Wave 4; Step 10 @Memory, 25.09.2026; Merge §87; deploy DEFERRED_TO_EPIC): `A6 memory-lookup-api-round1026`** — **COMPLETED + MERGED (§87, ADR-1026-18 → Accepted) + ARCHIVED (`plans/archive/memory-lookup-api-round1026/`, 5 файлов: spec, adr-1026-18, tasks, evidence, review-T-3622) + ACCEPTED INTO PENDING EPIC 3 RELEASE** (deploy `DEFERRED_TO_EPIC`, EPIC_ONLY — **НЕ deployed**, DevOps не вызывался; APP_VERSION **2.58.30** без bump, 2.58.31 — на границе агрегатного релиза Эпика 3; откат: hot `MEMORY_LOOKUP_ENABLED=OFF`, cold `git revert e8646af`, DDL-откат не нужен). Approved **единым Reviewer gate T-3622** (обе линзы) — **Approved C0/H0, Risk R2**; binding Reviewed-Commit `e8646af2bcaa79b55cadda756d0e8cc7789fe24f`, WTH `05d882ea5ff64ed982a46aabbd46ae22c0dbaf4c69b96c567395de9fb6d2f543`, Spec-Hash `4d81918024a43b883bc5491ed38d4b2e28af07defbcc0845d21f83d1eb79a36a` (отчёт — `plans/archive/memory-lookup-api-round1026/review-T-3622.md`). Реализация: новый LLM-видимый инструмент **`get_user_context(person, user_id, purpose, max_items)`** (`chat_id` из `ToolContext`; purpose enum 6: identity/appearance/speech_style/biography/relationships/general) поверх существующих досье+RAG (read-only, новой БД/DDL нет); **канон 11→12** атомарно (12-й в хвост, первые 11 байт-в-байт); purpose-роутинг + **lazy RAG только {general, speech_style}**; envelope §33 из 5 полей (`facts`/`sources`/`confidence`/`time_context`/`no_data`; unknown ≠ confirmed); капы (ceiling 20; slice ≤5/≤240 симв.; результат ≤4000); фактчек-набор = 3; env-only киль-свитч `MEMORY_LOOKUP_ENABLED` (default ON; OFF → активный набор 10). Числа (зеркало строки metrics **10.26-A6**, 25.09.2026): pytest **9213/0** (база 9170 + 43), JS **47/47**, каталог **470/427/445/101/99/21** (Δ каталога = 0), SQLite **v12** (Δ DDL=0), `git diff --check` **0**, канон **12**. Findings **L-A6-01…-06 — non-blocking** → watch-items агрегатного Reviewer-gate Эпика 3 (§87.11). KG: `ADR-1026-18-memory-lookup-and-canon-extension` (Accepted) + `metric-snapshot-round1026-a6-final`. **▶️ Следующая — A4 `image-context-memory`** (§22–§25 + §32–§34 + §37; зависит A3+A6; **потребитель контракта A6** `get_user_context`, не переписывать lookup) → затем A7 → A8 → A9 → A10; порядок `A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 → A7 → A8 → A9 → A10`. Агрегатный релиз Эпика 3 (bump 2.58.31 + DevOps) — **PENDING**; вход агрегатного gate — findings F1–F7 (A5, §86) + L-A6-01…-06 (A6, §87.11) + owner-гейт прод-проб изображений (PENDING OWNER VERIFICATION). Историческая Step 0-часть A5 — в баннерах ниже.
>
> **✅ A5 ЭПИКА 3 «Agentic Intelligence» (Wave 3; Step 10 @Memory, 25.09.2026; Merge §86; deploy DEFERRED_TO_EPIC): `A5 image-daily-limit-round1026`** — **COMPLETED + MERGED (§86, ADR-1026-17 → Accepted) + ARCHIVED (`plans/archive/image-daily-limit-round1026/`, 6 файлов: spec, adr, tasks, evidence, threat-failure-analysis, review-T-3604) + ACCEPTED INTO PENDING EPIC 3 RELEASE** (deploy `DEFERRED_TO_EPIC`, EPIC_ONLY — **НЕ deployed**, DevOps не вызывался; APP_VERSION **2.58.30** без bump, 2.58.31 — на границе агрегатного релиза Эпика 3; откат: hot `IMAGE_DAILY_LIMIT_ENABLED=OFF`, cold `git revert e8646af`, DDL-откат не нужен). Approved **единым Reviewer gate T-3604** (обе линзы) — **Approved C0/H0, Risk R3**; binding Reviewed-Commit `e8646af2bcaa79b55cadda756d0e8cc7789fe24f`, WTH `b57f1f0ede7563e2b8c31f63093b830fedf99cf29729609c50018c5cea23077c`, Spec-Hash `428c44b477b3d2c40bb59b258ad54f72d64d967b22c98aaacc6f976fdac80601` (отчёт — `plans/archive/image-daily-limit-round1026/review-T-3604.md`). Реализация: PG-ledger `image_reservation` + условный UPSERT `reserve → generate → commit/release`, idem-ключ `{chat_id}:{message_id}:{source}`, каталоговый `limits.image_daily_limit` + группа `limits_images` (секция «Лимиты» в `mod_images`), per-chat TZ `limits.chat_timezone`, учёт из журнала, env-киль-свитч `IMAGE_DAILY_LIMIT_ENABLED`. Числа (зеркало строки metrics **10.26-A5**, 25.09.2026): pytest **9170/0** (база 9156 + 14), JS **47/47**, каталог **470/427/445/101/99/21** (Δ +1/+1 sanctioned), SQLite **v12** (Δ DDL=0), PG DDL **≠ 0 sanctioned** (`image_reservation` + 2 индекса), `git diff --check` **0**, канон **11**. Findings **F1–F7 — non-blocking** → watch-items агрегатного Reviewer-gate Эпика 3 (§86). KG: `ADR-1026-17-image-daily-limit` (Accepted) + `metric-snapshot-round1026-a5-final`; `SpecDecision-a5-limit-scope` → **RESOLVED**. **▶️ Следующая — A6 `memory-lookup-api`** (§32–§35, structured `get_user_context`, P0, depends A0) → затем **A4 `image-context-memory`** (ПОСЛЕ A6; старые handoff'ы ошибочно называли A5 «image-context-memory» — это A4, ошибка исправлена) → A7 → A8 → A9 → A10; порядок `A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 → A4 → A7 → A8 → A9 → A10`. Агрегатный релиз Эпика 3 (bump 2.58.31 + DevOps) — **PENDING**. Исторический Step 0 этой фичи — в баннере ниже.
>
> **🗂️ A5 ЭПИКА 3 «Agentic Intelligence» (историческая Step 0-часть баннера, 25.09.2026 — итог см. в баннере выше; карта кода/гэпов дублируется в KG `A5 image-daily-limit` и в архиве фичи): `A5 image-daily-limit`** (§26–§31/§50; ТЗ `plans/current_task.md:4905–5064`). **ЧАСТЬ 1 — идентичность подтверждена (снято расхождение нумерации):** канон — таблица бэклога `plans/backlog.md:306–307`: **A4 = `image-context-memory`** (§22–§25/§32–§34/§37), **A5 = `image-daily-limit`** (§26–§31/§50); волновой порядок `:314` — `[A3✅ → A5] ∥ A6 → A4 → A7 → A8 → A9 → A10` → на момент Step 0 следующей была именно A5 image-daily-limit (сейчас ✅ — см. баннер выше); сечении ТЗ подтверждено (§22 «изображения на основе памяти» = контент A4 на `:4765–4903`). ⚠️ STATE_CONFLICT снят вердиктом: записи «следующая — A5 image-context-memory» (`round1025-architecture.md:201`, `ARCHITECTURE.md:3388`, `backlog.md:296/314` (актуализация 25.09), `metrics.md` 10.26-A3) — нумерационная ошибка архивации A3 (номер A5 приписан контенту A4); НЕ переписывать (R17/backlog — за @PM); указатель `ARCHITECTURE.md:3388` (§85.9 «Следующие») **ИСПРАВЛЕН @PM (T-3607, 25.09.2026)**: A5 ✅ → следующая A6 → затем A4. Машинный блок согласен: «select_next = image-daily-limit (A5 per backlog table)». **Карта кода:** счётчик image_calls — **PG** `worker_budget` (`pg_db.py:191–199`; в SQLite отсутствует; runtime-PgDatabase DI, `worker_budget.py:78–89`); лимит env-only `WORKER_DAILY_IMAGE_CALLS_GLOBAL/PER_CHAT` 200/60 (`settings.py:941–944`; ветка `_metric_limit` `worker_budget.py:275–305` — hot/per-chat overrides для image_calls НЕ резолвятся — gap §26/§30); списание `consume` `:211–242` — check-after-increment, без атомарного резерва и idempotency key (§29 gap); обвязка `image_generation._consume_budget` `:570–589` global→per-chat до генерации (`generate_and_send` `:840–852`), rollback при ошибке/сбое публикации отсутствует (§29 gap); учёт запросов/успехов/ошибок (§28) отсутствует; день — глобальный `WORKER_BUDGET_TZ` (Asia/Yekaterinburg, `worker_budget.py:58`, `settings.py:914`), per-chat TZ прецедент `limits.chat_timezone` (`direct_chat_service.py:1266–1273`, `tool_router.py:1480–1525`) — реюз для §31; UI N/M частично в `GET /api/workers/budget` (`get_day_summary` `:335–373`), маршрут «Модули → Генерация изображений» отсутствует (§27/§85-UI — отдельная санкция). **Конфликт-чек:** Δ DDL ожидается ≠ 0 (санкция за Step 2 @Architect; параллельные хранилища — запрет §87; не дублировать worker_budget/image_calls — A0 §EV-19); §85-UI — отдельная санкция; REUSE ExecutionGraph; идемпотентность поверх A3-маркера. **Baseline:** перемерено — HEAD `e8646af` == origin/master, APP_VERSION **2.58.30**, git status 32 незакоммиченных строки (pending epic-release код A2+A3 — не трогать, не коммитить); взято verified A3 (не перезамерялось) — pytest `.venv` **9156/0**, JS **47/47**, канон **11**, каталог **469/426/444/100/98/21**, прод **2.58.30** (MainPID **652183**), SQLite v12 (local-копия user_version=0 — не авторитетна). Deploy **EPIC_ONLY → deferred** (DevOps не вызывать, per-фича деплоя нет). **KG:** `A5 image-daily-limit` (+ верbatim §26–§31) + 4 риска (`Risk-a5-double-consumption`/`Risk-a5-concurrent-overshoot`/`Risk-a5-scope-tz-semantics` High, `Risk-a5-stale-map-duplicate-metric` Medium) + `SpecDecision-a5-limit-scope` (**RESOLVED** — ADR-1026-17) + 15 связей (part-of Эпик 3, depends-on A3/A2, reuses image_generation/worker_budget, governed-by ADR-1025-24, mapped-in round1025-architecture). **Вопросы Step 2 @Architect — ЗАКРЫТЫ ADR-1026-17 (D1–D12, §86):** хранилище/DDL-санкция (`image_reservation` + 2 индекса), дефолт/scope (`limits.image_daily_limit` ≠ shared `WORKER_DAILY_IMAGE_CALLS_*`), TZ/сброс, ключ идемпотентности, rollback (release/commit), UI «Лимиты» — санкционирована (D9), границы A4/A5/A6/A7. **▶️ Шаги 1–10 этой фичи выполнены — итог в баннере выше (Step 10 @Memory).** Не тронуто: current_task.md, машинный блок, backlog.md, metrics.md, ARCHITECTURE.md, durable-аудит, архивы A0–A3 (чтение).

> **✅ A3 ЭПИКА 3 «Agentic Intelligence» (Wave 2; Step 10 @Memory, 24.09.2026; Merge §85; deploy DEFERRED): `A3 unified-image-request` — **COMPLETED + MERGED (§85, T-3582 @Architect) + ARCHIVED (T-3583 @PM ✅, 25.09.2026 — папка перенесена в `plans/archive/unified-image-request-round1026/`; SHA-256 до/после переноса идентичны 7/7; финализирован только `tasks.md`) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deploy `DEFERRED_TO_EPIC`; release policy **EPIC_ONLY** — пер-фича деплоя нет; bump 2.58.30 → 2.58.31 — в агрегатном релизе Эпика 3; **release-узел не создаётся**; НЕ «DEPLOYED»/«VERIFIED»).** Approved **единым Reviewer gate** (Scanner удалён 24.09.2026; отдельного Scanner-approval нет): итер.1 **Needs Fixes** (блокеры B-1 High — документированные вердикты HY-01…HY-06 с веткой `INCONCLUSIVE`; B-2 Medium — R3-обязательный threat/failure-анализ) → **документальный rework T-3580 (1 проход; product-код не менялся)** → целевой re-review **Approved C0/H0/R3**; binding Reviewed-Commit `e8646af`, WTH `29e8da0a…`, Spec-Hash `a98d20d1…`; non-blocking doc-only мелочи `L-R1026A3-1…-8`. Реализация: единый **ImageRequest** + `build_final_prompt`/`run_image_request` в `services/image_generation.py` (оба входа direct → существующий `generate_and_send`; второй pipeline запрещён); защита от двойной генерации (пре-гейт + авторитетный маркер `ToolContext.image_request_handled` → `skipped/already_handled`); `TOOL_GENERATE_IMAGE` — текст описания (6 пунктов §21), канон **11**; env-only `UNIFIED_IMAGE_REQUEST_ENABLED`; память/досье — поля-заглушки (A4); §104 — AST-гейт генератора. Диагностика §21: **HY-06 механика подтверждена** (пре-гейт ловит только «бот+нарисуй/сгенерируй/создай» формы), **HY-03 env-слой опровергнут** (все ON), «виноват промпт» — опровергнуто A0; **HY-01/02/04/05 — INCONCLUSIVE** (билд-окружение недоступно: LLM-эндпоинт 500 `get_channel_failed`, fallback 401; прод-логи недоступны) + **owner-гейт прод-проб изображений (HY-01/02/04/05 + per-chat PG-флаг + живой A/B описания) — PENDING OWNER VERIFICATION**. Числа: pytest `.venv` 9125 → **9156/0** (+31), JS **47/47**, `git diff --check`=0; **Δ DDL=0 / Δ каталога=0** (469/426/444/100/98/21); канон **11**; 2-вызовность; **версия не бампалась** (2.58.30). Отчёты (архив) — `plans/archive/unified-image-request-round1026/{spec.md, adr-1026-16-…md, tasks.md, evidence.md, image-diagnostics.md, threat-failure-analysis.md, review.md}`; **`deployment.md` отсутствует** — при `DEFERRED_TO_EPIC` корректно. Архив — `plans/archive/unified-image-request-round1026/` (**T-3583 @PM ✅, 25.09.2026**). **▶️ Следующая (на 24.09.2026) — A5 `image-daily-limit`** (§26–§31/§50; в этой записи ранее ошибочно значилось «A5 image-context-memory» — это **A4** (§22–§25/§32–§34/§37), она идёт **после A6**; фактический порядок `A3 ✅ → A5 ✅ → A6 → A4 → A7 → A8 → A9 → A10`). R17/R18; Шаг 10 — только `plans/**` + KG (не коммичено).
>
> **🗂️ A3 ЭПИКА 3 «Agentic Intelligence» (историческая Step 0-часть баннера, 24.09.2026 — итог см. в баннере выше): `A3 unified-image-request`** (§18 «Генерация изображений» `plans/current_task.md:4634–4657` / §19 «Прямой вызов и tool calling» `:4660–4690` / §20 «Единый Image Request» `:4693–4729` / §21 «Tool calling генерации изображений» `:4732–4762`) — оба способа (прямая фраза «Бот, нарисуй …» и вызов инструмента из LLM) → **единый механизм подготовки и генерации**; **не создавать два независимых image pipeline**; **существующий рабочий прямой вызов сохранить без регрессий**; **исследовать, почему текущие tool-вызовы не работают** (пробы/логи, а не фикс на гипотезе HY-01…HY-06); результат инструмента корректно возвращается в цикл; **не заменять реальную ошибку генератора вымышленным отказом модели**; **§104 `generate_image` не переписывать** (менять только запрос/обвязку); §22 (память) — A4, §36–§37 — A7, §85-UI — отдельная санкция. **Вход — durable-аудит A0** `plans/docs/agentic-audit-round1026.md` (HY-01…HY-06, `#root-cause`, `#labels-hypothesis`) + **A2** (`plans/archive/tool-chains-round1026/`, §84: envelope `ToolLoopResult.tool_results`/`ToolContext.result_for`, §17-лимиты cap 6 / мягкий 360 c / дедуп ≤2 / платные ≤4, канон **11**) + A1 (§83 `CoordinatorDecision`). **Карта кода (факт, HEAD `e8646af` + незакоммиченный A2):** прямой путь — `services/image_generation.py` `IMAGE_KEYWORD_RE:93` / `extract_prompt:163` / `resolve_module_enabled:170` / `maybe_handle_keyword:968` / `generate_and_send:936` + `services/direct_chat_service.py` пре-гейт `:874–883` / `_image_pre_gate_block:1574` / `image_pre_gate_fired → image_enabled=False:934–939`; tool-путь — `services/tool_schemas.py` `TOOL_GENERATE_IMAGE:323` / `TOOL_CALLING_TOOLS:375` (канон 11) / `active_tools:417` + `services/tool_router.py` `dispatch:491` / `_generate_image:1553` + `services/tool_loop.py` `chat_with_tools` (роль `tool` :438, §17-лимиты); **точка дублирования** — интерлок `:934–939` + `<image_result>` (генератор уже единый `generate_and_send`; расходится только подготовка запроса); бюджет `worker_budget.METRIC_IMAGE_CALLS:35` (env global 200 / per-chat 60); наблюдаемость ExecutionGraph `STEP_KIND:73` (`image→llm`, `tool→tool`). **Baseline:** HEAD `e8646af` == `origin/master` (в дереве — незакоммиченный A2-код = pending epic-release candidate; **пер-фича deploy нет, policy EPIC_ONLY**); `APP_VERSION` **2.58.30** (без bump; 2.58.31 — в агрегате эпика); pytest `.venv` **9125/0**, JS **47/47** (verified A2, не перезамерялось); каталог **469/426/444/100/98/21** (REGISTRY/GROUPS/`_TAB_BY_GROUP`/TAB_RULES перемерены импортом); **Δ DDL=0** (SQLite v12); прод 2.58.30 (MainPID 652183). KG: `A3 unified-image-request` + 5 Risk (2 Critical / 3 High) + `SpecDecision-a3-image-request` (OPEN). **Следующий шаг — Step 1 @PM (`tasks.md`)**; открытые вопросы Step 2 @Architect — какие пробы/логи доказывают HY-01…HY-06.
>
> **✅ A2 ЭПИКА 3 «Agentic Intelligence» (Wave 2; Step 10 @Memory, 24.09.2026; Merge §84; deploy DEFERRED_TO_EPIC): `A2 tool-chains-round1026`** — **COMPLETED + MERGED (§84, T-3547 @Architect — merge/ADR Accepted параллельно) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deploy `DEFERRED_TO_EPIC`; release policy **EPIC_ONLY** — пер-фича деплоя нет; bump 2.58.30 → 2.58.31 — в агрегатном релизе Эпика 3; **release-узел не создаётся**)**. Approved **единым Reviewer gate** (Scanner удалён 24.09.2026; отдельного Scanner-approval нет): **Approved с 1-й итерации** C0/H0, **Risk R3**, R3-усиленные доказательства достаточны (threat/failure 14 угроз + adversarial 12 + независимая baseline-проба OFF); binding Reviewed-Commit `e8646af`, WTH `f1f1d544…`, Spec-Hash `e87e530f…`; отклонения D-a/D-b/D-c — non-blocking; non-blocking `L-R1026A2-1/-2/-3`. Реализация: **структурный спутник-envelope** (`ToolLoopResult.tool_results`, `ToolContext.result_for`/`ctx.tool_results`) при **неизменном** модельно-видимом канале (`role:"tool"`/`tool_context`) и ключах `tool_trace`; структурированные ошибки (`status:"error"`+`error_code`/`error_type`; ошибка инструмента ≠ общий отказ LLM); §17-лимиты **cap 6 / мягкий 360 c / дедуп ≤2 / платные ≤4** + частичный результат (`chain_timeout|chain_call_limit|chain_cost_limit`), env-only `TOOL_CHAIN_LIMITS_ENABLED` (default ON); **последовательность санкционирована** (без `asyncio.gather`); канон **+1 `fetch_article`** (**10→11**) атомарно, env-only `ARTICLE_TOOL_ENABLED` (default ON), `factcheck_tools`=3; общий резолв ссылки `smartmodule_urls.resolve_context_url`→`ctx.resolved_url`; reuse `tool_loop`/`ToolRouter`; §104/§85-UI/промпты/`param_catalog.py`/`db/**`/`web/**` вне diff. Числа: pytest `.venv` 9082→**9125/0** (+43; новый `tests/test_tool_chains_round1026.py`=43), JS **47/47**, `git diff --check`=0; **Δ DDL=0 / Δ каталога=0** (469/426/444/100/98/21); канон инструментов **11**; **версия не бампалась** (2.58.30); 2-вызовность (`await_count==2`); CSP/zero-build. **ADR-1026-15 → Accepted** (§84). Откат: soft env-only kill-switch'и (`TOOL_CHAIN_LIMITS_ENABLED=false`, `ARTICLE_TOOL_ENABLED=false`) / hard annotated-тег `pre-round1026-a2` (obj `a3ea44d4`) → `e8646af` + `git revert`. Архив — `plans/archive/tool-chains-round1026/` (**T-3548 @PM ✅ 24.09.2026**; папка перенесена из `plans/features/`; SHA-256 после переноса 6/6 идентичны, финализирован только `tasks.md`; **`deployment.md` отсутствует** — деплой отложен). **▶️ Следующая — A3 `unified-image-request`** (§18–§21/§25; порядок `A0 ✅ → A1 ✅ → A2 ✅ → [A3 → A5] ∥ A6 → A4 → A7 → A8 → A9 → A10`).
>
> **🗂️ A2 ЭПИКА 3 «Agentic Intelligence» (Wave 2; Step 0 @Memory, 24.09.2026; product code НЕ менялся — только `plans/**` + KG; **итог — в баннере выше**): `A2 tool-chains`** (§15 «Последовательные tool calls» `plans/current_task.md:4520–4566` / §16 «Контракты инструментов» `:4569–4607` / §17 «Ограничение цепочек» `:4610+`) — инструменты используют результаты предыдущих (зависимые A→B последовательно; независимые — параллельно, ЕСЛИ архитектура поддерживает); общий механизм цепочек, НЕ per-combination и НЕ костыль под фразу (§15); структурированные результаты/ошибки (§16: пример JSON `status/url/title/markdown/source_id` — пример, НЕ обязательная схема; существующие контракты REUSE; ошибка инструмента — структурно, НЕ общий отказ LLM); ограничение цепочек (§17: макс вызовов, общий тайм-аут, лимит ретраев, дедуп одинаковых вызовов, лимит расходов, частичный результат — без тяжёлой инфраструктуры). **P0. Вход — durable-аудит A0** `plans/docs/agentic-audit-round1026.md` (анкоры `#tool-map-llm`, `#s12-3-5`, `#epic3-reuse`, `#epic3-a1-a10`, `#epic3-summary`) + **A1** (`plans/archive/tool-coordinator-round1026/`, ADR-1026-14 D3, §83). **Карта кода (факт, HEAD `e8646af`):** цепочки уже есть — `services/tool_loop.py` `chat_with_tools:89` (многораундовый; `TOOL_MAX_ROUNDS=4` :38; `_TOOL_CALLS_PER_ROUND_MAX=2` :39; усечение :158–162; роли `assistant`/`tool` :167–190; plain-fallback :125–135; деградация :191–196; `ToolLoopResult` :48–73 с `tool_trace`/`tool_context`); зависимые A→B поддержаны (внутри раунда вызовы последовательны, результат возвращается модели ролью `tool` → следующий раунд может вызвать B); **параллельности нет** (`asyncio.gather` отсутствует) → §15 «если архитектура поддерживает» = сейчас последовательность; диспетчер `services/tool_router.py:455–480` (`dispatch` **всегда plain-строка**, ошибки «ОШИБКА {name}: {TypeName}»; `_generate_image` JSON-статус :1437–1468); канон **10** `services/tool_schemas.py:337–348` (freeze ADR-1026-13 D3; расширение — только A1/A2), `active_tools:367–389`, `factcheck_tools:405–415`. **GAP §16:** структурного контракта результата нет (строки). **GAP §15/§36:** сценарий «ссылка→извлечение→фактчек» каноном 10 не закрыт — `WebContentExtractor.extract` (`web_content_extractor.py:59`) и `FactCheckService.check_claim` (`factcheck_service.py:77`) — СЕРВИСЫ, не LLM-тулы; расширение канона — санкция A2 (ADR-1026-13 D3). §17: есть лишь макс раундов/вызовов + частичный результат; нет общего тайм-аута цепочки, дедупа одинаковых вызовов, лимита расходов (per-tool тайм-ауты: search 25с, download 180с, summarize/lore 300с, history 10с — `tool_router.py:79–130`). **Конфликт-чек:** freeze 10 ✓; без 3-го LLM-вызова ✓; общий механизм/не per-combination ✓; зависимые последовательно ✓; §104 `generate_image` не трогать ✓; §85-UI/Δ каталога вне scope ✓; REUSE ExecutionGraph ✓. **Baseline (перемерено):** HEAD `e8646af` == `origin/master`, мастер, дерево чистое (в рабочем дереве — только машинный блок `plans/workflow_state.md`, за Оркестратором); `APP_VERSION` **2.58.30**; каталог **469/426/444/100/98/21**; канон **10** / `factcheck_tools` **3**. Взято как verified A1 (НЕ перезамерялось): pytest `.venv` **9082/0**, JS **47/47**, **Δ DDL=0** (SQLite v12), прод активен (MainPID **652183**). **Риски (KG):** Critical — `Risk-a2-infinite-call-loop` / `Risk-a2-live-chat-regression`; High — `Risk-a2-phrase-hack-per-combination` / `Risk-a2-unstructured-handoff`. **Открытый вопрос @Architect:** `SpecDecision-a2-chain-contract` (OPEN) + лимиты §17 / параллельность / расширение канона / границы A3/A4/A7 / Δ DDL / deploy. **▶️ Следующий шаг — Step 1 @PM (`tasks.md`).** KG: `A2 tool-chains` (canonical; alias `A2 sequential-tool-calls`) + `Эпик 3` + 4 Risk + SpecDecision.
>
> **✅ A0 ЭПИКА 3 «Agentic Intelligence» (Step 10 @Memory, 24.09.2026; read-only; deploy NOT_APPLICABLE): `A0 agentic-audit` — обязательный архитектурный аудит §12 (**15/15** пунктов) / §54 п.1 (карта **10 инструментов × 8/8 полей**) + **durable-артефакт `plans/docs/agentic-audit-round1026.md`** (38 стабильных анкоров; матрица §12 15/15 · §54 п.1 1/1 · REQ 19/19 · SC 23/23 — **orphan'ов нет**; **34 EVIDENCE / 6 HYPOTHESIS**); первопричина image-tool-calling — доказанные code-path механизмы (пре-гейт ключевика до LLM; plain-fallback при провайдер-отказе tools; рубильники модуля OFF) + опровергнутое («только промпт»; второй шаг после tool response есть) + **6 гипотез HY-01…HY-06** (вход **A3/A4**). **COMPLETED (read-only; Approved — единый Reviewer gate C0/H0; binding Reviewed-Commit `e8065e9` / WTH `805c4680…` / Spec-Hash `1B2AE779…`; non-blocking `L-R1026A0-1/-2/-3`).** Deploy — **NOT_APPLICABLE** (0 изменений рантайма/ассетов/каталога/DDL/env; `APP_VERSION` **2.58.29** без bump; Δ DDL=0 / Δ каталога=0; тесты **9026/0**, JS **47/47** без изменений; канон инструментов 10 не изменён; прод не трогался; точка отката — тег `pre-round1026-a0` (obj `afe14278`) → `e8065e9`; прецедент F8 §67 / F10 §70). **ADR-1026-13 → Accepted** (по merge/архивации T-3499). Архивация — `plans/archive/agentic-audit-round1026/` (**T-3499 @PM ✅ выполнен 24.09.2026**; durable-артефакт **остаётся на месте**). **▶️ Следующая — A1 `tool-coordinator`** (§13–§14; handoff §8.3).
>
> **🗂️ A0 ЭПИКА 3 «Agentic Intelligence» (исторический Step 0 @Memory, 24.09.2026; read-only — только `plans/**` + KG; **итог — в строке выше**): `A0 agentic-audit` — обязательный архитектурный аудит §12 (15 пунктов) / §54 п.1 (карта инструментов); **Wave 0 enabler → A1–A10**; НЕ реализация, продуктовый код НЕ менялся; санкции/human gate не требуются (исследование допускалось заранее). Источник — `plans/current_task.md` §11 (:4324) / §12 (:4373–:4437) / §13–§54 (:4440–:5888) / §52 (:5749) / §53 (:5807) / §54 п.1 (:5877). **Карта кода (фактическая, HEAD `e8065e9`):** 10 инструментов `services/tool_schemas.py` `TOOL_CALLING_TOOLS` :337–:348 (канон R9=10, ADR-1020-4); `active_tools()` :367; `factcheck_tools()` :405; dispatch `services/tool_router.py:457–468`; `services/tool_loop.py` `TOOL_MAX_ROUNDS=4` :38 / `_TOOL_CALLS_PER_ROUND_MAX=2` :39 (ADR-1020-7); image — прямой §104 (`image_generation.generate_image_verbose` :860 / `generate_and_send` :936) vs tool (`tool_router._generate_image` :1437) + пре-гейт ключевика (`direct_chat_service._image_pre_gate_block` :1345 / `maybe_handle_keyword` :968); **причина отказа tool calling — кандидаты:** пре-гейт ключевика intercepts «Бот, нарисуй…» до LLM, FR-15 plain-fallback при провайдер-reject tools (`tool_loop.py:125–135`), env/per-chat рубильники (`image_generation.resolve_module_enabled` :170; `active_tools` :382), провайдер tool calling, `generate_and_send` reason — **НЕ подтверждена, §12 запрещает сводить к промпту**; досье (`dossier_prompts.format_dossier_block` :568 / `direct_chat_service.py:2135`) + RAG (`summary_memory.get_rag_context` :2479 / `vector_search` :1855 / `_knn_graph_facts` :2692; FTS5/vec0 `database.py` :286/:298/:366/:1252) + `dig_into_lore` (`tool_router.py:573`); **`get_user_context` ОТСУТСТВУЕТ** (0 вхождений; §33 — целевой API A6); фактчек `factcheck_service.check_claim` :77 / `_check_claim_two_call` :133; URL→Markdown `web_content_extractor.extract` :59 (trafilatura→Tavily→Exa); Decision Making `system2_handoff.normalize_response_mode` :68 / `parse_direct_synthesis` :272 (`response_mode`, ADR-1023-3); реакции `smartmodule_utils.react_moai` :93 (`setMessageReaction`, aiogram 3.29.1); L1↔L2 `summary_l1_contract.py` / `summary_fact_package.py` (top-level `service{response_mode,cover_prompt}` :418); ExecutionGraph F6 §65 (`execution_graph_source.build_graph` :485, `STEP_KIND` :73 = image→llm / tool→tool) + S8 §79 (`web/api/analytics.py` `/analytics/execution/latest` :281, `web/static/execution_graph.js` `fromExecution` :341); тумблеры/лимиты `param_catalog.py` REGISTRY :1893 / GROUPS :147 / TAB_RULES :2124 / `_TAB_BY_GROUP` :2253 + `feature_gates.py` + `worker_budget.py` `METRIC_IMAGE_CALLS` :35. **Baseline (перемерено):** HEAD **`e8065e9`** == `origin/master` (рабочее дерево — только машинный блок `plans/workflow_state.md`); `APP_VERSION` **2.58.29** (`config/settings.py:1827`); SQLite **v12** (`database.py:107`; Δ DDL=0); каталог **REGISTRY 469 / GROUPS 100 / `_TAB_BY_GROUP` 98 / TAB_RULES 21 / categorized 444** (импорт `services.param_catalog`). Взято как verified S10 (НЕ перезамерялось): pytest `.venv` **9026/0**, JS **47/47**, Settings **426**. **Конфликт-чек (для A1–A10):** `action` {reply/react/silent/tool} развести со `style` (§39) **без третьего LLM-вызова** (AMEND `physical-two-call-pipeline`, ADR-1022-3/4 + ADR-1023-3; `ARCHITECTURE.md:1153`); ExecutionGraph — REUSE (вторую аналитику не создавать); `services/tool_schemas.py` — AMEND ADR-1020-4 **без потери 10**; канон промптов — ADR-1013-3; §104-контур (`generate_image`/`generate_image_verbose`) и F6/S8/§85-UI-границы не ломать. **Риски (KG, 4):** Critical — аудит выдан за реализацию (`Risk-a0-audit-as-implementation`) / ложная причина отказа tool calling (`Risk-a0-false-toolcall-cause`); High — пропуск зависимости/дубликат инструмента (`Risk-a0-missed-dependency-duplicate-tool`); Medium — устаревшая карта кода (`Risk-a0-stale-code-map`). **KG:** `A0 agentic-audit` (feature; `part-of` `Эпик 3 (Agentic Intelligence)` — узел создан, ранее отсутствовал; `enables` A1–A10; `reuses` ExecutionGraph-adapter §65/§79; `mapped-in` `round1025-architecture`; `governed-by` ADR-1025-24) + 4 Risk + `SpecDecision-a0-audit-artifact` (**OPEN**). **Открытые вопросы Step 2 @Architect (НЕ решены):** формат/место durable аудит-отчёта (вероятно `plans/docs/…`); нужен ли ADR (карта инструментов / расширение канона ADR-1020-4); границы read-only; как оформить «причину отказа tool calling изображений» (доказательство vs гипотеза). **НЕ трогать:** product code, `plans/current_task.md`, машинный блок `OPENCODE_WORKFLOW_STATE_V1`, `plans/backlog.md` (@PM), `plans/metrics.md`, `plans/ARCHITECTURE.md`; @Scanner удалён — единый Reviewer gate. **▶️ Следующий шаг — Step 1 @PM (`tasks.md` A0).** R17/R18: не коммичено.

> **✅ A1 ЭПИКА 3 «Agentic Intelligence» (Wave 1; Step 10 @Memory, 24.09.2026; Merge §83; deploy VERIFIED): `A1 tool-coordinator`** — **COMPLETED + MERGED (§83, T-3520 @Architect — merge/ADR Accepted параллельно) + DEPLOYED (2.58.30 VERIFIED, T-3522 @DevOps)**. Approved **единым Reviewer gate** (Scanner удалён 24.09.2026; отдельного Scanner-approval нет): **Approved с 1-й итерации** C0/H0 (линза 1 + линза 2); binding Reviewed-Commit `e3ea367`, WTH `4361f011…`, Spec-Hash `b37b9a50…`; отклонения D-a/D-b/D-c — non-blocking; non-blocking `L-R1026A1-1/-2/-3`. Реализация: программный слой `CoordinatorDecision` внутри `services/direct_chat_service.py` (без 3-го LLM-вызова, без wire-`action`); модельный выбор инструментов сохранён; `tool_loop.py` не менялся (reuse цепочек); изоляция Вербализатора; env-only kill-switch `DIRECT_COORDINATOR_ENABLED` (default ON, `ClassVar`, `config/settings.py:552`); `APP_VERSION` 2.58.29→**2.58.30** + README; `tests/test_tool_coordinator_round1026.py` (56). **ADR-1026-14 → Accepted** (§83 + deploy). Числа: pytest `.venv` 9026→**9082/0** (+56), JS **47/47**, `git diff --check`=0; **Δ DDL=0 / Δ каталога=0** (469/426/444/100/98/21); 2-вызовность (`await_count==2`); CSP/zero-build. Deploy VERIFIED: коммиты `b32c46a` (код+тесты, 2.58.30) / `b1c02a3` (планы) / `7c79df3` (deploy-doc); прод ff `04f5ae1..b1c02a3`; MainPID **652183**; health 200; `/healthz` 2.58.30; served `?v=2.58.30`; **kill-switch effective ON** (без env-override); `database is locked`=0; промпты не менялись. Откат: soft `DIRECT_COORDINATOR_ENABLED=false`+рестарт / hard annotated-тег `pre-round1026-a1` (obj `3244c61e`) → `e3ea367` + `git revert`. Архив — `plans/archive/tool-coordinator-round1026/` (**T-3521 @PM ✅ выполнен 24.09.2026**). **▶️ Следующая — A2 `tool-chains`** (§15–§17). Pre-existing флак `test_betterstack_handler.py` (вне diff, известен с 10.19).
>
> **🗂️ A1 ЭПИКА 3 «Agentic Intelligence» (Wave 1; Step 0 @Memory, 24.09.2026; product code НЕ менялся — только `plans/**` + KG): `A1 tool-coordinator`** (§13 «Целевая архитектура» / §14 «Единый координатор инструментов») — Синтезатор расширяется до выбора/координации инструментов; **решение о действии и итоговый текст — разные задачи**; Вербализатор — только текст по фактам (без скрытых серверных операций); **общий механизм цепочек**, а не обработчик на каждую комбинацию; **не создавать дополнительный LLM-вызов там, где достаточно программной логики**. **Вход — durable-аудит A0** `plans/docs/agentic-audit-round1026.md` (по анкорам `#tool-map-llm`, `#tool-map-direct`, `#s12-12`, `#epic3-reuse`, `#epic3-a1-a10`, `#epic3-summary`). **Карта кода (HEAD `e3ea367`):** вход direct-чата `direct_chat_service.py:672–773` (dig-пре-гейт :679–683, image-пре-гейт :688–693, tool-loop :766–773, `active_tools` :768); **решение о действии сейчас принимает сама модель** в `tool_loop.chat_with_tools:89` (`tool_choice='auto'`; role `tool` :184; `TOOL_MAX_ROUNDS=4` :38; ≤2/раунд :39; plain-fallback :125–135; `ToolLoopResult` :48–73); **Вербализатор (Stage-2)** `_synthesize_direct_answer:899–988` (Stage-1 Синтезатор :932–935 → `parse_direct_synthesis` :936 → Stage-2 :972; гейт включения :814–818; `response_mode` :813/824/843); диспетчер `tool_router.py:455–480` (10 :457–468), канон `tool_schemas.py:337–348`, `active_tools:367–389`, `factcheck_tools:405–415`; **REUSE ExecutionGraph** `execution_graph_source.build_graph:485`/`STEP_KIND:73` → `web/api/analytics.py:281` → `web/static/execution_graph.js:341` (вторую аналитику не создавать, §65 F6/§79 S8); смежные контуры (досье `:2135`, RAG `:2259–2357`, фактчек `factcheck_service.py:77`, URL→MD `web_content_extractor.extract:59`, реакции `smartmodule_utils.react_moai:93`). **Конфликт-чек:** freeze канона **10** инструментов (ADR-1026-13 D3; расширение — только A1/A2); **без 3-го LLM-вызова** (ADR-1022-4/ADR-1023-3, решение внутри существующих Stage-1/2); общий механизм цепочек (§14); REUSE ExecutionGraph; **§104 `generate_image` не трогать**; **§85-UI — отдельная санкция** (Δ каталога≠0); канон промптов **ADR-1013-3**. **Риски (KG, 4):** Critical — 3-й вызов/регресс живого direct-чата; High — per-combination обработчики, дрейф канона промптов, дублирование инструментов/сервисов. **Baseline (Step 0):** HEAD **`e3ea367`** == `origin/master` (перемерено), дерево чистое (только машинный блок); `APP_VERSION` **2.58.29** (`config/settings.py:1827`, перемерено); каталог REGISTRY **469** / GROUPS 100 / TAB_RULES 21 / `_TAB_BY_GROUP` 98 (перемерено импортом `param_catalog`); Settings 426 / categorized 444 и **Δ DDL=0** (SQLite v12) — verified A0/S10; pytest **9026/0**, JS **47/47** — verified A0/S10 (повторно не перезамерялись). **KG:** `A1 tool-coordinator` + 4 Risk + `SpecDecision-a1-coordinator-scope` (**OPEN**) + `tool_loop-mechanism`; связи `part-of` Эпик 3 · `depends-on` A0 · `enables` A2/A7 · `reuses` tool_loop/ExecutionGraph · `governed-by` ADR-1025-24 (live-гейты)/ADR-1013-3/two-call · `mapped-in` round1025-architecture. **Регламент:** @Scanner удалён — обязанности в едином Reviewer gate (не назначать). **▶️ Следующий шаг — Step 1 @PM (`tasks.md`); открытые вопросы Step 2 @Architect (НЕ решены):** точный scope координатора (что меняется в Синтезаторе vs текущий `_synthesize_direct_answer`); нужны ли AMEND ADR-1022-4/ADR-1023-3 и канон промптов (ADR-1013-3); Δ DDL/Δ каталога (UI §85 — отдельная санкция); deploy/bump; границы A2 (последовательные вызовы) и A7 (`action`).
>
> **✅ S10 ЭПИКА 2 (Шаг 10 @Memory, 24.09.2026; Merge §81; deploy/активация VERIFIED): `summary-deploy-round1026` (§107/§114/§115/§116/§117) — COMPLETED + MERGED (§81, T-3476 @Architect — merge/ADR-1026-12 Accepted выполняются параллельно) + DEPLOYED/ACTIVATED (T-3478 @DevOps, **2.58.29 VERIFIED**; `415a861` код+тесты / `04f5ae1` планы / `7bf716e` deploy-doc; прод ff `cebd950..04f5ae1`, active MainPID **595858**, health 200, `/healthz` 2.58.29, served `?v=2.58.29`, `database is locked`=0, **эффективный `SUMMARY_HYBRID_L2_ENABLED`=True** (code-default ON, `.env` без override), фильтр ON; §115 пп.1–4 ✅, пп.5–7 PENDING OWNER; откат soft `SUMMARY_HYBRID_L2_ENABLED=false`+рестарт / hard annotated-тег `pre-round1026-s10` (obj `be7760295`) → `76abf91` + `git revert`; `deployment.md` VERIFIED).** Approved **единым Reviewer gate** (Scanner удалён 24.09.2026; отдельного Scanner-approval нет): **Approved с 1-й итерации** C0/H0 (линза 1 + линза 2); binding Reviewed-Commit `76abf91`, WTH `a17093d8…`, Spec-Hash `41ee0c6e…`; отклонения @Builder D-a (meta-штамп)/D-b (явный OFF в legacy-тестах)/D-c (флак `test_summary_memory.py`) — non-blocking; non-blocking `L-R1026S10-1/-2/-3`. Числа: pytest `.venv` 9000→**9026/0** (+26; S10-файл 25), JS **47/47**, `git diff --check`=0; **Δ DDL=0 / Δ каталога=0** (469/426/444/100/98/21; F8 не переиздавался); 2-вызовность; CSP/zero-build; §104/§110/§111/S9/routes/db/`telegram_send`/`param_catalog`/`bot.py` вне diff. Архив — `plans/archive/summary-deploy-round1026/` (**T-3477 @PM — ✅ выполнен 24.09.2026**: папка перенесена из `plans/features/`; 9 файлов, SHA-256 8/9 идентичны — финализирован только `tasks.md` (статусы/шапка); статус фичи — COMPLETED + MERGED (§81) + ARCHIVED + DEPLOYED/ACTIVATED (2.58.29)). **🧩 ЭПИК 2 «Summary Hybrid Pipeline» — авто-часть завершена (S1–S10, все COMPLETED + DEPLOYED 2.58.18–2.58.29); live-часть (первый рабочий запуск Саммари и публикация, §115/§117) — PENDING OWNER VERIFICATION.** Предусловие Эпика 3 (после Эпиков 1 и 2) формально достигнуто (решение о старте — select_next/@PM).
>
> **🗂️ S10 ЭПИКА 2 (исторический Step 0 @Memory, 24.09.2026 — итог см. баннер выше): `summary-deploy` (§107 прямой деплой без теневого режима/rollout/Legacy-интерфейса; §114 минимальные предполётные тесты; §115 первый рабочий запуск; §116 критерии «Эпик 2 не завершён»; §117 обязательные результаты; §85-UI — **отдельной санкцией**, Δ каталога ≠ 0) — последний пункт Эпика 2; Step 0 ✅ (initial context sync + исторический конфликт-чек; код НЕ менялся — только `plans/MEMORY.md` + KG).** Зависимости **S1–S9 ✅** (DEPLOYED/VERIFIED 2.58.18–2.58.28; S6 §80 — 2.58.28); гейт **D4 (ADR-1025-24) закрыт владельцем** (owner declaration 24.09.2026). **Baseline (переизмерено):** HEAD **`76abf91`** == `origin/master`, дерево чистое (в рабочем дереве — только машинный блок `plans/workflow_state.md`); `APP_VERSION` **2.58.28** (`config/settings.py:1823`); каталог **469/426/444/100/98/21** (REGISTRY/Settings/GROUPS/`_TAB_BY_GROUP`/TAB_RULES подтверждены импортом `services.param_catalog`); **Δ DDL=0** (SQLite v12). Взято как последний verified S6 (НЕ перезамерялось в Step 0): pytest `.venv` **9000/0**, JS **47/47**; прод активен 2.58.28 (MainPID **575712**). **Активация сейчас:** `SUMMARY_HYBRID_L2_ENABLED` — env-only `ClassVar`, default **OFF** (`config/settings.py:984–985`) + hot-first `flags.summary_hybrid_l2_enabled` + per-chat `_chat_limit` (`services/summary_generator.py:652–666`); ON-врезка `_run_hybrid_l2` (668–780, L1 699 / пакет 718–720 / L2 733 / доставка 755/760); фильтр S1 `SUMMARY_FILTER_ENABLED` default **ON**; слоты `SUMMARY_L1_*`/`SUMMARY_L2_*` env-only («не выбрано» → глобальная модель §82); публикация S6 (`_publish_rich_document` 969 / `_publish_plain_document` 849); планировщик cron 0/6/12/18 (`services/summary_scheduler.py:30–45`); S7-логи (`services/summary_run_log.py`); S8-адаптер (`services/execution_graph_source.py`); S9 dry-run (`services/summary_test_run.py`). **Риски (KG, 5):** Critical — активация ломает живые Саммари/реальную публикацию; High — неверные модели L1/L2, регресс фильтра/потеря контекста, двойная/неверная публикация, невозвратный откат; **R2 → возможно R3** (решение за Step 2 @Architect). `SpecDecision-s10-activation-scope-round1026` — **OPEN**. **Открытые вопросы Step 2 @Architect:** механика активации (§107 «включено после деплоя» vs текущий default OFF: default/env/per-chat/комбинация), уровень риска, §114-предполётные тесты без публикации в основной чат, процедура §115 первого запуска, состав §117-результатов, границы §85-UI, deploy/rollback (тег + soft-флаги). **▶️ Следующий шаг — Step 1 @PM** (декомпозиция `plans/features/summary-deploy-round1026/tasks.md` — ныне `plans/archive/summary-deploy-round1026/tasks.md`; папка фичи ещё не создана). НЕ трогать: product code, `plans/current_task.md`, машинный блок, `plans/backlog.md`, `plans/metrics.md`, `plans/ARCHITECTURE.md`; не коммитить; R17/R18.
>
> **🔓 ГЕЙТ D4 ЗАКРЫТ — LIVE-ПРИЁМКА ЭПИКА 1 ПОДТВЕРЖДЕНА ВЛАДЕЛЬЦЕМ (owner declaration, 24.09.2026).** Владелец подтвердил live-приёмку Эпика 1 (Telegram WebView) → гейт **ADR-1025-24 D4 закрыт**; все owner live-гейты (Эпик 1, S1–S5, S7, S8, S9) считаются подтверждёнными владельцем. **S6 `summary-publish-integration` разблокирован** (ранее ⛔ BLOCKED; смена существующего публикационного пайплайна теперь разрешена); **S10 `summary-deploy` зависит только от завершения S6**; Эпик 3 — предусловие Эпики 1–2 (live Эпика 1 подтверждён). Исторические формулировки «GATED D4» в старых записях — исторический слой, не переписываются. Источник — owner declaration + чекпоинт оркестратора (rev 22).
>
> **✅ S6 ЭПИКА 2 (Шаг 10 @Memory, 24.09.2026; Merge §80; deploy VERIFIED): `summary-publish-integration-round1026` (§100–§106/§103; Эпик 2, P0) — COMPLETED + MERGED (§80, T-3457 @Architect — merge выполнен 24.09.2026: ADR-1026-11 Accepted, spec §7 актуализирован) + DEPLOYED (T-3459 @DevOps, **2.58.28 VERIFIED**; `d0d634c` код+тесты / `cebd950` планы / `b889019` deploy-doc; прод ff `7c5338e..cebd950`, active MainPID **575712**, health 200, `/healthz` 2.58.28, served `?v=2.58.28`, `database is locked`=0, `PUBLISH_*`=0 (аддитивны), §104-контур не тронут; откат annotated-тег `pre-round1026-s6` (obj `6fd4a9d6`) → `197891f` + `git revert`; `deployment.md` VERIFIED).** Approved **единым Reviewer gate** (Scanner удалён намеренно 24.09.2026; отдельного Scanner-approval нет): итер.1 **Needs Fixes** (B-R1026S6-1/-2 Medium — тихая потеря текста/plain без абзацных `<b>`) → rework T-3456 → итер.2 **Approved** C0/H0 (обе линзы; Q1/Q2 — допустимы; L-R1026S6-D1 doc-drift → закрыт на merge §80 / L-R1026S6-D2 non-blocking); binding Reviewed-Commit `197891f`, WTH `4c7b2917…`, Spec-Hash `77c17775…`; pytest 8926→**9000/0** (+74; S6-файл 69), JS 46→**47/47**, `git diff --check`=0; **Δ DDL=0 / Δ каталога=0** (469/426/444/100/98/21; F8 не переиздавался); закрыт follow-up S5: **L-R1026S5-3/-4/-5/-6 + S-R1026S5-7 → CLOSED**; AMEND ADR-1026-7 D5 / ADR-1026-9 D2,D7 / ADR-1026-10 D1,D4,D8. Архив — `plans/archive/summary-publish-integration-round1026/` (**T-3458 @PM ✅ 24.09.2026** — папка перенесена из `plans/features/`; SHA-256 при переносе 6/6, содержательно изменён только `tasks.md` — статусы/шапка; T-3435…T-3459 ✅, T-3460 handoff `[ ]`). **▶️ Следующий — S10 `summary-deploy`** (§107/§114/§115/§117 + §85-UI отдельной санкцией) — последний пункт Эпика 2; live-приёмки S1–S6/S7/S8/S9 — ⏳ PENDING OWNER VERIFICATION.
>
> **🧩 S6 STEP 1–3 + BASELINE + СВЕРКА @PM (24.09.2026; `summary-publish-integration-round1026`, Эпик 2, §100–§106): дизайн принят, baseline зафиксирован; код НЕ менялся — только `plans/**` + KG.** Step 1 @PM ✅ (`tasks.md` T-3435…T-3460, 26; блоки 0/A–H) → Step 2 @Architect ✅ (T-3436: `spec.md` REQ-S6-01…-10 / SC-01…SC-20 / §8 §103-утверждения + **ADR-1026-11** D1–D10, Proposed → Accepted по T-3457, merge ожидается **§80**) → baseline @DevOps ✅ (T-3435: annotated-тег `pre-round1026-s6`, tag-object `6fd4a9d6` → `197891f`; `APP_VERSION` 2.58.27; pytest `.venv` 8926/0; JS 46/46; каталог 469/426/444/100/98/21; Δ DDL=0; бэкапы `var/backups/s6-round1026-20260924-070835/` + `.env.bak.round1026-s6`) → сверка @PM ✅ (T-3437: REQ→SC→ADR-колонки, карта D1–D10 → задачи, 20 инвариантов; GATED-тесты S7/S8 → T-3446/T-3450/T-3453; `_send_chunked` superseded только на публикационном пути) → **Step 3 @Memory ✅** (KG: `ADR-1026-11-summary-publication-integration` + связи; `SpecDecision-s6-publish-scope-round1026` → **RESOLVED**). **AMEND:** ADR-1026-7 D5 / ADR-1026-9 D2/D7 / ADR-1026-10 D1/D4/D8. **REUSE:** ADR-1022-4/-6, ADR-1023-6. Δ DDL=0 / Δ каталога=0 (санкции нет; F8 не переиздаётся); **deploy = ДА, bump 2.58.27 → 2.58.28**; откат `pre-round1026-s6` + `git revert`; нового kill-switch нет; остаток S10 — §107/§114/§115/§117. **▶️ Следующий шаг — Step 4 @Builder (T-3438…T-3453)** → Reviewer gate (T-3454/T-3455) → rework (T-3456) → merge §80 (T-3457) → архив (T-3458) → deploy (T-3459) → S10 (T-3460). ⚠️ Открыто (за @Architect, T-3436 follow-up): `spec.md` §4 D1 включает `web/api/analytics.py` (docstring), ADR-1026-11 D1 — нет.
>
> **🔧 S6 STEP 0 @Memory (24.09.2026; initial context sync + конфликт-чек; код НЕ менялся — только `plans/**` + KG):** `summary-publish-integration-round1026` (Эпик 2, ТЗ §100–§106/§103; источник `plans/current_task.md` стр. 3069–3248 + §107 3250–3276 + §114/§115) — **UNBLOCKED / PLANNING; Step 0 ✅ → Step 1 @PM**. Baseline: HEAD **`197891f`** == `origin/master` (в рабочем дереве — только машинный чекпоинт `plans/workflow_state.md`, обновлён оркестратором); `APP_VERSION` **2.58.27** (прод активен, MainPID 540872); pytest `.venv` **8926/0** (последний верифицированный S8; повторно не перезамерялся), JS **46/46**; каталог **469/426/444/100/98/21** (REGISTRY/GROUPS/`_TAB_BY_GROUP`/TAB_RULES подтверждены импортом + `gen_param_registry --check` OK); **Δ DDL=0** (SQLite v12); R17/R18. **Карта кода (файлы:строки):** `services/summary_generator.py` (`_run` 372–596 — фильтр 419–436, ON-ветка 442–445, OFF-доставка 553–566; `_hybrid_l2_enabled` 598–612; `_run_hybrid_l2` 614–721 — L1 645 / пакет 664 / L2 677 / доставка 694–704; `_deliver_l2_plain` 778–822 — `chunk_plain_blocks` 795, `send_text(parse_mode=HTML)` 797, даунгрейд 820–822; `_deliver_l2_rich` 824–908 — обложка 843/874 → `format_rich_html` 875–876 → `send_rich_message(content_format="html")` 877–879, cover-unavailable → plain 864–865, ошибка → plain 901–902; `_resolve_cover_prompt` 1432–1486; `_deliver_rich` 1537–1621 — `generate_image_verbose` 1575, `_send_rich_with_retry` 1623–1637 (1 retry), `_plain_fallback` 1639–1647; `_send_chunked` 1712–1726 — `_chunk_by_whitespace`, НЕ по абзацам); `services/telegram_send.py` (`send_text` 98, `build_cover_article_html` 152–165 — `<img tg://photo>` + `<p>`, БЕЗ `<h1>`; `build_cover_media` 168–187; `send_rich_message` 190–221 — auto/html/markdown); `services/summary_article_formatter.py` (лимиты 27–32; `format_rich_html` 110–146 — `<h1>` 122–123; `format_plain_html` 148–166; `chunk_plain_blocks` 210+ — по абзацам); `services/summary_run_log.py` (`SUMMARY_*` 186–233, `FORMAT_*` 242–275, `COVER_*` 277–305; `PUBLISH_*` отсутствуют — GATED); `services/execution_graph_source.py` (`KIND_PUBLISH` 37, `publication` 74, `PUBLICATION_STATUS_GATED` 89, publish не эмитится 363–371, `publication_status=gated` 448/525); `web/static/execution_graph.js` (publish отбрасывается 269–274, `publicationStatus` 'gated' 365–366); `services/summary_test_run.py` (dry-run `not_published/dry_run` 419/671; 0 публикаций); `services/summary_prompts.py` (L1 216–258, L2 270–303); kill-switch `config/settings.py:984-985` (`SUMMARY_HYBRID_L2_ENABLED` default False) + `SUMMARY_L2_*` 975–977; `services/image_generation.py` (`generate_image_verbose` 860 — §104 не трогать); §110-viewer `web/app.js:1777-1779/2782-2783/9629-9653`. **Конфликт-чек:** REUSE — `sendRichMessage`/`build_cover_media`/форматтер S5/события S7/adapter S8/§110-viewer; SUPERSEDE-кандидаты (решает Step 2) — авто-режим `build_cover_article_html` (без `<h1>`) и `downgrade_rich_to_plain` на OFF-пути, `_chunk_by_whitespace` vs §105 «по границам абзацев»; AMEND-кандидат — «OFF байт-в-байт» ADR-1026-7 D5 при изменении OFF-доставки. **Открытые вопросы Step 2 @Architect** (SpecDecision-s6-publish-scope-round1026, OPEN): границы diff живого пайплайна; `SUMMARY_HYBRID_L2_ENABLED`/OFF байт-в-байт; §103-сверка (источники документации); маппинг кодов §106; активация publish-узла S8/`PUBLISH_*`; менять ли форматтер/канон; deploy/bump; остаток S10 (§107/§114/§115). **Риски (Critical/High):** потеря текста/обложки при fallback; двойная публикация; нарушение §104; регресс живой публикации; неверные коды §106; преждевременная активация publish-узла S8; семантика OFF-пути. KG: `S6 summary-publish-integration` (обновлён), `ADR-1025-24-verification-gate` (gate resolved), `S10 summary-deploy` (awaiting S6), `Эпик 2`/`round1025-architecture`/`S8 summary-analytics-adapter` синхронизированы, 7 Risk + SpecDecision. **Следующий шаг — Step 1 @PM** (декомпозиция `tasks.md`).

> **✅ S8 ЭПИКА 2 (Шаг 10 @Memory, 24.09.2026; Merge §79; deploy VERIFIED): `summary-analytics-adapter-round1026` (§30/§111/§112, Раунд 10.26) — COMPLETED + MERGED (§79, T-3431 @Architect) + DEPLOYED (T-3433 @DevOps, **2.58.27 VERIFIED**; `64cdb0e` код+тесты / `7c5338e` планы / `f14ae56` deploy-doc; прод ff `aa42a04..7c5338e`, active MainPID **540872**, health 200, `/healthz` 2.58.27, served `?v=2.58.27`, новый роут жив (401), `database is locked`=0, `PUBLISH_*`=0; откат annotated-тег `pre-round1026-s8` → `2ffeb6a` + `git revert`; `deployment.md` VERIFIED); Approved **единым Reviewer gate** (Scanner удалён намеренно 24.09.2026 — обязанности в Reviewer; отдельного Scanner-approval нет), **1 проход** C0/H0, блокирующих 0; binding Reviewed-Commit `2ffeb6a`, WTH `4151d36d…`, Spec-Hash `f397f3fd…`; pytest 8890→**8926/0** (+36), JS 45→**46/46**; Δ DDL=0 / Δ каталога=0; закрыт F6-долг `L-F6S-1`; техдолг §79.7 (L-R1026S8-1/-2/-3 open); архив — `plans/archive/summary-analytics-adapter-round1026/` (**T-3432 @PM ✅ 24.09.2026** — папка перенесена из `plans/features/`; SHA-256 6/6 идентичны); live — ⏳ PENDING OWNER VERIFICATION; следующий actionable — **S6/S10 при снятии D4** (⛔ GATED).** Baseline @DevOps T-3410 ✅: HEAD **`2ffeb6a`** == `origin/master`, annotated-тег **`pre-round1026-s8`** (tag-object `eeb960b4`) → `2ffeb6a`; бэкапы `var/backups/s8-round1026-20260924-053821/` + `.env.bak.round1026-s8`; `APP_VERSION` **2.58.26**, pytest `.venv` **8890/0**, JS **45/45**, каталог **469/426/444/100/98/21**, **Δ DDL=0** (SQLite v12); прод активен 2.58.26 (MainPID 522392). Артефакты (архив): `plans/archive/summary-analytics-adapter-round1026/{spec.md (REQ-S8-01…-14, SC-01…SC-18), adr-1026-10-summary-execution-graph-adapter.md (**ADR-1026-10**, D1–D10, Proposed → Accepted по T-3431), tasks.md (T-3410…T-3434, 25; блоки 0/A–H)}`; сверка @PM T-3412 ✅ (SC↔ADR, mismatches 0). **Суть ADR-1026-10:** D1 — старт **ядром без S6**, publish-срез **GATED** (прецедент S7; `cover_status` не GATED); D2 — маппинг `step→kind` (`filter`→algorithm, `l1_clusterizer`/`l2_writer`→llm, `formatting`→format, `publication`→publish зарезервирован GATED) + два источника (PG `llm_usage_events` + in-memory снапшот S7 `RunContext`/`_filter_metrics`); D3 — backend-источник (аддитивный `GET /api/analytics/execution/latest` в `web/api/analytics.py` + `services/execution_graph_source.py` NEW) + клиентский `fromExecution`, `GraphViewer`/`NodeCard`/`DetailPanel` не переписываются; D4 — §112 по kind, «Нет данных»/«Без лимита» вместо `$0`, `publication_status="gated"`, **закрытие `L-F6S-1`** (аддитивный `price_known` в `/analytics/usage/summary`); D5 — единый `run_id`=`correlation_id`; D6 — два несмешиваемых режима; D7 — **Δ DDL=0 / Δ каталога=0**, новый флаг не вводится; D8 — publish вне diff; D9 — **deploy=ДА, bump 2.58.26 → 2.58.27**, откат `pre-round1026-s8`; D10 — REUSE/инварианты (2-вызовность, CSP/zero-build, R17/R18). **Границы:** S6 `summary-publish-integration` ⛔ GATED (D4) и S10 `summary-deploy` ⛔ GATED остаются закрытыми; ADR-1026-10 → S6 (gated-by), → S10 (bounded-by). SpecDecision `SpecDecision-s8-publish-slice-round1026` → **RESOLVED**. KG: узел `ADR-1026-10-summary-execution-graph-adapter` (adr, D1–D10) + связи (S8 specified-by; reuses ADR-1025-19/-1026-9/-1026-8/ADR-1023-7; governed-by ADR-1025-24 D4; closes `L-F6S-1`); обновлены `S8 summary-analytics-adapter`, `Эпик 2 (Summary Hybrid Pipeline)`, `round1025-architecture`, `ExecutionGraph-adapter`, `tech-debt-round10.25-f6`. ✅ `plans/ARCHITECTURE.md` **§79** создан фактом мержа T-3431 (ADR-1026-10 Accepted); архивация — T-3432 @PM ✅ (24.09.2026). **▶️ Следующий шаг — Step 4 @Builder** (блоки B–G, T-3413…T-3427: mapping step→kind; backend adapter/источник узлов; UI/узлы/режимы; §112-метрики; R17/регресс; границы), затем единый Reviewer gate (T-3428/T-3429) → rework T-3430 → merge §79 T-3431 → архив T-3432 → deploy T-3433 → handoff T-3434. Live-гейты S1–S5/S9/S7 и Эпика 1 — ⏳ PENDING OWNER VERIFICATION; S6/S10 — ⛔ GATED (D4).
