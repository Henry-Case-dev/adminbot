...
> **▶️ S3 ЭПИКА 2 (Step 0 @Memory ✅ + Step 1 @PM ✅ + Step 2 @Architect ✅ + Step 3 @Memory ✅, 23.09.2026): `summary-l1-clusterizer` — `spec.md` + **ADR-1026-5** (D1–D6, Accepted) + KG-синк; код НЕ менялся (только `plans/**` + KG).** Baseline: HEAD **`4007081`** (== `origin/master`, после S2 + финального синка), `APP_VERSION` **2.58.21**, каталог **467/426/442/100/98/21**, **Δ DDL=0**, pytest `.venv` **8569/0** (последний верифицированный S2; collection 8569), JS **43/43**; CSP/zero-build; R17/R18. Вход S3: `RestoreResult.kept` + `build_l1_payload` (§92) + `FilterResult.fragments` (§93; `Fragment.message_ids` — DB `id`, не TG). **РЕШЕНО (Step 2 @Architect → ADR-1026-5, Accepted):** **вариант (a): L1 заменяет Stage-1, L2 заменяет Stage-2 ⇒ ровно 2 вызова**; третий — блокер; L1-слой = 1 вызов, §93-фрагменты — один вход (ASC/дедуп), chunk-merge **BLOCKED**, при нехватке бюджета `truncated`+`skipped_ids`+лог; **AMEND ADR-1022-4** (Stage-1→Кластеризатор §95, вход L2→пакет фактов §96; effective S5; 2 вызова/изоляция/fallback single/доставка/kill-switch/validator-loop сохранены) и **AMEND ADR-1023-3/-6** (`cover_prompt`/`response_mode` — опциональные служебные поля того же JSON L1, вариант i; меняется только эмитент); в S3 живой путь не меняется; врезка — не раньше S5/S6 (гейт S6/S10 + D4). **§82:** в S3 — только слот summary L1; summary L2 — S5; direct L1/L2 — вне S1–S10. KG: `S3 summary-l1-clusterizer` + **ADR-1026-5-l1-clusterizer-contract-two-call-amend** (specified-by; amends ADR-1022-4/1023-3/-6; reuses ADR-1026-2/1013-3; governed-by ADR-1025-24) + 6 Risk (митигации D1–D5) + SpecDecision (RESOLVED D2). Каталог: +1 промпт-ключ `prompts.summary_l1_clusterizer_system_prompt` (санкция D4; REGISTRY 468/Settings 426/categorized 443) + переиздание F8 (ADR-1026-2); модель/провайдер/ключ — env-only `SUMMARY_L1_*` (Δ каталога=0). **Deploy = ДА** (bump 2.58.21→2.58.22). **▶️ Следующий шаг — baseline @DevOps** (тег `pre-round1026-s3`) **→ Step 4 @Builder** (`tasks.md` T-3253…T-3282).
>
