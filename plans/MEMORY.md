...
Индекс долговременной памяти. Архитектура — `plans/ARCHITECTURE.md` (§1–§59);
...
> **✅ АКТИВНЫЙ ПАКЕТ ЗАКРЫТ (Шаг 8 @PM, 22.09.2026): UPD-фикс `hotfix7-shell-glass-heartbeat-round1025` — COMPLETED + MERGED + ARCHIVED; ⏳ deploy — Шаг 9 @DevOps; ⏳ live-гейт владельца T-2682.** Архитектура — **`plans/ARCHITECTURE.md` §59** (Merge @Architect: §59.1 A — единый `--shell-h` + режимы normal/fullscreen, §59.2 B — premium ECG-сердцебиение §15 (AMEND ADR-1025-12 D4), §59.3 C — серо-графитовый `--shell-*` (AMEND ADR-1025-12 D2 / уточнение ADR-1025-9 D2), §59.4 D/E — рецепт glass + снятие «грязи» + AA + выравнивание, §59.5 F — матрица 5 режимов, §59.6 флаги, §59.7 SUPERSEDE/AMEND, §59.8 техдолг, §59.9 live-гейты, §59.10 ссылки). Архив — **`plans/archive/hotfix7-shell-glass-heartbeat-round1025/`** (`spec.md` + **ADR-1025-13** + `tasks.md` + `evidence.md` + `review.md`; `deployment.md` — Шаг 9; **T-2658…T-2694, 37**; сохранились только **T-2690 (deploy)** и **T-2682 (live)** как открытые). @Reviewer **Approved** (итер.2, F-1…F-4); @Scanner **Critical 0 / High 0 / Medium 0 / Low 3 / Info 3 → «к деплою ДА»** (`plans/reports/round1025_hotfix7_scanner_audit.md`); AA — `plans/reports/round1025_hotfix7_contrast.md`; матрица — `plans/reports/round1025_hotfix7_ui_report.md`; UPD 7 — `plans/reports/round1025_tz_remaining_audit.md` (+§9 итоговый отчёт). Числа: pytest **8207/0**, JS **27/27**, `APP_VERSION` **2.58.8**; **Δ DDL=0**, **Δ каталога=0** (459/98/96/21/418); код/тесты/отчёты в рабочем дереве, **НЕ закоммичены**. Техдолг **§59.8** (L-H7-1/-2/-3, Info I-H7-1/-2/-3). Откат — тег `pre-round1025-hotfix7`→`5a5465c` (запушен) + `git revert`; soft — env-флаги `UI_SHELL_GLASS_V2`/`UI_HEARTBEAT_PREMIUM`/`UI_SHELL_LAYOUT_V2` (+hotfix6-флаги), default ON. **▶️ Следующий шаг — Шаг 9 @DevOps** (коммит/push/деплой + `deployment.md` VERIFIED, APP_VERSION 2.58.8), **затем — возобновление F4** (Step 2 @Architect; `metrics.md` — Шаг 10 @Memory).
>
> **⏸ F4 (ПАУЗА, Step 0–1 done, 22.09.2026): `module-catalog-quickpanel-store-round1025`** (Эпик 1, Волна 2; ТЗ §31–§45/§72/§73/§79/§116). Baseline: HEAD **`5a5465c`**, `APP_VERSION` **2.58.7**; pytest **8185/0**, JS **26/26**; каталог **459/98/96/21/418** (Δ=0). Папка — `plans/features/module-catalog-quickpanel-store-round1025/` (`tasks.md` — заготовка Step 1 @PM; `spec.md`/ADR — Step 2 @Architect). **Суть:** каталог модулей + панель избранного + канонический `ModuleConfigurationStore` (одна мутация → все представления, ключ `scope_type+scope_id+module_id`, scoped keys), переиспользование F3 (`scopeEpoch`/`_scopeGuard`, `chat_source`/`global_value`, §43-хелперы) и F0-write-path (`persistItems`). **Открыто:** хранение избранного (§36), метаданные/синонимы поиска §44, «Настроить»→workspace F5, §43 «Ожидает применения» = N/A (read-through). KG: `F4-module-catalog-quickpanel-store-round1025` + constraint + risks + decision (`specdecision-f4-favorites-storage-round1025`, OPEN) + `metric-snapshot-round1025-f4-step0`. Шаг 0–1 done (код/коммиты/`current_task.md` не менялись). **▶️ Возврат — после deploy HOTFIX7 (Шаг 9 @DevOps); следующий шаг — Step 2 @Architect (`spec.md` + новый ADR — ⚠️ `ADR-1025-13` занят HOTFIX7, брать следующий свободный `ADR-1025-14`) → build.** Отдельного решения владельца не требует (UPD 7 п.7 «без human gate»).
