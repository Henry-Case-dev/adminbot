...
> **Статус:** Epics 1–27 ALL DEPLOYED ✅ (v2.25.0, `1d7bed4`, 939 тестов). **Epic 28 (Качество памяти, v2.26.0) — Шаг 6: IMPLEMENTED ✅ (ревью PASS, 995 тестов), НЕ закоммичен.** Epic 27 (промпт v2 + SUMMARY_ALIASES) DEPLOYED на проде: PID 934174, SUMMARY_ALIASES 36 пар активны (бэкап .env.bak.epic27), 0 traceback, T-207–T-210 Done. Ручные действия пользователя: живой тест `/summary` в чате, Н1 (BotFather `/setprivacy` → Disable), при WARNING удаления — выдать боту админ-права `delete_messages`.
> **Текущий коммит:** `1d7bed4` (feat(summary): Epic 27 — новый системный промпт бота-абьюзера v2 и SUMMARY_ALIASES (v2.25.0)) — в origin/master (github.com/Henry-Case-dev/adminbot.git). Прод версия **v2.25.0**. **Epic 28 реализован в рабочем дереве (T-211…T-219 Done, 995 тестов), но НЕ закоммичен — впереди T-220 (коммит `feat(summary): Epic 28 — … (v2.26.0)` + пуш + деплой).**
> **Сервер:** 198.46.175.136:/var/www/admin_bot, systemctl active (running), PID 934174 (был 926618), 0 traceback после рестарта; .env +SUMMARY_ALIASES (36 пар, бэкап .env.bak.epic27, JSON OK, sha1 совпал с репо). Известные не-блокеры (pre-existing): бот не отвечает на SIGTERM (~95с рестарт); L3 vector search failed — Dimension mismatch 3072 vs 768 (старые эмбеддинги) → FTS5-фоллбек штатный — **лечится Epic 28 (VecAutoHeal, T-216, реализовано)**: при первом рестарте после деплоя v2.26.0 автолечение выполнит DROP smart_archive + пересоздание под actual_dim (один раз, 768→3072 — ожидаемо); прод .env править НЕ нужно.

---

## ✅ Epic 28: Качество памяти — векторы, репосты, алиасы, очистка — v2.26.0 (IMPLEMENTED, Шаг 6 @Memory, 2026-08-16)

> **Статус:** Шаг 1 (PM: R28-1…R28-6, D76–D80) ✅ → Шаг 2 (@Architect, Section 37) ✅ → Шаг 3 (@Memory, DESIGN) ✅ → **Шаги 4–5: @Builder (T-211…T-219) ✅ + @Reviewer PASS ✅ → Шаг 6 (@Memory): IMPLEMENTED ✅ (not committed).** Прод пока v2.25.0 (коммит `1d7bed4`, PID 934174). Впереди T-220 (@DevOps): коммит + пуш + деплой (37.9, D80).
...
| **37.7 (T-217, D76/D77)** | SYSTEM_PROMPT v3: **правило 6** (алиас из атрибута author обязателен дословно, переименовывать нельзя; без алиаса — свобода + креативная интерпретация ника «человек с пейзажем в нике»; финал — имя из author) + **правило 7** (is_forward="true" → содержание принадлежит forward_source, не переславшему); пояснение финала: «реальный ник из контекста» → «имя участника из атрибута author» (D73 сохраняется); 23 строки; эталон backlog **1518–1540** (формула 1538+2), слайс `lines[1517:1540]`; плейсхолдеры {max_symbols} ×1, {username} ×2 БЕЗ изменений (атрибуты в правилах — без фигурных скобок) |
...
- **T-217 (SYSTEM_PROMPT v3):** правила 6/7; **байт-в-байт == backlog 1518–1540** (инвариант D74); хелпер `lines[1517:1540]`; плейсхолдеры `{max_symbols}` ×1, `{username}` ×2 без изменений.
...
- **Изменённые файлы (12–13):** `services/database.py`, `handlers/summary.py`, `services/summary_xml.py`, `services/summary_generator.py`, `services/summary_memory.py`, `services/summary_cleanup.py` (новый), `services/summary_prompts.py`, тесты (7 файлов), планы (board.md/backlog.md/ARCHITECTURE.md/MEMORY.md). **НЕ закоммичено.**

### 📋 Задачи (статусы Шага 6)
...
- **T-220** @DevOps+@PM (P1, ←T-219) — коммит + деплой + инструкция (37.9, D80) → **PENDING**

### 🚀 Резюме для T-220 (коммит + деплой)

- **Коммит:** на русском, conventional commits: `feat(summary): Epic 28 — качество памяти: репосты, алиасы, векторное автолечение и cleanup (v2.26.0)` — **12–13 файлов** (код + тесты + планы). Перед коммитом: полный pytest (995 passed), `git diff --check`.
- **Деплой:** прод 198.46.175.136:/var/www/admin_bot: `git pull` → `systemctl restart admin_bot` (SIGTERM ~95с — pre-existing, не паниковать) → проверка логов: **НЕТ «Dimension mismatch»**, есть «sqlite-vec loaded (dim=3072)» (или WARNING probe → FTS5), алиасы работают, 0 traceback.
- **DROP smart_archive выполнится ОДИН раз на проде** при расхождении 768→3072 — это ожидаемо (D78/D79, факты smart_archive_facts сохраняются).
- **Прод .env править НЕ нужно** (автолечение чинит само); опционально EMBEDDING_DIM=3072 с бэкапом `.env.bak.epic28` — только для чистых логов.

### ⚠️ Предупреждения для @DevOps (T-220)

- **Хелпер-диапазон `lines[1517:1540]`** — уже реализован и сверен (v3 байт-в-байт == backlog 1518–1540). При правках backlog выше блока пересчитывать (формула `1538 + N_вставленных`).
- **SYSTEM_PROMPT v3 — байт-в-байт** == backlog 1518–1540 (инвариант D74); перед коммитом `git diff --check` и полный pytest (995 passed).
...
- **DROP smart_archive на проде выполнится один раз** при 768→3072 — ожидаемо (D78/D79); `smart_archive_facts` сохраняется; после рестарта в логах НЕ должно быть «Dimension mismatch».
- **Прод .env НЕ править** (автолечение чинит само); EMBEDDING_DIM=3072 — опционально, только с бэкапом .env.bak.epic28.
- **Коммит на русском**, conventional commits: `feat(summary): Epic 28 — … (v2.26.0)`; 12–13 файлов; планы (board/backlog/ARCHITECTURE/MEMORY) в состав коммита.
...
> **Требования R27-1…R27-4, решения D72–D75:** `plans/backlog.md` (Epic 27, эталон промпта R11 v2 на строках 1518–1538; с Epic 28 эталон v3 — 1518–1540). **Дизайн:** `plans/ARCHITECTURE.md` Section 36 (36.1–36.8).

### 📐 Дизайн (Section 36, @Architect, 2026-08-16)

| Блок | Суть |
|---|---|
| **C1** | Единый источник истины — ТОЛЬКО backlog.md (R11 v2, 1518–1538, 21 строка; с Epic 28 v3 — 1518–1540, 23 строки); ARCHITECTURE ссылается, не дублирует (дубль = риск рассинхрона эталонов) |
...
- **SYSTEM_PROMPT v2 (T-207):** `services/summary_prompts.py` строки 8–28 (21 строка) — байт-в-байт == эталон `plans/backlog.md` 1518–1538 (с Epic 28 v3 — 1518–1540) (инвариант D74, `git diff --check` чист); плейсхолдеры `{max_symbols}` ×1, `{username}` ×2; подстановка `.replace` в `summary_generator.py:113` НЕ тронута (C2); `COMPRESS_PROMPT`/`EXTRACT_PROMPT` не изменены (C4).
- **Тесты:** хелпер `_backlog_system_prompt` — слайс `lines[1517:1538]` (с Epic 28 v3: `lines[1517:1540]`); `test_max_symbols_is_the_only_placeholder` переписан по D72 (regex `\{(\w+)\}` → set `{max_symbols, username}`); полный pytest **939 passed / 0 failed**; точечно 50 passed.
...
- **T-207** @Builder (P0) — SYSTEM_PROMPT = v2 дословно (backlog 1518–1538, без хвостовых пробелов; с Epic 28 v3 — 1518–1540) + docstring; тесты 36.4; полный pytest 939 → **DONE**
...
3. **Верификация:** WARNING «SUMMARY_ALIASES invalid JSON» в логах ОТСУТСТВУЕТ → AliasResolver распарсил все 36 пар. Тесты: 939 passed. Ревью PASS. SYSTEM_PROMPT v2 байт-в-байт == backlog 1518–1538 (с Epic 28 v3 — 1518–1540); плейсхолдеры {max_symbols} ×1, {username} ×2; подстановка .replace не тронута; COMPRESS/EXTRACT заморожены.
...
- Байт-в-байт инвариант (D74): backlog-блок 1518–1540 (v3 — Epic 28) == константа `SYSTEM_PROMPT`, без хвостовых пробелов; контроль `git diff --check`.
- Диапазон хелпера `1517:1540` хрупкий — при сдвиге строк backlog выше блока обновить (T-217-C).
...
| R11 | Системный промпт — захардкодить ДОСЛОВНО: токсичный «бот-абьюзер» (v3 — Epic 28: правила 6/7 — алиасы из атрибута author обязательны дословно, репосты не приписывать переславшему; ленивая печать, запрет маркдауна/эмодзи/тире-ёлочек, абзацы), приписка «самым главным шизом объявляется {username}» (эталон в backlog.md, строки 1518–1540) |
...
| **v2.26.0** | **2026-08-16** | **Epic 28 (Качество памяти: векторы/репосты/алиасы/очистка)** | **T-211–T-219 (IMPLEMENTED, ревью PASS; T-220 коммит+деплой PENDING)** | **995** |
...
| **IMPLEMENTED** | Epic 28: T-211 – T-219 (IMPLEMENTED + ревью PASS ✅, 995 тестов, **НЕ закоммичен**) — репосты/алиасы/векторное автолечение/cleanup/SYSTEM_PROMPT v3; T-220 (коммит+пуш+деплой) PENDING → @DevOps |
...
| **Epic 28** | Качество памяти — **IMPLEMENTED (Шаг 6 @Memory), НЕ закоммичен**: T-211–T-219 Done, ревью PASS, 995 тестов (939+56), T-220 PENDING — коммит `feat(summary): Epic 28 — … (v2.26.0)` (12–13 файлов) + деплой git pull + restart + проверка логов |
...
| **Backlog-кандидаты (pre-existing)** | (а) L3 dimension mismatch: эмбеддинги 3072 dim vs БД 768 → vector search failed → FTS5 fallback (штатная деградация; **лечится Epic 28 VecAutoHeal — реализовано, применится при деплое v2.26.0**: DROP smart_archive один раз + пересоздание под 3072); (б) stop-timeout systemd при рестарте (~90-95с → SIGKILL, сервис поднимается корректно); (в) **SIGTERM graceful shutdown** — бот не отвечает на SIGTERM, рекомендуется отдельный тикет |
