...
> **Статус:** Epics 1–28 ALL DEPLOYED ✅ (v2.26.0, `ac80ce8`, 995 тестов, PID 936542, dim=3072). **Epic 28 (Качество памяти, v2.26.0) — Шаг 8: DEPLOYED ✅.** **Epic 29 (UX-полировка, target v2.27.0) — Шаг 6: IMPLEMENTED ✅ (ревью PASS, 1002 тестов). T-221…T-225 DONE; T-226 PENDING (@DevOps) — коммита ещё нет.** Вся цепочка воркфлоу (0–8) завершена. Ручные действия пользователя: живой тест `/summary` в чате, Н1 (BotFather `/setprivacy` → Disable), при WARNING удаления — выдать боту админ-права `delete_messages`.
> **Текущий коммит:** `ac80ce8` (feat(summary): Epic 28 — качество памяти: репосты, алиасы, векторное автолечение и cleanup (v2.26.0)) — в origin/master (github.com/Henry-Case-dev/adminbot.git). Прод версия **v2.26.0**. **Epic 28 DEPLOYED (T-211…T-220 Done, 995 тестов, PID 936542).** **Epic 29: target v2.27.0 — IMPLEMENTED (Шаг 6, ревью PASS, 1002 тестов, коммита НЕТ — впереди T-226).**
...
## ✅ Epic 29: UX-полировка — удаление команды, ack-вариации, промпт v4 — v2.27.0 (IMPLEMENTED, Шаг 6 @Memory, 2026-08-16)

> **Статус:** Шаг 1 (PM: R29-1…R29-6, D81–D84) ✅ → Шаг 2 (@Architect, T-221, Section 38) ✅ → Шаг 3 (@Memory, DESIGN) ✅ → Шаги 4–5: @Builder (T-222…T-225) ✅ + @Reviewer PASS ✅ → **Шаг 6 (@Memory): IMPLEMENTED ✅** → Шаг 7: @DevOps (T-226) → Шаг 8 (@Memory): DEPLOYED. Прод v2.26.0 (`ac80ce8`, PID 936542, 995 тестов), target **v2.27.0**. Коммита ещё НЕТ.
...
| **38.3 (T-223, D83/D84)** | Промпт v4: пункт 3 удалён (типографика → `cleanup_llm_text` 37.6); пункт 6 — канон пользователя ДОСЛОВНО (уже в дереве `M services/summary_prompts.py:18`: «нечитаемая херня», «чел с пейзажем в нике», «используй СТРОГО дословное значение из атрибута author» — D83, НЕ переписывать/не чинить); нумерация — зазор «1,2,4,5,6,7» НЕ перенумеровывать (D84); эталон backlog → 22 строки, 1518–1539, слайс `lines[1517:1539]`; плейсхолдеры `{max_symbols}`/`{username}` без изменений; канон содержит «—» и `"` — не чинить |
...
- **T-223 (SYSTEM_PROMPT v4, D83/D84):** пункт 3 (типографика) удалён; нумерация-зазор «1,2,4,5,6,7» НЕ перенумерована; пункт 6 — канон пользователя ДОСЛОВНО (незакоммиченная правка НЕ откачена); эталон backlog.md **1518–1539** (22 строки), слайс `lines[1517:1539]`; тест :59 — новый ассерт «используй СТРОГО дословное значение из атрибута author»; **байт-в-байт ЗЕЛЁНЫЙ**; плейсхолдеры `{max_symbols}`/`{username}` без изменений; COMPRESS/EXTRACT не тронуты.
...
- **Изменённые файлы (9):** `handlers/summary.py`, `services/summary_prompts.py`, `plans/backlog.md`, `plans/ARCHITECTURE.md`, `plans/board.md`, `README.md`, `tests/test_summary_handlers.py`, `tests/test_summary_prompts.py`, `plans/MEMORY.md`. **Коммита ещё НЕТ — код в рабочем дереве.**

### 📋 Задачи (статусы, Шаг 6)
...
- **T-223** @Builder (P0, ←дизайн) — промпт v4: пункт 3 удалён, пункт 6 канон дословно, зазор-нумерация, эталон 1518–1539, слайс `lines[1517:1539]`, тесты :11/:13/:14/:59 → **DONE** (Шаг 6, байт-в-байт зелёный)
...
- **T-226** @DevOps+@PM (P1, ←T-225) — коммит (включая канон пользователя) + пуш + деплой + верификация (38.6) → **PENDING**

### ⚠️ Предупреждения для @DevOps (T-226)

- **Коммит на русском**, conventional commits: `feat(summary): Epic 29 — UX-полировка: удаление команды, ack-вариации, промпт v4 (v2.27.0)` — **ВКЛЮЧАЯ канон пользователя** (`M services/summary_prompts.py` — незакоммиченная правка в строке 18, D83, НЕ откатывать, НЕ «чинить» «—» и `"` в каноне). В коммит входят 9 файлов (включая plans/MEMORY.md).
- **Деплой:** прод 198.46.175.136:/var/www/admin_bot — `git pull` (ff) → `systemctl restart admin_bot` (**~95с SIGTERM pre-existing — не паниковать**) → верификация: 0 traceback, active (running), новый PID.
- **Верификация порядка логов:** `triggered → command deleted → ack sent` (при WARNING `command delete failed` — проверить права `delete_messages` у бота; pre-existing Н1: BotFather `/setprivacy` → Disable).
- **Байт-в-байт:** в проде до деплоя активен SYSTEM_PROMPT v3; после — v4 (22 строки, эталон backlog 1518–1539, слайс `lines[1517:1539]`). При правках backlog выше блока R11 пересчитывать диапазон (формула: конец = 1539 + N_вставленных).
- **НЕ трогать:** `COMPRESS_PROMPT`/`EXTRACT_PROMPT`, `llm_client.py`, `summary_generator.py`, `summary_cleanup.py`, `bot.py`, scheduler, продовый `.env` (изменений env в релизе нет).
