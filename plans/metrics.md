...
| **R18-history-risk** | **Risk (открыт)** | Исходное значение пароля осталось в истории git (коммиты `eb3fd4a…995cf83`); рабочее дерево и HEAD чисты — требуется решение владельца (rotation и/или rewrite history). KG `risk-secret-in-git-history` |
