...
- **hotfix4 (10.25, ADR-1025-8 D1):** блок `SUMMARY_EDITOR_COVER_PROMPT_BLOCK`
  переформулирован (T-2511) в **self-contained** вид: Редактор Stage-1 не видит
  авторский «Стиль обложки» (он конкатенируется к image-промпту отдельно),
  поэтому в блоке нет ссылки на невидимый стиль и нет внутреннего противоречия
  («без надписей, НО включи заголовок»): свои надписи не добавляем, короткий
  заголовок владельца задаёт сам стиль. Слепок прежнего канона (F6) —
...
## SUMMARY_L1_CLUSTERIZER_SYSTEM_PROMPT (раунд 10.26, S3/ADR-1026-5 D2/D4; канон L1 «Кластеризатор» §94–§95; источник — services/summary_prompts.py)

Отдельный канон L1 (Conversation Disentanglement): темы, переплетённые разговоры,
хронология, проверяемые факты с evidence, строгий JSON §95; L1 НЕ пишет саммари.
Служебные поля `response_mode`/`cover_prompt` — того же JSON (AMEND ADR-1023-3/-6,
эмитент — L1; нормализаторы/обрезка/§104 без изменений). Слепок базы канона S3
(без общего блока маркировки целевого сообщения, прецедент
`PREV_SUMMARY_EDITOR_R1023`) — `PREV_SUMMARY_L1_CLUSTERIZER_R1026`; ступень
`PROMPT_MIGRATIONS` (правило маркировки) + документированный
`ROLLBACK_MIGRATIONS` (`services/prompt_migrations.py`); PG-ключ
`prompts.summary_l1_clusterizer_system_prompt` (Δ каталога +1, санкция D4); в живой
путь L1 не врезан (врезка — S5/S6 по санкции).

SUMMARY_L1_CLUSTERIZER_SYSTEM_PROMPT = """СИСТЕМНАЯ РОЛЬ:
...
Если в истории чата есть сообщение с пометкой [ЭТО ТВОЯ ТЕКУЩАЯ КОМАНДА] (перед пометкой может стоять <<<), считай его инструкцией от пользователя. Это НЕ событие диалога: не пересказывай этот запрос как реплику чата, не упоминай его в выжимке и не разбирай как часть беседы. Просто выполни написанное."""


## SUMMARY_L2_WRITER_SYSTEM_PROMPT (раунд 10.26, S5/ADR-1026-7 D1/D4; канон L2 «Писатель» §97–§99; источник — services/summary_prompts.py)

Отдельный канон L2 (Писатель статьи): качественная связная статья по **пакету
фактов §96** (темы/хронология/факты/фрагменты), БЕЗ стиля повседневной переписки
(никакого сленга/беглой речи/бессвязного потока/мемов — §97); умеренная ирония
допустима, приоритет — точность; структура §98 (H1, вступление, события
отдельными абзацами, логичный порядок, необязательное заключение, ≤1 жирный
акцент на абзац); запрет выдуманных цитат и приписанных реплик. Выход — строгий
JSON-документ §99 `{schema_version:1, title, paragraphs:[{text, emphasis|null}]}`;
L2 **не** форматирует HTML (это код-форматтер
`services/summary_article_formatter.py`). Слепок базы канона S5 — PREV — ступень
идемпотентной `PROMPT_MIGRATIONS` и документированный `ROLLBACK_MIGRATIONS`
(`services/prompt_migrations.py`); PG-ключ
`prompts.summary_l2_writer_system_prompt` (Δ каталога +1, санкция D3); врезка —
за kill-switch `SUMMARY_HYBRID_L2_ENABLED` (default OFF, S5), слот `SUMMARY_L2_*`
— env-only. Рассказчик/Редактор/`digest`/`system2_handoff` — REUSE без изменений.

SUMMARY_L2_WRITER_SYSTEM_PROMPT = """СИСТЕМНАЯ РОЛЬ:
