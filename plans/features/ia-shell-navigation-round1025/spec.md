...
- **Объём OFF (M-2 Scanner):** откату подлежит **навигация/шелл** только. Layout «Справки» (оглавление/поиск/якоря), мобильный редактор «Доступов» и label «Аналитика» **не откатываются** — осознанный nav-scope D5/§6.4, «мёртвых» состояний нет.
...
> **Происхождение baseline-фикстуры (review F1):** снимок снят со `services/param_catalog.py` на ветке F1; поскольку F1 меняет **только** nav-метаданные (`NAV_ORDER`/`NAV_TITLES`/`TAB_NAV`), а `REGISTRY`/`GROUPS`/`TAB_RULES` не трогает, замороженные множества **идентичны** до-правковому (F0/F8) состоянию — фикстура легитимно фиксирует неизменный каталог (проверка равенством множеств, не размеров). Происхождение зафиксировано и в самом файле (`_provenance`).
