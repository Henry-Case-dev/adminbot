...
> **Статус: 🔴 REOPENED** (17.09.2026). Эпик **10.20** «Летописец + RAG + UI» возвращён владельцем на доработку
> после **визуальной инспекции мини-аппа (скриншоты)**: **бэкенд — ок**, **БЛОК 3 (UX/UI) провален почти полностью**.
> Предыдущий отчёт Оркестратора о готовности UI признан **галлюцинацией**; @Reviewer получил **выговор** за пропуск.
...
- [ ] `.modal`/`.modal-card`, `.card`, панели (`.oversight-panel`, `.hub-card`, `.module-card`, `.prov-block`, `details.advanced`, `.scope-panel`) → фон `rgba(20,25,30,0.5)` **вместе** с `backdrop-filter: blur(16px)` и `-webkit-backdrop-filter: blur(16px)`.
- [ ] Никакой класс/правило (`card-solid`, `--surface-glass`) не должен «съедать» стекло в модалках и карточках.
- [ ] Фон/бордер **снимаются** с grid-контейнеров (`.module-list`, `.hub-grid`) — стекло живёт на карточках, а не на их обёртке.
- [ ] Фолбэки сохраняются: `@supports not (backdrop-filter)` → плотный фон; `prefers-reduced-motion`/`prefers-contrast` — как есть.

**Дефект 2 (CSS Grid):**
- [ ] `.module-list` и `.hub-grid` → строго `display:grid; grid-template-columns: repeat(auto-fit, minmax(320px,1fr)); gap:1rem;` (заполняют ширину; центровка сохраняется).
- [ ] Убедиться, что Tailwind-утилиты родителя (`grid-cols-1`) и порядок подключения CSS не ломают треки.

**Дефект 3 (binding / маска секрета — CRITICAL):**
- [ ] Единый sentinel `SECRET_MASK = '••••••••••••'`; `configured:true` → инпут показывает маску; `null`/`configured:false` → пусто.
- [ ] Покрыть **все** пути: provider-блоки (`blockFieldValue`), модалку модуля (basic + advanced), generic config (keys-шаблоны `index.html:544`, `632`).
- [ ] Маска **никогда** не отправляется на сервер (`saveBlock`/`saveKeyItem`) и не становится `baseline`/`draft`.
- [ ] Поля Betterstack SQL (user + password) — проверены отдельно (эталон кейса владельца).

**Дефект 4 (градиент):**
- [ ] `animation-duration` градиента ∈ **5–8s**; в палитре — **оранжевый** рядом с сиреневым и циан/бирюзовым; углы настроены так, чтобы переливы были **явно видны** (не «еле заметная подсветка»).
- [ ] Градиент остаётся фоном (не под текстом напрямую) и не ухудшает контраст WCAG AA.

**Дефект 5 (sticky-save):**
- [ ] Панель прилипает к низу скролл-контейнера, не перекрывает последний элемент/контент, не ломает padding/margin модалки, учитывает `safe-area-inset-bottom`.
- [ ] Сохраняется функциональность: `<sticky-save>` — единственный способ сохранения на config-вкладках (регресс S10.20-1 не вернуть).

**Процессное (обязательное):**
- [ ] Тесты — **DOM/CSS-ассерты** (каскад/вычисленные значения/поведение JS), а не «строка есть в файле».
- [ ] @Reviewer проверяет **фактический рендер**: computed-стили, классы в DOM, значения инпутов, отсутствие перекрытия.
...
- [ ] **Дефект 1:** каскад-резолвер (T-1939) показывает итоговый фон модалки/карточки = `rgba(20,25,30,0.5)` и `backdrop-filter`/`-webkit-backdrop-filter` = `blur(16px)`; ни один `.modal-card` не нейтрализован `.card-solid`/`--surface-glass`; grid-контейнеры без фона.
- [ ] **Дефект 2:** `.module-list`/`.hub-grid` содержат точную тройку grid-правил; при ширине ≥ 1000px — **≥ 2 трека** (замер `gridTemplateColumns`).
- [ ] **Дефект 3:** для `{configured:true}` во **всех** модулях инпут показывает `••••••••••••`; пусто только при `null`; маска **не** уходит на сервер (тест); поля `Пароль/Пользователь Betterstack SQL` подтверждены.
- [ ] **Дефект 4:** `animation-duration` ∈ [5s, 8s]; оранжевый присутствует в палитре градиента; углы/видимость подтверждены; `prefers-reduced-motion`/`prefers-contrast` работают.
- [ ] **Дефект 5:** панель `position:sticky; bottom:0`, контент не перекрыт (offset-замер + визуально), отступы модалки не сломаны, safe-area учтён.
- [ ] **Тесты red→green:** новые тесты (T-1939) **падают до фикса** и зелёные после (red-прогон зафиксирован в отчёте) — исключает ложную приёмку.
- [ ] Полный `pytest` — **0 failed**; JS-гейты (`node --check web/app.js`, `JS-UNIT-OK`, `VUE-MOUNT-OK`) чистые; `git diff --check` clean.
- [ ] **Меню/навигация не изменены** (снимок навигации 25 вкладок + navbar + 12 карточек модулей зелёный).
- [ ] **R18-скан** `plans/**` чист (секретов нет; значения не цитируются).
- [ ] **Фактический рендер подтверждён @Reviewer** (верификационный отчёт с сырыми выводами CSSOM-зондов + served-CSS) и **независимо @Scanner**; доказательство вида «класс где-то есть» **не принимается**.
- [ ] **Деплой** @DevOps выполнен; на прод-URL подтверждено, что отдаётся новый CSS (без секретов в выводе), бот `systemctl active`.

## 5. Чек-лист задач

### T-1933 (@Architect, гейт) — UI-контракт доработки (класс-карта + каскад + sentinel маски)

- [ ] Зафиксировать **класс-карту**: какой DOM-узел обязан нести glass-фон (`modal-card`, `card`, `hub-card`, `module-card`, `oversight-panel`, `prov-block`, `details.advanced`, `scope-panel`), а какой — **нет** (grid-контейнеры `.module-list`/`.hub-grid`).
- [ ] Зафиксировать **каскадный контракт**: порядок `vendor/tailwind.css` → `app.css`; правило, кто выигрывает у `.card-solid`/`--surface-glass`; решение «убрать `card-solid` из разметки **или** явный override `.modal-card.card-solid`» (одно, не оба).
- [ ] Зафиксировать **sentinel маски** `SECRET_MASK = '••••••••••••'` и инвариант «маска никогда не сохраняется и не становится baseline/draft».
- [ ] Токены/значения: `--glass-bg: rgba(20,25,30,.5)`, `--glass-blur: blur(16px)`; grid-тройка; градиент (5–8s + оранжевый токен + углы); sticky-save (отступы + safe-area).
- [ ] Артефакт: `plans/features/round1020-ui-rework/ui-contract.md` (+ при необходимости правки `spec.md`). **Зависит:** ничего. **Гейт для T-1934…T-1938.**
...
- [ ] Пуш + деплой по протоколу (`git pull`, `systemctl restart admin_bot`, `systemctl status admin_bot`); секреты в репо/выводе **не хранить** (R17/R18).
- [ ] Прод-проверка: бот `active`; отдаётся **новый** CSS (проверка без печати секретов); UI-страница открывается.
- [ ] Зафиксировать в отчёте: коммит, статус сервиса, подтверждение доставки CSS/`APP_VERSION`.
...
- **Статус для @Orchestrator:** 🔴 **REOPENED** — **никаких отчётов о готовности до реального фикса**. Критерий выхода:
