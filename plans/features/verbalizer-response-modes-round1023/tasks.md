...
- [x] **T-2186** [@Builder] **Тесты «Markdown разрешён в rich-канале»:** `FORMAT_RICH_BLOCK` подключён для Article; `plain_no_tables` не активен; таблицы/сетки не бракуются и сохраняются в `InputRichMessage`. *(UPD review iter1: тест КОНТРАКТНЫЙ — проверяет хелперы `compose_verbalizer_system(..., "rich")`/`channel_enabled_rules("rich", …)`; сквозная `sendRichMessage`-интеграция — зона F6.)*
- [x] **T-2187** [@Builder] *(review iter1)* H1: снятие конфликта «буллиты запрещены/обязательны» в composed `deep_research` (`_DEEP_RESEARCH_OVERRIDES`) + тест состава. H2: text-only plain для саммари/фактчека + очистка `<b>` (`strip_lore_html`) + тесты финальной доставки; direct `deep_research` — HTML-capable plain. M1: флаг гейтит маршрут доставки; M2/M5: `plain_no_tables` через `detect_plain_tables`; M3/L4: фиксация в spec/ADR. *(review iter2: контекстная детекция таблиц — одиночный `|` не бракуется; таблица = `<table`/ASCII/separator/≥2 подряд pipe-строк.)*
