...
- **Воспроизведено @Reviewer:** pytest `.venv` **8890/0** (115.51 s); S7-файл **35/35**; JS **45/45**; `git diff --check`=0; каталог **469/426/444/100/98/21**, `APP_VERSION` **2.58.26**; R18 OK; **Δ DDL=0**; `PUBLISH_*`=0; `routes.py`/`param_catalog.py`/`image_generation.py`/`telegram_send.py`/`summary_xml.py`/`summary_article_formatter.py`/`db/**` вне diff; маркер-тесты не ослаблены (в tests — только пины версии + регистрация JS-теста).
- **[B-R1026S7-2] Medium (блокирующая, requirement gap §109 / SC-07) — RESOLVED ✅:** `services/summary_run_log.py:71-93` — `attempts_of()` сначала R17-safe разбор текста (`_ATTEMPTS_RE`: `after N attempts?` / `attempts=N`), затем fallback `exc.attempts`, затем `None`. Независимая проба @Reviewer на реальных классах `llm_client` без атрибута: `LLMRateLimitError`→3/429, `LLMServerError`→3/502, `LLMTimeoutError`→2/None, `LLMTransportError`→4; fallback→4; приоритет текста над атрибутом→5; без текста/атрибута→None; в лог уходит только число (сырой текст/URL не текут). Пины без искусственного атрибута: `test_attempts_of_real_llm_texts`, `test_llm_failure_summary_failed_http` (`assert not hasattr(exc,"attempts")` → `attempts=3`/`http_status=429`/`error_type=LLMRateLimitError`), L1 502/3, L2 429/3.
- **[L-R1026S7-3] Low (R17, попутно) — RESOLVED ✅:** `services/summary_test_run.py` `SUMMARY_TEST_FORMAT_ERROR` без `exc_info`, +`error_type`/`reason=formatter_error`; пин `test_dry_run_format_error_test_code` (`rec.exc_info is None`, сырой текст не течёт).
- **OPEN Low/Info (non-blocking):** L-R1026S7-1 (`FILTER_ERROR` `exc_info=True`, pre-existing S1; строка вне S7-diff — подтверждено) → S8/хотфикс; I-R1026S7-1 (машинный чекпоинт `plans/workflow_state.md` устарел) → @Orchestrator.
- **Недоступно:** live-приёмка владельца (Telegram/WebView) — PENDING OWNER VERIFICATION; deploy T-3408 не выполнялся (вне Step 5); baseline 8854/0 и JS 44/44 не перемерялись (worktree занят).
- **Handoff:** @Orchestrator → **APPROVED**; далее T-3404 @Scanner (по регламенту) / T-3406 merge / T-3408 deploy; live — PENDING OWNER.

## Эпик 2 / S7 `summary-logging-runid-round1026` (T-3403, Step 5 @Reviewer, **ИТЕРАЦИЯ 2 — rework T-3405**) — 24.09.2026 — **CHANGES REQUESTED (C0/H0; 1 блокирующая Medium B-R1026S7-2; B-R1026S7-1/L-R1026S7-2 закрыты; live — PENDING OWNER VERIFICATION)**

- **База:** HEAD `f774ecc6af823c4191dd5f833fa2cce1d0a5bad2` (== `origin/master` == тег `pre-round1026-s7`); правки **НЕ закоммичены** (34 M + 4 ??). Отчёт: `plans/features/summary-logging-runid-round1026/review.md`. Working-Tree-Hash `41142927dcc5d39879c1a4b9cf6747947ed4416f8531b85e17e217b1e2c578db` (рецепт — в review.md; diff-SHA `684a7c3f…`); Spec-Hash `d9d11797fbc1e290825a53053ac90d7a49b16af324375305506cae54b74a9ac0` (не менялась).
- **Воспроизведено @Reviewer:** pytest `.venv` **8887/0** (118.78 s); новый файл **32/32**; legacy `test_token_analytics_round1023` **46/46**; JS **45/45**; `git diff --check`=0; каталог **469/426/444/100/98/21**, `APP_VERSION` **2.58.26**; R18 OK; **Δ DDL=0**; `PUBLISH_*`=0 (grep+тест); `routes.py`/`param_catalog.py`/`image_generation.py`/`telegram_send.py`/`summary_xml.py`/`summary_article_formatter.py`/`db/**` вне diff; маркер-тесты не ослаблены (в tests — только пины версии).
- **[B-R1026S7-1] Medium (итер.1) — RESOLVED ✅:** `summary_generator.py:372-380` — `ctx.model`/`ctx.provider` заполняются на OFF-пути best-effort (`getattr(self, "llm", None)`), ON-ветка сохранена. Проба @Reviewer: `SUMMARY_FAILED | stage=run | model=model-x | provider=api.example.com | http_status=429 | error_type=LLMRateLimitError | reason=LLMRateLimitError | attempts=-`; ключ не течёт; `SUMMARY_COMPLETE` не эмитится; legacy `object.__new__` не падает.
- **[L-R1026S7-2] Low (итер.1) — RESOLVED ✅:** `web/app.js:9566-9581` — OFF сохраняет ручной уровень (`logLevel === 'INFO'`); JS-кейс ctx3 нетривиален. **[I-R1026S7-2] — RESOLVED ✅** (T-3402 ✅).
- **[B-R1026S7-2] Medium (requirement gap §109 / приёмочный инвариант 5 / SC-07, BLOCKING) — OPEN:** `services/summary_run_log.py:65-71` (`attempts_of`) читает только атрибут `exc.attempts`; реальные исключения `llm_client` атрибута не имеют — число попыток только в тексте («…after 3 attempts…», `llm_client.py:751-753,767-770,725-729`), при этом `http_status_of` текст парсит. Проба @Reviewer на реальном `LLMRateLimitError("LLM rate limited (429) after 3 attempts: …")`: `SUMMARY_FAILED | … | attempts=-` (http_status=429). Пин `test_llm_failure_summary_failed_http` зелёный лишь за счёт искусственного `exc.attempts = 3` на моке → **ложное подтверждение SC-07**; в проде «Количество попыток» (§109 verbatim) не выводится (то же `L1_ERROR`/`L2_ERROR`). **Fix:** R17-safe разбор `after (\d+) attempts` в `attempts_of()` (fallback на атрибут) + пин на реальный текст `LLMRateLimitError`/`LLMServerError` без атрибута; повторный прогон. **Verify:** OFF-проба + полный pytest/JS.
- **Low/Info OPEN (non-blocking):** L-R1026S7-1 (`FILTER_ERROR` `exc_info=True` — pre-existing вне diff), **L-R1026S7-3 (new, Low, pre-existing)**: `services/summary_test_run.py:653-657` `SUMMARY_TEST_FORMAT_ERROR` сохраняет `exc_info=True` (T-3400 покрыл только `_execute`/чтение окна; тест-контур) — R17-хардненинг follow-up; I-R1026S7-1 (чекпоинт `plans/workflow_state.md` устарел → @Orchestrator).
- **Недоступно:** live-приёмка владельца — PENDING OWNER VERIFICATION; deploy T-3408 не выполнялся; baseline 8854/0 и JS 44/44 не перемерялись (дельта +33/+1 сходится).
- **Handoff:** @Orchestrator → **CHANGES REQUESTED**; B-R1026S7-2 → @Builder; после фикса — точечный re-review T-3403; T-3404 @Scanner — по регламенту.

## Эпик 2 / S7 `summary-logging-runid-round1026` (T-3403, Step 5 @Reviewer, ИТЕРАЦИЯ 1) — 24.09.2026 — **CHANGES REQUESTED (C0/H0; 1 блокирующая Medium B-R1026S7-1; live — PENDING OWNER VERIFICATION)**

- **База:** HEAD `f774ecc6af823c4191dd5f833fa2cce1d0a5bad2` == annotated-тег `pre-round1026-s7` (`bd7c822`→`f774ecc`); правки **НЕ закоммичены** (34 M + 4 ??). Отчёт: `plans/features/summary-logging-runid-round1026/review.md`. Working-Tree-Hash `cac7b07c4c7a7b43c99d167c204398267a986f3a23807832ea4febc6d679dd07`; Spec-Hash `d9d11797fbc1e290825a53053ac90d7a49b16af324375305506cae54b74a9ac0`.
- **Воспроизведено @Reviewer:** pytest `.venv` **8887/0** (114.07 s); новый файл 32/32; JS **45/45**; `git diff --check`=0; каталог импортом **469/426/444/100/98/21**, `APP_VERSION` **2.58.26**; R18 OK; **Δ DDL=0**; `PUBLISH_*`=0 (grep+тест); `routes.py`/`param_catalog.py`/`image_generation.py`/`telegram_send.py`/`summary_xml.py`/`summary_article_formatter.py`/`db/**` вне diff.
- **[B-R1026S7-1] Medium (requirement gap §109 / приёмочный инвариант 5, BLOCKING) — OPEN:** `services/summary_generator.py:602-605` — `ctx.model`/`ctx.provider` заполняются только в `_run_hybrid_l2`; на **OFF-пути (default, hybrid GATED)** `SUMMARY_FAILED` эмитится с `model=- | provider=-`, хотя §109/инвариант 5 требуют модель/провайдер при ошибке; OFF-путь — единственная error-поверхность (L1/L2-событий там нет). Воспроизведено (`--log-cli-level=INFO`): `SUMMARY_FAILED | run_id=RID-S7-0001 | chat_id=-1001 | stage=run | model=- | provider=- | http_status=429 | error_type=LLMError | reason=LLMError | attempts=- | duration_ms=1`. **Fix:** заполнять ctx.model/ctx.provider из `self.llm._chat_model`/`_base_url` в `_run` (как в ON-ветке) + пин в `test_llm_failure_summary_failed_http`. **Verify:** тест OFF-пути + полный pytest.
- **Low/Info OPEN (non-blocking):** L-R1026S7-1 (`FILTER_ERROR` `exc_info=True` — pre-existing вне diff, R17-хардненинг follow-up), L-R1026S7-2 (UI: ручная смена `logLevel` при активном чипе → OFF вернёт «до-чиповый» уровень), I-R1026S7-1 (`plans/workflow_state.md` чекпоинт устарел), I-R1026S7-2 (T-3402 `[ ]` при фактически выполненных границах).
- **Недоступно:** live-приёмка владельца — PENDING OWNER VERIFICATION; deploy T-3408 не выполнялся; baseline pytest 8854/0 на `f774ecc` не перемерялся (worktree занят; дельта +33 сходится).
- **Handoff:** @Orchestrator → **CHANGES REQUESTED**; B-R1026S7-1 → @Builder T-3405; после фикса — re-review T-3403; T-3404 @Scanner — по регламенту.
...
- **База:** HEAD `cc6105c` == annotated-тег `pre-round1026-s9^{commit}` (`for-each-ref` tag `9f4305a` → `git rev-list -n 1` = `cc6105c`); правки **НЕ закоммичены**. Отчёт: `plans/reports/round1026_s9_scanner_audit.md` (итерация 2; итерация 1 — историческая запись там же).
- **Воспроизведено @Scanner (итерация 2, свои пробы):** pytest `.venv` **8854/0** (120.75 s); JS **44/44** + `node --check` OK; свой шпион-прогон `run_summary_test` → **0** публикаций/памяти/`generate_image(_verbose)`, `await_count==2` (`l1_clusterizer`,`l2_writer`), `SUMMARY_HYBRID_L2_ENABLED` до==после, hybrid не вызван; каталог импортом **469/426/444/100/98/21**; `APP_VERSION` 2.58.25; `SUMMARY_TEST_UI_ENABLED=True`; `git diff --check`=0; R18 OK; `summary_generator.py` **55+/0−**; `routes.py`/`image_generation.py`/`telegram_send.py`/`param_catalog.py`/`database.py`/`pg_db.py` вне diff; **Δ DDL=0**.
- **Контракт error-payload (4 пути) — RESOLVED:** `no-generator`/`error_result`/`running`/`window-read-error` → `present_result` даёт полные `metrics`+`artifacts`+`display`, `metrics.tokens.l1` присутствует, `artifacts.source==[]`, диагностика ≥1; нет `undefined.l1`. UI-guards (`v-if="summaryTestMetrics"/"summaryTestArtifacts"`) на error/частичном payload → `null` (без падения), на полном → **пропускают** (успешные секции не скрыты) — JS-блок (g).
- **[B-R1026S9-1] Medium (§112 «Процент отсева») — RESOLVED:** `data-summary-test-drop` + computed `summaryTestDropPercent` (`null`/`undefined`/`''` → «Нет данных»; `12.5` → «12.5%»); JS-блок (h).
- **[B-R1026S9-2] High / [S-R1026S9-1] Medium (UI-render) — RESOLVED** (см. контракт выше). **[S-R1026S9-2] Low (== L-R1026S9-3, purge running) — RESOLVED** (пробы: KEPT после TTL; сохранён при эвикции). **[S-R1026S9-4] Low (== L-R1026S9-5, fail-open метрик) — RESOLVED** (проба: `{}`/0). **[S-R1026S9-3] Low (== L-R1026S9-4, spec/README 503) — RESOLVED** для spec §5.2/README. **[L-R1026S9-1] Low (R18 docs) — RESOLVED.**
- **[S-R1026S9-5] Low (contract/docs) — NEW, OPEN (non-blocking):** `services/summary_test_run.py:489` docstring и `adr-1026-8` D4 всё ещё указывают `503 (TEST_NO_GENERATOR)`; реализация + spec §5.2 — `202`/`200`+`status="error"`. Fix: убрать `503` из docstring + ADR. Non-blocking.
- **Low/Info OPEN (non-blocking, owned follow-up):** L-R1026S9-2 (5 пред-существующих падений @ global `py -3`/aiogram 3.29.1 — воспроизведены на base, не регресс S9), L-R1026S9-6 (`_filter_metrics` shared state), L-R1026S9-7 (`window_hours` clamp vs 422), L-R1026S9-8 (R17 `exc_info=True` → S7), I-R1026S9-1 (`has_more` — эвристика), I-R1026S9-2.
- **Новых Critical/High нет.** Low-фиксы регрессий не дали (retention/fail-open пробы + полный pytest 8854/0). Живой ON-путь (S6/S10) — GATED; live-приёмка владельца — PENDING OWNER VERIFICATION.
- **Handoff:** @Orchestrator → **SCANNED — к деплою ДА**. Low/Info — owned follow-up.

## Эпик 2 / S9 `summary-testing-ui-round1026` (T-3373, Step 5 @Reviewer, ИТЕРАЦИЯ 2) — 23.09.2026 — **APPROVED (оба блокера закрыты, подтверждено независимо; остаток — Low/Info non-blocking)**

- **База:** HEAD `cc6105c` == annotated-тег `pre-round1026-s9^{commit}` (`git cat-file -p` = `cc6105c`); правки **НЕ закоммичены** (33 M + 5 ??). Отчёт: `plans/features/summary-testing-ui-round1026/review.md`.
- **Воспроизведённые числа @Reviewer (итерация 2):** pytest `.venv` **8854/0** (116.6 s), JS **44/44** + `node --check` OK, каталог **469/426/444/100/98/21**, `APP_VERSION` 2.58.25, `git diff --check`=0, `summary_generator.py` **55+/0−**, **Δ DDL=0** (`database.py`/`pg_db.py`/`param_catalog.py`/`routes.py` вне diff), R18 OK; маркер-тесты не ослаблены (только bump версии).
- **[B-R1026S9-2] High (UI-render) — RESOLVED ✅.** `_base_result` всегда отдаёт полные `metrics/artifacts/display`; `present_result` нормализует (merge); `error_result`+`empty_payload`; API `_result_payload`/`_execute` отдают контракт-валидный payload с диагностикой; `index.html` guard `v-if="summaryTestMetrics"/"summaryTestArtifacts"`; `app.js` computed-guard. Тесты: `test_no_generator_error_payload_contract`, `test_error_result_has_diagnostics_and_full_structures`, `test_empty_payload_running_contract`, `test_run_no_generator_still_polls_error`, `test_execute_exception_payload_contract`, JS (g). Диагностика видна (guard структурный, ошибки не маскирует).
- **[B-R1026S9-1] Medium (§112 «Процент отсева») — RESOLVED ✅.** `index.html:491` `data-summary-test-drop`; `app.js` computed `summaryTestDropPercent` (null/undefined/'' → «Нет данных»); JS (h) нетривиален (null→«Нет данных», 12.5→«12.5%»).
- **[L-R1026S9-1] RESOLVED ✅** (тег → `cc6105c`). **[L-R1026S9-3] RESOLVED ✅** (`_purge`/эвикция исключают running; `test_store_ttl_keeps_running`/`test_store_eviction_keeps_running`). **[L-R1026S9-4] RESOLVED ✅** (spec §5.2 без 503, §7 `TEST_RUN_FAILED`, README-оговорка об обложке). **[L-R1026S9-5] RESOLVED ✅** (reset `_filter_metrics` в `build_test_rows` + тест).
- **OPEN Low/Info (non-blocking):** [L-R1026S9-2] (5 пред-существующих падений @ global `py -3`, не регресс S9), [L-R1026S9-6] (write в `_filter_metrics`), [L-R1026S9-7] (кламп вместо 422), [L-R1026S9-8] (`exc_info=True`), [I-R1026S9-1] (`has_more` — эвристика).
- **Handoff:** @Orchestrator → **APPROVED**; далее @Scanner T-3374 (повторный независимый аудит) и @DevOps T-3378; live — PENDING OWNER VERIFICATION.

## Эпик 2 / S9 `summary-testing-ui-round1026` (T-3373, Step 5 @Reviewer, ИТЕРАЦИЯ 1) — 23.09.2026 — **CHANGES REQUESTED (2 блокирующих: High UI-краш error-результатов + Medium §112 «Процент отсева»; live — PENDING OWNER VERIFICATION)**

- **База:** HEAD `cc6105c` == annotated-тег `pre-round1026-s9^{commit}` (`git rev-parse` = `cc6105c`); правки **НЕ закоммичены** (33 M + 5 ??). Отчёт: `plans/features/summary-testing-ui-round1026/review.md`.
- **Воспроизведённые числа @Reviewer:** pytest `.venv` **8847/0** (120.0 s), JS **44/44** + `node --check` OK, каталог импортом **469/426/444/100/98/21**, `git diff --check`=0, `summary_generator.py` **51+/0−**, **Δ DDL=0**, `APP_VERSION` 2.58.25, R18 OK.
- **[B-R1026S9-2] High (UI-render, BLOCKING) — OPEN, подтверждён независимо (совпадает с M-R1026S9-1 @Scanner):** `web/index.html:490` (`metrics.tokens.l1.input`)/`:511` (`artifacts.source.length`) без guard; error-пути (`TEST_NO_GENERATOR`, ошибка чтения окна, исключение в `_execute`) → `metrics={}`/`artifacts={}` или payload вовсе без них → **TypeError в render Vue**, диагностика §5.3/§7 не видна. Проба: `present_result(_base_result(status="error"))` → `metrics={}`, `artifacts={}`. **Fix:** guard в HTML и/или пустые `metrics`/`artifacts`/`display` из `_result_payload`. **Verify:** JS-маркер рендера error-payload + re-review.
- **[B-R1026S9-1] Medium (requirement gap) — OPEN:** `web/index.html` L484–497 (`data-summary-test-metrics`) не рендерит `metrics.drop_percent` (§112 «Процент отсева»; tasks инвариант 8; REQ-S9-09). `Select-String "отсев|drop_percent" web/index.html web/app.js` → 0; API метрику отдаёт. **Fix:** строка «Процент отсева: …» (null → «Нет данных»).
- **[L-R1026S9-1] Low (docs):** `evidence.md`/`tasks.md` T-3345 заявляют тег → `ae5a147`; фактически → `cc6105c`. OPEN.
- **[L-R1026S9-2] Low (env/evidence):** на глобальном `py -3` (aiogram 3.29.1) — 5 пред-существующих падений rich-media; **воспроизведены идентично на base-worktree `cc6105c`** → не регресс S9; `.venv` (3.31.0) — 0. OPEN.
- **[L-R1026S9-3] Low (robustness):** `_RunStore._purge`/эвикция не исключают `status=="running"` (прогон >15 мин вычищается; возможен повторный параллельный запуск). OPEN.
- **[L-R1026S9-4] Low (contract/docs):** spec §5.2 обещает 503 — реализация 202+`status="error"`; README/ADR «0 `generate_image`» без оговорки про подтверждение обложки. OPEN.
- **[L-R1026S9-5] Low (metrics):** fail-open `_apply_filter` не пишет `_filter_metrics` → возможны устаревшие `status`/`restored_count`. OPEN.
- **[L-R1026S9-6] Low (shared state):** dry-run пишет `SummaryGenerator._filter_metrics[chat_id]`; живых читателей нет. OPEN.
- **[L-R1026S9-7] Low (API):** `window_hours` клампится (1..720) вместо 422 — spec §5.2 vs §4.2. OPEN.
- **[L-R1026S9-8] Low (R17):** catch-all `exc_info=True` в `_execute`/чтении окна — пересмотреть с S7. OPEN.
- **Подтверждено ✅:** 0 публикаций/0 памяти/0 `generate_image` (шпионы); ON per-run ровно 2 вызова (`await_count==2`), флаг/`_hybrid_l2_enabled` не читаются; `_run`/`_run_hybrid_l2` байт-в-байт (0−); асинхронный 202+polling + in-memory TTL/≤20; global admin; флаг default ON + hot-OFF false → 404+скрытие; fail-closed-коды `TEST_*`; S6 GATED; Δ DDL=0; Δ каталога=0; CSP/zero-build; R17/R18; bump 2.58.25; маркер-тесты не ослаблены.
- **Handoff:** @Orchestrator → **CHANGES REQUESTED**; @Builder — закрыть [B-R1026S9-2] и [B-R1026S9-1] → повторный прогон pytest+JS → re-review. Live — PENDING OWNER VERIFICATION.

## Эпик 2 / S9 `summary-testing-ui-round1026` (T-3374, Step 6 @Scanner) — 23.09.2026 — **NOT READY — 2 блокирующих (B-R1026S9-1 Medium «Процент отсева» + UI-render error-результатов: S-R1026S9-1 Medium, эскалирован @Reviewer в B-R1026S9-2 High); C0/H0; dry-run 0/0/0 подтверждён**

- **База:** HEAD `cc6105c`; правки **НЕ закоммичены** (33 M + 5 ?? кода/тестов + каталог фичи). Отчёт: `plans/reports/round1026_s9_scanner_audit.md`.
- **ID @Scanner — `S-R1026S9-*`** (переименовано из исходных `M-R1026S9-1`/`L-R1026S9-1..4` во избежание конфликта с новыми `B-R1026S9-1..2`/`L-R1026S9-1..8` @Reviewer). **Карта соответствия:** `S-R1026S9-1` == `M-R1026S9-1 @Scanner` (эскалирован @Reviewer → `B-R1026S9-2 High`); `S-R1026S9-2` == `L-R1026S9-1 @Scanner` (== `L-R1026S9-3 @Reviewer`); `S-R1026S9-3` == `L-R1026S9-2 @Scanner` (== `L-R1026S9-4 @Reviewer`); `S-R1026S9-4` == `L-R1026S9-3 @Scanner` (== `L-R1026S9-5 @Reviewer`); `I-R1026S9-2` == `L-R1026S9-8 @Reviewer`.
- **Воспроизведено @Scanner:** pytest `.venv` **8847/0** (119.8 s); JS **44/44** + `node --check web/app.js` OK; независимый шпион-прогон `run_summary_test` → **0** `send_text`/`send_rich_message`/`send_photo`/`build_cover_media`/`generate_image(_verbose)`/`summary_memory.*`, `await_count==2` (`l1_clusterizer`,`l2_writer`), `SUMMARY_HYBRID_L2_ENABLED` до==после, `memory.writes==0`; каталог **469/426/444/100/98/21**; `APP_VERSION` 2.58.25; `git diff --check`=0; R18 (annotated-тег `pre-round1026-s9`→`cc6105c`, `stash@{0}`).
- **[B-R1026S9-1] Medium (requirement gap, BLOCKING) — @Reviewer Step 5, независимо подтверждён @Scanner (OPEN):** §112 «Процент отсева» не рендерится; `Select-String "отсев|drop_percent" web/index.html web/app.js` → **0**, при том что `_metrics` возвращает `drop_percent` и API его отдаёт. Fix: строка «Процент отсева: …» (`null` → «Нет данных»).
- **[S-R1026S9-1] Medium (UI-render, BLOCKING; == `M-R1026S9-1 @Scanner`, эскалирован @Reviewer в `B-R1026S9-2 High`):** `web/index.html:490` (`summaryTest.result.metrics.tokens.l1.input`), `:511` (`artifacts.source.length`) — без guard; error-пути (`TEST_NO_GENERATOR`, ошибка чтения окна, исключение в `_execute`) отдают результат без `metrics`/`artifacts` → TypeError в render Vue, диагностика §5.3/§7 не отображается. Проба: `present_result(_base_result(status="error"))` → `metrics={}`, `artifacts={}`. Fix: guard в HTML и/или всегда отдавать пустые `metrics`/`artifacts` из `_base_result`.
- **[S-R1026S9-2] Low (== `L-R1026S9-1 @Scanner` == `L-R1026S9-3 @Reviewer`):** `web/api/summary_test.py:84-102` — `_purge`/эвикция не исключают running → прогон >15 мин вычищается (404 + повторный параллельный запуск возможен). **[S-R1026S9-3] Low (== `L-R1026S9-2 @Scanner` == `L-R1026S9-4 @Reviewer`):** spec §5.2 обещает 503 для `TEST_NO_GENERATOR` (реализация — 202/200/error); README/ADR «0 `generate_image`» без оговорки про подтверждение обложки. **[S-R1026S9-4] Low (== `L-R1026S9-3 @Scanner` == `L-R1026S9-5 @Reviewer`):** fail-open `_apply_filter` не пишет `_filter_metrics` → устаревшие `status`/`restored_count` в тест-отчёте. **Info:** I-R1026S9-1 (`has_more` — эвристика), I-R1026S9-2 (== `L-R1026S9-8 @Reviewer`: `exc_info=True` в WARNING-логах).
- **Инварианты OK:** Δ DDL=0 (`database.py`/`pg_db.py` вне diff); Δ каталога=0; `routes.py`/`image_generation.py`/`telegram_send.py`/`param_catalog.py` вне diff; `summary_generator.py` = 51+/0− (аддитивно); ON-путь живого пайплайна не активируется; CSP/zero-build (0 CDN/inline/eval; `v-html` предпросмотров экранирован сервером); флаг default ON, OFF → 404 + скрытие секции; §112 «Нет данных»/«Без лимита»; маркер-тесты не ослаблены (только bump).
- **Handoff:** @Orchestrator → **NOT READY** (блокируют B-R1026S9-1 + S-R1026S9-1/B-R1026S9-2; возврат в @Builder → повторная проверка @Reviewer + @Scanner). Low/Info — owned follow-up.

## Эпик 2 / S5 `summary-l2-writer-formatter-round1026` (T-3338, Step 6 @Scanner) — re-audit итерация 2 (rework T-3339) — 23.09.2026 — **SCANNED — к деплою ДА по коду (OFF-безопасное; ON — GATED; C0/H0/блокирующих Medium 0; live — PENDING OWNER VERIFICATION)**

- **База:** HEAD `e3ea608` == annotated-тег `pre-round1026-s5` (`for-each-ref`: tag `a6be444` → `e3ea608e19ea…`); правки **НЕ закоммичены** (59 M + 6 ??). Отчёт: `plans/reports/round1026_s5_scanner_audit.md` (итерация 2). Итерация 1 (v1, `НЕ к деплою`) — историческая запись ниже.
- **Воспроизведённые числа (@Scanner, итерация 2):** pytest `.venv` **8807/0** (112.4 s), JS `tests/test_webapp_js_unit.py` **30/0** + `node --check` **43/43**, целевые L1/L2/F8/IA **222 passed**, каталог импортом **469/426/444/100/98/21**, F8 `--check` **CHECK OK** (exit 0), **Δ DDL=0**, `APP_VERSION` 2.58.24, `git diff --check`=0, R18 (тег→`e3ea608`, `stash@{0}`, `.env.bak.round1026-s5`).
- **[B-R1026S5-1] Medium (data-integrity) — RESOLVED, подтверждено @Scanner независимо:** фикстура без BOM (`7B 0D 0A`), **0** mojibake (U+2550–256C=0, U+2591–2593=0, байт `E2 95 A8`=0); `nav_titles` = «Модули»/«ИИ»/«Память» байт-точно; `git diff HEAD --numstat` = **3+/2−**; изменены ровно `_provenance.note` (+S5-абзац), `counts.REGISTRY 468→469`, +ключ `prompts.summary_l2_writer_system_prompt`; отступ 1; LF↔CRLF нормализуется git.
- **[S-R1026S5-1] Medium (ON-drift) — RESOLVED, подтверждено @Scanner независимо:** ON-guard (`summary_generator.py:373-380`) стоит **после** S1/S2 (`_apply_filter` = `filter_window`+`restore_context` → `xml_rows`); `_run_hybrid_l2(chat_id, xml_rows, focus, correlation_id)`; `trigger_message_id` → S1; `focus` → `focus_block` в `run_l1` (`summary_l1_clusterizer.py:538-541`). `SUMMARY_FILTER_ENABLED` default True. ON = **ровно 2** вызова (`run_l1` :548 + `run_l2` :757, по одному `await call(messages)`); OFF-тело `_run` не изменено.
- **Low/Info — статусы:** S-R1026S5-2/-3/-4/-6 и L-R1026S5-1/-2 **RESOLVED** (подтверждено пробами). OPEN (non-blocking): L-R1026S5-3 (+1 `_chat_limit` на OFF), L-R1026S5-4 (§106-классы → S6), L-R1026S5-5 (sanitize после escape), **L-R1026S5-6** (`'[^']*'` снимает апострофы `It's John's` → `Its Johns`, `Don't` → `Dont`; содержимое сохраняется; **подтверждён независимо** — совпал с находкой @Reviewer, не дублируется), S-R1026S5-5 (pytest только на `.venv`/aiogram 3.31.0). **Новое [S-R1026S5-7] Info (ON-gated S6-note):** ON-ветка пропускает `memorize_facts(chat_history)` (`:378-380` return до `:405-409`) — соответствует ADR-1026-7 D5/spec §7.1 (не дефект кода), но при активации ON память перестанет получать `chat_history`-факты → решение за S6.
- **Новые Critical/High — нет.** Проба нарезки (S-R1026S5-4): limit=20 с `&`/сущностями → 0 висячих `&`, осколки ≤ limit, конкатенация сохраняется. Проба одиночных кавычек — S-R1026S5-2 закрыт (кавычки снимаются), побочка = L-R1026S5-6.
- **Handoff:** @Orchestrator → **SCANNED — к деплою ДА по коду** (OFF-безопасное; ON — только после live-приёмки Эпика 1). Low/Info — owned follow-up (S6: L-R1026S5-4, S-R1026S5-7).

## Эпик 2 / S5 `summary-l2-writer-formatter-round1026` (T-3337, Step 5 @Reviewer) — re-review итерация 2 (rework T-3339) — 23.09.2026 — **APPROVED (C0/H0/блокирующих Medium 0; ON — GATED; live — PENDING OWNER VERIFICATION)**

- **База:** HEAD `e3ea608` == annotated-тег `pre-round1026-s5` (`git rev-parse pre-round1026-s5^{commit}` = `e3ea608e19ea…`); правки **НЕ закоммичены** (59 M + 6 ??). Отчёт: `plans/features/summary-l2-writer-formatter-round1026/review.md`.
- **Воспроизведённые числа (@Reviewer, итерация 2):** pytest `.venv` **8807/0** (108.7 s), JS `tests/test_webapp_js_unit.py` **30/0** + **43** файла (`tests/js/*.js`), F8/IA/JS-набор **66 passed**, 3 L2-тест-файла **70 passed** (64 → +6 rework), `git diff --check`=0, каталог **469/426/444/100/98/21**, F8 `--check` **CHECK OK**, **Δ DDL=0**, `APP_VERSION` 2.58.24, R18 (тег→`e3ea608`, `stash@{0}`), `web/**`/`summary_xml.py`/`summary_filter.py`/`summary_context_restore.py`/`image_generation.py`/`pg_db.py`/`database.py`/`current_task.md` вне diff.
- **[B-R1026S5-1] — RESOLVED 23.09.2026 (T-3339 @Builder; подтверждено @Reviewer):** `tests/fixtures/round1025/catalog_baseline.json` восстановлен из blob `e3ea608`; `git diff HEAD --numstat` = **3+/2−**; UTF-8 **без BOM**, отступ **1**, **0** mojibake (U+2550–256C/U+2591–2593; любых U+2500–257F — 0). JSON-дифф к базе: ровно `_provenance.note` (+S5-абзац), `counts.REGISTRY 468→469`, +1 ключ `prompts.summary_l2_writer_system_prompt`; `group_ids`/`group_tab`/`nav_order`/`nav_titles`/`tab_nav` — семантически идентичны. `test_round1025_f8_registry`/`test_ia_inventory_round1025`+F8 `--check` — зелёные.
- **[S-R1026S5-1] — RESOLVED 23.09.2026 (T-3339; подтверждено @Reviewer):** ON-ветка перенесена **после S1/S2**: `rows → _apply_filter (S1 `filter_window` + S2 `restore_context`) → xml_rows → ON`; `_run_hybrid_l2(chat_id, xml_rows, focus, correlation_id)`; `trigger_message_id` учтён S1; `focus` → `focus_block` в L1 (`run_l1` префиксит к `user_content`, как legacy). Доказано `TestOnPathAppliesS1S2`. Соответствует ADR-1026-7 D5/§80.
- **Low — статусы:** L-R1026S5-1 **RESOLVED** (мёртвый импорт удалён); L-R1026S5-2 **RESOLVED** (`TestOffPathDirect`); S-R1026S5-2 **RESOLVED** (`_QUOTE_RE`/`_strip_quotes` — одиночные/восточные кавычки); S-R1026S5-3 **RESOLVED** (`TestCanon::test_canon_doc_byte_identical`); S-R1026S5-4 **RESOLVED** (`_split_safe` не рвёт HTML-сущность, `test_chunk_does_not_split_html_entity`); S-R1026S5-6 **RESOLVED** (переформатирование устранено фиксом B). **OPEN (non-blocking):** L-R1026S5-3 (доп. `get_chat_param` на OFF), L-R1026S5-4 (§106-классы → S6, закреплено в `tasks.md`/`evidence.md`), L-R1026S5-5 (sanitize после escape), **L-R1026S5-6 [low, new]** (`'[^']*'` может склеить апострофы `don't, it's` → `dont, its` — побочный от S-R1026S5-2; продукт русскоязычный, ON gated → не блокер). Info S-R1026S5-5 (нужен `.venv`/aiogram 3.31.0) — без изменений.
- **Blocking findings:** нет (Critical 0 / High 0 / блокирующих Medium 0). ON-путь в проде — GATED (default OFF, гейт S6/S10+D4, моки); live-приёмка Эпика 1 — PENDING OWNER VERIFICATION.
- **Handoff:** @Orchestrator → **APPROVED** (OFF-безопасное состояние к merge/деплою); ON — только после live-приёмки Эпика 1.

## Эпик 2 / S5 `summary-l2-writer-formatter-round1026` (T-3338, Step 6 @Scanner) — 23.09.2026 — **SCANNED — НЕ к деплою (1 блокирующий Medium: F8-фикстура; live — PENDING OWNER VERIFICATION)** [историческая запись; findings закрыты в итерации 2 — см. выше]

- **База:** HEAD `e3ea608` == annotated-тег `pre-round1026-s5`; правки **НЕ закоммичены** (59 M + 6 ??; новые `services/summary_l2_writer.py` 800, `services/summary_article_formatter.py` 217, 3 тест-файла = 64 теста). Отчёт: `plans/reports/round1026_s5_scanner_audit.md`.
- **Severity:** Critical 0 / High 0 / **Medium 1 блокирующий (data-integrity)** / **Medium 1 ON-gated (S6)** / Low 3 (+5 @Reviewer подтверждены) / Info 2.
- **Воспроизведённые числа @Scanner:** pytest `.venv` **8801/0** (109.7 s; на глобальном `py -3` aiogram 3.29.1 → 5 ложных `InputRichMessageMedia`-падений — нужен `.venv` 3.31.0), JS **30/0** (43 файла), 64 теста, `git diff --check`=0, каталог **469/426/444/100/98/21**, F8 `--check` OK, sha256 repin OK, **Δ DDL=0**, R18 (тег→`e3ea608`, `stash@{0}`, `.env.bak.round1026-s5`), `APP_VERSION` 2.58.24.
- **[B-R1026S5-1] Medium (блокирующий, data-integrity) — ПОДТВЕРЖДЁН независимо (hex-дамп):** `tests/fixtures/round1025/catalog_baseline.json` — 302 mojibake-символа (U+2550–256C/U+2591–2593) в `L4` (`_provenance.note`) и `L224–226` (`nav_titles.{modules,ai,memory}`); bytes `E2 95 A8 D0 AC…` = двойная перекодировка `"Модули"`; в `e3ea608` — 0. Нарушает ADR-1026-2/tasks S5 инв. №12; тесты/F8 `--check` эти поля не читают → ложно-зелёный. **Fix:** восстановить из `e3ea608` + 2 санкц. правки (ключ + counts), UTF-8 без BOM. **RESOLVED 23.09.2026 (T-3339; подтверждено @Reviewer в итерации 2: 3+/2−, 0 mojibake, JSON-дифф = только санкц.).**
- **[S-R1026S5-1] Medium (ON-gated, S6-owned; не блокирует OFF-деплой):** ON-ветка `_run_hybrid_l2` (`summary_generator.py:349-354`, `:522-597`) стоит **до** S1/S2 и берёт сырые `rows` → `build_l1_payload` → `run_l1`; `_apply_filter` (S1+S2 `restore_context`) вызывается только в OFF (`:377`, `_restore` `:772/:799`). Аргументы `focus`/`trigger_message_id` не используются (grep=0). Нарушает ADR-1026-7 D5 («врезка … после S1/S2») и §80 («фильтр → восстановление → L1»); spec §7.1 противоречит D5. Impact — регрессия S1/S2/`focus` при включении ON (сейчас флаг OFF). **Fix (S6):** прогонять `_apply_filter` до `build_l1_payload` + прокинуть `focus`/`trigger_message_id`, либо AMEND D5/§80. **RESOLVED 23.09.2026 (T-3339; подтверждено @Reviewer в итерации 2: ON после S1/S2, `TestOnPathAppliesS1S2`).**
- **[S-R1026S5-2] Low:** `summary_l2_writer.py:101` `_QUOTE_RE` не покрывает одиночные кавычки `'…'` → проба `Он 'придумал такое' на ходу.` даёт `ok` с сохранённой цитатой (для `"…"`/`«…»` снимается). Defense-in-depth gap. **RESOLVED 23.09.2026 (T-3339; `_QUOTE_RE` расширен; тесты `test_single_quoted_*`/`test_corner_quoted_*`).**
- **[S-R1026S5-3] Low (evidence):** нет byte-identity теста канона L2 в `plans/docs/canon/architecture.md` (`test_summary_l2_writer.py::TestCanon` его не содержит; у L1 — есть). Док фактически байт-идентичен (проверено @Scanner), но `evidence.md`/`review.md` заявляют покрытие неверно. **RESOLVED 23.09.2026 (T-3339; добавлен `TestCanon::test_canon_doc_byte_identical`).**
- **[S-R1026S5-4] Low:** `summary_article_formatter.py:207-211` при вынужденной нарезке (`len(block)>limit`) может разорвать HTML-сущность; в проде недостижимо (абзац ≤900 < 4096). **RESOLVED 23.09.2026 (T-3339; `_split_safe` + `test_chunk_does_not_split_html_entity`).**
- **Info:** S-R1026S5-5 (полный pytest зелёный только на `.venv`/aiogram 3.31.0); S-R1026S5-6 (`catalog_baseline.json` переформатирован 1→2 отступа — устраняется фиксом B-R1026S5-1).
- **Инварианты ✅:** OFF-тело `_generate_two_call` не изменено + ленивые импорты L2; ON = ровно 2 вызова (моки), 3-й/legacy-фолбэк невозможен; `content_format="auto"` default байт-в-байт; `web/**`/XML/`pg_db.py`/`database.py` вне diff; нет CDN/inline/eval; форматтер экранирует `<script>`/`&`/`"`; fail-closed §106 (L1/пакет не usable → L2 не вызывается; L2-провал → нет публикации; обложка-провал → plain); канон `PREV_*`/миграция/ROLLBACK идемпотентны; Рассказчик/Редактор/`digest` REUSE; Δ DDL=0; Δ каталога=+1; маркер-тесты заменены, не отключены; R17/R18; гигиена индекса.
- **Handoff:** @Orchestrator → **SCANNED — НЕ к деплою**. @Builder — закрыть [B-R1026S5-1] → повторный прогон → re-review; [S-R1026S5-1] — обязательный S6-item до включения ON. Low/Info — owned follow-up.

## Эпик 2 / S5 `summary-l2-writer-formatter-round1026` (T-3337, Step 5 @Reviewer) — 23.09.2026 — **CHANGES REQUESTED (1 блокирующий Medium: F8-фикстура; live — PENDING OWNER VERIFICATION)**

- **База:** HEAD `e3ea608` == annotated-тег `pre-round1026-s5`; diff `e3ea608..working tree` (59 M + 6 ??; новые `services/summary_l2_writer.py` 800 стр., `services/summary_article_formatter.py` 217 стр., 3 новых тест-файла = 64 теста). Отчёт: `plans/features/summary-l2-writer-formatter-round1026/review.md`.
- **Severity:** Critical 0 / High 0 / **Medium 1 (блокирующий, data-integrity)** / Low 5 / Info 0.
- **Воспроизведённые числа (@Reviewer):** pytest `.venv` **8801/0** (111.6 s; baseline 8737 + 64), JS **30/0** (43 файла), новые тесты **64**, `git diff --check`=0, каталог **469/426/444/100/98/21**, F8 `--check` OK (exit 0), `sha256(param_catalog.py)`=`8928c1d1…e0e` == f8_baseline, **Δ DDL=0**, R18 (тег→`e3ea608`, `stash@{0}`), `APP_VERSION` 2.58.24 == README.
- **Покрытие:** SC-01…SC-14 подтверждены (L2 1 вызов `step="l2_writer"`; §99-контракт; анти-цитаты + fail-closed `quote_attribution`; форматтер rich `<h1>`/`<p>`/≤1 `<b>` + `sanitize→html.escape` (адверсивный тест: `<script>`/`onerror`/`&`/`"` экранируются); OFF байт-в-байт тело; ON ровно 2 вызова, 3-й невозможен, без legacy-фолбэка; канон+`PREV_*`+миграция/ROLLBACK+эталон; env-only слот/флаг; R17-логи). §104/`web/**`/XML/`pg_db.py`/`database.py` вне diff. ✔
- **[B-R1026S5-1] Medium (блокирующий, data-integrity):** `tests/fixtures/round1025/catalog_baseline.json` — весь файл перезаписан (721+/720−, отступ 1→2) с **двойной перекодировкой** русского текста: `nav_titles.{modules,ai,memory}` и `_provenance.note` → mojibake (302 символа U+2550–256C/U+2591–2593; в `e3ea608` — 0). Было `"Модули"` → стало `"╨М╨╛╨┤╤Г╨╗╨╕"`. Нарушает ADR-1026-2 (F8 = «обновление фикстуры, не отключение») и tasks.md S5 инв. №12. Runtime/тесты S5 не затронуты (F8 `--check` и тесты эти поля не читают → ложно-зелёный). **Fix:** восстановить из `e3ea608` + наложить только 2 санкц. правки (`registry_keys += prompts.summary_l2_writer_system_prompt`, counts 468→469), UTF-8 без BOM, исходный отступ; перегнать `test_round1025_f8_registry`/`test_ia_inventory_round1025` + JSON-дифф. **RESOLVED 23.09.2026 (T-3339; см. re-review выше).**
- **Low (non-blocking, OPEN):** L-R1026S5-1 мёртвый импорт `format_plain_html` в `_deliver_l2_plain`; L-R1026S5-2 нет теста прямого сравнения вывода `_run` OFF с baseline (тело не менялось); L-R1026S5-3 доп. `get_chat_param` на OFF (fail-safe); L-R1026S5-4 §106-классы `SUMMARY_GENERATION_FAILED`/`COVER_GENERATION_FAILED`/`RICH_MESSAGE_SEND_FAILED`/`TEXT_FALLBACK_FAILED` не реализованы (0 вхождений), хотя T-3325 `[x]`/SC-09 их требует — backlog S6 закрепляет «коды ошибок §106» за S6 (дефер приемлем, отметку согласовать); L-R1026S5-5 rich-путь применяет `_maybe_sanitize` ПОСЛЕ escape (инверсия порядка spec §5.2; функционально безопасно).
- **Unavailable:** ON-путь в проде — GATED (S6/S10+D4, проверен на моках); live-приёмка Эпика 1 — PENDING OWNER VERIFICATION.
- **Handoff:** @Builder — закрыть [B-R1026S5-1] → повторный прогон → @Orchestrator (re-review).

## Эпик 2 / S4 `summary-fact-package-round1026` (T-3300, Step 5 @Reviewer) — 23.09.2026 — **APPROVED (автономный контур; live — PENDING OWNER VERIFICATION)**

- **Границы:** baseline `59f5921`, annotated-тег `pre-round1026-s4` → `59f5921`; правки НЕ закоммичены — focused diff (24 M + 6 ??: new `services/summary_fact_package.py`, `tests/test_summary_fact_package.py`, папка фичи; только version-пины в тестах). Живой путь/публикация/обложка/XML/`web/**`/`param_catalog.py`/`database.py`/`pg_db.py` — вне diff. Отчёт: `plans/features/summary-fact-package-round1026/review.md`.
- **Severity:** Critical 0 / High 0 / requirement-blocking Medium 0 / Low 3 (owned follow-up S5) / Info 2 → **Approved**.
- **Воспроизведено лично:** pytest **8737/0** (109.8 s; baseline 8685 + 52), `tests/test_summary_fact_package.py` **52/0**, JS **43/43**, `git diff --check`=0, каталог импортом **468/426/443/100/98/21**, F8 `tools/gen_param_registry_round1025.py --check` **OK (exit 0)**, Δ DDL=0, 0 LLM-вызовов (AST-импорты модуля — только stdlib+проект; `llm_client`/`async def` отсутствуют), R18 (тег annotated→`59f5921`, бэкап s4, `.env.bak.round1026-s4`, `stash@{0}`).
- **Оба ленза:** §96-полнота (name/description/chronology/facts/evidence_ids/fragments), компактность (сырой лог не дублируется; fragments ⊆ темы), доказательства не генерируются, fail-closed `ok/truncated/empty/invalid/error/not_built`, ID TG/DB + `evidence_ids ⊆ message_ids`, детерминизм (байт-идентичность, вход не мутируется), транзит `service` вне L2-контента, врезки нет (GATED). Критичных дефектов безопасности/целостности не выявлено; R17-логи чисты.
- **Новые/подтверждённые findings (@Reviewer):** L-R1026S4-1 (char-обрезка фрагмента не в `truncated`/WARN — только `metrics.fragment_char_truncated_count`; проба 1500→1000, `status=ok`) — **подтверждён лично**; L-R1026S4-2 (O(n²) `_enforce_budget`) — подтверждён по коду; **L-R1026S4-3 (new)** — §9/ADR D5 «проброс `skipped_ids`/`skipped_tg_ids`» реализован как счётчики (`l1_skipped_count`/`l1_chunk_count`), сами id-списки/`skipped_tg_ids` не переносятся (данные остаются на `L1Result`; §10 — «счётно») → Low, уточнить §9 либо донести в S5; I-R1026S4-1 (`unassigned` не усекается, `cut` не используется); I-R1026S4-2 (`invalid`/`error` без `invalid_reason` → `reason=None`, `metrics.reason='ok'`; проба `invalid_result(None)`). Все — не блокеры. **M-R1026S4-1 (Scanner, process gate) — RESOLVED** (review.md создан).
- **Unavailable:** live-приёмка — PENDING OWNER VERIFICATION; врезка L1→пакет→L2 — GATED (S5/S6).
- **Handoff:** @Orchestrator → **APPROVED**; далее T-3302 (при необходимости) → T-3303 @Architect → T-3304 @PM → T-3305 @DevOps.

## Эпик 2 / S4 `summary-fact-package-round1026` (Шаг 6, T-3301) — 23.09.2026, Step 6 @Scanner — **SCANNED (к деплою ДА по коду; процессный gate — T-3300 @Reviewer не закрыт)**

- **Baseline:** HEAD `59f5921` (annotated-тег `pre-round1026-s4` → `59f5921`), `APP_VERSION` 2.58.22; правки **НЕ закоммичены** (24 M + 6 ??, focused diff). Отчёт: `plans/reports/round1026_s4_scanner_audit.md`. Артефакты: `plans/features/summary-fact-package-round1026/{spec.md,adr-1026-6-*.md,tasks.md,evidence.md}`.
- **Severity:** Critical 0 / High 0 / requirement-blocking Medium 0 / **Medium 1 (процесс/gate)** / Low 2 (owned follow-up) / Info 2 — **к деплою ДА по коду**.
- **Изменённые артефакты:** new `services/summary_fact_package.py` (чистый модуль, 694 стр., 0 LLM-вызовов; `FactPackage` v1: `name`=topic verbatim, `description`=детерминированная агрегация `facts[].text`, `chronology`=ASC `(timestamp,message_id)` из §92, `facts`/`evidence_ids` verbatim+union, `fragments` evidence-first с лимитами, `service{response_mode,cover_prompt}`; бюджет — reuse `limits.summary_max_context_*` через `resolve_chat_limit`; fail-closed `ok/truncated/empty/invalid/error`+`not_built`); new `tests/test_summary_fact_package.py` (**52**); `config/settings.py` bump **2.58.23**; `README.md`/`param-registry-*.meta.md`/`plans/*` + 13 Py/4 JS теста (только version-пины).
- **D4/§96 (independent):** §96-полнота темы; **сырой лог повторно не дублируется** (`fragments` ⊆ `message_ids`/`evidence_ids` темы; проба — тексты прочих сообщений отсутствуют); «доказательства» не генерируются (`description` — из `facts[].text`, `chronology` — из §92); `evidence_ids ⊆ message_ids`; ID — TG `message_id` (DB `id` → `invalid`); fail-closed подтверждён; **0 LLM-вызовов** (`llm_client` в модуле отсутствует), живой путь `await_count==2`/`steps=[stage1,stage2]`.
- **Инварианты OK:** **Δ DDL=0** (`database.py`/`pg_db.py` вне diff); **Δ каталога=0** — импортом **468/426/443/100/98/21**, `param_catalog.py` вне diff (F8 не переиздавался); `APP_VERSION` 2.58.23 синхронен; маркер-тесты не ослаблены (только version-пины); R17 (логи — числа/коды/id; секрет-тест) / R18 (тег `pre-round1026-s4`→`59f5921`, `var/backups/s4-round1026-20260923-202326/`, `.env.bak.round1026-s4`, `stash@{0}`); CSP/zero-build (новых либ/CDN/inline/eval нет); `git diff --check`=0; гигиена индекса OK.
- **[M-R1026S4-1] Medium (OPEN, процесс/gate, не код-дефект):** `plans/features/summary-fact-package-round1026/review.md` **отсутствует** (T-3300 @Reviewer не закрыт; T-3301 по `tasks.md` формально зависит от T-3300). **Fix/путь:** выполнить T-3300 → `review.md`, при findings — rework + повторные проверки. Независимые проверки @Scanner — зелёные (не код-блокер).
- **[L-R1026S4-1] Low (OPEN, owned, S5):** `services/summary_fact_package.py:251-253` — обрезка фрагмента >`FRAGMENT_MAX_CHARS=1000` увеличивает только `metrics.fragment_char_truncated_count`, но не ставит `truncated` и не даёт WARN (проба: 1500 симв. → `status=ok`, `truncated_count=0`). Спек §4.5 санкционирует сам лимит, но это расходится с политикой модуля «любое вытеснение → truncated/WARN» (§93 «не резать молча»). **Fix:** учитывать char-обрезку в `truncated`/WARN либо явно задокументировать. Не блокирует (модуль не в живом пути).
- **[L-R1026S4-2] Low (OPEN, owned, perf, S5):** `services/summary_fact_package.py:382-412` — `_enforce_budget` на каждом вытеснении заново сериализует JSON и считает `count_tokens` по всему остатку (O(n²)); проба 17 тем × 30 фрагментов × 1000 симв. при limit=199000 → 341 вытеснение, **0.29 s** CPU. **Fix:** инкрементальная оценка/батч-вытеснение. Не блокирует.
- **Info:** I-R1026S4-1 (`unassigned_message_ids` не усекаются бюджетом; `cut` не используется — при превышении одними unassigned `status=empty`/`fits=False`); I-R1026S4-2 (`invalid`/`error` без `invalid_reason` → `result.reason=None`, а `metrics.reason`/лог = `ok`).
- **Прогоны @Scanner:** `tests/test_summary_fact_package.py` **52 passed / 0 failed**; импорт каталога **468/426/443/100/98/21**; `git diff --check`=0; тег `pre-round1026-s4`→`59f5921`.
- **Unavailable:** полный pytest/JS повторно не гонялись (данные Builder: 8737/0, JS 43/43 — приняты как evidence); live-приёмка — PENDING OWNER VERIFICATION; врезка L1→пакет→L2 — GATED (S5/S6).
- **Handoff:** @Orchestrator → **SCANNED** (к деплою ДА по коду; процессный gate — T-3300 @Reviewer; L-R1026S4-1/-2 — owned follow-up к S5).

## Эпик 2 / S3 `summary-l1-clusterizer-round1026` (T-3275, Step 5 @Reviewer) — 23.09.2026 — **APPROVED (автономный контур; live — PENDING OWNER VERIFICATION)**

- **Границы:** baseline `4007081` (== `origin/master`, тег `pre-round1026-s3`), правки НЕ закоммичены — focused diff (57 M + 4 ??: 2 модуля, тест-файл, папка фичи); `summary_generator.py`/`summary_xml.py`/публикация/обложка/`web/**`/`current_task.md` вне diff. Отчёт: `plans/features/summary-l1-clusterizer-round1026/review.md`.
- **Вердикт: Approved** — оба мандатных ленса (requirements/correctness + focused change audit) пройдены независимо; **Critical 0 / High 0; блокирующих Medium нет**. Живой путь не тронут (врезки нет, T-3273 DEFERRED).
- **Воспроизведено @Reviewer (не по evidence):** pytest `.venv` **8685/0** (111.25 s) == baseline 8569 + 116; новый файл **116/0**; целевые регрессы two-call/cover/filter/restore-integration **65/0**; JS **43/43** (все 43 файла); F8 `--check` exit 0 (`реестр 468 == REGISTRY`); `git diff --check` exit 0; каталог импортом **468/426/443/100/98/21** (prompts 21→22); `sha256(param_catalog.py)`=`6531f653…7812` == фикстура (delta 56→57); тег/бэкап/`stash@{0}` на месте; `APP_VERSION` 2.58.22 синхронен.
- **Контракт/ядро (проверено по коду):** ровно **1** LLM-вызов L1 (живой `await_count==2`, `steps=[stage1,stage2]`, третьего вызова нет); строгая схема §95 (лишние поля → invalid; `schema_version` int==1); лимиты 100/30/1000/200/500; ID-пространства (payload TG; DB `id` — только упаковка §93; неоднозначность → `id_space_mismatch`); evidence ⊆ треда; авто-`unassigned`; канонизация/дедуп/ASC детерминированы; fail-closed все 5 статусов; «L1 не пишет саммари»; канон L1 == эталон `plans/docs/canon/architecture.md` байт-в-байт; миграция/ROLLBACK идемпотентны, кастом не перезаписывается.
- **[M-R1026S3-1] RESOLVED (процесс, @Scanner → @Reviewer):** `review.md` T-3275 создан этим проходом; код-дефектов за ним не стояло (независимые прогоны зелёные).
- **[L-R1026S3-1] Low (OPEN, owned, S5):** `summary_l1_clusterizer.py:470` — `resolve_l1_slot()` вне fail-closed `try:` (:477), проброс исключения вопреки docstring; в S3 не в живом пути. **Fix:** перенести резолв в `try` до врезки S5. Подтверждено независимо.
- **[L-R1026S3-2] Low (OPEN, owned, contract):** `summary_l1_contract.py:399-413` — `evidence_message_ids: []` принимается (`status=ok`; подтверждено пробой) при том, что §94 подразумевает evidence. **Fix:** требовать ≥1 evidence либо документировать. Не блокирует.
- **[L-R1026S3-3] Low (OPEN, owned, edge):** `summary_l1_clusterizer.py:199-215` — строка без `tg_message_id`: single-chunk → `message_id: null`, fragmented → `IdSpaceMismatch` (несогласованность; S1/S2 гарантируют поле). **Fix:** унифицировать fail-closed. Не блокирует.
- **[R-R1026S3-1] Info (S5-note):** dedicated-путь `_post` не пишет analytics (`_record_analytics` module/step/correlation_id) и не учитывает global usage / per-chat BYOK, в отличие от `generate`; в S3 нет потребителя — решить при врезке публичным методом клиента.
- **[R-R1026S3-2] Info (S5-note):** оценка бюджета L1 (`estimate_and_split`) считает только токены текста (+маркеры), без system-промпта и JSON-обёртки §92 → фактический ввод больше оценки (спека предписывает существующий эстиматор); наблюдать на S5.
- **Подтверждено чисто:** Δ DDL=0; каталог ровно +1 (468/426/443; промпт PG-only `settings_field=None`); env-only `SUMMARY_L1_*` (Settings 426); F8 фикстуры/ассерты не ослаблены (re-pin чисел/версии); CSP/zero-build; R17 (логи — числа/коды/host; контент не логируется); R18; §57–§73/F0–F11/S1/S2 зелёные.
- **Unavailable:** live-приёмка владельца — **PENDING OWNER VERIFICATION**; E2E §113 — S9; реальные ответы провайдера L1 — после врезки S5/S6.
- **Handoff:** @Orchestrator → **APPROVED** (T-3275 закрыт `review.md`; деплой-гейт T-3275 снят). Далее — T-3278 Merge (§-раздел @Architect), T-3279 архивация, T-3280 deploy (bump 2.58.22).

## Эпик 2 / S3 `summary-l1-clusterizer-round1026` (Шаг 6, T-3276) — 23.09.2026, Step 6 @Scanner — **SCANNED (к деплою ДА после закрытия процессного гейта T-3275)**

- **Baseline:** HEAD `4007081` (annotated-тег `pre-round1026-s3` → `4007081`), правки **НЕ закоммичены** — focused diff-based аудит дерева (57 M + 4 ??). Отчёт: `plans/reports/round1026_s3_scanner_audit.md`. Спека/ADR: `plans/features/summary-l1-clusterizer-round1026/{spec.md,adr-1026-5-l1-clusterizer-contract-two-call-amend.md,evidence.md}`.
- **Severity:** Critical 0 / High 0 / **Medium 1 (процесс: отсутствует `review.md` T-3275)** / Low 3 (owned follow-up) / Info 5 — **дефектов кода-блокеров нет**.
- **Изменённые файлы:** new `services/summary_l1_contract.py` (строгий JSON §95, `IdSpace` TG, `L1Result` fail-closed, канонизация/дедуп/ASC, `id_space_mismatch`), `services/summary_l1_clusterizer.py` (§92-вход → §93-упаковка в один вход → **ровно 1 LLM-вызов** → валидатор; env-слот §82; логи `L1_*`), `tests/test_summary_l1_clusterizer.py` (116); M `summary_prompts.py` (+канон L1/PREV), `prompt_migrations.py` (+ступень/ROLLBACK), `param_catalog.py` (+1 промпт-ключ), `config/settings.py` (env-only `SUMMARY_L1_*` + bump 2.58.22), `.env.example`, `README.md`, канон-эталон, F8-артефакты/фикстуры, ~43 теста (version/count re-pin).
- **D4-гейт:** `summary_generator.py`/`summary_xml.py`/`telegram_send.py`/`image_generation.py`/`web/**` **вне diff**; врезки L1 нет (T-3273 DEFERRED); 2-вызовность живая (`await_count==2`, `steps=[stage1,stage2]`, OFF байт-в-байт); третьего вызова нет (1 `await call(`).
- **Инварианты ✅:** Δ DDL=0 (`pg_db.py` вне diff, `sha256` == фикстура); Δ каталога = **ровно +1** (REGISTRY **468**/Settings fields **426** без изменений/categorized **443**/GROUPS **100**/`_TAB_BY_GROUP` **98**/TAB_RULES **21**; prompts 21→22; у ключа `settings_field=None`/`env_name=None`); F8 переиздан (`--check` OK; delta 56→57; `sha256(param_catalog.py)`=`6531f6…7812` == фикстура; `ROUTES_SHA256_F11` цел); `APP_VERSION` **2.58.22** синхронен; маркер-тесты не ослаблены (только re-pin); CSP/zero-build; R17 (логи — числа/коды/id; канон/эталон без секретов); R18 (тег `pre-round1026-s3`, `var/backups/s3-round1026-20260923-183119/`, `.env.bak.round1026-s3`, `stash@{0}`); `git diff --check`=0; гигиена индекса OK.
- **Канон (ADR-1013-3):** эталон == коду байт-в-байт (2846 симв.); `PROMPT_MIGRATIONS`/`ROLLBACK_MIGRATIONS` корректны; идемпотентность/кастом-не-перезаписывается/pre-seed-skip — зелёные.
- **Логика:** fail-closed на всех статусах (`ok|empty|invalid|truncated|error`, `usable`=ok/truncated); ID-пространства явные (DB `id`→TG через таблицу, неоднозначность → `id_space_mismatch`); авто-`unassigned`; детерминизм (двойной прогон); «L1 не пишет саммари» (структурные капы); `kept`/S1/S2 не мутируются.
- **[M-R1026S3-1] Medium (OPEN, процесс):** `review.md` T-3275 @Reviewer отсутствует (workflow_state/backlog/MEMORY не отражают Step 4 @Builder). **Fix:** прогнать T-3275 → review.md; при findings — rework + повторные проверки; либо явное решение @Orchestrator о порядке шагов.
- **[L-R1026S3-1] Low (OPEN, hardening):** `summary_l1_clusterizer.py:469-475` — `resolve_l1_slot()` вне fail-closed try (воспроизведён проброс `RuntimeError`). **Fix:** обернуть/перенести внутрь try до врезки S5.
- **[L-R1026S3-2] Low (OPEN, contract):** `summary_l1_contract.py:399-413` — `evidence_message_ids: []` принимается (`status=ok`). **Fix:** требовать ≥1 evidence либо документировать.
- **[L-R1026S3-3] Low (OPEN, edge):** `summary_l1_clusterizer.py:193-215` — строка без `tg_message_id`: single-chunk → молча `message_id: null`, fragmented → `IdSpaceMismatch` (воспроизведено). **Fix:** унифицировать fail-closed.
- **Info:** I-R1026S3-1 (в 4 JS-тестах метка assert осталась `2.58.21` при regex `2.58.22` — косметика); I-R1026S3-2 (`THREAD_ID_RE` `$` пропускает trailing `\n`; значение перенумеровывается); I-R1026S3-3 (канон L1 содержит `TARGET_INSTRUCTION_BLOCK`, но маркер текущей команды не ставится — инертно, решить в S5); I-R1026S3-4 (`_post(api_key=…)` подменяет общий chat-клиент — pre-existing паттерн `generate_worker`, учесть per-key кэш в S5); I-R1026S3-5 (docs-lag workflow_state/backlog/MEMORY до Step 8–10).
- **Тесты @Scanner:** полный `.venv` pytest **8685/0** (113.18 s; +116 к baseline); новый файл **116 passed**; JS **43/43**; `node --check` OK; F8 `--check` exit 0; `git diff --check`=0; импорт-проверки каталога/версии/хешей — OK.
- **Handoff:** @Orchestrator → **SCANNED** (код/инварианты чистые; деплой — после T-3275; L-R1026S3-1/-2/-3 — owned follow-up).
...
- **Вердикт: Critical 0 / High 0 / Medium 0 / Low 3 (новые, non-blocking) / Info 4 — к деплою ДА.**
- **[L-POLY1026-1] Low (OPEN, perf/GC):** `web/static/polygon-background.js:425-534` (`drawFacets`/`drawNodes`) — на каждый кадр аллоцируются массивы `ca/cb/mix` (по грани, ~189/93) и `rgba()`-строки (~500–1000+ объектов/кадр). SC-33 (нет аллокаций «все точки каждый кадр») не нарушен (точки не аллоцируют), но GC-давление на mobile WebView. Fix: скретч-массивы/кэш `rgba`. Live-перф — PENDING OWNER.
- **[L-POLY1026-2] Low (OPEN, gate-coverage):** `tools/ui_round1025_matrix.py:133-142` — матрица §71 зафиксирована на `UI_POLYGON_BG_ENABLED=False` (Aurora-пробы), активный default-ON путь покрыт только `tools/ui_round1026_polygon.py`. Будущая регрессия `_syncBgLayer`/CSS активного пути матрицей не ловится. Fix: второй режим ON в матрице.
- **[L-POLY1026-3] Low (OPEN, evidence):** в `round1026_polygon_background_ui_report.md:7,80`/`spec.md:272` CSP записана `script-src 'self'`, фактически `web/app.py:50-61` — `script-src 'self' 'unsafe-eval'` (pre-existing, diff `app.py` пуст). Инвариант «без CDN/inline/eval в новом коде» соблюдён.
- **Info:** I-POLY1026-1 (`glass.js::detectMode` webgl→refraction без отдельного регресс-теста; влияние — атрибут `data-lg-mode`); I-POLY1026-2 (`fallbackNone` без `contextrestored` → фон off до след. `_syncBgLayer`, по дизайну); I-POLY1026-3 (`uiFlag` default `true` до `/api/me` — pre-existing); I-POLY1026-4 (`sample()` `getImageData` полного DPR-буфера, только диагностика).
- **Инварианты ✅:** Δ DDL=0 (`services/**`/миграции/`db/**` пусты, `sha256(pg_db.py)` == фикстура); Δ каталога=0 (467/426/442/100/98/21; `UI_POLYGON_BG_ENABLED ∉ pc.REGISTRY`; `sha256(param_catalog.py)` == фикстура); CSP/zero-build (only `/static/**`, `eval`/`innerHTML` нет, SHA `7707D7FE…C1BE`); ровно один активный рендерер (`_syncBgLayer` + `html.polygon-bg`; JS-тест C); `Math.random` в рендере нет (SEED+mulberry32); нет утечек rAF/RO/буферов (`stop`/`pause` идемпотентны); a11y `pointer-events:none`/`aria-hidden`/вне контейнеров; `APP_VERSION` 2.58.19 синхронен; R17 (0 секретов; логов нет)/R18 (`pre-round1026-visual`, `var/backups/`, `stash@{0}`); D4-гейт (публикация/`summary_*`/S6/S10/IA/сердцебиение не тронуты); стекло default OFF, J заблокирован §13.2; `git diff --check`=0; маркер-тесты не ослаблены (только bump + re-pin `ROUTES_SHA256_F11`=`4b652cb1…551b`).
- **Прогоны @Scanner:** JS `round1026` → `POLYGON-LUMINESCENCE-OK`; hotfix7/8/9/10 JS OK; `tests/test_webapp_round1026_polygon.py` **18 passed**; `tests/test_webapp_js_unit.py` + `test_round1025_f8_registry.py` **59 passed**; каталог 467/426/442/100/98/21; `git diff --check`=0. Документированные @Builder (полный pytest 8520/0, JS 43/43, matrix §71 и `ui_round1026_polygon.py` 0 failures) — согласованы с артефактами, не переигрывались (объём).
- **Handoff:** @Orchestrator → **SCANNED** (блокеров нет; Low3 — owned follow-up; live-гейты стекла/перфа/WebView — PENDING OWNER).
...
- **Вердикт: приёмка Эпика 1 (авто) — готова, SCANNED.** Critical 0 / High 0 / Medium 0 / Low 2 / Info 4. Блокеров нет. Live-гейт Telegram WebView — **PENDING OWNER VERIFICATION**; Эпик 1 не объявляется завершённым.
- **Read-only инвариант ✔ (программно):** `git diff HEAD -- services web handlers config migrations param_catalog.py web/api` пуст, untracked в рантайме нет; изменения только `tools/**` + `plans/**`; `tests/**` не менялись; `deploy_commands.txt`/`plans/current_task.md` не тронуты.
- **Честность приёмки ✔:** §72 «ровно 1 write» — поведенческий (`writes != 1` краснеет при 0 и >1; три представления сверены, reload=сервер); §73 stale — ловит целевой дефект (поздний ответ A не меняет B); §78 секреты — реальный `saveKeyItem` (маска/композит → 0 write, ключ → 1 write); §79 — не заглушка (300 passed подмножества + пустой рантайм-дифф).
- **§117 п.1–12 ✔:** все 12 пунктов закрыты ссылками, ни один не пропущен; все артефакты существуют (реестр/карты F8, diff, чек-лист секретов F9, e2e, 8 PNG, heartbeat, Playwright, тег отката).
- **Инварианты ✔:** Δ DDL=0; Δ каталога=0 (независимый импорт: `REGISTRY=459`, `GROUPS=98`, `_TAB_BY_GROUP=96`, `TAB_RULES`/`TAB_NAV`/`CONFIG_TAB_TITLES=21`); CSP/zero-build; R17 (regex-скан F10-артефактов 0); R18 (тег `pre-round1025-f10`→`57b325c` запушен, бэкап `f10-round1025-20260923-091131`, `.env.bak.round1025-f10`, `stash@{0}` цел); маркер-тесты не ослаблены; гигиена (`git diff --check`=0; скриншоты/`_ui_*`/`.env`/zip/`var/` вне индекса).
- **Прогоны @Scanner (независимо):** `node --check web/app.js` OK; `node tests/js/*.js` **42/42 OK**; `py -3 -m pytest -q` **8457 passed / 5 failed / 1 skipped** (5 env pre-existing `rich`/`InputRichMessageMedia`, вне диффа); `.venv` pytest **8463 passed** (совпадает с отчётом); `git diff --check`=0.
- **Low (OPEN, owned follow-up, не блокируют):** **L-F10S-1** — `tools/ui_round1025_e2e.py:320,346-350`: §73 не assert'ит `deferred_captured`/положительный эффект A (вакуумный проход при неотправленной мутации; текущий прогон `captured=true`). **L-F10S-2** — `tools/ui_round1025_matrix.py:1451-1481,2610-2690`: low-TG `1280×400` не содержит `.bottom-nav`/`minTouch` → клаузы «нижние панели/touch» неактивны, покрытие = h-scroll + bounds sidebar; формулировка отчёта шире факта.
- **Info:** I-F10S-1 (`sendRichMessage` evidence — grep; поведенческие тесты `outgoing_guard_round1022` в subset не включены); I-F10S-2 («8463 passed» лишь в `.venv`; system `py -3` = 8457/5env/1 — указывать интерпретатор); I-F10S-3 (синтетический `sk-…-probe` в `tools/`, не секрет); I-F10S-4 (nit: `tests/**` фактически не менялись).
- **Handoff:** @Orchestrator → **SCANNED** (live-гейт Telegram WebView — PENDING OWNER; EPIC 1 не завершён).

## Round 10.25 F11 `status-showcase-dashboard-round1025` (витрина «Статус» §11–§21: 12-кол. сетка §12, Hero/метрики §13/§14, граф §16, сон §17, «Новые факты»/бюджеты §19, счётчики §20 + kill-switch `UI_STATUS_GRID_V2`) — 23.09.2026, Step 6 @Scanner (T-3092) — **SCANNED (итер.2, финал)**

- **Baseline:** HEAD `25cc19c` (тег `pre-round1025-f11`), правки **НЕ закоммичены** — focused diff-based аудит (31 M + 10 ??, вкл. правки планов). Отчёт: `plans/reports/round1025_f11_scanner_audit.md`. Вход: `spec.md`/`adr-1025-23`/`evidence.md`/`tasks.md`/`round1025_f11_verification.md`/`round1025_f11_ui_report.md`. Тип: UI-композиция (`web/**`) + bump + тесты.
- **Вердикт (итер.2, финал): К ДЕПЛОЮ — ДА.** Critical 0 / High 0 / Medium 0 / Low 0 / Info 2. (Итер.1: High 1 + Medium 1 + Low 3 — все закрыты и воспроизведены @Scanner.)
- **[H-F11S-1] High (RESOLVED, итер.2):** аддитивный `counts` в существующем `/api/status/logs` (`routes.py:1675-1685`) + read-only `LogRingHandler.level_counts()` (`services/log_ring.py:185-199`, агрегация по всему буферу); `loadLogCounts` (`web/app.js:9308-9318`) — один запрос `level=ALL&limit=1`, читает `counts.ERROR`/`counts.WARNING` (реальные числа; семантика раздельная, без «warn и выше»); нет `counts`/ошибка → «—». `count`/`logs` без изменений (обратная совместимость, `loadLogs` не тронут). Гейты: `test_level_counts_exact` (7→7 / 3→3), `test_level_counts_not_capped_by_limit` (`get_entries(limit=1)=1`, `level_counts=5>1`), API `test_logs_counts_not_capped_by_limit` (`count=1`, `counts.ERROR=5>1`), JS `F11-LOG-COUNTS-OK`.
- **[M-F11S-1] Medium (RESOLVED, итер.2):** формулировки приведены к факту — OFF `UI_STATUS_GRID_V2` = «одноколоночный безопасный режим» (`.status-grid--legacy`), НЕ byte-identical legacy-DOM (`spec.md:70/149`, `adr:48/52/73`, `.env.example`); код композиции не менялся.
- **Low (RESOLVED, итер.2):** L-F11S-1 — строгий byte-freeze `web/api/routes.py` восстановлен через `ROUTES_SHA256_F11` (SHA-256 совпал с константой `76bcab3c…3306`, тест PASSED); L-F11S-2 — `UI_STATUS_GRID_V2` задокументирован в `.env.example`; L-F11S-3 — initial focus (`ref="graphFullPanel" tabindex="-1"` + `_focusGraphFull`) + Esc-close (`escClose` → `closeGraphFull`, `web/app.js:4876-4879`).
- **Info:** I-F11S-1 (RESOLVED) мёртвая ветка `$refs.statusLogs` удалена; I-F11S-3 (live Telegram WebView/TMA — PENDING OWNER VERIFICATION, T-3097). Остаточно (Info, не блокер): нет полного focus-trap; тач-цели чипов/счётчиков < 44px.
- **Info (не блокируют):** I-F11S-2 — один лёгкий запрос вместо двух в 30-сек. поллинге (новых таймеров нет).
- **Инварианты ✔:** Δ DDL=0 (`web/api/routes.py` — аддитивно; `services/log_ring.py` — read-only `level_counts()`, без схемы/DDL/миграций); Δ каталога=0 (459/98/96/21; `param_catalog.py` не тронут; `UI_STATUS_GRID_V2` в каталоге нет); CSP/zero-build (`node --check` OK; 0 `v-html`/`innerHTML`/`onclick`/`eval`/CDN); `APP_VERSION` **2.58.17** синхронен (settings/README/`?v=__APP_VERSION__`/строгие пины); флаг env-only вне каталога; R17 (`counts` — только имена уровней; секретов нет); R18 (`pre-round1025-f11`→`25cc19c` (+origin), `.env.bak.round1025-f11`, `stash@{0}`); гигиена (`git diff --check`=0; `.env`/`current_task.md`/zip/скриншоты/`tools/_ui_*`/`var/backups` вне индекса; `ARCHITECTURE.md`/`MEMORY.md`/`metrics.md` не тронуты); маркер-тесты — строгие (byte-freeze `routes.py` восстановлен); 5 pytest-падений env pre-existing (воспроизведены: `outgoing_guard_round1022` ×2 + `summary_cover_round1023` ×3, `ImportError` aiogram rich).
- **Прогоны @Scanner (независимо, итер.2):** `node --check` OK; `tests/js/*.js` **42/42 PASS**; `py -3 -m pytest -q` **8457 passed / 5 failed / 1 skipped** (5 env pre-existing: `outgoing_guard_round1022` ×2 + `summary_cover_round1023` ×3, `ImportError` aiogram rich); `tests/test_webapp_f11_round1025.py` **33 passed**; каталог 459/98/96/21; `git diff --check`=0; R17 0.
- **Handoff:** @Orchestrator → **SCANNED** (к деплою ДА, блокеров нет; live-гейт T-3097 — PENDING OWNER).

## Round 10.25 F9 `secrets-and-save-states-round1025` (UI секретов §50 + состояния §51 + визуальный добор SaveBar §69/§78) — 23.09.2026, Step 6 @Scanner (T-3055) — **итерация 2 (финал)**

- **Baseline:** HEAD `93432c0` (тег `pre-round1025-f9`), правки **НЕ закоммичены** — focused diff-based аудит (32 M + 9 ??). Отчёт: `plans/reports/round1025_f9_scanner_audit.md`. Вход: `spec.md`/`adr-1025-22`/`evidence.md`/`review.md`/`round1025_f9_secrets_checklist.md`. Тип: UI-слой (`web/**`) + bump + тесты.
- **Вердикт (итер.2, финал): К ДЕПЛОЮ — ДА.** Critical 0 / High 0 / Medium 0 / Low 0 / Info 2. (Итер.1: High 1 + Low 4 — все закрыты и воспроизведены @Scanner.)
- **H-F9S-1 (RESOLVED):** `web/app.js:7978-7986` — image-ветка `deleteKeyItem` → `DELETE /api/config/keys/own/{key}` c `{method:'DELETE', global:true}`; `api()` (`:3295`) при `global:true` не подставляет `X-Chat-Id` → сервер идёт в глобальную ветку (`routes.py:814-833`, аудит), chat-ветка `delete_chat_key`/422 не задействуется. Паритет с `saveProviderSecret` (`:3864`); регресс-гейт `round1025_f9_secret_field_test.js:236-239` (`opts.global === true`). Прочие ветки (non-image empty-write `per_chat:false`→global, BYOK OFF, confirm=false, legacy) — корректны.
- **Low (RESOLVED):** **L-F9S-1** — `secret-field.maskText` (`:11464-11468`) делегирует в `secretDisplayOf`; мёртвый `blockSecretText` удалён, 2 теста переведены на `secretDisplay` (assert неизменён). **L-F9S-2** — недостижимая ветка `stateLabel['saved']` убрана (`:11398-11404`); тест ждёт `''`; «Сохранено» — тост F0. **L-F9S-3** — строгий контракт восстановлен: `FIXTURE == "2.58.15"` и `APP_VERSION == "2.58.16"` (`tests/test_round1025_f8_registry.py:102-107`). **L-F9S-4** — `evidence.md:25/53`: fixture убран из bump, помечен историческим.
- **Info:** I-F9S-1 осиротевший комментарий от удалённого `blockSecretText` (`web/app.js:5935-5936`, косметика); I-F9S-2 live TMA/WebView keyboard/safe-area — PENDING OWNER VERIFICATION.
- **Инварианты ✔:** **R17** (сырых секретов в DOM/POST/логах/тестах/артефактах нет — regex-скан 0; поле ввода секрета всегда пусто; guard'ы `hasSecretMask` целы в `dirtyKeyItems/saveKeyItem/saveBlock/testBlock/testField`; маска в API не уходит); **R18** (тег `pre-round1025-f9`, `.env.bak.round1025-f9`, `stash@{0}`); **Δ DDL=0** (`services/**`/`web/api/**`/миграции не тронуты, новый endpoint не создан); **Δ каталога=0** (459/98/96/21/418; `param_catalog.py` не тронут); CSP/zero-build (x-template); `APP_VERSION` **2.58.16** синхронен; XSS-safe (`{{ }}`/`:attr`, без `v-html`); F0-движок не переписан (`persistItems`×1, `notify`×1); §52–§67/F1/F4/F5/F6/F7/F8/BYOK не сломаны; маркер-тесты не ослаблены; удаление — отдельное действие + confirm; safe-area ровно один раз; гигиена (`git diff --check`=0, `.env`/`current_task.md`/zip/скриншоты/`tools/_ui_*`/`var/backups` в индексе отсутствуют).
- **Прогоны @Scanner (итер.2, независимо):** `node --check` OK; `tests/js/*.js` **40/40 OK**; `py -3 -m pytest -q` → **8421 passed / 0 failed / 1 skipped**, 5 env-failed (`rich`-fallback, `services/**` вне диффа, не регресс); каталог 459/98/96/21/418; `git diff --check`=0; R17-regex 0.
