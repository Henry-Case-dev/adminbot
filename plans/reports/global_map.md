...
## Эпик 2 / S9 `summary-testing-ui-round1026` (dry-run тест-контур «Тестирование» §113/§112) — 24.09.2026, Step 6 @Scanner (T-3374, итерация 2 после rework T-3375) + Step 5 @Reviewer (итерация 2 APPROVED) — **SCANNED: к деплою ДА (C0/H0/блокирующих Medium 0; Low 5 + 1 новый Low doc-drift `503`); dry-run 0/0/0 подтверждён; live — PENDING OWNER VERIFICATION**

- **Новые узлы (фактические зависимости):**
  - `services/summary_test_run.py` (`run_summary_test`/`TestRunResult`/`present_result`/`resolve_window_hours`; коды `TEST_*`, метки §112 `NO_DATA`/`UNLIMITED_LABEL`) → `services.summary_generator.SummaryGenerator.build_test_rows` (read-only окно `memory.db.get_smart_window` + S1 `_apply_filter` + S2 `_restore`) → `services.summary_context_restore.build_l1_payload` → `services.summary_l1_clusterizer.run_l1` (1 LLM, `step="l1_clusterizer"`) → `services.summary_fact_package.build_fact_package` (0 LLM) → `services.summary_l2_writer.run_l2` (1 LLM, `step="l2_writer"`) → `services.summary_article_formatter.format_rich_html`/`format_plain_html` (локальный рендер, не отправка). Метрики §112 ← `services.usage_events` + PG `llm_usage_events` по `correlation_id`; бюджет ← `services.budget_limits.context_state` + `limits.summary_max_context_tokens`. **Модуль НЕ вызывает** `telegram_send.*` / `SummaryGenerator._deliver_*`/`_send_*` / `summary_memory.*` / `generate_image` — dry-run инвариант 0 публикаций / 0 памяти / 0 image.
  - `web/api/summary_test.py` (`summary_test_router`, async 202 + polling, in-memory `_RunStore` TTL 15 мин/≤20/`MAX_CONCURRENT=2`, rate-limit 10 c) → `web/api/deps.requires_global_admin` + `services.roles.access_for` + `config.settings.SUMMARY_TEST_UI_ENABLED` (env-only ClassVar, default ON; OFF → 404) + `services.web_runtime.get_summary_generator`; включается в `web/app.py` (`routes.py` вне diff). Обложка — отдельным `POST …/cover` (`{"confirm":true}`) через существующий `services.image_generation.generate_image_verbose` (0 новых LLM-вызовов), превью `GET …/cover-image` (server-side temp, no-store).
  - `services/web_runtime.py` +`set_summary_generator`/`get_summary_generator` (+reset); вызов из `bot.py` после создания `SummaryGenerator`.
  - UI: `web/app.js` (`summaryTest` state, `summaryTestVisible`, watcher `workspaceTab`, `maybeLoadSummaryTest`/`summaryTestRun`/`summaryTestPoll`/`summaryTestConfirmCover`) + `web/index.html` (секция `data-summary-test` внутри существующей вкладки `testing` модуля `mod_summary`); tab-id `testing` **уже существовал** → **Δ каталога=0**.
- **Инварианты:** **Δ DDL=0**; **Δ каталога=0** (469/426/444/100/98/21); `APP_VERSION` 2.58.24→2.58.25; `summary_generator.py` аддитивно (55+/0−; тело `_run`/`_run_hybrid_l2` не тронуто); ON-путь живого пайплайна тест-контуром не активируется (`_hybrid_l2_enabled`/`_run_hybrid_l2` не вызываются); CSP/zero-build; R17/R18 (annotated-тег `pre-round1026-s9`→`cc6105c`).
- **Находки (итерация 2):** **[B-R1026S9-1, medium, requirement-gap] — RESOLVED** (§112 «Процент отсева»: `data-summary-test-drop` + computed `summaryTestDropPercent`, null → «Нет данных»). **[B-R1026S9-2 High / S-R1026S9-1 Medium, ui-render] — RESOLVED** (error-payload всегда с полными `metrics`/`artifacts`/`display` + диагностика; UI-guards не скрывают успех). **[S-R1026S9-2] Low (== L-R1026S9-3, purge running) — RESOLVED.** **[S-R1026S9-3] Low (== L-R1026S9-4, spec/README 503) — RESOLVED** для spec/README. **[S-R1026S9-4] Low (== L-R1026S9-5, fail-open `_filter_metrics`) — RESOLVED.** **[S-R1026S9-5] Low (NEW, non-blocking):** `services/summary_test_run.py:489` docstring + `adr-1026-8` D4 всё ещё упоминают `503 (TEST_NO_GENERATOR)` — реализация/spec §5.2 = `202`/`200`+`status="error"`. Прочие @Reviewer OPEN (non-blocking): L-R1026S9-2/-6/-7/-8. **ID-карта @Scanner:** `S-R1026S9-1`==`B-R1026S9-2`, `S-R1026S9-2`==`L-R1026S9-3`, `S-R1026S9-3`==`L-R1026S9-4`, `S-R1026S9-4`==`L-R1026S9-5`, `I-R1026S9-2`==`L-R1026S9-8`. Отчёт: `plans/reports/round1026_s9_scanner_audit.md`.

## Эпик 2 / S5 `summary-l2-writer-formatter-round1026` (L2 «Писатель» §97–§99 + серверный форматтер §98/§101/§102/§105; врезка за kill-switch) — 23.09.2026, Step 5 @Reviewer (T-3337) + Step 6 @Scanner (T-3338, итерация 2 после rework T-3339) — **SCANNED: к деплою ДА по коду (OFF-безопасное; ON — GATED, default OFF; C0/H0; блокирующих Medium 0)**

- **Новые узлы (фактические зависимости):**
  - `services/summary_l2_writer.py` (`build_l2_input`/`run_l2`/`parse_l2_document`/`validate_l2_document`/`resolve_l2_slot[_safe]`/`resolve_l2_max_paragraphs`/`L2Result`/`L2Slot`) → `services/summary_prompts.SUMMARY_L2_WRITER_SYSTEM_PROMPT` + `services/prompt_style_blocks.resolve_prompt` + `services/system2_handoff.parse_json_object` + `services/llm_client` (`generate` / dedicated `_post`+`_fallback_with_retries`) + `services/token_counter.count_tokens` + `services.hot_config.get` + `config.settings` (env-only `SUMMARY_L2_BASE_URL`/`SUMMARY_L2_MODEL_NAME`/`SUMMARY_L2_API_KEY`). Модуль изолирован: НЕ импортирует фильтр/восстановление/саммари-генератор, видит только `FactPackage` (§96). 1 LLM-вызов, `module="summary"`, `step="l2_writer"`.
  - `services/summary_article_formatter.py` (чистый stdlib; 0 LLM/БД/сети) → `services/outgoing_guard.sanitize_outgoing` (по умолчанию) для rich/plain. Теги только `img/h1/p/b`.
  - `services/summary_generator.py::_run` → (за флагом `SUMMARY_HYBRID_L2_ENABLED`, default OFF) `summary_l1_clusterizer.run_l1` → `summary_fact_package.build_fact_package` (`summary_context_restore.build_l1_payload`) → `summary_l2_writer.run_l2` → `summary_article_formatter.*` → существующие `send_rich_message(content_format="html")`/`send_text(..., parse_mode="HTML")`. Ветка `_generate_two_call` (OFF) сохранена.
  - `services/param_catalog.py` +1 промпт-ключ `prompts.summary_l2_writer_system_prompt` (группа `prompts_summary`, advanced, synthesizer); `services/prompt_migrations.py` — `PROMPT_MIGRATIONS`/`ROLLBACK_MIGRATIONS` + `PREV_SUMMARY_L2_WRITER_R1026`; эталон `plans/docs/canon/architecture.md`.
  - `services/telegram_send.send_rich_message(..., content_format="auto")` — новый параметр; `"html"` (ON) → `InputRichMessage(html=…)` без авто-детектора `_looks_rich`; `"auto"` (default) — байт-в-байт прежнее.
- **Инварианты:** **Δ DDL=0**; **Δ каталога = +1** (санкция ADR-1026-7 D3; F8 переиздан: REGISTRY 468→469, categorized 443→444, delta 57→58; sha256 `param_catalog.py` repin); `APP_VERSION` 2.58.23→2.58.24; **ровно 2 LLM-вызова** на обоих путях; §104/`web/**`/XML вне diff; R17/R18.
- **Подтверждено @Scanner (Step 6):** зависимости узлов — без изменений (см. выше). **Уточнение архитектуры:** ветка `_run_hybrid_l2` врезана **до** S1/S2 (`summary_generator.py:349-354`) и берёт сырые `rows` → `build_l1_payload` → `run_l1`, т.е. ON-путь **не** применяет S1 `_apply_filter`/S2 `restore_context` (вызов только в OFF `:377`) и **не** использует `focus`/`trigger_message_id` — drift от ADR-1026-7 D5/§80 [S-R1026S5-1, S6]. `telegram_send.send_rich_message(content_format="auto")` default сохраняет legacy-маршрут `_looks_rich`; `"html"` → `InputRichMessage(html=…)` без авто-детектора.
- **Открытый блокер:** `tests/fixtures/round1025/catalog_baseline.json` повреждён двойной перекодировкой (`nav_titles.{modules,ai,memory}`/`_provenance.note`, 302 mojibake-символа; hex `E2 95 A8…`) → `plans/features/summary-l2-writer-formatter-round1026/review.md` [B-R1026S5-1], подтверждён @Scanner. Отчёт @Scanner: `plans/reports/round1026_s5_scanner_audit.md`.
- **Unavailable:** ON-путь в проде — GATED (S6/S10+D4, моки); live-приёмка Эпика 1 — PENDING OWNER VERIFICATION.
- **Итерация 2 @Scanner (T-3338, re-audit после rework T-3339) — зависимости без изменений.** [B-R1026S5-1] **RESOLVED** (фикстура `catalog_baseline.json`: 0 mojibake, без BOM, `git diff HEAD`=3+/2−, JSON-дифф = только санкц.). [S-R1026S5-1] **RESOLVED**: врезка ON-ветки перенесена **после S1/S2** (`summary_generator.py:373-380`: `_apply_filter` = S1 `filter_window`+S2 `restore_context` → `xml_rows` → `_run_hybrid_l2(chat_id, xml_rows, focus, correlation_id)`); `trigger_message_id`→S1, `focus`→`focus_block` в `run_l1`; ON = ровно 2 вызова. Правки выше (стр. 12–13) отражают **итерацию 1** и на момент итерации 2 неактуальны. Новое [S-R1026S5-7] Info (S6-note): ON пропускает `memorize_facts(chat_history)` — по принятому ADR D5/spec §7.1, решение за S6. Инварианты: Δ DDL=0; Δ каталога=+1 (469/426/444/100/98/21); pytest 8807/0; JS 43/43; F8 `--check` OK; R17/R18.

## Эпик 2 / S4 `summary-fact-package-round1026` («Пакет фактов §96» — детерминированный сборщик L2-входа, 0 LLM) — 23.09.2026, Step 6 @Scanner (T-3301) — **SCANNED**

- **Новая подтверждённая зависимость (чистый слой сборки, изолирован от живого пути):** `services/summary_fact_package.py` (`build_fact_package(l1_result, payload_items, *, budget=None, correlation_id=None) -> FactPackageResult`; `resolve_fact_package_budget`; `serialize_package`; `FactPackage` v1) → `services/summary_l1_contract.py` (`IdSpace`/`build_id_space`, `STATUS_*`, `STATUS_NOT_BUILT`-семантика) + `services/token_counter.count_tokens`/`resolve_chat_limit` + `config.settings` (reuse `limits.summary_max_context_tokens`/`_chars`). Модуль — только stdlib + проектные модули: **0 LLM-вызовов, без БД/сети**; вход не мутируется; выход байт-идентичен на одном входе.
- **Поток данных (D1/D5):** `L1Result.payload` §95 (TG `message_id`) + §92-payload (`summary_context_restore.build_l1_payload`) → `FactPackage{threads[]}`: `name`=topic verbatim, `description`=агрегация `facts[].text` (дедуп/кап 500), `chronology`=ASC `(timestamp,message_id)`, `facts`/`evidence_ids` verbatim+union, `fragments` evidence-first (`FRAGMENT_MAX_CHARS=1000`, 30/тред, 500 всего), `service{response_mode,cover_prompt}` (транзит вне L2-контента). Fail-closed: `ok/truncated` — доставляемы; `empty/invalid/error` → `threads=[]`; нет `usable`/`payload` → `not_built`.
- **Δ/границы:** **Δ DDL=0**; **Δ каталога=0** (468/426/443/100/98/21; `param_catalog.py` вне diff; F8 не переиздавался); `APP_VERSION` 2.58.23; **врезка L1→пакет→L2 НЕ сделана** — GATED (S5/S6, ADR-1025-24 D4); `summary_generator.py`/`summary_xml.py`/публикация/обложка/`web/**` вне diff.
- **Оценка:** Critical 0 / High 0 / Medium 1 (процесс: нет `review.md` T-3300) / Low 2 (char-обрезка без WARN; O(n²)-оценка бюджета) / Info 2. **К деплою ДА по коду.** Отчёт: `plans/reports/round1026_s4_scanner_audit.md`. Handoff: @Orchestrator → SCANNED.

## Эпик 2 / S3 `summary-l1-clusterizer-round1026` (L1 «Кластеризатор» §94–§95: строгий JSON, ID-пространства, fail-closed; канон +1; env-only слот) — 23.09.2026, Step 6 @Scanner (T-3276) — **SCANNED**

- **Новая подтверждённая зависимость (серверный слой, изолирован от живого пути):** `services/summary_l1_clusterizer.py` (`pack_l1_input`/`build_l1_user_content`/`resolve_l1_slot`/`resolve_l1_budget`/`run_l1`; `MODULE="summary"`, `STEP="l1_clusterizer"`, `PROMPT_PG_KEY`) → `services/summary_l1_contract.py` (строгая схема §95: `validate_l1_response`/`parse_l1_response`/`build_id_space`/`L1Result{ok|empty|invalid|truncated|error}`) → `services/system2_handoff.py` (`parse_json_object`/`normalize_response_mode`/`normalize_cover_prompt`/`contains_system_ids`); `pack_l1_input` ← `services/summary_context_restore.build_l1_payload` (§92; TG `message_id`) + `services/summary_filter.estimate_and_split` (overlap=1, DB `id`) + `services/token_counter.count_tokens`/`resolve_chat_limit`; `run_l1` → `services/llm_client.LLMClient.generate(module="summary", step="l1_clusterizer")` (наследование глобальной модели) либо dedicated `_post(api_key=…, base_url=…)` + `_fallback_with_retries` (существующая `LLM_FALLBACK_*`-политика); промпт ← `services/prompt_style_blocks.resolve_prompt` + `services.summary_prompts.SUMMARY_L1_CLUSTERIZER_SYSTEM_PROMPT`; слот ← `services.hot_config.get` (hot-first, forward-compatible `models.summary_l1_*`/`keys.summary_l1_api_key`).
- **Пространства ID (D5):** вход/выход L1 — **TG** `message_id` (§92-payload); `Fragment.message_ids` — **DB `id`** → конвертация только через `build_db_tg_map` (неоднозначность/пропуск → `IdSpaceMismatch` → `invalid` `id_space_mismatch`); авто-`unassigned` для неупомянутых payload-id; `evidence_message_ids` ⊆ `message_ids` своего треда.
- **Канон (ADR-1013-3):** новый PG-ключ `prompts.summary_l1_clusterizer_system_prompt` (каталог +1: REGISTRY **468**/Settings **426**/categorized **443**; `settings_field=None`) → `services/summary_prompts.SUMMARY_L1_CLUSTERIZER_SYSTEM_PROMPT` + слепок `PREV_SUMMARY_L1_CLUSTERIZER_R1026` → ступень `PROMPT_MIGRATIONS`/`ROLLBACK_MIGRATIONS` (`services/prompt_migrations.py`) → эталон `plans/docs/canon/architecture.md` (байт-в-байт). Слот модели — **env-only** `SUMMARY_L1_*` (ClassVar, Δ каталога=0 в части слота; UI-слот — S5).
- **Гейт/врезка:** в живой путь **не врезан** (T-3273 DEFERRED; врезка — S5/S6 по санкции, гейт S6/S10 + ADR-1025-24 D4); `services/summary_generator.py`/`summary_xml.py`/публикация/обложка/`web/**` вне diff; 2-вызовность живая (`await_count==2`); логи `L1_START/COMPLETE/ERROR` (R17: числа/коды/id).
- **Инварианты:** Δ DDL=0; Δ каталога = +1; F8 переиздан (delta 56→57, `--check` OK, `sha256(param_catalog.py)`=`6531f6…7812`, `ROUTES_SHA256_F11`/`pg_db.py` целы); `APP_VERSION` 2.58.22; CSP/zero-build.
- **Находки:** Critical 0 / High 0 / Medium 1 (процесс: нет `review.md` T-3275) / Low 3 (L-R1026S3-1 fail-closed-обёртка слота; L-R1026S3-2 пустой evidence; L-R1026S3-3 `tg_message_id=None` edge) / Info 5. **Деплой — после T-3275.** Отчёт: `plans/reports/round1026_s3_scanner_audit.md`. Handoff: @Orchestrator → SCANNED.

## Эпик 2 / S2 `summary-context-restore-round1026` (детерминированное восстановление контекста Саммари §90–§92; pure-core + async-адаптер) — 23.09.2026, Step 6 @Scanner (T-3246) — **SCANNED**

- **Архитектурные зависимости (подтверждены):** `services/summary_context_restore.py` (`restore_context(kept, dropped, window, params, *, extra_parents, token_limit, char_limit, bot_id) -> RestoreResult`; `RestoreParams(context_neighbors=1, context_max_messages=50)`; `build_l1_payload` §92; `RESTORE_CHAIN_DEPTH=10`) ← `services/token_counter.count_tokens`, `services/database.row_get`; `services/summary_generator.py::_apply_filter` (врезка S2 между `filter_window` и `xml.build`) → `_restore` / `_collect_extra_parents` **reuse** `services/thread_chain.collect_thread_chain(db, chat_id, tg, depth)` + `db.get_smart_message_by_tg_id` / `get_bot_reply` / `get_bot_reply_parent`. `FilterResult` (S1) `kept`/`dropped`/`restored_count` — контракт передачи S1→S2; `RestoreResult` аддитивен, XML-промпты/публикация не затронуты.
- **Гейт/логи:** мастер-гейт `flags.summary_filter_reply_context_enabled` (per-chat, OFF → S2 не вызывается, S1 байт-в-байт); логи `RESTORE_START/COMPLETE/ERROR` + `FILTER_COMPLETE.restored_count` с `run_id`; метрики §109/S8 в `self._filter_metrics[chat_id]` (аддитивно, при OFF байт-в-байт как S1).
- **Инварианты:** Δ DDL=0; Δ каталога=0 (467/426/442/100/98/21); 2 LLM-вызова; CSP/zero-build; `APP_VERSION` 2.58.21.
- **Находки:** Critical 0 / High 0 / Medium 0 / Low 2 (L-R1026S2-1 docstring «без часов» vs `perf_counter`; L-R1026S2-2 perf watch `_collect_extra_parents` ≤50 chain-walk) / Info 2. **К деплою ДА.** Отчёт: `plans/reports/round1026_s2_scanner_audit.md`. Handoff: @Orchestrator → SCANNED.

## EXTRA-визуальный эпик `polygonal-luminescence-round1026` (Polygon Canvas2D + Delaunator 5.0.0, адаптер `__AuroraFlow`, env-only `UI_POLYGON_BG_ENABLED`) — 23.09.2026, Step 6 @Scanner (T-3214) — **SCANNED**

- **Новая подтверждённая зависимость (клиентский слой, zero-build, CSP-safe):** `web/index.html` `<script src="/static/vendor/delaunator.5.0.0.min.js?v=__APP_VERSION__">` (vendored IIFE, ISC, SHA-256 `7707D7FE750559D4D31DBDEBEA5E41024487520BC1CC9967AC9795B4A682C1BE`) → `web/static/polygon-background.js` (`window.__PolygonBackground` = Canvas 2D рендерер: триангуляция `Delaunator.from`, `buildScene`/`rebuildTopology`/`draw*`, SEED+mulberry32) → `web/app.js::_syncBgLayer` (выбор ровно одного активного рендерера по `polygonBgEnabled`). Сборка — `tools/vendor/{package.json,package-lock.json,build.mjs,.delaunator-entry.mjs}` (esbuild 0.25.9, `globalThis.Delaunator`); лицензия/версия/SHA — `web/static/vendor/README.md`. **CDN/инлайна нет**; SHA пин `ROUTES_SHA256_F11` переиздан `76bcab3c…3306` → `4b652cb1…551b` (аддитивный bool во `ui_flags`).
- **Адаптер совместимости (вызовы `app.js` НЕ переписаны):** `polygon-background.js` сохраняет исходный Aurora как `window.__AuroraFlowLegacy` и подменяет `window.__AuroraFlow` фасадом `{start,stop,resize,setPaused,sample,mode,__polygonAdapter}`; `app.js::_legacyAuroraFlow()` предпочитает `__AuroraFlowLegacy` и игнорирует адаптер (маркер `__polygonAdapter`). `aurora-flow.js` (OGL) сохранён как **soft-откат** (`UI_POLYGON_BG_ENABLED=false`), не удалён.
- **CSS/слой:** `web/static/app.css` `.polygon-background-canvas { position:fixed; inset:0; z-index:0; pointer-events:none }` + `html.polygon-bg` гасит `.aurora-flow-canvas`/`.aurora-bg`/`body::before/::after`; canvas — прямой child `body` (вне max-width-контейнеров), `#app { z-index:1 }` выше. Второй механизм высоты/`ResizeObserver` не создан (`measure()` по `documentElement`, как в `aurora-flow.js`).
- **Флаг/доставка:** `config/settings.py` env-only `UI_POLYGON_BG_ENABLED: ClassVar[bool]` (default ON, ∉ `param_catalog`) → `bool()` в `/api/me.ui_flags` (`web/api/routes.py`) → computed `polygonBgEnabled` (`web/app.js`) → `_syncBgLayer`. Δ каталога = 0 (467/426/442/100/98/21); Δ DDL = 0.
- **Стекло (D6/J, без интеграции):** `web/static/glass.js::detectMode` получил честный путь `data-render="webgl"`→`refraction`; `mountGlass` в рантайме по-прежнему вызывается только для изолированного `[data-glass-surface]` (функциональные селектор/⛶/карточка — без стекла); `UI_LIQUID_GLASS_LIB` default **OFF**; блок J **заблокирован** гейтом §13.2 (7 проверок не воспроизводимы headless); hotfix10 не повторён.
- **Миграция Aurora (SUPERSEDE/AMEND):** SUPERSEDE ADR-1025-17 D6 (активный рендерер → Polygon); AMEND ADR-1025-18 D1 (стекло) и D6 (flags-матрица + `UI_POLYGON_BG_ENABLED`); REUSE ADR-1025-13 D1 (единый источник высоты). Матрица §71 `tools/ui_round1025_matrix.py` фиксирует `UI_POLYGON_BG_ENABLED=False` (проверяет сохранённый Aurora-путь); полигон-путь — `tools/ui_round1026_polygon.py`.
- **Вердикт (Step 6 @Scanner):** Critical 0 / High 0 / Medium 0 / Low 3 / Info 4 — **к деплою ДА** (блокеров нет). Инварианты ✅: Δ DDL=0, Δ каталога=0, CSP/zero-build, один активный рендерер, детерминизм (без `Math.random`), R17/R18, a11y. Прогоны @Scanner: JS `POLYGON-LUMINESCENCE-OK`, `test_webapp_round1026_polygon.py` 18, `js_unit`+`f8_registry` 59, `git diff --check`=0. Live-гейты (перф/стекло/WebView) — PENDING OWNER. Отчёт: `plans/reports/round1026_visual_scanner_audit.md`. Handoff: @Orchestrator → SCANNED.

## Эпик 2 / S1 `summary-filter-round1026` (алгоритмический префильтр Саммари §87–§89/§93; каталог `summary_filter_*` +8/+2; врезка во вход L1) — 23.09.2026, Step 6 @Scanner (T-3153) — **SCANNED (итер.2, финал)**

- **Новая подтверждённая зависимость (серверный слой, 0 LLM):** `services/summary_filter.py` (новый, чистый: `score_message`/`filter_window`/`estimate_and_split`; вход — строки `get_window_messages`; `children_by_tg` по **`tg_message_id`**) → импортируется `services/summary_generator.py` (`_apply_filter`, врезка строго между `get_window_messages` и `xml.build`; фильтруется **только XML-история** `xml_rows`, RAG/память/graph/memorize — на исходных `rows`); `summary_filter` → `services/token_counter.count_tokens`. Fail-open + OFF = `xml.build(rows)`. **Per-chat резолвинг** (итер.2): мастер-тумблер `flags.summary_filter_enabled` — через `_chat_limit(chat_id, …)` (чат A ≠ чат B, spec §6.1/SC-05); потолок токенов — через `resolve_context_tokens`.
- **Каталог/UI (санкция ADR-1026-1 D1):** `config/settings.py` +8 `SUMMARY_FILTER_*`; `services/param_catalog.py` +8 записей, +2 группы `flags_summary_filter`/`limits_summary_filter` → `TAB_RULES[TAB_MOD_SUMMARY]`; `web/app.js` — новый workspace-таб `prep` (`workspaceGroupTab` → 'prep', `mod_summary.tabs` уже содержит `prep`), `web/index.html` — prep рендерит generic-сетку (placeholder снят), placeholder остаётся для clusterizer/writer. Δ JS-каталога = 0.
- **Метрики/логи:** `self._filter_metrics[chat_id]` (аддитивно, без узлов ExecutionGraph — kind `algorithm` эмитит S8); `FILTER_START/COMPLETE/ERROR` (+`FILTER_EMPTY_FALLBACK`) с `run_id=correlation_id`, только числа/коды/id (R17). Контракт для будущих S2/S3: `kept`/`dropped`/`fragments`/`budget`/`scores`.
- **Тесты (итер.2):** `tests/test_summary_filter.py` (31 unit) + **новый** `tests/test_summary_filter_integration.py` (7: per-chat A/B, OFF байт-в-байт, RAG по исходному окну, fail-open, ровно 2 LLM-вызова, sentinel-потолок).
- **Frozen F8 / ADR-1026-2 (итер.2):** переиздание замороженных артефактов F8 при санкционированном Δ каталога — **штатная процедура** (AMEND ADR-1025-21 D6); `plans/ARCHITECTURE.md §67.6` + `adr-1026-1` AMEND. Байтфризы `pg_db.py`/`routes.py` и строгий sha256-пин сохранены (ребазированы, не ослаблены); 467/100/98/21, delta 56; `app_version` фикстуры исторический 2.58.15.
- **Вердикт (итер.2, финал):** Critical 0 / High 0 / Medium 0 / Low 1 (L-R1026S1-3, non-blocking) → **к деплою ДА**. Закрыты: M-R1026S1-1, L-R1026S1-1, L-R1026S1-2 (rework @Builder), B-1 @Reviewer (@Architect ADR-1026-2). Инварианты ✅: D4-гейт, 2-вызовность, Δ DDL=0, каталог 467/426/442/100/98/21, R17/R18, CSP/zero-build, `git diff --check`=0. Прогоны @Scanner: `.venv` pytest **8501/0**, JS **42/42**. Отчёт: `plans/reports/round1026_s1_scanner_audit.md`. Handoff: @Orchestrator → SCANNED.

## Round 10.25 F10 `epic1-verification-round1025` (верификационный стоп-гейт Эпика 1) — 23.09.2026, Step 6 @Scanner (T-3122) — **SCANNED**

- **Новая подтверждённая связность — только инструменты (НЕ рантайм, zero-build):** `tools/ui_round1025_e2e.py` (E2E §72/§73/§74) и `tools/ui_round1025_secrets_ui.py` (§78 UI-секреты) **импортируют** `tools/ui_round1025_matrix.py` (`M._Handler`/`M.CONFIG_STUB`/`M.CONFIG_STUB_CHAT`/`M.TMA_STUB`/`M._stub_for`/`M.REPO`/`M.PORT`+offset) — переиспользование локального `ThreadingHTTPServer` + `page.route('**/api/**')` + init-stub `Telegram.WebApp`, без новых библиотек. Артефакты прогонов `tools/_ui_round1025_{e2e,secrets_ui,raw}.json` + `_ui_round1025_shots/` — вне git (root `.gitignore:101-102` + `tools/.gitignore`). Рантайм (`services/**`,`web/**`,`config/**`, миграции, `param_catalog.py`) **не тронут** — F10 не создаёт рантайм-зависимостей.
- **Аддитивное расширение харнесса:** `tools/ui_round1025_matrix.py` — `LOW_TG_WINDOW={1280×400, stable_offset=120}` + `_low_tg_failures()` + отдельный low-window проход (базовые 10 вьюпортов × 5 режимов не изменены). Стимул задаётся через CSS-переменные (`--tg-viewport-stable-height`/`--app-usable-height`/`--tg-viewport-bottom-offset`) — те же, что читает `web/static/app.css` (установленная с hotfix4/6/9 техника).
- **Точка отката Эпика 1:** annotated-тег `pre-round1025-f10` → `57b325c` (== прод, `APP_VERSION` 2.58.17); deploy F10 **NOT_APPLICABLE** (ADR-1025-24 D1).
- **Вердикт (Step 6 @Scanner):** Critical 0 / High 0 / Medium 0 / Low 2 / Info 4 — **блокеров нет; приёмка Эпика 1 (авто) — ДА**, live-гейт Telegram WebView **PENDING OWNER**; Эпик 1 не объявляется завершённым. Инварианты: Δ DDL=0, Δ каталога=0 (459/98/96/21), CSP/zero-build, R17/R18; `py -3` pytest 8457/5env/1, `.venv` 8463/0, JS 42/42, `git diff --check`=0. Отчёт: `plans/reports/round1025_f10_scanner_audit.md`. Handoff: @Orchestrator → SCANNED.

## Round 10.25 F11 `status-showcase-dashboard-round1025` (витрина «Статус» §11–§21; kill-switch `UI_STATUS_GRID_V2`) — 23.09.2026, Step 6 @Scanner (T-3092) — **SCANNED (итер.2, финал)**

- **Новая подтверждённая зависимость (клиентский слой, zero-build):** `web/index.html` `.status-grid` (12 кол.) + обёртки `.sg-5/7/8/4/12` + `.sg-12` + `.status-graph-full` (полный экран графа) → `web/static/app.css` `.status-grid`(12→6→1)/`.status-hero`/`.status-counts`/`.status-sleep`/`.status-facts`/`.status-budgets`/`.status-graph-full`/`.status-grid--legacy` → `web/app.js` computed `statusGridV2`/`statusSys`/`botLastActivity`/`sleepWidget`/`graphGroupOptions`/`factsFeed`/`graphFullVisible` + методы `loadLogCounts`/`scrollToLogs`/`graphSelectNode`/`focusGraphNode`/`applyGraphFilter`/`graphResetView`/`openGraphFull`/`closeGraphFull`.
- **Новые маршруты/флаги:** аддитивный hash-маршрут `#/status/graph` (`ROUTE_TO_TAB`→`status`, `ROUTE_PARENT`→`#/`, depth 1; fallback-шторка `graphFullOpen` при `IA_V2_ENABLED=false`) — владелец F1. env-only `UI_STATUS_GRID_V2` (`config/settings.py` ClassVar default ON → `bool()` в `/api/me.ui_flags` → `statusGridV2`); **Δ каталога=0**, `web/api/routes.py` — аддитивно (bool `UI_STATUS_GRID_V2` в `ui_flags` + вычисляемое `counts` в `/api/status/logs`); `services/log_ring.py` — read-only `LogRingHandler.level_counts()`.
- **Потребители (reuse, без новых API/поллеров):** `GET /api/status` (Hero/метрики), `GET /api/status/logs` (счётчики §20 — аддитивное `counts`), `GET /api/memory/cognition/status` (сон §17, единый серверный `generated_at`), `GET /api/memory/graph` (граф §16; вес/`barnesHut`/`_graphSignature` не менялись, только клиентские фильтр/фокус/соседи), `GET /api/oversight/dossier_feed` («Новые факты»), `GET /api/workers/budget` + `status.context` (бюджеты), `GET /api/status/logs` (§20 счётчики). F6 `execution_graph.js`/`/analytics/*` не тронуты; §15 heartbeat, §18-ленты, §20 viewer — не переписаны.
- **Вердикт (итер.2, финал):** Critical 0 / High 0 / Medium 0 / Low 0 / Info 2 — **К ДЕПЛОЮ ДА**. Итер.1 закрыта: H-F11S-1 (счётчики — аддитивное `counts`, реальные числа), M-F11S-1 (OFF = одноколоночный безопасный режим, формулировки исправлены), L-F11S-1/2/3 + I-F11S-1. Δ DDL=0; Δ каталога=0 (459/98/96/21); `APP_VERSION` 2.58.17; JS 42/42; pytest 8457/5 env/1 skip, F11 33; 5 env-fail pre-existing; R17/R18. Отчёт: `plans/reports/round1025_f11_scanner_audit.md`. Handoff: @Orchestrator → SCANNED.

## Round 10.25 F9 `secrets-and-save-states-round1025` (UI секретов §50 — маска как display-индикатор; состояния §51; визуальный добор SaveBar §69/§78) — 23.09.2026, Step 6 @Scanner (T-3055)

- **Новая подтверждённая зависимость (клиентский слой, zero-build):** `web/index.html` `secret-field` (7 шаблонов: generic ×3 `:1436/:1553/:1832`, provider ×3 `:495/:718/:1196`, BYOK `scope='chat'` `:1601`) → `web/static/app.css` `.secret-field*` → `web/app.js` (`secretDisplayOf` — единый источник маски: `secretDisplay`/компонент `maskText`, `deleteKeyItem`, `_syncKeyboardOffset`) → существующие серверные поверхности **без нового endpoint**: `DELETE /api/config/keys/own/{key}` **`global:true`** (глобальная ветка + аудит `record_global_secret_audit`) и F0-write-path `persistItems([{value:''}])` → `POST /api/config` (global). **`web/api/routes.py` не менялся (Δ DDL=0).**
- **SaveBar (§69/§78, добор поверх F0 без переписывания движка):** `visualViewport` → `--kb-offset = innerHeight − (vv.height + vv.offsetTop)` на `documentElement` → `.modal-backdrop`/`.modal-actions`/`.sticky-save`/`.field`; safe-area ровно один раз (`.sticky-save`); `.sticky-spacer`/`scroll-padding-bottom`/`scroll-margin-bottom`; SaveBar остаётся в `footer.modal-actions` вне `.modal-body`.
- **Потребители/связи:** F8-реестр/чек-лист (`plans/reports/round1025_f9_secrets_checklist.md`, 28 секретов = 20 UI + 8 env-only); BYOK (`keyStatusOwn` → `byokSecret`); F7-PERMsoc/F5-workspace/F6/F4/F1 — reuse без изменений; `services/param_catalog.py` не тронут (Δ каталога=0, 459/98/96/21/418).
- **Вердикт (итер.2, финал):** Critical 0 / High 0 / Medium 0 / Low 0 / Info 2 — **К ДЕПЛОЮ ДА**. Итер.1 (High 1 + Low 4) закрыта: **H-F9S-1** — `deleteKeyItem` image-ветка теперь `DELETE /api/config/keys/own/{key}` с `{global:true}` (иначе `api()` подставлял `X-Chat-Id` → chat-ветка → `delete_chat_key` 422); **L-F9S-1** — `maskText` делегирует в `secretDisplayOf`, `blockSecretText` удалён; **L-F9S-2** — ветка `stateLabel['saved']` убрана; **L-F9S-3** — строгий `APP_VERSION == "2.58.16"`; **L-F9S-4** — evidence про fixture исправлен. Инварианты: Δ DDL=0, Δ каталога=0, CSP/zero-build, R17 (сырых секретов нет; поле ввода всегда пусто; guard'ы целы), R18 (`pre-round1025-f9`, `.env.bak.round1025-f9`, `stash@{0}`), `APP_VERSION` 2.58.16. Отчёт: `plans/reports/round1025_f9_scanner_audit.md`. Handoff: @Orchestrator → `SCANNED`.

## Round 10.25 F8 `parameter-registry-widget-map-round1025` (реестр параметров §2 + сохранность конфигурации §3 + §117 п.1–4) — 23.09.2026, Step 6 @Scanner (T-3016)

- **Артефакты-первоисточники (committed, additive, read-only):** `plans/docs/param-registry-round1025.tsv` (459×23, `internal_key == ParamSpec.pg_key`) → `plans/docs/screen-map-round1025.md` (459, `old_screen→param_key→new_screen→read/write_api→ui_visibility`) + `plans/docs/widget-map-round1025.md` (`old_widget→data_source→new_location→preserved_actions`, `api-only` §) + провенанс `plans/docs/param-registry-round1025.meta.md` (HEAD/APP_VERSION/счётчики/дельта 48). Генератор — `tools/gen_param_registry_round1025.py` (stdlib + `services.param_catalog` + `config.settings`, **вне рантайма**, `--check` = exit≠0 при расхождении; вход для дельты — `plans/archive/settings-persistence-audit-round1014/inventory.tsv` 411).
- **Сохранность конфигурации:** `tools/config_snapshot_diff.py` (read-only, 8 срезов global/chats/dm/permsoc/prompts/models/connections/roles; классификация по `pg_key`/`chat_id`; R17-safe) → `plans/reports/round1025_f8_config_diff.md`. Снимки — `var/backups/f8-round1025-config/` (вне git); живой PG-контур отложен на @DevOps T-2998/T-3000.
- **Потребители/связи (проверено):** F9 (секреты-UI читает `{configured,last4}`/реестр), **F10** (переиспользует method snapshot+diff как гейт — см. L-F8S-2), F11 (карта виджетов). Тесты `tests/test_round1025_f8_registry.py` импортируют оба `tools/*` (тесты ≠ рантайм). Рантайм-импортов F8-инструментов нет (маркер `test_tools_not_imported_by_runtime`); `tools/` исторически не полностью изолирован (`services/tool_router.py` → `tools.video_downloader`, pre-existing, не F8).
- **Вердикт (итер.1):** Critical 0 / High 0 / Medium 0 / Low 3 / Info 4. **Блокеров нет; к приёмке — ДА.** Инварианты: Δ DDL=0, Δ каталога=0 (459/98/96/21/418), роуты 31 без изменений, `APP_VERSION` 2.58.15, CSP/zero-build, R17 (28 секретов — только маска)/R18 (`pre-round1025-f8`@`a40f244`, `.env.bak.round1025-f8`, `stash@{0}`). Deploy — **NOT_APPLICABLE**. Отчёт: `plans/reports/round1025_f8_scanner_audit.md`. Handoff: @Orchestrator → `SCANNED`.
