...
- Рабочий untracked-файл `deploy_commands.txt` (`.gitignore`) содержит plaintext-учётки/токены. В git/индекс/коммиты **не попал** (проверено `git status --ignored`, скан диффа — чисто). Рекомендация владельцу: хранить через менеджер секретов; ротация — по решению владельца (прецедент R17/UPD3 §4). **✅ ЗАКРЫТО решением владельца (22.09.2026): `deploy_commands.txt` не трогать, ротация НЕ выполняется** — осознанный отказ владельца (не техдолг, действий не требует; KG `deploy_commands_policy_round1025`).
