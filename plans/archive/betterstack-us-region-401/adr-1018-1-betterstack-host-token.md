...
- **Статус:** Proposed (**подтверждён владельцем, UPD п.4:** «BetterStack (env host) … принимается „как есть“» — решения D2/D3/D4/D5/D6/D7/D8 без изменений; итерация 2, 15.09.2026)
- **Дата:** 2026-09-15
...
- **SUPERSEDE/AMEND:** **AMEND** неявный контракт раунда 4/5 (`DEFAULT_HOST` = EU; «сравнение с `SENTRY_DSN` не делаем»). Не supersede целиком — заменяется только host-политика и стартовая диагностика.
...
В отличие от снятого в раунде 5 запрета на любые сравнения, вводится **точное** сравнение: `token_equals_sentry_public_key(token, dsn)` (извлечь userinfo из `SENTRY_DSN` → `hmac.compare_digest`). При совпадении — **WARNING на старте** (не блокирует старт). Это устраняет главный источник 401 без риска ложных срабатываний.
...
Маркер `[betterstack] attached | host=<host> | token_len=N | last4=XXXX | from=LOGTAIL_SOURCE_TOKEN`; хост не секрет, токен — нет. При пустом host/token — `skipped` с причиной. `_HINT_401` дополняется указанием «host/токен одного проекта/региона».
...
Обязательный наблюдательный шаг (T-1704): 4 комбинации {US, EU} × {Source Token, public key}; ожидаемый 200 только на (US × Source Token). Вывод — только HTTP-код (R17).
