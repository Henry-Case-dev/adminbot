...
- Политика усечения окна: **keep-end с приоритетом якоря** (R1023F2-10) — сохраняются целевое сообщение (`anchor_message_id`), свежие `after`, затем цепочка, затем ближние `before`; первыми вытесняются самые старые `before`.
- Бюджет рендера: `format_chat_context(..., reply_chains=..., anchor_message_id=...)` гарантирует **длину всего `<chat_context>` ≤ `max_chars`** (обёртка + окно + цепочка); ходы цепочки вытесняются с дальнего конца, при нехватке места под-блок опускается. Отдельный жёсткий потолок цепочки — `_REPLY_CHAINS_MAX_CHARS = 1200` (R1023F2-01). Для окон сверх бюджета усечение может отличаться от до-F2 алгоритма (R1023F2-11).
...
- **Legacy `FACTCHECK_CONTEXT_MESSAGES`:** активный код-путь **не читает**; одноразовая идемпотентная миграция `migrate_factcheck_context_defaults` переносит его значение в `before` при guard'ах (R1023F2-08): legacy == своему code-дефолту → skip (мигрировать нечего); `before` задан не-дефолтным значением → skip (кастом/env не затираем); `before` отсутствует или равен дефолту → переносим. Явный `before == дефолту` неотличим от сид-дефолта — задокументированное ограничение (ADR §«Известные ограничения»). Ключ остаётся в реестре (пины/права), но уходит из UI-вида фактчека (внутренний).
