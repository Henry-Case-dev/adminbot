...
### Epic 57: /info → rich text статья (заголовки H1/H2, цитаты → italic+подчёркивание+жирный) — 🚧 IN PROGRESS (пользовательский запрос, 2026-08-23, Шаг 1 @PM ✅, target v2.41.0)
...
- [ ] T-445 (@DevOps, P0) — бэкап `info_text.md.bak.epic57` ДО pull (серверный == HEAD `1d9bf61` — сверить); коммит (код+канон+тесты+планы, БЕЗ untracked mp4 `danger_boom_gif-03.mp4`) + пуш origin/master; деплой: `git pull --ff-only`, restart, 0 traceback, smoke /info (grep-сверки). **DoD:** бэкап есть; коммит запушен (без mp4); pull ff; новый PID; 0 traceback; smoke OK. **READY (Шаг 6 @Memory, после T-444 ✅ APPROVED WITH NOTES).**
- [ ] T-446 (@Docs, P1) — README v2.41.0 (иронично) + MEMORY; версия v2.41.0 (minor, D226 — прецедент D223). **DoD:** доки актуализированы; Epic 57 CLOSED. **Docs-часть (T-446-A) ✅ DONE (Шаг 6 @Memory): README v2.41.0 + MEMORY + board; финализация + Epic 57 CLOSED — после деплоя T-445.**

**Updated:** 2026-08-23 — **Epic 57 🚧 IN PROGRESS (Шаг 6 @Memory): ✅ IMPLEMENTED + REVIEWED (APPROVED WITH NOTES) — DEPLOY PENDING (@DevOps T-445)**. T-442 ✅ DONE (@Architect, Шаг 2) — Rich Messages ОТВЕРГНУТЫ (блокерная деградация «This message is not supported»); эмуляция H1=`<b><u>` / H2=`<b>` / цитаты=`<b><i><u>`, parse_mode HTML; aiogram 3.29.1 апгрейд НЕ нужен (sendRichMessage уже есть с v3.29.0); Section 53.3 «Дополнение Epic 57». T-443 ✅ DONE (@Builder, Шаг 4) — текст verbatim, 5 мест синхронны, счётчики 34 b / 27 i / 28 u / 2 a, 91 сущность (<100), 2964 симв. (<4096), 2354 passed / 0 failed. T-444 ✅ DONE (@Reviewer, Шаг 5) — APPROVED WITH NOTES (замечания закрыты @Docs). T-446-A (@Docs) — README v2.41.0 + MEMORY + board ✅ (T-446 — финализация после деплоя, Epic 57 CLOSED). T-445 (@DevOps) READY (Шаг 7). База: прод v2.40.0 (`1d9bf61`, PID 1054487), 2354 теста, Epics 1–56 ALL CLOSED. Без @Orchestrator.**
