...
### Epic 59: Документационно-исследовательский: память бота + замеры на проде + полная переписка README — 🚧 IN PROGRESS (пользовательский запрос, Шаг 5: re-review, target v2.42.1, 2026-08-24)

> Полный трек — `plans/backlog.md` (Epic 59). Требования R59-1…R59-5, решения D232–D234.
> Три направления, изначально docs-only, но включена канон-синхронизация /info с правкой
> пользователя (D224) — код затронут точечно (`services/info_service.py::DEFAULT_INFO_TEXT`):
> (1) RESEARCH_HUMAN — новый Блок 8 «Память: как улучшить
> и оптимизировать» (после Блока 7, №52+, без дублей с №10–20, без рекомендаций по приватности) +
> удаление «Итоговой шпаргалки» (~646–703) и «Что дальше» (~707–713); (2) замеры памяти на проде
> (df -h, local_database.db* + WAL/SHM, media/venv/.git, journalctl --disk-usage, таблицы БД read-only)
> + оценка роста ретеншена FULL 30→180/365, ARCHIVE 90→180/365, «без лимита»=вечность (раздельно
> L2/L3/GraphRAG, суммарно, МБ); (3) полная переписка README (без changelog — суть в «Функции» ДО
> удаления; env-таблица 141 поле settings.py; понятно не-программисту, иронично). v2.42.1 (patch/chore,
> D232 — прецедент D218, исходно docs-only + канон-синхронизация D224). Канон /info синхронизирован
> с правкой пользователя (D224); тесты/RESEARCH.md НЕ трогаем; деплой ТРЕБУЕТСЯ — сверка серверного
> `info_text.md` == HEAD `c455728`, бэкап `info_text.md.bak.epic59` ДО pull, `git pull --ff-only`,
> grep-сверки, `systemctl restart admin_bot` ОБЯЗАТЕЛЕН, 0 traceback, новый PID (D234). База: прод
> v2.42.0 (`c455728`, PID 1063356), 2360 тестов, Epics 1–58 ALL CLOSED. Без @Orchestrator.

- [x] T-452 (@Researcher, P0) — ресёрч duckduckgo → exa (context7 НЕ нужен) по долгосрочной памяти LLM-ботов / GraphRAG / RAG retention/decay/consolidation; RESEARCH_HUMAN: новый «Блок 8» после Блока 7 (№52+), человеческим языком, без дублей с №10–20, без приватность-рекомендаций; УДАЛИТЬ «Итоговую шпаргалку» + «Что дальше»; RESEARCH.md не тронут. **DoD:** рекомендации с что/зачем/цена/приоритет; стиль сохранён; Блоки 1–7 нетронуты; шпаргалка удалена; чекбоксы не проставлены. ✅ **DONE (Шаг 2 @Researcher):** Блок 8 добавлен (№52+, что/зачем/цена/приоритет), шпаргалка и «Что дальше» удалены без битых ссылок, RESEARCH.md не тронут, чекбоксы не проставлены.
- [x] T-453 (@DevOps, P0) — SSH nik@198.46.175.136: `df -h`, `ls -la local_database.db*`, `du -sh media/ venv/ .git/`, `sudo journalctl --disk-usage`, размеры таблиц БД read-only (sqlite3, без изменения данных); оценка роста FULL 30→180/365, ARCHIVE 90→180/365, «без лимита» (вечность) — раздельно L2/L3/GraphRAG + сумма (МБ), с учётом сжатия L2→L3 и возраста чата. **DoD:** цифры в отчёте (МБ); пароль не залогирован; прод-конфиг не изменён. ✅ **DONE (Шаг 2 @DevOps):** БД 14.9 MiB (+WAL 4.0 MiB, суммарно ~19.5 MiB), свободно 17 ГБ (диск 24G), media 96M, venv 166M, journalctl 1.4G. Темпы: L2 0.31 МБ/д (11923 строк за ~7.9 д, 213 Б/сообщение), L3 ~0.48–1.45 МБ/д (после старта компрессии; сейчас 0), GraphRAG ~1.0 МБ/д (79 фактов/д × 12.3 КБ). Сценарии +1 год: S0 692 МБ (4.0%), S1 597, S2/S3 478. Находка: векторы graph_facts_vec не чистятся при TTL — кандидат в бэклог. Допущения: линейная экстраполяция, L3 — оценка.
- [x] T-454 (@Docs, P0) — полная переписка README.md: Что это → Функции (все 20+, суть из changelog: DirectChat/SmartCache/Checkup/YouTube/Web/инфо/GraphRAG/Summary/CB+фолбэк — перенести ДО удаления) → Параметры/env (полная таблица 141 параметр .env (139 полей settings.py + SENTRY_DSN/LOGTAIL_SOURCE_TOKEN из bot.py): дефолты, на что влияют, где менять — прод .env vs .env.example) → Как менять тексты → Telegram ID → Запуск/тесты → Мониторинг → Нюансы; без changelog, иронично, не-программисту понятно. **DoD:** все функции описаны; env-таблица полная; «где менять» понятно; тесты-счётчики README проверены (PM: их нет); 0 регрессий (2360). ✅ **DONE (Шаг 4 @Docs):** README переписан полностью, changelog удалён (суть в «Функции»), env-таблица 141 параметр .env (139 полей settings.py + SENTRY_DSN/LOGTAIL_SOURCE_TOKEN из bot.py), 2360/0, git diff --check чист.
- [x] T-455 (@Reviewer, P0) — RESEARCH_HUMAN (шпаргалка удалена, Блок 8 без дублей, стиль) + README (полнота функций/env, понятность) + арифметика DevOps-оценки + полный pytest 0 регрессий (2360). ✅ **DONE (Шаг 5 @Reviewer): REJECTED — БЛОКЕР-1** (заявлено docs-only «без кода/деплоя/рестарта» при фактической канон-синхронизации D224 с точечной правкой кода; 145 ≠ 141 поле env). Правки внесены @PM — **после правок APPROVED pending, re-review**.
- [ ] T-456 (@DevOps, P1) — коммит `docs(epic59): …` — 8 файлов (README, plans/RESEARCH_HUMAN, plans/backlog, plans/board, plans/MEMORY, info_text.md, services/info_service.py, plans/ARCHITECTURE; БЕЗ untracked mp4 `danger_boom_gif-03.mp4`/`.env`/db) + пуш origin/master; деплой ТРЕБУЕТСЯ — сверка серверного `info_text.md` == HEAD `c455728`, бэкап `info_text.md.bak.epic59` ДО pull, `git pull --ff-only`, grep-сверки (H1 без «с выходом в сеть internet», «Способ 1/2/3» без ведущих пробелов, h1=1/h2=6/b=27/i=27/u=0/a=2), `systemctl restart admin_bot` ОБЯЗАТЕЛЕН, 0 traceback, новый PID.
- [ ] T-457 (@Docs/PM, P1) — MEMORY.md (Epic 59, v2.42.1, фактура замеров) + board [x]; Epic 59 CLOSED.

**Updated:** 2026-08-24 — **Epic 59 🚧 IN PROGRESS (Шаг 5: re-review)**: Шаг 1 @PM ✅ (R59-1…R59-5, D232–D234, T-452…T-457). Шаги 2–4 ✅ DONE: T-452 @Researcher (Блок 8, шпаргалка удалена), T-453 @DevOps (замеры + оценка роста, сводка в строке задачи), T-454 @Docs (README переписан, 141 поле env, 2360/0). Шаг 5 @Reviewer: **REJECTED — БЛОКЕР-1** (план/факт рассинхрон: docs-only «без кода/деплоя/рестарта» vs канон-синхронизация D224 с точечной правкой кода; 145 ≠ 141). Правки внесены @PM: деплой ТРЕБУЕТСЯ, restart ОБЯЗАТЕЛЕН, 8 файлов — re-review @Reviewer.

### Epic 58: /info → настоящий Rich Message (sendRichMessage; H1/H2 честные, цитаты → жирный+курсив) — ✅ DEPLOYED & CLOSED (v2.42.0, коммит `c455728`, прод PID 1063356, 2360 тестов, 2026-08-24, Шаг 8 @Memory)

> Полный трек — `plans/backlog.md` (Epic 58). Требования R58-1…R58-7, решения D229–D231.
> Ревизия решения Epic 57 (D227): пользователь утверждает, что rich-формат сейчас
> читается ВСЕМИ клиентами (десктоп + мобильный) → (1) /info → настоящий Rich Message
> (`sendRichMessage`, Bot API 10.1/10.2, `InputRichMessage`); (2) у примеров фраз-вызовов
> (27 цитат) убрать подчёркивание → жирный+курсив (`<b><i>`); (3) у заголовка убрать
> подчёркивание, ЧЕСТНЫЕ H1 (интро «Гайд по фичам…») и H2 (6 нумерованных подзаголовков).
> D229 (product): риск деградации на старых клиентах остаётся в риск-реестре; D231: фолбек
> на `sendMessage`+HTML при rich-ошибке (эмуляция Epic 57 — фолбек-канон) — стратегию
> решает @Architect (T-447). Канон-цепочка (5 мест) синхронно: `DEFAULT_INFO_TEXT` =
> `info_text.md` = ARCHITECTURE 53.3 (оба блока, + пометка Epic 58) = backlog «Канон R44-1»
> (plain — суть не меняется). aiogram 3.29.1 уже содержит sendRichMessage — апгрейд НЕ нужен
> (пин <4.0.0). v2.42.0 (minor, D230 — прецедент D226). База: прод v2.41.0 (`ba91035`,
> PID 1062574), 2354 теста, Epics 1–57 ALL CLOSED. Без @Orchestrator.

- [x] T-447 (@Architect, P0) — обязательный ресёрч context7 → duckduckgo → exa: точный синтаксис `InputRichMessage` для aiogram 3.29.1 (html или blocks), лимиты длины rich (4096? больше?), `<a href>`/`<b>`/`<i>` в rich-HTML, `reply_to`, исключение при невалидном rich (для фолбека); вердикт по формату + фолбек-стратегия D231; новый канон разметки в Section 53.3 (оба блока, + пометка Epic 58) + дизайн переписи `handlers/info.py` (sendRichMessage, chunking, /edit_info DM-превью rich); правки тестов дословно (баланс h1/h2/b/i, u=0; strip-список; маркер «Гайд по фичам»). **DoD:** вердикт зафиксирован; Section 53.3 обновлена обоими блоками (+ пометка Epic 58); дизайн handlers/info.py зафиксирован; правки тестов перечислены дословно; маркер «Гайд по фичам» подтверждён; вопросы 1–6 закрыты. ✅ **DONE (Шаг 2 @Architect: sendRichMessage + InputRichMessage(html) — в aiogram 3.29.1 только html/markdown, blocks нет; лимиты rich: 32 768 симв. / 500 блоков / 16 уровней вложенности; ошибка — TelegramBadRequest (rich-специфичного исключения нет) = триггер фолбека D231; reply_to → reply_parameters; Section 53.3 «Дополнение Epic 58»; счётчики h1=1 / h2=6 / b=27 / i=27 / u=0 / a=2, 2782 симв. — chunking не нужен).**
- [x] T-448 (@Builder, P0) — rich-разметка (интро→H1 без `<u>`, 6 подзаголовков→H2, 27 цитат→`<b><i>`; текст не менять) + канон-цепочка 5 мест синхронно + перепись `handlers/info.py` (sendRichMessage + фолбек D231 + /edit_info превью + chunking) + тесты (баланс тегов/strip/маркеры; НОВЫЕ тесты sendRichMessage-вызова и фолбека) + полный pytest 0 регрессий (2354+) + `git diff --check`. **DoD:** дифф только разметки + handlers/info.py; 5 мест синхронны; тесты зелёные (вкл. sendRichMessage и фолбек); 0 регрессий (2354+). ✅ **DONE (Шаг 4 @Builder: честные `<h1>`/`<h2>`, 27 цитат `<b><i>` без `<u>`, текст verbatim; канон-цепочка 5 мест синхронна; handlers/info.py — sendRichMessage + фолбек D231 (TelegramBadRequest → legacy-HTML-эмуляция Epic 57 → plain) + /edit_info rich-превью; 2360 passed / 0 failed (+6); git diff --check чист).**
- [x] T-449 (@Reviewer, P0) — байт-в-байт 5 мест; дифф; ревью rich-логики `handlers/info.py` (sendRichMessage, фолбек D231, chunking, /edit_info-валидация); 0 регрессий; APPROVED. **DoD:** APPROVED; 5 мест синхронны; дифф = только разметка + handlers/info.py; 0 регрессий. ✅ **DONE (Шаг 5 @Reviewer: ✅ APPROVED WITH NOTES — M1/L1 закрыты @Docs (T-451); 5 мест синхронны; дифф = разметка + handlers/info.py; 0 регрессий).**
- [x] T-450 (@DevOps, P0) — бэкап `info_text.md.bak.epic58` ДО pull (серверный == HEAD `ba91035` — сверить); коммит (код+канон+тесты+планы, БЕЗ untracked mp4 `danger_boom_gif-03.mp4`) + пуш origin/master; деплой: `git pull --ff-only`, restart, 0 traceback, smoke /info (grep-сверки: h1/h2/b+i, u=0). **DoD:** бэкап есть; коммит запушен (без mp4); pull ff; новый PID; 0 traceback; smoke OK. ✅ **DONE (Шаг 7 @DevOps: коммит `c455728` «feat(epic58): v2.42.0 — …», пуш `ba91035..c455728` origin/master, 10 файлов, без mp4; деплой — сверка серверного HEAD == `ba91035` и `info_text.md` == HEAD, бэкап `info_text.md.bak.epic58` ДО pull, pull ff `ba91035..c455728`; grep: h1=1, h2=6, b=27, i=27, u=0, a=2, «Гайд по фичам»=1; restart OK, PID 1063356, 0 traceback).**
- [x] T-451 (@Docs, P1) — README v2.42.0 (иронично) + MEMORY; версия v2.42.0 (minor, D230 — прецедент D226). **DoD:** доки актуализированы; Epic 58 CLOSED. ✅ **DONE (Шаг 6 @Docs: README v2.42.0 + MEMORY + board; финализация Epic 58 (CLOSED) — после деплоя T-450).**

**Updated:** 2026-08-24 — **Epic 58 ✅ DEPLOYED & CLOSED (v2.42.0, Шаг 8 @Memory)**: все T-447…T-451 DONE ([x]). Шаг 7 @DevOps: коммит `c455728` (10 файлов, без mp4) + пуш origin/master (`ba91035..c455728`); деплой — бэкап info_text.md.bak.epic58 ДО pull, сверка HEAD == ba91035 и info_text.md == HEAD, pull ff `ba91035..c455728`, restart OK, PID 1063356, 0 traceback; grep-сверка: h1=1, h2=6, b=27, i=27, u=0, a=2, «Гайд по фичам»=1. Тесты 2360/0. /info на настоящих Rich Messages (sendRichMessage, H1/H2, D229/D231) на проде. Epics 1–58 ALL CLOSED. Без @Orchestrator.**

### Epic 57: /info → rich text статья (заголовки H1/H2, цитаты → italic+подчёркивание+жирный) — ✅ DEPLOYED & CLOSED (v2.41.0, коммит `ba91035`, прод PID 1062574, 2354 теста, 2026-08-23, Шаг 8 @Memory)

> Полный трек — `plans/backlog.md` (Epic 57). Требования R57-1…R57-7, решения D226–D228.
> Переоформить /info в статью нового формата rich text: интро-строка «Гайд по фичам бота
> с выходом в сеть internet. Никаких слеш-команд, всё работает нативно прямо в диалоге.» → H1;
> 6 пронумерованных подзаголовков → H2; 27 цитат (`<blockquote>`) → italic + подчёркивание +
> жирный. Текст НЕ менять — только разметка. Обязательный ресёрч (context7 + duckduckgo,
> fallback exa): Telegram Bot API / aiogram rich text (заголовки, версия Bot API, `parse_mode`
> vs entities, хватит ли aiogram 3.29.1 (пин <4.0.0) или нужен апгрейд). Канон-цепочка (5 мест)
> синхронно: `DEFAULT_INFO_TEXT` = `info_text.md` = ARCHITECTURE 53.3 (оба блока, пометка
> Epic 57) = backlog «Канон R44-1» (plain — суть не меняется). v2.41.0 (minor, D226 — прецедент
> D223). База: прод v2.40.0 (`1d9bf61`, PID 1054487), 2354 теста, Epics 1–56 ALL CLOSED.
> Без @Orchestrator.

- [x] T-442 (@Architect, P0) — ресёрч context7 + duckduckgo/exa (rich text: H1–H4? синтаксис, версия Bot API, aiogram-совместимость, нужен ли апгрейд в рамках <4.0.0 и его риски); вердикт по формату (parse_mode vs entities); новый канон разметки в Section 53.3 (+ пометка Epic 57); перечень правок тестов (баланс тегов, strip-список, маркеры — интро-строка остаётся в тексте как H1, проверить, что маркер «Гайд по фичам» выживет); риск: chunking (3065 симв. сейчас, новая разметка может добавить — пересчитать <4096), plain-фоллбек и /edit_info-валидация. **DoD:** вердикт зафиксирован; Section 53.3 обновлена обоими блоками (+ пометка Epic 57); правки тестов перечислены дословно; маркер «Гайд по фичам» подтверждён; chunking пересчитан (<4096); вопросы 1–5 закрыты. ✅ **DONE (Шаг 2 @Architect, зафиксировано Шаг 3 @Memory: Rich Messages (Bot API 10.1/10.2, sendRichMessage) для /info ОТВЕРГНУТЫ — деградация блокерная «This message is not supported» на неподдерживающих клиентах; заголовков-entity в sendMessage НЕТ; aiogram 3.29.1 уже содержит sendRichMessage — апгрейд НЕ нужен (пин <4.0.0); решение — эмуляция: H1 = `<b><u>`, H2 = `<b>`, цитаты = `<b><i><u>`, parse_mode HTML; счётчики 34 b / 27 i / 28 u / 0 blockquote / 2 a, 91 сущность (<100), 2964 симв. (<4096); маркер «Гайд по фичам» выживает; Section 53.3 «Дополнение Epic 57»; далее — @Builder (T-443)).**
- [x] T-443 (@Builder, P0) — разметка (интро→H1, 6 подзаголовков→H2, 27 blockquote→italic+подчёркивание+жирный; текст не менять) + канон-цепочка 5 мест синхронно (DEFAULT_INFO_TEXT, info_text.md, ARCHITECTURE 53.3 оба блока, backlog R44-1 plain — суть без тегов не меняется) + правки тестов + полный pytest 0 регрессий (2354) + git diff --check. **DoD:** дифф только разметки; 5 мест синхронны; тесты зелёные; 0 регрессий (2354). ✅ **DONE (Шаг 4 @Builder: интро H1=`<b><u>`, 6 подзаголовков H2=`<b>`, 27 цитат `<b><i><u>`, текст verbatim — 2270 симв. strip-теги идентичны старой версии; канон-цепочка 5 мест синхронна; счётчики 34 b / 27 i / 28 u / 2 a, 91 сущность (<100), 2964 симв. (<4096 — один чанк); 2354 passed / 0 failed; git diff --check чист).**
- [x] T-444 (@Reviewer, P0) — байт-в-байт 5 мест; дифф только разметки; риски aiogram-апгрейда для других сервисов (если вердикт — апгрейд); 0 регрессий; APPROVED. **DoD:** APPROVED; 5 мест синхронны; дифф = только разметка; 0 регрессий. ✅ **DONE (Шаг 5 @Reviewer: ✅ APPROVED WITH NOTES — замечания закрыты @Docs (T-446); апгрейд aiogram не требовался; 5 мест синхронны; дифф = только разметка; 0 регрессий).**
- [x] T-445 (@DevOps, P0) — бэкап `info_text.md.bak.epic57` ДО pull (серверный == HEAD `1d9bf61` — сверить); коммит (код+канон+тесты+планы, БЕЗ untracked mp4 `danger_boom_gif-03.mp4`) + пуш origin/master; деплой: `git pull --ff-only`, restart, 0 traceback, smoke /info (grep-сверки). **DoD:** бэкап есть; коммит запушен (без mp4); pull ff; новый PID; 0 traceback; smoke OK. ✅ **DONE (Шаг 7 @DevOps: коммит `ba91035`, пуш `1d9bf61..ba91035`, 8 файлов, без mp4; бэкап info_text.md.bak.epic57 ДО pull, pull ff `1d9bf61..ba91035`; grep: b=34, i=27, u=28, blockquote=0, a href=2, «Гайд по фичам»=1; PID 1062574, 0 traceback).**
- [x] T-446 (@Docs, P1) — README v2.41.0 (иронично) + MEMORY; версия v2.41.0 (minor, D226 — прецедент D223). **DoD:** доки актуализированы; Epic 57 CLOSED. ✅ **DONE (T-446-A — README v2.41.0 + MEMORY + board, Шаг 6; финализация Шаг 8: Epic 57 помечен CLOSED).**

**Updated:** 2026-08-23 — **Epic 57 ✅ DEPLOYED & CLOSED (v2.41.0, Шаг 8 @Memory)**: все T-442…T-446 DONE ([x]). Шаг 7 @DevOps: коммит `ba91035` (8 файлов, без mp4) + пуш origin/master (`1d9bf61..ba91035`); деплой — бэкап info_text.md.bak.epic57 ДО pull, pull ff `1d9bf61..ba91035`, restart OK, PID 1062574, 0 traceback; grep-сверка: b=34, i=27, u=28, blockquote=0, a href=2, «Гайд по фичам»=1. Тесты 2354/0. /info в rich-разметке (эмуляция H1/H2, D227) на проде. Epics 1–57 ALL CLOSED. Заметка: старый процесс при рестарте убит SIGKILL после TimeoutStopSec=90s — штатно, но наблюдать.**

### Epic 56: /info — правка пользователя (раздел 6 «Прямое обращение к Богу Машине») + `<code>`→`<blockquote>` — ✅ DEPLOYED & CLOSED (v2.40.0, коммит `1d9bf61`, прод PID 1054487, 2354 теста, 2026-08-23, Шаг 8 @Memory)
...
- [x] T-439 (@Reviewer, P0) — байт-в-байт сверка 5 мест; дифф только теги; 0 регрессий; APPROVED. **DoD:** APPROVED; 5 мест синхронны; дифф = только замены тегов. ✅ **DONE (Шаг 5 @Reviewer: APPROVED WITH NOTES — Medium-1/Low-1 закрыты @Docs T-441).**
- [x] T-440 (@DevOps, P0) — бэкап `info_text.md.bak.epic56` ДО всего; коммит (код+канон+тесты+info_text.md, без mp4) + пуш; деплой: `git pull` (при локальных изменениях серверного файла — сверка/checkout, правку НЕ терять), restart, 0 traceback, smoke /info. **DoD:** бэкап есть; коммит запушен; правка не потеряна; новый PID; 0 traceback; smoke OK. ✅ **DONE (Шаг 7: коммит `1d9bf61`, пуш c7a6da5..1d9bf61, 8 файлов, без mp4; бэкап info_text.md.bak.epic56 ДО pull, pull ff c7a6da5..1d9bf61; grep: blockquote 27 в 11 строках / code 0 / «Богу Машине» 1 / старые секции 4; PID 1054487, 0 traceback).**
- [x] T-441 (@Docs, P1) — README v2.40.0 + MEMORY. **DoD:** доки актуализированы; Epic 56 CLOSED. ✅ **DONE (Шаг 8: README v2.40.0 + фиксы Medium-1/Low-1 + MEMORY; Epic 56 CLOSED).**

**Updated:** 2026-08-23 — **Epic 56 ✅ DEPLOYED & CLOSED (v2.40.0, Шаг 8 @Memory)**: все T-437…T-441 DONE ([x]). Шаг 7 @DevOps: коммит `1d9bf61` (8 файлов, без mp4) + пуш origin/master (c7a6da5..1d9bf61); деплой — бэкап info_text.md.bak.epic56 ДО pull, pull ff c7a6da5..1d9bf61, restart OK, PID 1054487, 0 traceback; grep-сверка: blockquote 27 (в 11 строках), `<code>` 0, «Богу Машине» 1, старые секции 4. Тесты 2354/0. Раздел 6 «Прямое обращение к Богу Машине» + 27 blockquote на проде. Epics 1–56 ALL CLOSED.**
