...
### Epic 58: /info → настоящий Rich Message (sendRichMessage; H1/H2 честные, цитаты → жирный+курсив) — ✅ IMPLEMENTED + REVIEWED (APPROVED WITH NOTES) — ⏳ DEPLOY PENDING (v2.42.0, Шаг 6 @Memory, 2026-08-24)
...
**Updated:** 2026-08-24 — **Epic 58 (v2.42.0) — Шаг 6 @Memory: ✅ IMPLEMENTED + REVIEWED (APPROVED WITH NOTES — M1/L1 закрыты @Docs T-451) — ⏳ DEPLOY PENDING (@DevOps T-450).** @Architect T-447 DONE (sendRichMessage + InputRichMessage(html), Bot API 10.1; фолбек D231 по TelegramBadRequest). @Builder T-448 DONE (честные `<h1>`/`<h2>`, 27 цитат `<b><i>` без `<u>`, текст verbatim; 2360 passed / 0 failed). @Reviewer T-449 ✅ APPROVED WITH NOTES. @Docs T-451 DONE (README v2.42.0 + MEMORY + board). Прод остаётся v2.41.0 (`ba91035`, PID 1062574) до T-450. Далее — Шаг 7 @DevOps (T-450: бэкап `info_text.md.bak.epic58` ДО pull, коммит без mp4, пуш origin/master, `git pull --ff-only`, restart, 0 traceback, smoke /info). Без @Orchestrator.**
