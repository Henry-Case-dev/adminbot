...
### ✅ Хотфикс-9 (Волна 1.8, ПРИОРИТЕТНЫЙ по UPD3) — `hotfix9-shell-liquidglass-darkaurora-round1025` (внеплановый, выше F6) — ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 22.09.2026; Merge §63); deploy ⏳ Шаг 9 @DevOps (код НЕ закоммичен); live ⏳ PENDING OWNER VERIFICATION
...
**Статус:** ✅ **COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 22.09.2026).** Merge — `plans/ARCHITECTURE.md` **§63** (@Architect, Step 7); **архив — `plans/archive/hotfix9-shell-liquidglass-darkaurora-round1025/`** (`spec.md` + **ADR-1025-17-shell-flex-aurora-glass-csp** + `tasks.md` + `evidence.md` + `review.md`; содержимое/чекбоксы сохранены, UTF-8; **47 [x] / 3 [ ]** — открыты **T-2830** (live PENDING OWNER VERIFICATION), **T-2837** (deploy, Шаг 9), **T-2839** (продолжение F6); **T-2838** (Шаг 8 @PM — архивация/синхронизация) ✅). **Задачи — T-2790…T-2839 (50)**; дублей ID нет (максимум занятого до пакета — HOTFIX8 **T-2789**). @Reviewer **Approved** (T-2835, итер.2; блокер H-H9S-1 и M-H9R-1 закрыты; `review.md`); @Scanner **Critical 0 / High 0 / Medium 0 / Low 0 / Info 3 → «к деплою ГОТОВО»** (T-2836; `plans/reports/round1025_hotfix9_scanner_audit.md`). Baseline HEAD **`b374c0f`** (= tag `pre-round1025-hotfix9`), `APP_VERSION` **2.58.11**. Прогоны: pytest **8291 passed / 0 failed** (baseline 8272; hotfix9 17/0); матрица `tools/ui_round1025_matrix.py` — **failures 0** (10 вьюпортов × 5 режимов); `tools/glass_prototype_probe.py` — **failures 0** (`displacementScale=14.56`, `diffGlass=15.97`, `diffHole=63.817`); `node --check` OK; `git diff --check` = 0. **Δ DDL=0, Δ каталога=0** (459/98/96/21/418), CSP same-origin vendored (без CDN/inline/eval), SHA-256 vendored 3/3. **⏳ deploy — Шаг 9 @DevOps (T-2837; bump `APP_VERSION` 2.58.11 → 2.58.12 в дереве; «HTTP 200 ≠ корректный layout»; коммиты/`?v=`/`deployment.md` появятся при деплое); ⏳ live — T-2830 PENDING OWNER VERIFICATION (реальный Telegram WebView не проведён).** Техдолг — **§63.11** (Info 3 + Low 0, не блокеры). **Следующая — F6 `memory-analytics-reorg-round1025` (немедленно после деплоя HOTFIX9, §17, без human gate).**
...
| **F6** | `memory-analytics-reorg` *(в ТЗ @Memory — `memory-access-reorg`)* — раздел «Память» §52–§59 + «Сводка»→«Аналитика» §21 + карта вызовов §22–§30 (adapter ExecutionGraph, 2 режима, mobile) — **⏭️ СЛЕДУЮЩАЯ НЕМЕДЛЕННО ПОСЛЕ ДЕПЛОЯ HOTFIX9** (UPD3 §17, без human gate). HOTFIX9 ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 22.09.2026; Merge §63; архив — `plans/archive/hotfix9-shell-liquidglass-darkaurora-round1025/`), deploy ⏳ Шаг 9 @DevOps; HOTFIX8 ✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (2.58.11); live T-2776 ⏳. Step 0 @Memory для F6 выполнен (см. `plans/features/memory-analytics-reorg-round1025/tasks.md`); Step 1 @PM — при старте. | §4, §21–§30, §52–§59, §75, §76 | UI+adapter (web+api) | **P0** | F1, F4 | детализация при старте |
...
> **Порядок фич (единый, по UPD):** **F0 → F1 → F2 → F3 → hotfix6 (Волна 1.5) → HOTFIX7 (Волна 1.6, приоритетный UPD) → F4 → F5 → HOTFIX8 (Волна 1.7) → HOTFIX9 (Волна 1.8, приоритетный UPD3) → F6 → F7 → F11 → F9 → F10**. Изменение относительно исходной декомпозиции: F0 добавлена в начало (Wave 0), F11 передвинута **перед** F9 (обе после F4/F5/F6/F7), F9 сужена; **hotfix6** вставлен между F3 и F4 по итогам живой приёмки владельца (блоки A–D), **§15 вынесено из F11 в hotfix6**; **HOTFIX7** вставлен между hotfix6 и F4 по новому приоритетному UPD «Срочный фикс текущего фронта» (§6073–6154), **F4 — ✅ ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 22.09.2026; Merge §60; 2.58.9; live T-2656 ⏳); HOTFIX9 — ✅ ARCHIVED (Шаг 8 @PM, 22.09.2026; Merge §63; `APP_VERSION` 2.58.12 в дереве; deploy ⏳ Шаг 9 @DevOps, live T-2830 ⏳ PENDING OWNER VERIFICATION).**
>
> **▶️ Текущий шаг (Шаг 8 @PM, 22.09.2026):** **HOTFIX9 `hotfix9-shell-liquidglass-darkaurora-round1025` (Волна 1.8, приоритетный UPD3) — ✅ COMPLETED + MERGED + ARCHIVED** (Merge **§63**; архив — `plans/archive/hotfix9-shell-liquidglass-darkaurora-round1025/` + **ADR-1025-17** + `evidence.md` + `review.md`; @Reviewer **Approved** (итер.2), @Scanner **C0/H0/M0/L0/I3** → «к деплою ГОТОВО»; pytest 8291/0, матрица failures 0). **Дальше: `deploy — Шаг 9 @DevOps` ⏳ (T-2837; `APP_VERSION` 2.58.11 → 2.58.12 в дереве) → `Шаг 10 @Memory: метрики 10.25-HOTFIX9 + KG + техдолг §63.11` → затем следующая фича — F6 `memory-analytics-reorg-round1025` (T-2839, немедленно, без human gate).** Ранее закрыт **HOTFIX8** `hotfix8-shell-glass-aurora-round1025` (deploy VERIFIED — задеплоено 2.58.11; коммиты `98551b2`/`5124522`/`1a8ed18`/`d3179c1`) и **F5** `module-workspace-tabs-round1025` (deploy `63dddd3`/`0d3ea40`/`af137cd`, `APP_VERSION` 2.58.10). Живые гейты владельца (в т.ч. **T-2830** HOTFIX9 PENDING, **T-2776** HOTFIX8, **T-2742**, **T-2682**, **T-2617**, **T-2656**) закрываются параллельно и не блокируют старт (UPD3 §15/§16 «без human gate»).
