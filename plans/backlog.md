...
### ⚡ EXTRA-визуальный эпик (round 10.26) — «POLYGONAL LUMINESCENCE» — ✅ **COMPLETED + MERGED + ARCHIVED + DEPLOYED (2.58.19 + правка v2.58.20)** (Шаг 8 @PM 23.09.2026; Merge §72; deploy VERIFIED Шаг 9 @DevOps + addendum-правка); блок J/стекло — PENDING OWNER

**Триггер:** новое авторитетное ТЗ владельца `plans/current_task.md`, **строки 7485–8755** («# EXTRA: ВИЗУАЛЬНЫЙ ЭПИК — POLYGONAL LUMINESCENCE», §0–§17) — прочитано **дословно**; файл **НЕ изменялся** (R17/R18). Приоритет **выше Эпика 2** → **Эпик 2 S2 `summary-context-restore` — ⏸ PAUSED** (возобновление **после деплоя** EXTRA-эпика, Шаг 9; §17; см. `plans/workflow_state.md`, раздел «ПАУЗА»).

**Суть (§0–§1):** заменить **Dark Aurora Flow** (SUPERSEDE — ADR-1025-17 D6 / hotfix9-10 / `UI_AURORA_FLOW_V2`; файл сохранить для отката, но **не в DOM**) на живую полигональную светящуюся композицию: полупрозрачные треугольные грани, тонкие линии, светящиеся узлы, сине-фиолетовые/сиреневые области, циановые акценты, локальные источники света, переливающееся свечение **из самой геометрии**. Стек §2: **Delaunator 5.0.0** (`--save-exact`, локальная сборка esbuild через `tools/vendor` → `web/static/vendor/`, без CDN, фикс-версия + SHA-256), **Canvas 2D** основной, **OGL 1.0.11** сохранён для отката/GPU, `@liquidglassjs/core` 0.5.3 — без второй библиотеки стекла. Модуль §3.1 — `web/static/polygon-background.js` (контракт `start/stop/pause/resume/resize/getDiagnostics()` + адаптер `window.__AuroraFlow`); §3.3 — **один активный фоновый рендерер**. §12 — Liquid Glass **сначала изолированный прототип (7 проверок), затем точечно** (AMEND ADR-1025-18 D1; Sidebar/Header графитовые; frosted ≠ рефракция; **белые прямоугольники не повторять**). §14 — `getDiagnostics()` (без UI) + Playwright 0/5/10/20 с (движется **положение**, не только яркость) + композиция/материалы; **«не объявлять готово по pixel diff»**.

**План:** `plans/archive/polygonal-luminescence-round1026/tasks.md` — блоки **0, A–Q**; **T-3162…T-3219 (58)**; трассируемость REQ→§ТЗ. **Статус:** ✅ **COMPLETED + MERGED + ARCHIVED** (Шаг 8 @PM, T-3216, 23.09.2026) — @Builder (блоки 0/A–I, K) → @Reviewer **Approved** (T-3213) → @Scanner **Critical 0 / High 0 / Medium 0 / Low 3 / Info 4 → «к деплою ДА»** (T-3214; `plans/reports/round1026_visual_scanner_audit.md`) → Merge `plans/ARCHITECTURE.md` **§72** (T-3215) → архивация. **Архив:** `plans/archive/polygonal-luminescence-round1026/` (`spec.md` + **ADR-1026-3** + `tasks.md` + `evidence.md` + `review.md` + `deployment.md`; содержимое/чекбоксы сохранены, UTF-8). **Deploy — ✅ VERIFIED (Шаг 9 @DevOps, T-3217 + addendum):** базовый — коммиты `76fc5e1`/`bf46360`/`1ad98ca` (`APP_VERSION` 2.58.18→**2.58.19**); правка владельца v2.58.20 (мерцание свечения фона ×2 медленнее) — `306778a`/`a50b014`/`54c6445` (**2.58.20**); push origin/master без force, прод ff, `/api/health` 200, served `?v=2.58.20`, `database is locked`=0, `deployment.md` **VERIFIED**; откат — annotated-тег `pre-round1026-visual` → `9d046e5` + `var/backups/visual-round1026-*`. **Метрики — ✅ T-3218 (Шаг 10 @Memory):** строка `plans/metrics.md` + раздел **10.26-VISUAL** + KG-синк (закрывающий docs-коммит — @DevOps). **Техдолг — §72.5** (L-POLY1026-1 GC-аллокации / L-POLY1026-2 покрытие матрицы §71 / L-POLY1026-3 CSP-формулировка + Info 1–4). **Блок J / Liquid Glass — PENDING OWNER VERIFICATION** (гейт §13.2: реальная рефракция п.1 / внешний ореол п.5 / перф WebView п.7; `UI_LIQUID_GLASS_LIB` default OFF, hotfix10 не повторяем). **Live-гейты — `[ ]` PENDING OWNER VERIFICATION.** **▶️ После деплоя возобновляется приостановленная Эпик 2 S2 `summary-context-restore`** (§17/§90–§92; Step 1 @PM — следующий); на декоре не зацикливаться. Инварианты: **Δ DDL=0**, **Δ каталога=0**, CSP/zero-build, один активный рендерер.

**Baseline (Step 0):** HEAD **`9d046e5`** (== `origin/master`), `APP_VERSION` **2.58.18**; pytest `.venv` **8501/0**, JS **42/42**; каталог **467/426/442/100/98/21**; **Δ DDL=0**; **Δ каталога** — предварительно **0** (env-only флаг, окончательно за @Architect).

**Границы/инварианты:** не редизайн; не менять IA/layout/настройки/бизнес-логику/API; сердцебиение (§11/§15) не переделывать; `AppShell` не менять; CSP/zero-build; **Δ DDL=0**; **гейт S6/S10** (публикация/hybrid Эпика 2) **не трогать** (закрыт до live-приёмки Эпика 1, ADR-1025-24 D4); §17 — после деплоя не останавливаться, **не зацикливаться на декоре**, живой Telegram — PENDING OWNER VERIFICATION. Откат — тег `pre-round1026-visual` + `git revert`; бэкапы/теги/`stash@{0}` не удалять (R18).

**Открытые вопросы для @Architect:** (a) новый модуль vs замена `aurora-flow.js`; (b) env-only `UI_POLYGON_BG` vs каталог (Δ каталога?); (c) место прототипа (gitignored `tools/`?); (d) судьба `aurora-flow.js`/`UI_AURORA_FLOW_V2`; (e) OGL оставить или Canvas-2D-only; (f) deploy+bump; (g) форма приёмочного отчёта.
...
- **Эпик 3 «Agentic Intelligence» (после Эпиков 1 и 2; исследование допустимо заранее, но не блокирует):** единый координатор инструментов, последовательные tool calls, единый Image Request (direct|tool), генерация из досье/RAG, дневной лимит изображений (atomic reserve + idempotency, scope global/chat), structured memory lookup `get_user_context(purpose)`, новый Decision Making (`action` {reply/react/silent/tool} отдельно от `style`), реакции Telegram (`setMessageReaction`), события `DECISION_*`/`TOOL_*`/`REACTION_SENT`/`MESSAGE_IGNORED`/`IMAGE_*`, интеграция в существующий ExecutionGraph и Mini App; 23 сценария §52 и приёмка §53.
...
### 🗺️ ЭПИК 1 (Раунд 10.25) — «Liquid Glass Control Center» — 12 фич (F0 + F1–F11; F0 добавлена UPD, F11 добавлена @PM) — **✅ АВТО-ЧАСТЬ ЗАКРЫТА (F10 §70, 23.09.2026): все 12 фич + хотфиксы завершены; единый live-гейт владельца (Telegram WebView) — [ ] PENDING OWNER VERIFICATION (закрывается параллельно, не блокирует старт Эпика 2)**
...
| **F9** ✅ | `secrets-and-save-states` — **✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026); Merge `plans/ARCHITECTURE.md` §68; архив — `plans/archive/secrets-and-save-states-round1025/` (`spec.md` + **ADR-1025-22** + `tasks.md` + `evidence.md` + `review.md`); deploy ✅ задеплоено 2.58.16 (Шаг 9 @DevOps, T-3058, `deployment.md` VERIFIED), live ⏳ PENDING OWNER VERIFICATION (T-3059); техдолг §68.8** — только **UI-слой секретов §50** (`{configured,last4}`, «Ключ установлен», маска ≠ значение input, отдельные замена/удаление) + **визуальная часть SaveBar §69/§78** (safe area, клавиатура, последнее поле); **persistence/409/412/state-machine/безопасный merge/версия — в F0 (Wave 0)**, не дублируются. @Reviewer **Approved** (итер.2); @Scanner **C0/H0/M0/L0/Info 2** → «к деплою ДА» (`plans/reports/round1025_f9_scanner_audit.md`). Числа: JS **40/40**, pytest **8421/0/1 skipped** (5 env вне scope), **Δ DDL=0**, **Δ каталога=0** (459/418/434/98/96/21; секреты 28), `APP_VERSION` **2.58.16**. Закрыты T-3058 (deploy — ✅ 2.58.16) и T-3060 (метрики/KG — ✅ Шаг 10); открыт только T-3059 (live PENDING OWNER VERIFICATION). **▶️ Следующая — F11** `status-showcase-dashboard-round1025`, затем **F10** `epic1-verification-round1025`. | §50, §51, §69, §78 | UI+API | **P0** | **F0 ✅**, F4 ✅, F5 ✅ | **T-3024…T-3062 (39) — `plans/archive/secrets-and-save-states-round1025/tasks.md`** |
| **F10** ✅ | `epic1-verification-round1025` — **✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge `plans/ARCHITECTURE.md` §70; архив — `plans/archive/epic1-verification-round1025/`); deploy — NOT_APPLICABLE (обосновано: read-only verification-гейт — нет Δ рантайма/ассетов/каталога/DDL, `tools/**`/`tests/**` рантаймом не импортируются; `APP_VERSION` 2.58.17 без bump; Шаг 9 @DevOps T-3126, `deployment.md`); авто-приёмка Эпика 1 ✅** (Step 5 @Reviewer **Approved** T-3121; Step 6 @Scanner **C0/H0/M0** T-3122, `plans/reports/round1025_f10_scanner_audit.md`; `.venv` pytest **8463/0**, JS **42/42**, матрица §71 + E2E §72–§74 + §78 UI — **failures: 0**; §79-стоп-гейт пройден — существующие пайплайны не изменены: 300 passed; Δ DDL=0, Δ каталога=0 = 459/418/434/98/96/21). **Live-приёмка владельца (Telegram WebView) — [ ] PENDING OWNER VERIFICATION** (Chromium ≠ Telegram WebView; T-3097/T-3059/T-3021/T-2961/T-2917/T-2862/… открыты). Точка отката §117 п.12 — annotated-тег `pre-round1025-f10` → `57b325c`. Приёмочный отчёт — `plans/reports/round1025_f10_acceptance.md` (9 разделов, D3). | §69–§79, §116, §117 | verification (gate) | **P0** | **F0**, F1…F9, F11 | **T-3100…T-3128 (29) — `plans/archive/epic1-verification-round1025/tasks.md`** |
| **F11** ✅ | `status-showcase-dashboard` — **[добавлено @PM, адаптация: §11–§20 не покрыты в F1–F10 по существу]** витрина «Статус» §11–§20: 12-кол. сетка §12, Hero §13, метрики §14, ~~живое сердцебиение §15 (Canvas 2D+rAF)~~ → **§15 SUPERSEDE/вынесено в `hotfix6-webview-shell-heartbeat-round1025` (Волна 1.5)**, граф §16, сон/бейджи §17, мониторинг интеллекта §18, «Новые факты» §19, логи §20 + превью карты §21 | §11–§21, §70, §77 | UI (web) | **P0** | F1, F2, F6 | **✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026); Merge §69; архив — `plans/archive/status-showcase-dashboard-round1025/` (`spec.md` + **ADR-1025-23** + `tasks.md` + `evidence.md` + `review.md`; T-3063…T-3099, 37; закрыты T-3063…T-3096 и T-3098/T-3099; открыт T-3097 live); @Reviewer Approved (итер.2, T-3091), @Scanner C0/H0/M0/L0/Info 2 → «к деплою ДА» (T-3092); JS 42/42, матрица §70 `failures: 0`, Δ DDL=0/Δ каталога=0 (459/418/434/98/96/21); `APP_VERSION` 2.58.16 → 2.58.17; kill-switch `UI_STATUS_GRID_V2`; техдолг §69.6; `deployment.md` — Шаг 9 @DevOps VERIFIED (коммиты `f3e9195`/`e7170b7`/`5c8fa88`, health 200, `database is locked`=0). ✅ задеплоено 2.58.17; live ⏳ PENDING OWNER VERIFICATION (T-3097).** |
...
7. **Волна 3:** **F5** (workspace+промпты) ✅ → **F6** (Память+Аналитика) ✅ **COMPLETED + MERGED + ARCHIVED + DEPLOYED** (Шаги 8–9 @PM/@DevOps, 23.09.2026; Merge §65; deploy ✅ 2.58.14; live ⏳ PENDING) → **F7** `permsoc-local-space-round1025` ✅ **COMPLETED + MERGED + ARCHIVED + DEPLOYED** (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026; Merge §66; архив — `plans/archive/permsoc-local-space-round1025/`; deploy ✅ задеплоено 2.58.15 (`deployment.md` VERIFIED); live ⏳ PENDING OWNER VERIFICATION) → **F8** `parameter-registry-widget-map-round1025` ✅ **COMPLETED + MERGED + ARCHIVED** (Шаг 8 @PM, 23.09.2026; Merge §67; архив — `plans/archive/parameter-registry-widget-map-round1025/`; deploy — **NOT_APPLICABLE**; живой PG-снимок ⏳ PENDING OWNER VERIFICATION) → **F9** `secrets-and-save-states-round1025` ✅ **COMPLETED + MERGED + ARCHIVED + DEPLOYED** (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026; Merge §68; архив — `plans/archive/secrets-and-save-states-round1025/`; @Scanner C0/H0/M0/L0/Info 2; deploy ✅ задеплоено 2.58.16, live ⏳ PENDING OWNER VERIFICATION) → **F11** ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §69; архив — `plans/archive/status-showcase-dashboard-round1025/`; deploy ✅ задеплоено 2.58.17 VERIFIED (health 200, `database is locked`=0), live ⏳ PENDING OWNER VERIFICATION) → **✅ F10** `epic1-verification-round1025` — COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §70; архив — `plans/archive/epic1-verification-round1025/`; deploy **NOT_APPLICABLE**; авто-приёмка ✅; live ⏳ PENDING OWNER VERIFICATION) → далее **Эпик 2 «Summary Hybrid Pipeline»** (§80–§115; старт по ADR-1025-24 D4), затем **Эпик 3 «Agentic Intelligence»**.
8. **Волна 4:** **F9** ✅ **COMPLETED + MERGED + ARCHIVED + DEPLOYED** (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026; Merge §68; только секреты §50 + визуальный SaveBar; серверный persistence/409/state-machine — в F0) — deploy ✅ задеплоено 2.58.16, live ⏳ PENDING OWNER VERIFICATION.
9. **Волна 5:** **F10** ✅ **COMPLETED + MERGED + ARCHIVED** (Шаг 8 @PM, 23.09.2026; Merge §70; архив — `plans/archive/epic1-verification-round1025/`; deploy **NOT_APPLICABLE** (обосновано); авто-приёмка ✅). → **СТОП-ГЕЙТ Эпика 1:** авто-часть ✅ закрыта, **live-приёмка владельца — [ ] PENDING OWNER VERIFICATION** → **следующий — Эпик 2 «Summary Hybrid Pipeline»** (§80–§115; старт по ADR-1025-24 D4) → затем **Эпик 3 «Agentic Intelligence»**.

> **Порядок фич (единый, по UPD):** **F0 → F1 → F2 → F3 → hotfix6 (Волна 1.5) → HOTFIX7 (Волна 1.6, приоритетный UPD) → F4 → F5 → HOTFIX8 (Волна 1.7) → HOTFIX9 (Волна 1.8, приоритетный UPD3) → **HOTFIX10 (Волна 1.9, приоритетный UPD4)** → F6 ✅ (Merge §65, Шаг 8 @PM 23.09.2026; deploy ✅ Шаг 9 @DevOps, 2.58.14) → **F7** ✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026; Merge §66; архив — `plans/archive/permsoc-local-space-round1025/`; deploy ✅ задеплоено 2.58.15 (`deployment.md` VERIFIED); live ⏳ PENDING OWNER) → **F8** ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §67; deploy **NOT_APPLICABLE**; живой PG-снимок ⏳ PENDING OWNER VERIFICATION) → **F9** `secrets-and-save-states-round1025` ✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026; Merge §68; архив — `plans/archive/secrets-and-save-states-round1025/`; deploy ✅ задеплоено 2.58.16, live ⏳ PENDING OWNER VERIFICATION) → **F11** ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §69) → **✅ F10** `epic1-verification-round1025` — COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §70; архив — `plans/archive/epic1-verification-round1025/`; deploy **NOT_APPLICABLE**; авто-приёмка ✅; live ⏳ PENDING OWNER VERIFICATION) → далее **Эпик 2 «Summary Hybrid Pipeline»** (§80–§115; старт по ADR-1025-24 D4), затем **Эпик 3 «Agentic Intelligence»****. Изменение относительно исходной декомпозиции: F0 добавлена в начало (Wave 0), F11 передвинута **перед** F9 (обе после F4/F5/F6/F7), F9 сужена; **hotfix6** вставлен между F3 и F4 по итогам живой приёмки владельца (блоки A–D), **§15 вынесено из F11 в hotfix6**; **HOTFIX7** вставлен между hotfix6 и F4 по новому приоритетному UPD «Срочный фикс текущего фронта» (§6073–6154), **F4 — ✅ ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 22.09.2026; Merge §60; 2.58.9; live T-2656 ⏳); HOTFIX9 — ✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 22.09.2026; Merge §63; 2.58.12; live T-2830 ⏳ PENDING OWNER VERIFICATION); HOTFIX10 — ✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (Шаги 8–9 @PM/@DevOps, 23.09.2026; Merge §64; задеплоено 2.58.13; live T-2862 ⏳ PENDING OWNER VERIFICATION).**
>
> **▶️ Текущий шаг (F9 `secrets-and-save-states-round1025` закрыта, задеплоена и заархивирована 23.09.2026 — Шаги 0–10 ✅; deploy VERIFIED 2.58.16):** **F9 `secrets-and-save-states-round1025` (Эпик 1, §50 + визуальный SaveBar §69/§78) — ✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED** (Merge `plans/ARCHITECTURE.md` **§68**; архив — `plans/archive/secrets-and-save-states-round1025/` (`spec.md` + **ADR-1025-22** + `tasks.md` + `evidence.md` + `review.md`); **T-3024…T-3062, 39**; @Reviewer **Approved** итер.2, @Scanner **C0/H0/M0/L0/Info 2** → «к деплою ДА»; техдолг **§68.8**; JS **40/40**, pytest **8421/0/1 skipped** (5 env вне scope), **Δ DDL=0**, **Δ каталога=0**, `APP_VERSION` **2.58.16**; закрыты **T-3058** (deploy — ✅ Шаг 9 @DevOps, `f5fbd5f`/`c610c5f`/`49c1ae1`, health 200, `database is locked`=0) и **T-3060** (метрики/KG — ✅ Шаг 10 @Memory); открыт только **T-3059** (live-гейт реального TMA/WebView — ⏳ PENDING OWNER VERIFICATION)). **F11 — ✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026; Merge §69; архив — `plans/archive/status-showcase-dashboard-round1025/`; deploy ✅ задеплоено 2.58.17 VERIFIED (health 200, `database is locked`=0), live ⏳ PENDING OWNER VERIFICATION). F10 `epic1-verification-round1025` — ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §70; архив — `plans/archive/epic1-verification-round1025/`; deploy **NOT_APPLICABLE** (обосновано); авто-приёмка ✅; live ⏳ PENDING OWNER VERIFICATION). Эпик 1 — авто-часть закрыта; следующий — Эпик 2 «Summary Hybrid Pipeline» (§80–§115, старт по ADR-1025-24 D4), затем Эпик 3 «Agentic Intelligence».** **Ранее закрыт и задеплоен HOTFIX10** (Merge §64; архив — `plans/archive/hotfix10-liquidglass-rollback-shell-geometry-round1025/`; T-2840…T-2869, 30; задеплоено 2.58.13, live T-2862 ⏳). Ранее закрыт и задеплоен **HOTFIX9** (Merge **§63**; deploy Шаг 9 @DevOps **VERIFIED** — `470b63a`/`8d61926`/`dba76e8`, `APP_VERSION` 2.58.11 → **2.58.12**, health 200, `database is locked`=0; @Reviewer Approved, @Scanner C0/H0/M0/L0/I3; pytest 8291/0). Живые гейты владельца (в т.ч. **T-2830** HOTFIX9 PENDING OWNER VERIFICATION, **T-2776**, **T-2742**, **T-2682**, **T-2617**, **T-2656**) закрываются параллельно и не блокируют старт (UPD4 §8 «без human gate»).
...
### 🗺️ ЭПИК 2 (Раунд 10.26) — «Summary Hybrid Pipeline» — 10 фич (S1–S10) — ✅ **COMPLETED (авто-часть: S1–S10)**; live-приёмка владельца — ⏳ **PENDING OWNER VERIFICATION** (Эпик 1 live подтверждён владельцем 24.09.2026, гейт ADR-1025-24 D4 закрыт); **авто-итог: S1–S10 COMPLETED + MERGED + ARCHIVED + DEPLOYED (2.58.18 → 2.58.29; Merge §71, §73–§81; архив S10 — `plans/archive/summary-deploy-round1026/`; ревью S6–S10 — единый Reviewer gate, S10 Approved C0/H0 с 1-й итерации; pytest `.venv` 9026/0, JS 47/47, Δ DDL=0 / Δ каталога=0; техдолг §81.10: L-R1026S10-1/-3 OPEN, L-R1026S10-2 CLOSED на merge §81 — `review.md` исторический снимок; live §115/§117 — PENDING OWNER; §85-UI — отдельная санкция; предусловие Эпика 3 формально достигнуто — следующий шаг `select_next`/оценка Эпика 3).** Историческая детализация: **S1 `summary-filter-round1026` ✅ COMPLETED + MERGED + ARCHIVED (§71, 23.09.2026) → **S2 `summary-context-restore-round1026` ✅ COMPLETED + MERGED (§73) + ARCHIVED (Шаг 8 @PM, T-3249, 23.09.2026); deploy — ✅ задеплоено 2.58.21 (Шаг 9 @DevOps), live — [ ] PENDING OWNER VERIFICATION**; **✅ S3 `summary-l1-clusterizer` — COMPLETED (автономный контур: @Reviewer Approved C0/H0, @Scanner C0/H0/M1-процесс(закрыт)/L3/I5) + ARCHIVED (Шаг 8 @PM, T-3279, 23.09.2026; архив — `plans/archive/summary-l1-clusterizer-round1026/`; deploy VERIFIED 2.58.22 (T-3280); live — PENDING OWNER VERIFICATION); ✅ S4 `summary-fact-package` — DONE / VERIFIED (автономный контур: @Reviewer Approved C0/H0, @Scanner C0/H0/M1-процесс(закрыт `review.md`)/L2/I2) + ARCHIVED (Шаг 8 @PM, T-3304, 23.09.2026; архив — `plans/archive/summary-fact-package-round1026/`; deploy — **VERIFIED 2.58.23** (`08219ab`/`7722d66`); врезка GATED (S5/S6); live — ⏳ PENDING OWNER VERIFICATION); ✅ **S5 `summary-l2-writer-formatter` — DONE / VERIFIED (автономный контур) + ARCHIVED** (Шаг 8 @PM, T-3341, 23.09.2026; архив — `plans/archive/summary-l2-writer-formatter-round1026/`; @Reviewer Approved итер.2 C0/H0, @Scanner C0/H0/блокирующих 0/L4/I2; pytest 8807/0 (+70), JS 43/43, каталог 469/426/444, F8 переиздан, Δ DDL=0, bump 2.58.24; deploy **VERIFIED 2.58.24** (`03a4d55`/`ae5a147`); **ON-путь — GATED**, `SUMMARY_HYBRID_L2_ENABLED` default OFF; live — PENDING OWNER VERIFICATION) → **S6 ✅ COMPLETED + MERGED (§80) + ARCHIVED + DEPLOYED (2.58.28; Шаг 8 @PM, T-3458, 24.09.2026; архив — `plans/archive/summary-publish-integration-round1026/`; единый Reviewer gate C0/H0 после rework 1; D4 закрыт владельцем 24.09.2026) → S9 ✅ DONE/VERIFIED + ARCHIVED (24.09.2026) → S7 ✅ COMPLETED + MERGED (§78) + ARCHIVED + DEPLOYED (2.58.26) → S8 ✅ COMPLETED + MERGED (§79) + ARCHIVED + DEPLOYED (2.58.27); после S6 остаётся только S10 `summary-deploy` (§107/§114/§115/§117 + §85-UI отдельной санкцией) — следующий и последний пункт Эпика 2; live S1–S6/S7/S8/S9 — PENDING OWNER VERIFICATION** (S9 не зависит от S6, §113; Step 1 @PM ✅ 23.09.2026, `tasks.md` T-3345…T-3379); исходно: Step 1 @PM ✅ + Step 2 @Architect ✅ 23.09.2026 (spec + ADR-1026-7 Accepted D1–D7) + сверка @PM ✅ T-3310; `tasks.md` T-3308…T-3344, блоки 0/A–H; врезка за kill-switch `flags.summary_hybrid_l2_enabled=false` + байт-в-байт OFF-путь) (§97–§99/§101/§102; зависимость S4 ✅; врезка L1→пакет→L2 и слот/UI L2 — S5; гейт S6/S10+D4 и live — без изменений)). **▶️ S10 `summary-deploy` ✅ COMPLETED + MERGED (§81) + ARCHIVED + DEPLOYED/ACTIVATED (2.58.29) — последний пункт Эпика 2; Эпик 2 — ✅ COMPLETED (авто-часть: S1–S10).**
...
|---|---|---|---|---|---|---|
| **S1** ✅ | `summary-filter-round1026` — **✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (Шаг 8 @PM + Шаг 9 @DevOps, 23.09.2026; Merge `plans/ARCHITECTURE.md` §71 @Architect; архив — `plans/archive/summary-filter-round1026/`; `deployment.md` VERIFIED).** Алгоритмический фильтр §87–§89 + вес-алгоритм §88 + тонкая настройка §89 + лимит/чанки §93, namespace `summary.filter.*` (§87), дефолт **ВКЛ**; `tasks.md` — блоки 0/A–H, **T-3129…T-3161** (закрыты T-3129…T-3156, T-3157/T-3158/T-3159/T-3160/T-3161; открыт T-3151 live — браузерный UI/E2E). @Reviewer **Approved** (итер.2), @Scanner **C0/H0/M0/L1** (`plans/reports/round1026_s1_scanner_audit.md`); pytest 8501/0, JS 42/42; **Δ DDL=0**, Δ каталога = санкция ADR-1026-1 D1 (467/426/442/100/98/21); `APP_VERSION` 2.58.18. Техдолг — **§71.5** (L-R1026S1-3, L-2/-3/-4, T-3151). **deploy — ✅ задеплоено 2.58.18 VERIFIED (Шаг 9 @DevOps, T-3157; `4cd4ae6`/`89bda3d`/`31763ab`, health 200, `database is locked`=0)**; live-приёмка — ⏳ PENDING OWNER VERIFICATION (D4). | §80, §87–§89, §93 | backend/алгоритм | **P0** | Эпик 1 | **T-3129…T-3161 — `plans/archive/summary-filter-round1026/tasks.md`** |
| **S2** ✅ | `summary-context-restore-round1026` — **✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, T-3249, 23.09.2026; Merge `plans/ARCHITECTURE.md` §73 @Architect T-3248; архив — `plans/archive/summary-context-restore-round1026/`).** Восстановление контекста §90 + спец-случаи §91 + подготовка данных §92 (reply_to-родители через reuse `thread_chain`, ограниченный соседний контекст, единая ASC-хронология/дедуп, исходные `message_id`, `restored_count`); `tasks.md` — блоки 0, A–I, **T-3224…T-3252 (29)** (закрыты T-3224…T-3249 и T-3252; закрыты **T-3250** deploy — ✅ задеплоено 2.58.21 и **T-3251** метрики — ✅ Шаг 10; блок H/T-3244 — NOT_APPLICABLE). @Reviewer **Approved** (T-3245), @Scanner **C0/H0/M0/L2/I2 → «к деплою ДА»** (T-3246; `plans/reports/round1026_s2_scanner_audit.md`); pytest `.venv` **8569/0**, JS **43/43**; **Δ DDL=0**, **Δ каталога=0** (467/426/442/100/98/21); публикация/промпты/XML вне diff (D4); ровно 2 LLM-вызова. Техдолг — **§73.5** (L-R1026S2-1/-2/-3 + Info). `APP_VERSION` 2.58.20 → **2.58.21** (bump @Builder; закоммичен/задеплоен — Шаг 9). **deploy — ✅ задеплоено 2.58.21 VERIFIED (Шаг 9 @DevOps, T-3250; `eda7325`/`6fa456c`/`989ff8d`, health 200, `database is locked`=0); live-приёмка — `[ ]` PENDING OWNER VERIFICATION (D4).** | §80, §89–§93, §96 | backend/алгоритм | **P0** | S1 | **T-3224…T-3252 — `plans/archive/summary-context-restore-round1026/tasks.md`** |
| **S3** ✅ | `summary-l1-clusterizer` — L1 Кластеризатор §94–§95 (Conversation Disentanglement; **L1 не пишет Саммари**; строгий JSON-контракт; валидация `message_id`/`evidence_message_ids`; невалидный JSON не передаётся в L2). **✅ COMPLETED (автономный контур: @Reviewer Approved C0/H0; @Scanner C0/H0/M1-процесс(закрыт)/L3/I5) + ARCHIVED (Шаг 8 @PM, T-3279, 23.09.2026; архив — `plans/archive/summary-l1-clusterizer-round1026/`).** `tasks.md` — блоки 0/A–H, **T-3253…T-3282 (30)**; pytest **8685/0**, JS 43/43; каталог **468/426/443** (санкция D4: +1 промпт-ключ), F8 delta 57, **Δ DDL=0**, `APP_VERSION` bump **2.58.22**; **врезка L1 — DEFERRED → S5/S6** (живой путь/публикация вне diff; ровно 2 LLM-вызова); **UI-слот модели → S5**; follow-up L-R1026S3-1/-2/-3 + R-R1026S3-1/-2 → S5. Открытые/закрытые: T-3278 (merge §74) ✅, T-3279 (архивация) ✅, T-3280 (deploy VERIFIED 2.58.22) ✅, T-3281 (метрики) ✅; live — PENDING OWNER VERIFICATION. | §81, §94, §95, §92–§93, §106 | backend/LLM+канон | **P0** | S1, S2 | **T-3253…T-3282 — `plans/archive/summary-l1-clusterizer-round1026/tasks.md`** |
| **S4** ✅ | `summary-fact-package` — пакет фактов §96 (название/описание/хронология/факты/подтверждающие ID/фрагменты; без повторного сырого лога; не генерировать «доказательства» самостоятельно) — **✅ DONE / VERIFIED (автономный контур: @Reviewer Approved C0/H0; @Scanner C0/H0/M1-процесс(закрыт `review.md`)/L2/I2) + ARCHIVED (Шаг 8 @PM, T-3304, 23.09.2026; архив — `plans/archive/summary-fact-package-round1026/`).** `spec.md` (SC-01…SC-10) + **ADR-1026-6 Accepted (D1–D6)**; `tasks.md` — блоки 0/A–H, **T-3283…T-3307 (25)**; модуль `services/summary_fact_package.py` + 52 теста, интерфейс `build_fact_package(l1_result, payload_items, *, budget=None, correlation_id=None) -> FactPackageResult`; сборка из `L1Result` (fail-closed §95/§106), 0 LLM-вызовов, `description`/`chronology` — детерминированно (не LLM), `service{response_mode,cover_prompt}` транзитом, врезка — **GATED (не раньше S5/S6)**, pytest **8737/0** (+52), JS 43/43, Δ DDL=0, **Δ каталога=0** (468/426/443; F8 не переиздавался — N/A), `APP_VERSION` bump **2.58.23**; **deploy — VERIFIED 2.58.23 (`08219ab`/`7722d66`); врезка GATED (S5/S6)**; live — ⏳ PENDING OWNER VERIFICATION. Follow-up L-R1026S4-1/-2/-3 + I-R1026S4-1/-2 → S5. | §96, §93–§95, §106 | backend/сборка | **P0** | S3 ✅ | **T-3283…T-3307 — `plans/archive/summary-fact-package-round1026/tasks.md`** |
| **S5** ✅ | `summary-l2-writer-formatter` — L2 Писатель §97–§99 (отдельная модель/провайдер/системный промпт; качественная структурированная статья; без сленга/беглой речи/бессвязного потока/мемов; умеренная ирония; не выдумывать цитаты/не приписывать реплики; структура статьи §98, структурированный вывод §99, серверный форматтер → Rich Message; H1 §101, абзацы/акценты §102, экранирование; plain-fallback §105; fail-closed §106) — **✅ DONE / VERIFIED (автономный контур) + ARCHIVED (Шаг 8 @PM, T-3341, 23.09.2026; архив — `plans/archive/summary-l2-writer-formatter-round1026/`).** Step 1 @PM ✅ + Step 2 @Architect ✅ 23.09.2026 (`spec.md` SC-01…SC-14 + ADR-1026-7 Accepted D1–D7); сверка @PM T-3310 ✅; @Reviewer **Approved** (T-3337, итер.2, C0/H0, блокирующих Medium 0) + @Scanner **C0/H0/блокирующих Medium 0/L4/I2** (T-3338, итер.2; `plans/reports/round1026_s5_scanner_audit.md`); rework T-3339 закрыл **B-R1026S5-1** (фикстура F8 — 3+/2−, 0 mojibake) + **S-R1026S5-1** (ON-guard перенесён после S1/S2); pytest **8807/0** (+70), JS **43/43**, каталог **469/426/444**, **F8 переиздан** (REGISTRY 468→469, categorized 443→444, delta 57→58), **Δ DDL=0**, bump **2.58.24**; **ON-путь — GATED** (`SUMMARY_HYBRID_L2_ENABLED` default OFF, env-only; включение — только после live-приёмки Эпика 1, ADR-1025-24 D4); deploy — **VERIFIED 2.58.24** (`03a4d55`/`ae5a147`); live — ⏳ PENDING OWNER VERIFICATION. Follow-up → S6 (**L-R1026S5-3/-4/-5/-6, S-R1026S5-7**, §106-классы). Откат — `git reset --hard pre-round1026-s5` → `e3ea608`; soft — `flags.summary_hybrid_l2_enabled=false`. (решения ADR-1026-7: модули `services/summary_l2_writer.py` + `services/summary_article_formatter.py`; слот модели/ключ — **env-only `SUMMARY_L2_*`** (Δ=0), промпт — **каталог +1** `prompts.summary_l2_writer_system_prompt` → **F8 переиздание (ADR-1026-2, T-3312)**; kill-switch `SUMMARY_HYBRID_L2_ENABLED` (default OFF, env-only) + per-chat `_chat_limit`; лимиты `title`≤200/`limits.max_summary_parts`/rich≤32000; **Δ DDL=0**; **Deploy = ДА, bump 2.58.23 → 2.58.24**; OFF-путь байт-в-байт, ON — ровно 2 вызова, `await_count==2` оба пути, ON GATED; зависимость **S4 ✅ DONE/DEPLOYED 2.58.23**; сюда же — **врезка L1→пакет фактов→L2** (ровно 2 вызова, AMEND ADR-1022-4 effective S5), слот/UI модели L2 §82 и закрытие follow-up S3 `L-R1026S3-*`/`R-R1026S3-*` + S4 `L-R1026S4-1/-2/-3`/`I-R1026S4-1/-2` — **L-R1026S3-1 закрывается в S5 до врезки**, остальные документируются/передаются (spec §10); **гейт S6/S10+D4: врезка за kill-switch `flags.summary_hybrid_l2_enabled` (default OFF) + байт-в-байт OFF-путь — S5 верифицируем без владельца**; live — без изменений | §97–§99, §101, §102 | backend/LLM+формат | **P0** | S4 ✅ | **T-3308…T-3344 — `plans/archive/summary-l2-writer-formatter-round1026/tasks.md`** |
| **S6** ✅ | `summary-publish-integration` — публикация §100–§106: reuse `sendRichMessage` (§100) + настоящий H1, **`generate_image` НЕ трогать** (§104), fallback `sendMessage` (§105), коды ошибок §106 (в т.ч. L-R1026S5-4: `SUMMARY_GENERATION_FAILED`/`COVER_GENERATION_FAILED`/`RICH_MESSAGE_SEND_FAILED`/`TEXT_FALLBACK_FAILED`), сверка документации Telegram §103 — **✅ COMPLETED + MERGED (§80) + ARCHIVED + DEPLOYED (2.58.28)** (Шаг 8 @PM, T-3458, 24.09.2026; архив — `plans/archive/summary-publish-integration-round1026/` — `spec.md` + **ADR-1026-11** (Accepted) + `tasks.md` (T-3435…T-3460, 26; **T-3460 handoff — `[ ]` OPEN**) + `evidence.md` (в т.ч. §Rework) + `review.md` + `deployment.md` (VERIFIED); Merge §80 @Architect T-3457). Ревью — **единый Reviewer gate** (Scanner удалён 24.09.2026): итер.1 **Needs Fixes** (B-R1026S6-1/-2 Medium) → rework **1** → итер.2 **Approved C0/H0** (binding `197891f`/`4c7b2917…`/`77c17775…`). Числа: pytest **9000/0** (+74), JS **47/47**; **Δ DDL=0**, **Δ каталога=0** (469/426/444/100/98/21; F8 не переиздавался); deploy VERIFIED (`d0d634c`/`cebd950`/`b889019`, MainPID 575712, health 200, `database is locked`=0). Техдолг: **L-R1026S6-D1 CLOSED** (merge §80), **L-R1026S6-D2 OPEN**; Q1/Q2 допустимы; закрыты follow-up S5 **L-R1026S5-3/-4/-5/-6 + S-R1026S5-7**. Live — ⏳ PENDING OWNER VERIFICATION. | §100–§106, §103 | backend/публикация | **P0** | S5 ✅ | **T-3435…T-3460 — `plans/archive/summary-publish-integration-round1026/tasks.md`** |
| **S7** ✅ | `summary-logging-runid` — `run_id` + события §108, детали логов §109, фильтр в log viewer §110 (без ключей) | §108–§110 | backend/лог+UI | **P0** | **S5 ✅** (ядро `run_id`/событий FILTER/L1/L2/FORMAT/обложки независимо от S6; события `PUBLISH_RICH_*`/`PUBLISH_TEXT_*` §108 — **GATED вместе с S6/D4**, не реализованы) | **✅ COMPLETED + MERGED (§78) + ARCHIVED + DEPLOYED (2.58.26)** (Шаг 8 @PM, T-3407, 24.09.2026; архив — `plans/archive/summary-logging-runid-round1026/` — `spec.md` + **ADR-1026-9** (Accepted) + `tasks.md` (T-3380…T-3409, 30) + `evidence.md` + `review.md` + `deployment.md` (VERIFIED); Merge §78 @Architect T-3406). Ревью — **единый Reviewer gate** (Scanner удалён намеренно 24.09.2026; отдельного Scanner-approval нет): T-3403 итер.3 + T-3404 (линза 2, focused change audit + миграция) — **Approved** C0/H0/M0, блокирующих 0. Deploy **VERIFIED**: `5cba5df` (код+тесты+README) / `aa42a04` (планы/миграция) / `1684da9` / `4c22e2c` (deploy-doc); MainPID **522392**, health 200, `database is locked`=0; откат — annotated-тег `pre-round1026-s7` → `f774ecc`. Числа: pytest `.venv` **8890/0** (+36), JS **45/45**; **Δ DDL=0**, **Δ каталога=0** (469/426/444/100/98/21; F8 не переиздавался); `PUBLISH_*` GATED (S6/D4) — в коде отсутствуют; dry-run 0/0/0; R17/R18. **Техдолг — §78.7:** `L-R1026S7-1` (Low, pre-existing S1, вне diff → S8/хотфикс) + follow-up S9 → S8. **▶️ Следующий шаг — выбор S8/гейты: S8 `summary-analytics-adapter` (зависит от S7 ✅ и S6 ⛔ GATED/D4); S6/S10 — ⛔ GATED (D4, live-приёмка Эпика 1); live S1–S5/S9/S7/Эпика 1 — ⏳ PENDING OWNER VERIFICATION.** |
| **S8** ✅ | `summary-analytics-adapter` — §111 adapter ExecutionGraph + узлы Filter/Clusterizer/Writer/Formatting/Publication (kind: algorithm/llm/format/publish; **GraphViewer не переписывать**), метрики §112 | §30, §111, §112 | web+api adapter | **P0** | **S7 ✅, S6 ⛔ (GATED/D4), F6 ✅** | **✅ COMPLETED + MERGED (§79) + ARCHIVED + DEPLOYED (2.58.27)** (Шаг 8 @PM, T-3432, 24.09.2026; архив — `plans/archive/summary-analytics-adapter-round1026/` — `spec.md` + **ADR-1026-10** (Accepted) + `tasks.md` (T-3410…T-3434, 25; закрыты T-3410…T-3433; **T-3434 handoff — `[ ]` OPEN**) + `evidence.md` + `review.md` + `deployment.md` (VERIFIED); Merge §79 @Architect T-3431; перенос — SHA-256 6/6 идентичны). Ревью — **единый Reviewer gate** (Scanner удалён намеренно 24.09.2026; отдельного Scanner-approval нет): линза 1 T-3428 (requirements/correctness) + линза 2 T-3429 (focused change audit) — **Approved C0/H0**, блокирующих 0, 1 проход; rework 0 (T-3430 — ✅ N/A); binding Reviewed-Commit `2ffeb6a`, WTH `4151d36d…`, Spec-Hash `f397f3fd…`. Deploy **VERIFIED**: `64cdb0e` (код+тесты) / `7c5338e` (планы) / `f14ae56` (deploy-doc); прод MainPID **540872**, health 200, `/healthz` 2.58.27, `database is locked`=0, роут `GET /api/analytics/execution/latest` жив; откат — annotated-тег `pre-round1026-s8` → `2ffeb6a`. Числа: pytest `.venv` **8926/0** (+36), JS **46/46**; **Δ DDL=0**, **Δ каталога=0** (469/426/444/100/98/21; F8 не переиздавался). **Техдолг §79.7:** `L-R1026S8-1/-2/-3` (Low/Info, OPEN); **`L-F6S-1` — CLOSED**. ▶️ **Далее — только гейтовые: S6 ⛔ / S10 ⛔ при снятии D4 (ADR-1025-24, live-приёмка Эпика 1); других независимых задач Эпика 2 не осталось.** live S1–S5/S7/S8/S9/Эпика 1 — ⏳ PENDING OWNER VERIFICATION. **Актуализация 24.09.2026 (T-3458 @PM): S6 ✅ COMPLETED + MERGED (§80) + ARCHIVED + DEPLOYED (2.58.28); остаётся только S10 `summary-deploy` — следующий и последний пункт Эпика 2 (D4 закрыт владельцем).** |
| **S9** ✅ DONE | `summary-testing-ui` — тестирование из Mini App §113 (чат/окно/«Проверить пайплайн», **без публикации**, без изменений памяти, без `generate_image` без подтверждения; предпросмотр фильтр→контекст→L1→пакет→L2 + форматтер; метрики §112; диагностика) | §113, §112, §114 | UI+api | **P0** | **S5 ✅** (зависимость от S6 не подтверждена — см. примечание ниже) | **✅ DONE / VERIFIED (автономный контур) + ARCHIVED (Шаг 8 @PM, T-3377, 24.09.2026; архив — `plans/archive/summary-testing-ui-round1026/`) — итер.2 после rework T-3375: закрыты B-R1026S9-2 (High, краш вкладки на error-путях) и B-R1026S9-1 (Medium, §112 «Процент отсева»); @Reviewer Approved C0/H0 + @Scanner C0/H0/блокирующих 0/L5/I2; pytest 8854/0 (+47), JS 44/44, каталог 469/426/444 без Δ, Δ DDL=0, bump 2.58.25; флаг default ON (hot-OFF false); deploy **VERIFIED 2.58.25** (`ac3f8fc`/`59e5b12`); live — PENDING OWNER VERIFICATION; follow-up L-R1026S9-2/-6/-7/-8 + S-R1026S9-5 + I-R1026S9-1/-2 → S7/S8; откат — `git reset --hard pre-round1026-s9` → `cc6105c`. История Step 1 @PM ✅ (23.09.2026; `plans/features/summary-testing-ui-round1026/tasks.md`, блоки 0/A–H, **T-3345…T-3379, 35**; baseline `ae5a147`/2.58.24, pytest 8807/0, JS 43/43, каталог 469/426/444, Δ DDL=0) → Step 2 @Architect ✅ (`spec.md` SC-01…SC-15 + **ADR-1026-8** D1–D8) → сверка @PM ✅ (**T-3347**, 23.09.2026; карта ADR→задачи; **T-3346 ✅**, **T-3345 `[ ]`** — тег `pre-round1026-s9` ещё не создан) → baseline @DevOps ⏳ → @Builder B–F → G → H; **deploy = ДА (D8), bump 2.58.24 → 2.58.25**, Δ DDL=0, **Δ каталога=0 (F8 N/A)**, hot-OFF `SUMMARY_TEST_UI_ENABLED=false`; **независима от S6** (§113: тестовый прогон **не публикует**; порядок `S5 → [S6 ∥ S9] → S7 → S8 → S10`) |
| **S10** ✅ | `summary-deploy` — прямой деплой §107 + проверка §114 + первый рабочий запуск §115 + результаты §117 (+ §85-UI — при отдельной санкции) | §107, §114, §115, §117 | deploy/ops (gate) | **P0** | S1…S9 ✅ | **✅ COMPLETED + MERGED (§81) + ARCHIVED + DEPLOYED/ACTIVATED (2.58.29)** — Шаг 8 @PM (T-3477, 24.09.2026; архив — `plans/archive/summary-deploy-round1026/` — `spec.md` + **ADR-1026-12** (Accepted) + `tasks.md` (T-3461…T-3480) + `review.md` + `deployment.md` (VERIFIED) + `evidence.md` + `results.md` + `procedure-115.md` + `sec114-harness.md`; Merge §81 @Architect T-3476). Ревью — **единый Reviewer gate** (Scanner удалён 24.09.2026; отдельного Scanner-approval нет): линза 1 T-3474 + линза 2 T-3475 — **Approved C0/H0** с 1-й итерации (binding Reviewed-Commit `76abf91`, WTH `a17093d8…`, Spec-Hash `41ee0c6e…`; rework 0). Deploy/активация **VERIFIED**: `415a861` (код+тесты+`config/settings.py`) / `04f5ae1` (планы) / `7bf716e` (deploy-doc); прод ff `cebd950..04f5ae1`, active MainPID **595858**, health 200, `/healthz` 2.58.29, served `?v=2.58.29`, `database is locked`=0, **эффективный `SUMMARY_HYBRID_L2_ENABLED`=True** (code-default ON; `.env` без override), фильтр ON; откат — annotated-тег `pre-round1026-s10` (obj `be7760295`) → `76abf91` + soft `SUMMARY_HYBRID_L2_ENABLED=false` + `git revert 415a861 04f5ae1`. Числа: pytest `.venv` **9026/0** (+26), JS **47/47**, `git diff --check`=0; **Δ DDL=0 / Δ каталога=0** (469/426/444/100/98/21; F8 не переиздавался); 2-вызовность; CSP/zero-build. **Техдолг §81.10:** `L-R1026S10-1` (Info) / `L-R1026S10-3` (Low) — **OPEN**; `L-R1026S10-2` (Info, doc-drift номеров строк) — **CLOSED на merge §81** (`review.md` — исторический снимок, не переписывается). **Live §115 пп.5–7 / §117 пп.7/8/11–14 — ⏳ PENDING OWNER VERIFICATION.** |
...
**Порядок Эпика 2:** `S1 → S2 → S3 → S4 → S5 → [S6 ∥ S9 (после S5)] → S7 ✅ → S8 ✅ → S10`. **▶️ S1 ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §71) → S2 `summary-context-restore-round1026` ✅ COMPLETED + MERGED (§73) + ARCHIVED (Шаг 8 @PM, T-3249, 23.09.2026; `tasks.md` T-3224…T-3252, 29; deploy — ✅ задеплоено 2.58.21 (Шаг 9 @DevOps); live — `[ ]` PENDING OWNER VERIFICATION) → ✅ **S3 `summary-l1-clusterizer` — COMPLETED (автономный контур) + ARCHIVED (Шаг 8 @PM, T-3279, 23.09.2026; архив — `plans/archive/summary-l1-clusterizer-round1026/`; deploy — после T-3280; live — PENDING OWNER VERIFICATION) → ✅ **S4 `summary-fact-package` — DONE / VERIFIED (автономный контур) + ARCHIVED (Шаг 8 @PM, T-3304, 23.09.2026; архив — `plans/archive/summary-fact-package-round1026/`; deploy — **VERIFIED 2.58.23** (`08219ab`/`7722d66`); врезка GATED (S5/S6); live — ⏳ PENDING OWNER VERIFICATION) → ✅ **S5 `summary-l2-writer-formatter` — DONE / VERIFIED (автономный контур) + ARCHIVED** (Шаг 8 @PM, T-3341, 23.09.2026; архив — `plans/archive/summary-l2-writer-formatter-round1026/`; @Reviewer Approved итер.2 C0/H0, @Scanner C0/H0/блокирующих 0/L4/I2; pytest 8807/0 (+70), JS 43/43, каталог 469/426/444, F8 переиздан, Δ DDL=0, bump 2.58.24; deploy **VERIFIED 2.58.24** (`03a4d55`/`ae5a147`); ON-путь — GATED (`SUMMARY_HYBRID_L2_ENABLED` default OFF); live — PENDING OWNER VERIFICATION) → **S6 ⛔ BLOCKED (GATED D4) → S9 `summary-testing-ui` ✅ DONE / VERIFIED + ARCHIVED** (Шаг 8 @PM, T-3377, 24.09.2026; архив — `plans/archive/summary-testing-ui-round1026/`; @Reviewer Approved итер.2 C0/H0, @Scanner C0/H0/блок.0/L5/I2; pytest 8854/0 (+47), JS 44/44, каталог 469/426/444 без Δ, Δ DDL=0, bump 2.58.25; флаг default ON; deploy **VERIFIED 2.58.25** (`ac3f8fc`/`59e5b12`); live — PENDING OWNER VERIFICATION) → **S7 `summary-logging-runid` ✅ COMPLETED + MERGED (§78) + ARCHIVED + DEPLOYED (2.58.26)** (Шаг 8 @PM, T-3407, 24.09.2026; архив — `plans/archive/summary-logging-runid-round1026/` — `tasks.md` T-3380…T-3409, 30; единый Reviewer gate — Approved C0/H0/M0; deploy VERIFIED `5cba5df`/`aa42a04`/`1684da9`/`4c22e2c`, health 200, `database is locked`=0; live — ⏳ PENDING OWNER VERIFICATION; откат — `pre-round1026-s7` → `f774ecc`; порядок `S5 → [S6 ∥ S9 ✅] → S7 ✅ → S8 ✅ → S10`).** **▶️ S8 `summary-analytics-adapter` ✅ COMPLETED + MERGED (§79) + ARCHIVED + DEPLOYED (2.58.27)** (Шаг 8 @PM, T-3432, 24.09.2026; архив — `plans/archive/summary-analytics-adapter-round1026/`; единый Reviewer gate Approved C0/H0; deploy VERIFIED `64cdb0e`/`7c5338e`/`f14ae56`, MainPID 540872, health 200, `database is locked`=0; Δ DDL=0 / Δ каталога=0; откат `pre-round1026-s8` → `2ffeb6a`). **▶️ S6 `summary-publish-integration` ✅ COMPLETED + MERGED (§80) + ARCHIVED + DEPLOYED (2.58.28)** (Шаг 8 @PM, T-3458, 24.09.2026; архив — `plans/archive/summary-publish-integration-round1026/`; единый Reviewer gate C0/H0 (rework 1); deploy VERIFIED `d0d634c`/`cebd950`/`b889019`, MainPID 575712, health 200, `database is locked`=0; откат `pre-round1026-s6` → `197891f`). **▶️ S10 `summary-deploy` ✅ COMPLETED + MERGED (§81) + ARCHIVED + DEPLOYED/ACTIVATED (2.58.29) — последний пункт Эпика 2 (архив — `plans/archive/summary-deploy-round1026/`); Эпик 2 — ✅ COMPLETED (авто-часть: S1–S10); live S1–S10 и Эпика 1 — ⏳ PENDING OWNER VERIFICATION; T-3460 (handoff S6) и T-3479/T-3480 (handoff S10) — `[ ]` OPEN.**
 Канон-цепочка **S3 → S4 → S5 → S6** атомарна (ADR-1013-3). **Δ DDL / новые параметры `summary.filter.*` и слоты моделей** — санкционировать в `spec.md` (параллельное хранилище §87 запрещено). **СТОП-ГЕЙТ:** старт Эпика 2 — только после приёмки Эпика 1 (§0/§79).

**Примечание (Шаг 8 @PM, T-3341, 23.09.2026) — S9 ▶️ NEXT, S6 ⛔ BLOCKED:** в таблице у S9 ранее была указана зависимость `S5, S6`; проверка **§113** (тестовый прогон **не публикует** сообщение, **не меняет память**, `generate_image` — только с отдельным подтверждением) и порядок `S5 → [S6 ∥ S9]` показывают, что **функционально S9 зависит только от S5** (результаты S1–S5: фильтр, восстановление контекста, L1, пакет фактов, статья L2 + форматтер → предпросмотр Rich Message в тестовом режиме). Публикационный слой S6 тестовому контуру не требуется → **S9 ▶️ NEXT (eligible)**, **S6 ⛔ BLOCKED** до live-приёмки Эпика 1 (GATED, ADR-1025-24 D4). Если Step 2 @Architect подтвердит переиспользование тест-API адаптера публикации из S6 — этот фрагмент помечается GATED, тест остаётся «предпросмотр без публикации». **✅ Step 2 подтвердил (ADR-1026-8 D6 / spec §8): S6 — GATED, не вызывается; предпросмотр строится напрямую из форматтера S5; сверка @PM T-3347 ✅ (23.09.2026).**

**Примечание (Шаг 8 @PM, T-3377, 24.09.2026; актуализировано Шаг 8 @PM, T-3407, 24.09.2026 — S7 ✅ COMPLETED + MERGED (§78) + ARCHIVED + DEPLOYED (2.58.26); `tasks.md` T-3380…T-3409, 30):** S9 закрыта автономным контуром (итерация 2: rework T-3375 закрыл **B-R1026S9-2** High «краш вкладки на error-путях» и **B-R1026S9-1** Medium «§112 Процент отсева»; @Reviewer Approved C0/H0 + @Scanner C0/H0/блокирующих 0/L5/I2), архив — `plans/archive/summary-testing-ui-round1026/`; deploy **VERIFIED 2.58.25** (`ac3f8fc`/`59e5b12`); live — PENDING OWNER VERIFICATION. **Проверка зависимостей S7** (`summary-logging-runid`, §108–§110): ядро `run_id`/события FILTER/L1/L2/FORMAT/обложки **не требует S6** → может стартовать (после S9); события `PUBLISH_RICH_*`/`PUBLISH_TEXT_*` §108 относятся к публикации → **GATED вместе с S6 (D4)**, окончательная нарезка — Step 2 @Architect. **S8 зависит от S7 ✅** (и S6 ⛔/F6 ✅). **S6/S10 остаются ⛔ GATED (D4 — live-приёмка Эпика 1).** ▶️ **Следующий шаг — выбор S8/гейты** (архивация S7 — T-3407 ✅; handoff T-3409 — открыт). **Актуализация (Шаг 8 @PM, T-3458, 24.09.2026): S6 ✅ COMPLETED + MERGED (§80) + ARCHIVED + DEPLOYED (2.58.28); D4 закрыт владельцем (owner declaration 24.09.2026); после S6 остаётся только S10 `summary-deploy` — следующий и последний пункт Эпика 2; live S1–S6/S7/S8/S9 — ⏳ PENDING OWNER VERIFICATION.**

**D4-гейт S1:** **S1 не меняет публикацию** (`generate_image`/`sendRichMessage`/`build_cover_media`/`send_rich_message`/fallback `sendMessage`) — вход/алгоритм/промпты безопасны; **S6 `summary-publish-integration` и S10 `summary-deploy` — отдельный гейт** после **live-приёмки Эпика 1** + независимого решения @Architect (ADR-1025-24 D4). S1 держит System-2 **ровно 2 LLM-вызова** (ADR-1022-3/4) и **Δ DDL=0**. **✅ S1 закрыта, не открывая гейт:** **S6 `summary-publish-integration` / S10 `summary-deploy` остаются за гейтом** (публикация/промпты вне diff; live-приёмка Эпика 1 — ⏳ PENDING OWNER VERIFICATION). **Актуализация 24.09.2026 (T-3458): D4 закрыт владельцем; S6 ✅ COMPLETED + MERGED (§80) + ARCHIVED + DEPLOYED (2.58.28); S10 `summary-deploy` — следующий и последний пункт Эпика 2.**

### 🗺️ ЭПИК 3 (Раунд 10.26+ / 10.27; старт после приёмки Эпиков 1 и 2) — «Agentic Intelligence» — 11 фич (A0–A10; верхнеуровнево, детализация — Step 1 @PM при старте фичи) — ✅ **ПРЕДУСЛОВИЕ ДОСТИГНУТО** (Эпик 1 — авто ✅ §70; Эпик 2 — авто ✅ §71–§81); **Wave 0 — A0 `agentic-audit-round1026` ✅ COMPLETED (read-only) + APPROVED (C0/H0) + ARCHIVED (T-3499 @PM, 24.09.2026) + deploy NOT_APPLICABLE; Wave 1 — A1 `tool-coordinator` ✅ COMPLETED + MERGED (§83) + ARCHIVED + DEPLOYED (2.58.30, T-3521 @PM, 24.09.2026); Wave 2 — A2 `tool-chains` ✅ COMPLETED + MERGED (§84) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3548 @PM, 24.09.2026; ADR-1026-15 Accepted); **Wave 2 (продолжение) — A3 `unified-image-request` ✅ COMPLETED + MERGED (§85) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3583 @PM, 25.09.2026; ADR-1026-16 Accepted; единый Reviewer gate Approved C0/H0, R3); **Wave 3 — A5 `image-daily-limit` ✅ COMPLETED + MERGED (§86) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3605/T-3606/T-3607 @PM, 25.09.2026; ADR-1026-17 Accepted; единый Reviewer gate Approved C0/H0, R3; архив `plans/archive/image-daily-limit-round1026/`)**; **Wave 4 — A6 `memory-lookup-api` ✅ COMPLETED + MERGED (§87) + ARCHIVED (`plans/archive/memory-lookup-api-round1026/`, 5 файлов) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3623/T-3624/T-3625 @PM, 25.09.2026; ADR-1026-18 Accepted; единый Reviewer gate Approved C0/H0, R2; канон 11→12; Δ DDL=0 / Δ каталога=0)**; **Wave 4 (продолжение) — A4 `image-context-memory` ✅ COMPLETED + MERGED (§88) + ARCHIVED (`plans/archive/image-context-memory-round1026/`, 6 файлов) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3641/T-3642/T-3643 @PM, 25.09.2026; ADR-1026-19 Accepted; единый Reviewer gate Approved C0/H0 на 4-м цикле, R3; spec `63064ccb…`)**; **A8 `telegram-reactions` ✅ COMPLETED + MERGED (§90) + ARCHIVED (`plans/archive/telegram-reactions-round1026/`, 5 файлов) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3684/T-3685/T-3686 @PM, 25.09.2026; ADR-1026-21 Accepted; единый Reviewer gate Approved C0/H0, R2)** — порядок: `A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → A7 ✅ → A8 ✅ → A9 ✅ → A10 ✅`; ✅ **все фичи A0–A10 APPROVED**; ▶️ следующий шаг — **АГРЕГАТНЫЙ REVIEWER RELEASE GATE ЭПИКА 3** (A9 §91 ✅ + A10 §92 ✅); live-гейты владельца (Эпик 1 WebView, live S1–S10/§115/§117, live-приёмка §13/§14 A1) — ⏳ PENDING OWNER VERIFICATION и независимые задачи не останавливают
...
|---|---|---|---|---|---|
| **A0** | `agentic-audit` — обязательный read-only архитектурный аудит §12: карта всех инструментов (название/назначение/аргументы/результат/ошибки/доступность/зависимости/возможность последовательного вызова), схема JSON L1↔L2, исследование отказа tool calling генерации изображений, досье/RAG, фактчек, извлечение Markdown из URL, Decision Making, реакции, тумблеры/лимиты. **Enabler (Wave 0 Эпика 3)** — ✅ **COMPLETED (read-only; Approved — единый Reviewer gate C0/H0) + ARCHIVED + deploy NOT_APPLICABLE** (2.58.29 без bump); архив — `plans/archive/agentic-audit-round1026/`; durable-артефакт — `plans/docs/agentic-audit-round1026.md` | §12, §54 п.1 | audit (read-only) | **P0** | Эпики 1–2 |
| **A1** | `tool-coordinator` — единый координатор инструментов: Синтезатор расширяется (намерение / адресат / необходимость памяти / выбор инструментов / оценка результатов / выбор действия); Вербализатор — только текст по подготовленным фактам; **общий** механизм цепочек, не отдельный обработчик на каждую комбинацию. **✅ COMPLETED + MERGED (§83) + ARCHIVED + DEPLOYED (2.58.30)**; архив — `plans/archive/tool-coordinator-round1026/`; ADR-1026-14 Accepted | §13, §14 | backend/LLM-оркестрация | **P0** | A0 |
| **A2** | `sequential-tool-calls` — последовательные зависимые вызовы (A→B; зависимые не параллелить), структурированные контракты результатов/ошибок (данные, не инструкции), ограничения: max вызовов, общий тайм-аут, лимит повторных попыток, защита от повторного одинакового вызова, ограничение расходов, частичный результат. **✅ COMPLETED + MERGED (§84) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; T-3548 @PM, 24.09.2026)**; архив — `plans/archive/tool-chains-round1026/`; ADR-1026-15 Accepted | §15–§17, §36, §37 | backend/tool-loop | **P0** | A1 |
| **A3** | `unified-image-request` — единое представление `ImageRequest` (source direct/tool, chat_id, requester_id, original_message_id, user_request, resolved_subjects, context_required/context_sources, final_prompt, generator_config); устранение двойной генерации; фикс tool calling изображений; оба пути → существующий генератор. **✅ COMPLETED + MERGED (§85) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; T-3583 @PM, 25.09.2026)**; архив — `plans/archive/unified-image-request-round1026/`; ADR-1026-16 Accepted | §18–§21, §25 | backend/tool+image | **P0** | A1, A2 |
| **A4** | `image-context-memory` — генерация из досье/RAG: разрешение имён/алиасов (chat_id/user_id/alias/username/real_name/контекст; при неоднозначности — уточнение/отказ от персонализации), сборка промпта из независимых частей, безопасность (данные ≠ инструкции; арт ≠ достоверный портрет; не придумывать черты лица). **✅ COMPLETED + MERGED (§88) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; T-3641/T-3642/T-3643 @PM, 25.09.2026)**; архив — `plans/archive/image-context-memory-round1026/` (6 файлов); ADR-1026-19 Accepted; единый Reviewer gate Approved C0/H0 на 4-м цикле (R3) | §22–§25, §32–§34, §37 | backend/memory+LLM | **P0** | A3, A6 (**выполнена после A6**) |
| **A5** | `image-daily-limit` — дневной лимит генерации: **atomic reserve** квоты + **idempotency key** (повторный update не списывает дважды), scope global/chat с наследованием (не смешивать «глобальный дефолт» и «общую квоту всех чатов»), календарный день в TZ чата + время сброса, семантика учёта §28, отдельный учёт запросов/успехов/ошибок; UI «Модули → Генерация изображений → Лимиты» с «Использовано сегодня: N/M», источником и временем сброса. **✅ COMPLETED + MERGED (§86) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; T-3605/T-3606/T-3607 @PM, 25.09.2026)**; архив — `plans/archive/image-daily-limit-round1026/`; ADR-1026-17 Accepted | §26–§31, §50 | backend+DDL+UI | **P0** | A3 |
| **A6** | `memory-lookup-api` — structured memory lookup `get_user_context(user_id, chat_id, purpose, max_items)`; purpose: identity/appearance/speech_style/biography/relationships/general; поверх существующих досье+RAG (`dig_into_lore`), **новую базу памяти не создавать**; результат: факты + источники + подтверждённость + временной контекст + признак отсутствия данных; полные досье в каждый запрос не подмешивать. **✅ COMPLETED + MERGED (§87) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; T-3623/T-3624/T-3625 @PM, 25.09.2026)**; архив — `plans/archive/memory-lookup-api-round1026/` (5 файлов); ADR-1026-18 Accepted; единый Reviewer gate Approved C0/H0, R2 | §32–§35 | backend/memory tool | **P0** | A0 ✅; **выполнена** — вход A6 см. «A5 handoff» выше; next — **A4** |
| **A7** ✅ | `decision-making` — **✅ COMPLETED + MERGED (§89) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3664/T-3665/T-3666 @PM, 25.09.2026; ADR-1026-20 Accepted; единый Reviewer gate Approved на 2-м цикле, R3; архив `plans/archive/decision-making-round1026/`, 6 файлов; binding `e8646af`/`4b3df140…`/`950c5f80…`)** — расширение существующего механизма: `action` {reply/react/silent/tool} **отдельно** от `style` {casual/serious/deep_research} (не `style=silent`; Вербализатор не генерирует пустой текст); контракт решения (action/target_message_id/tools/style/reason_code); правила когда отвечать/реагировать/молчать с учётом треда/reply_to/предыдущего действия бота; `silent` не запускает L2; контекст предыдущих действий (изображение/статья/вопрос/ожидание инструмента); настройки §48; **без третьего LLM-вызова** (AMEND `physical-two-call-pipeline`) | §38–§40, §42–§48 | backend/LLM+UI | **P0** | A1, A2 |
| **A8** ✅ | `telegram-reactions` — `setMessageReaction` (официальный Bot API): проверка доступности реакции в чате, разрешённая альтернатива/отказ, реакция на **правильное** сообщение (не путать reply_to_id/message_id/ID прошлого сообщения бота); не превращать ошибку реакции в длинный текст; существующий `react_moai` (🗿) — база. **✅ COMPLETED + MERGED (§90) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; T-3684/T-3685/T-3686 @PM, 25.09.2026)**; ADR-1026-21 Accepted; единый Reviewer gate Approved C0/H0 на 1-м цикле, R2; архив `plans/archive/telegram-reactions-round1026/` (5 файлов) | §41, §44 | backend/Telegram API | **P0** | A7 |
| **A9** ✅ | `agentic-events-graph` — события `DECISION_START/COMPLETE`, `TOOL_PLAN_CREATED`, `TOOL_CALL_START/COMPLETE/FAILED`, `REACTION_SENT`, `MESSAGE_IGNORED`, `IMAGE_CONTEXT_RESOLVED`, `IMAGE_GENERATION_START/COMPLETE/FAILED` (+ `ANTI_CLICHE_*` из F0.3) с полями run_id/chat_id/message_id/action/инструменты/причина/длительность/ошибки; **интеграция в существующий ExecutionGraph** (узлы Decision / Memory Lookup / RAG / Web Extraction / Factcheck / Image Prompt Preparation / Image Generation / Reaction / Text Generation; `silent` — без фиктивного Вербализатора) + Mini App §50; приватное содержимое досье в публичные логи **не** выводить. **✅ COMPLETED + MERGED (§91) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; T-3703/T-3704/T-3705 @PM, 25.09.2026)**; ADR-1026-22 Accepted; единый Reviewer gate Approved C0/H0 на 1-м цикле, R2; архив `plans/archive/agentic-events-graph-round1026/` (5 файлов) | §49, §51 | backend/лог+web adapter | P1 | A1–A8 |
| **A10** ✅ | `agentic-verification` — 23 сценария §52 + критерии приёмки §53 + результаты §54 (12 пунктов): tool calling изображений, совместимость direct/tool, досье, лимит только на сервере, параллельные запросы, цепочки, отказ инструмента, невалидные аргументы, превышение лимита вызовов, молчание/реакция, недоступная реакция, глобальные/локальные настройки, отсутствие двойной генерации; карта инструментов, архитектура координатора, контракты, схема вызовов, схема Image Request, досье/RAG, схема лимитов, JSON Schema решения, логи цепочек, сохранность старых функций. **✅ COMPLETED + MERGED (§92) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE — deployment `epic deployment not applicable` (T-3727/T-3728/T-3729 @PM, 25.09.2026; ADR-1026-23 Accepted; единый Reviewer gate Approved, R1; архив `plans/archive/agentic-verification-round1026/` (5 файлов); приёмка `plans/reports/round1026_a10_acceptance.md`; следующий шаг — агрегатный Reviewer release gate Эпика 3)** | §52–§54 | verification (gate) | **P0** | A0–A9 ✅ |

**Порядок Эпика 3 (предварительный):** **A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → A7 ✅ → A8 ✅ → A9 ✅ → A10 ✅** (все A0–A10 ✅). Канон-атомарность при правке промптов — ADR-1013-3. **Δ DDL** (дневной лимит изображений: резерв/учёт + idempotency) — санкционировать в `spec.md`. **СТОП-ГЕЙТ:** старт Эпика 3 — только после приёмки Эпиков 1 и 2 (§0/§10); исследование A0 допустимо заранее, но не блокирует предыдущие эпики. **✅ Актуализация 24.09.2026: авто-часть Эпиков 1–2 закрыта (Эпик 1 §70; Эпик 2 §71–§81) — предусловие Эпика 3 формально достигнуто; A0 ✅ COMPLETED + ARCHIVED → A1 ✅ COMPLETED + MERGED (§83) + ARCHIVED + DEPLOYED (2.58.30) → A2 ✅ COMPLETED + MERGED (§84) + ARCHIVED + deployment DEFERRED_TO_EPIC — ▶️ следующая **A3 `unified-image-request`** (§18–§21/§25) ∥ A6.** **✅ Актуализация 25.09.2026 (архивация T-3583 @PM): A3 `unified-image-request` ✅ COMPLETED + MERGED (§85) + ARCHIVED + deployment DEFERRED_TO_EPIC (2.58.30 без bump) — порядок: A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ∥ A6 → A4 → A7 ✅ → A8 → A9 → A10; ▶️ следующая — **A5 `image-daily-limit`** (§26–§31/§50; A4 = `image-context-memory`, §22–§25/§32–§34/§37, идёт **позже** по волновому порядку), handoff T-3585 (открыт).** **✅ Актуализация 25.09.2026 (handoff T-3607 @PM): A5 `image-daily-limit` ✅ COMPLETED + MERGED (§86) + ARCHIVED (T-3605 → `plans/archive/image-daily-limit-round1026/`, 6 файлов) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3606; `APP_VERSION` 2.58.30 без bump; ADR-1026-17 Accepted; единый Reviewer gate Approved C0/H0, R3)** — порядок: A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → **A5 ✅** → A6 → A4 → A7 ✅ → A8 → A9 → A10; ▶️ следующая — **A6 `memory-lookup-api`** (§32–§35), затем **A4 `image-context-memory`** (после A6); handoff T-3607 закрыт, **Вход A6 — блок «A5 handoff» ниже.** **✅ Актуализация 25.09.2026 (закрытие T-3623/T-3624/T-3625 @PM): A6 `memory-lookup-api` ✅ COMPLETED + MERGED (§87) + ARCHIVED (T-3623 → `plans/archive/memory-lookup-api-round1026/`, 5 файлов) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3624; `APP_VERSION` 2.58.30 без bump; ADR-1026-18 Accepted; единый Reviewer gate Approved C0/H0, R2)** — порядок: A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → **A6 ✅** → A4 → A7 ✅ → A8 → A9 → A10; ▶️ следующая — **A4 `image-context-memory`** (§22–§25/§32–§34/§37, после A6), затем A7 ✅ → A8 → A9 → A10; handoff T-3625 закрыт, **Вход A4 — блок «A6 handoff» ниже.** **✅ Актуализация 25.09.2026 (закрытие T-3641/T-3642/T-3643 @PM): A4 `image-context-memory` ✅ COMPLETED + MERGED (§88) + ARCHIVED (T-3641 → `plans/archive/image-context-memory-round1026/`, 6 файлов; SHA-256 до/после идентичны, spec `63064ccb…`) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3642; `APP_VERSION` 2.58.30 без bump; ADR-1026-19 Accepted; единый Reviewer gate Approved C0/H0 на 4-м цикле, R3)** — порядок: A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → **A4 ✅** → A7 ✅ → A8 → A9 → A10; ▶️ следующая — **A8 `telegram-reactions`** (§41/§44-механика), затем A9 → A10; handoff T-3643 закрыт, **Вход A7 — блок «A4 handoff» ниже.** **✅ Актуализация 25.09.2026 (закрытие T-3684/T-3685/T-3686 @PM): A8 `telegram-reactions` ✅ COMPLETED + MERGED (§90) + ARCHIVED (T-3684 → `plans/archive/telegram-reactions-round1026/`, 5 файлов; SHA-256 до/после идентичны, spec `6e5cfc80…` байт-в-байт; `threat-failure-analysis.md` отсутствует — `NOT_APPLICABLE` при R2/D12) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3685; `APP_VERSION` 2.58.30 без bump; ADR-1026-21 Accepted; единый Reviewer gate Approved C0/H0 на 1-м цикле, R2)** — порядок: A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → A7 ✅ → **A8 ✅** → A9 → A10; ▶️ следующая — **A9 `agentic-events-graph`** (§49/§51; P1), затем A10; handoff T-3686 закрыт, **Вход A9 — блок «A8 handoff» ниже.** **✅ Актуализация 25.09.2026 (закрытие T-3702/T-3703/T-3704/T-3705 @PM): A9 `agentic-events-graph` ✅ COMPLETED + MERGED (§91) + ARCHIVED (T-3703 → `plans/archive/agentic-events-graph-round1026/`, 5 файлов; SHA-256 до/после идентичны, spec `3f703584…` байт-в-байт; `threat-failure-analysis.md` отсутствует — `NOT_APPLICABLE` при R2/D1) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC, T-3704; `APP_VERSION` 2.58.30 без bump; ADR-1026-22 Accepted; единый Reviewer gate Approved C0/H0 на 1-м цикле, R2)** — порядок: A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → A7 ✅ → A8 ✅ → **A9 ✅** → ▶️ A10; ▶️ следующая — **A10 `agentic-verification`** (§52–§54; P0; зависит A0–A9) — **ПОСЛЕДНЯЯ фича Эпика 3** перед агрегатным Reviewer release gate (bump 2.58.30 → 2.58.31); handoff T-3705 ✅, **Вход A10 — блок «A9 handoff» ниже.** **✅ Актуализация 25.09.2026 (A10 reconcile @PM; T-3726…T-3729): A10 `agentic-verification-round1026` ✅ COMPLETED + MERGED (§92) + ARCHIVED (T-3727 → `plans/archive/agentic-verification-round1026/`, 5 файлов; spec `a50b7cb0…`; ADR re-pin `37d8cef7…`) + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment A10 = `epic deployment not applicable`, T-3728; `APP_VERSION` 2.58.30 без bump; ADR-1026-23 Accepted; единый Reviewer gate Approved, R1)** — порядок: A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → A7 ✅ → A8 ✅ → A9 ✅ → **A10 ✅**; ▶️ **следующий шаг — АГРЕГАТНЫЙ REVIEWER RELEASE GATE ЭПИКА 3** (не фича): агрегатное ревью релиза → доставка + @DevOps bump 2.58.30 → 2.58.31; handoff T-3729 ✅, **Вход агрегатного gate — блок «A10 handoff / aggregate-gate inputs» ниже.**
...
**✅ A0 handoff (T-3497 @PM; закрыт архивацией T-3499 @PM, 24.09.2026):** A0 `agentic-audit-round1026` — обязательный read-only аудит §12 **завершён** (T-3484…T-3495 @Builder ✅, T-3496 @Architect реконсиляция ✅, T-3497 @PM handoff ✅, **T-3498 @Reviewer ✅ — единый gate Approved C0/H0, обе линзы**, **T-3499 @PM ✅ — архивация**). **Статус A0: COMPLETED (read-only) + ARCHIVED + deploy NOT_APPLICABLE** (`APP_VERSION` **2.58.29** без bump; архив — `plans/archive/agentic-audit-round1026/`; binding Reviewed-Commit `e8065e9` / WTH `805c4680…` / Spec-Hash `1B2AE779…`). **Следующая фича — A1 `tool-coordinator`** (§13–§14). Вход A1 — durable-артефакт **`plans/docs/agentic-audit-round1026.md`** (не архивируется; ссылки по анкорам, реестр `#anchors`). **A1 обязан учесть / НЕ дублировать:** freeze каталога инструментов **10** (ADR-1026-13 D3; `#duplicates`, `#tool-map`); **`physical-two-call-pipeline` без 3-го LLM-вызова** (§13; `#epic3-reuse`); разведение **`action`/`style`** (ADR-1023-3, §39; `#epic3-summary`); **REUSE ExecutionGraph** (`plans/ARCHITECTURE.md` §65 F6 + §79 S8; `#epic3-reuse` — вторую аналитику не создавать); **§104 `generate_image` не трогать**; канон промптов **ADR-1013-3 атомарно**; **§85 UI Саммари — отдельная санкция**. **Незакрытые гипотезы HY-01…HY-06 — вход для A3/A4** (`#labels-hypothesis`; первопричина image-tool-calling в A0 не закрыта). **ADR-1026-13 (D1–D3) — Accepted** (по архивации T-3499). Порядок Эпика 3 — без изменений: **A0 ✅ → A1 ✅ → A2 ✅ → [A3 → A5] ∥ A6 → A4 → A7 ✅ → A8 → A9 → A10** (следующая — **A3**).

**✅ A1 handoff (T-3521 @PM; архивация 24.09.2026):** A1 `tool-coordinator-round1026` — единый координатор инструментов **завершён** (T-3500…T-3522 ✅; **T-3523 handoff → A2 открыт**; T-3524 резерв N/A). **Статус A1: COMPLETED + MERGED (§83) + ARCHIVED + DEPLOYED (2.58.30 VERIFIED)** — единый Reviewer gate **Approved C0/H0** (binding `e3ea367`/`4361f011…`/`b37b9a50…`); ADR-1026-14 Accepted; deploy `b32c46a`/`b1c02a3`/`7c79df3`; архив — `plans/archive/tool-coordinator-round1026/`. **Результат:** программный слой `CoordinatorDecision` внутри Синтезатора (`direct_chat_service.py`), без 3-го LLM-вызова и без wire-`action`; reuse `tool_loop` как общий механизм цепочек; изоляция Вербализатора; env-only kill-switch `DIRECT_COORDINATOR_ENABLED` (default ON). **▶️ Следующая — A2 `tool-chains`** (§15–§17: полноценные последовательные вызовы/контракты/лимиты цепочек). **Вход A2 (зафиксирован T-3523):** reuse `tool_loop` (`TOOL_MAX_ROUNDS=4`, ≤2/раунд) как общий механизм цепочек — A2 расширяет до полных последовательных зависимых вызовов, структурированных контрактов §16, ограничения цепочек §17; лимиты/контракты — в `spec.md`/ADR при старте A2 (Step 1/2). **Границы A1 ✅ соблюдены:** §15–§17 (A2), §18–§25 (A3/A4), §32–§35 (A6), §38–§40/§42–§48 (A7), §41/§44 (A8), §49/§51 (A9) — не реализованы в A1. **Техдолг (non-blocking, открыт):** D-a/D-b/D-c + `L-R1026A1-1/-2/-3` + pre-existing флак `test_betterstack_handler.py`; live-приёмка §13/§14 — ⏳ PENDING OWNER VERIFICATION. Порядок Эпика 3 — без изменений: **A0 ✅ → A1 ✅ → A2 ✅ → [A3 → A5] ∥ A6 → A4 → A7 ✅ → A8 → A9 → A10** (следующая — **A3 `unified-image-request`**).

**✅ A2 handoff (T-3548 @PM; архивация 24.09.2026):** A2 `tool-chains-round1026` — Wave 2 Эпика 3 **завершён** (T-3525…T-3548 ✅; **T-3549 = `DEFERRED_TO_EPIC`** — агрегатный релиз Эпика 3, НЕ per-фича/НЕ «done/VERIFIED»; **T-3550 handoff → A3 открыт**; T-3551 резерв N/A). **Статус A2: COMPLETED + MERGED (§84) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC)** — единый Reviewer gate **Approved C0/H0** (R3; binding `e8646af`/`f1f1d544…`/`e87e530f…`); **ADR-1026-15 (D1–D12) Accepted**; архив — `plans/archive/tool-chains-round1026/` (6 файлов; **`deployment.md` отсутствует** — деплой отложен, это корректно). **Результат:** out-of-band **envelope §16** (`ToolLoopResult.tool_results`, `ToolContext.result_for`) при неизменном модельно-видимом канале; структурированные ошибки; зависимые цепочки §15 + общий `ctx.resolved_url`; §17-лимиты **cap 6 / мягкий 360 c / дедуп ≤2 / платные ≤4** + частичный результат; канон **+1 `fetch_article` (10→11)**; последовательность санкционирована (без `asyncio.gather`); Δ DDL=0 / Δ каталога=0. **▶️ Следующая — A3 `unified-image-request`** (§18–§21/§25) ∥ A6. **Вход A3/A4 (зафиксирован T-3548):** reuse **общего механизма цепочек** (`tool_loop`/`ToolRouter`) и **envelope-контракта** результата (`ToolContext.result_for`/`ctx.tool_results`) для единого `ImageRequest` (§18–§25); учесть **§17-лимиты** (cap 6 / 360 c / дедуп ≤2 / платные ≤4; env-only `TOOL_CHAIN_LIMITS_ENABLED`) и **канон 11** (`fetch_article` — 11-й в хвосте; env-only `ARTICLE_TOOL_ENABLED`); **общий резолв ссылки** `smartmodule_urls.resolve_context_url` → `ctx.resolved_url`; **дубликаты инструментов/сервисов запрещены** (санкция расширения ADR-1026-13 D3 **использована** A2 — повторно не расширять без новой санкции); **§104 `generate_image` не трогать без санкции**; незакрытые гипотезы **HY-01…HY-06** — вход A3/A4 (`plans/docs/agentic-audit-round1026.md`). **Техдолг (non-blocking, открыт):** `L-R1026A2-1/-2` закрыты doc-only в §84, `L-R1026A2-3` — при следующем касании; live-прогон §15–§17 — часть `DEFERRED_TO_EPIC`. Порядок Эпика 3 — без изменений: **A0 ✅ → A1 ✅ → A2 ✅ → [A3 → A5] ∥ A6 → A4 → A7 ✅ → A8 → A9 → A10** (следующая — **A3 `unified-image-request`**).

**✅ A3 handoff (T-3583 @PM; архивация 25.09.2026):** A3 `unified-image-request-round1026` — Wave 2 Эпика 3 **завершён** (T-3552…T-3583 ✅; **T-3584 = `DEFERRED_TO_EPIC`** — агрегатный релиз Эпика 3, НЕ per-фича/НЕ «done/VERIFIED»; **T-3585 handoff → A5 открыт**; T-3586 резерв N/A). **Статус A3: COMPLETED + MERGED (§85) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; `APP_VERSION` 2.58.30 без bump)** — единый Reviewer gate **Approved C0/H0 (R3)** после remediation B-1/B-2 (документальный rework T-3580; product-код не менялся); binding Reviewed-Commit `e8646af` / Working-Tree-Hash `29e8da0a…` / Spec-Hash `a98d20d1…`; **ADR-1026-16 (D1–D11) Accepted**; архив — `plans/archive/unified-image-request-round1026/` (7 файлов; **`deployment.md` отсутствует** — деплой отложен, это корректно). **Результат:** единый **ImageRequest** + `build_final_prompt`/`run_image_request` (оба входа direct/tool → существующий `generate_and_send`; второй pipeline запрещён); защита от двойной генерации (пре-гейт + авторитетный маркер `ToolContext.image_request_handled` → `skipped/already_handled`); `TOOL_GENERATE_IMAGE` — текст описания (6 пунктов §21), канон **11**; env-only `UNIFIED_IMAGE_REQUEST_ENABLED`; память/досье — поля-заглушки; §104 — AST-гейт; pytest **9156/0** (+31), JS **47/47**; Δ DDL=0 / Δ каталога=0. **▶️ Следующая — A5 `image-daily-limit`** (§26–§31/§50) ∥ A6. **Вход A5 (зафиксирован T-3583):** **единый ImageRequest с полями-заглушками памяти** (`context_required`/`context_sources`/`resolved_subjects`) — **реализация памяти/досье в A4 `image-context-memory`** («изображения на основе памяти», §22–§25); **envelope/лимиты A2** (§16 `ToolLoopResult.tool_results`/`ToolContext.result_for`, §17 cap 6 / 360 c / дедуп ≤2 / платные ≤4, env-only `TOOL_CHAIN_LIMITS_ENABLED`); **канон 11** (дубликаты/новые инструменты — только по санкции); вход `resolve_module_enabled` (env AND per-chat); env-only `UNIFIED_IMAGE_REQUEST_ENABLED` (default ON); вердикты гипотез HY-01…HY-06 (`plans/docs/agentic-audit-round1026.md`; «данные ≠ инструкции» §37 для tool-контекста изображений). **Owner-гейт прод-проб изображений — ⏳ PENDING OWNER VERIFICATION** (живые пробы HY-01/02/04/05 + per-chat флаг из PG + живая A/B описания `TOOL_GENERATE_IMAGE`); **фикс на недоказанной причине по-прежнему запрещён** (ADR-1026-16 D1). **Техдолг (non-blocking, открыт):** `L-R1026A3-1…-8` (**-6 реконсилирован doc-only при архивации**); live-прогон §18–§21 — часть `DEFERRED_TO_EPIC`. *Примечание нумерации: в таблице раунда 10.26 фича «изображения+контекст/память» указана как **A4 `image-context-memory`** (§22–§25/§32–§34/§37), а **A5 `image-daily-limit`** (§26–§31) — иная фича; актуальные указатели (`backlog.md:296/314`) после Step 1 @PM (25.09.2026) фиксируют следующей **A5 `image-daily-limit`** (§26–§31/§50); **A4** идёт позже по волновому порядку* Порядок Эпика 3: **A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → ▶️ A7 ✅ → A8 → A9 → A10** (следующая — **A8 `telegram-reactions`**, §41/§44-механика).

**✅ A5 handoff (T-3607 @PM; архивация 25.09.2026):** A5 `image-daily-limit-round1026` — Wave 3 Эпика 3 **завершён** (T-3587…T-3607 ✅; **T-3606 = `DEFERRED_TO_EPIC`** — агрегатный релиз Эпика 3, НЕ per-фича/НЕ «done/VERIFIED»; **T-3607 handoff → A6 закрыт**). **Статус A5: COMPLETED + MERGED (§86) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; `APP_VERSION` 2.58.30 без bump)** — единый Reviewer gate **Approved C0/H0 (R3)**; binding Reviewed-Commit `e8646af` / Working-Tree-Hash `b57f1f0e…` / Spec-Hash `428c44b4…`; **ADR-1026-17 (D1–D12) Accepted**; архив — `plans/archive/image-daily-limit-round1026/` (6 файлов: spec + ADR + tasks + evidence + threat-failure-analysis + review-T-3604; SHA-256 до/после переноса идентичны; `deployment.md` отсутствует — деплой отложен, это корректно). **Результат:** атомарный резерв `reserve_image`/`commit_image`/`release_image` (условный UPSERT, одна транзакция shared+per-chat), idem-ключ `{chat_id}:{message_id}:{source}`, PG-журнал `image_reservation` (ledger, +2 индекса, идемпотентный DDL — применяется `PgDatabase.init()`), scope `limits.image_daily_limit` (chat→global→env 60) **≠** shared `WORKER_DAILY_IMAGE_CALLS_GLOBAL` (200, env-only), per-chat TZ `limits.chat_timezone`, учёт §28 из журнала (`image_usage_summary`), UI-врезка «Лимиты» в `mod_images`; env-киль-свитч `IMAGE_DAILY_LIMIT_ENABLED`; Δ каталога 470/427/445/101/99/21; pytest **9170/0**, JS **47/47**; risk **R3** (findings F1–F7 — non-blocking watch-items агрегатного gate). **▶️ Следующая — A6 `memory-lookup-api`** (§32–§35), затем **A4 `image-context-memory`** (§22–§25/§32–§34/§37 — **после A6**). **Вход A6 (зафиксирован T-3607):** (1) контракт резерва уже существует — `services/worker_budget.py`: `reserve_image`/`commit_image`/`release_image`/`image_usage_summary` (+ `image_limits`/`image_day`/`image_next_reset`) — **не вводить второй счётчик/параллельный механизм** (A0 `#epic3-reuse`); (2) каталожный ключ **`limits.image_daily_limit`** (группа `limits_images`, tab `mod_images`) — референс scope-наследования `resolve_setting_cached`; (3) kill-switch-прецедент env-only `IMAGE_DAILY_LIMIT_ENABLED` (default ON) + `IMAGE_RESERVATION_RETENTION_DAYS`; (4) контракты/границы/rollback — **§86** `plans/ARCHITECTURE.md` + **ADR-1026-17 Accepted**; (5) архив R3-доказательств — `plans/archive/image-daily-limit-round1026/` (threat-failure-analysis, race-тесты SC-A5-04, review-T-3604). **Откат вклада A5:** hot = `IMAGE_DAILY_LIMIT_ENABLED=OFF`; cold = `git revert` к анкеру `e8646af`; DDL-откат не требуется (таблица инертна). Порядок Эпика 3 — без изменений: **A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 → A4 → A7 ✅ → A8 → A9 → A10** (следующая — **A6 `memory-lookup-api`**).

**✅ A6 handoff (T-3625 @PM; архивация 25.09.2026):** A6 `memory-lookup-api-round1026` — Wave 4 Эпика 3 **завершён** (T-3608…T-3625 ✅; **T-3624 = `DEFERRED_TO_EPIC`** — агрегатный релиз Эпика 3, НЕ per-фича/НЕ «done/VERIFIED»; **T-3625 handoff → A4 закрыт**). **Статус A6: COMPLETED + MERGED (§87) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; `APP_VERSION` 2.58.30 без bump)** — единый Reviewer gate **Approved C0/H0 (R2)**; binding Reviewed-Commit `e8646af` / Working-Tree-Hash `05d882ea…` / Spec-Hash `4d819180…`; **ADR-1026-18 (D1–D11) Accepted**; архив — `plans/archive/memory-lookup-api-round1026/` (5 файлов: spec + ADR + tasks + evidence + review-T-3622; SHA-256 до/после переноса идентичны 5/5; `deployment.md` отсутствует — деплой отложен, это корректно). **Результат:** новый LLM-видимый инструмент **`get_user_context`** поверх существующих досье+RAG (read-only, без новой БД); канон инструментов **11→12** атомарно (хвост; первые 11 байт-в-байт); контракт `person`(required)/`user_id`(optional)/`purpose`(enum 6)/`max_items`; `chat_id` из `ToolContext`; purpose-роутинг 6 значений; lazy RAG только `{general, speech_style}`; envelope 5 полей §33 (`facts`/`sources`/`confidence`/`time_context`/`no_data`); капы (hard 20; slice ≤5/≤240 симв.; результат ≤4000); env-only киль-свитч `MEMORY_LOOKUP_ENABLED` (default ON); Δ DDL=0 / Δ каталога=0; pytest **9213/0** (+43), JS **47/47**; risk **R2** (findings L-A6-01…-06 — non-blocking watch-items агрегатного gate). **▶️ Следующая — A4 `image-context-memory`** (§22–§25/§32–§34/§37 — **после A6**). **Вход A4 (зафиксирован T-3625):** (1) **generic-контракт A6 уже существует — `get_user_context`** (`services/tool_schemas.py` схема/канон-12/`active_tools`; `services/tool_router.py` диспетчер + `_get_user_context` + purpose-роутинг + envelope + R17-лог; read-only `services/database.py::get_user_context_facts`) — **A4 — потребитель, НЕ переписывать lookup** (§87.9); (2) **purpose-роутинг D3** (`identity`/`appearance`/`speech_style`/`biography`/`relationships`/`general`; `appearance` в A6 = generic-фильтр + honest `no_data`, извлечение/классификация внешности — граница **A4**); (3) **envelope 5 полей §33** + A2-проводка (`ToolContext.result_for`/`ctx.tool_results`); (4) **lazy RAG** `{general, speech_style}`; (5) **капы** (hard-ceiling 20; message-slice ≤5/hard 10; ≤240 симв./фрагмент; результат ≤4000/`truncated`); (6) **киль-свитч** env-only `MEMORY_LOOKUP_ENABLED` (default ON; OFF → инструмент скрыт, канон остаётся 12); (7) **архив** `plans/archive/memory-lookup-api-round1026/` + **§87** `plans/ARCHITECTURE.md` + **ADR-1026-18 Accepted**; (8) A4 **заполняет memory-заглушки A3** (`ImageRequest.context_required`/`context_sources`/`resolved_subjects` — §22–§25), выполняет **резолв имён/алиасов**, **визуальный срез** и **сборку image-prompt**, **не дублируя** `get_user_context`/RAG/резолвер; (9) анти-дубликаты/новая санкция канона — по прецеденту (санкция ADR-1026-13 D3 исчерпана A2; A6 использовал **новую** санкцию ADR-1026-18 D1 — повторно не расширять без новой санкции); (10) owner-гейт прод-проб изображений — `PENDING OWNER VERIFICATION` (живые HY-01/02/04/05 + per-chat флаг; фикс на недоказанной причине запрещён). **Откат вклада A6:** hot = `MEMORY_LOOKUP_ENABLED=OFF`; cold = `git revert` к анкеру `e8646af`; DDL-откат не требуется. Порядок Эпика 3 — без изменений: **A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 → A7 ✅ → A8 → A9 → A10** (следующая — **A4 `image-context-memory`**, §22–§25/§32–§34/§37).

**✅ A4 handoff (T-3643 @PM; архивация 25.09.2026):** A4 `image-context-memory-round1026` — Wave 4 Эпика 3 **завершён** (T-3626…T-3646 ✅; **T-3642 = `DEFERRED_TO_EPIC`** — агрегатный релиз Эпика 3, НЕ per-фича/НЕ «done/VERIFIED»; **T-3643 handoff → A7 закрыт**). **Статус A4: COMPLETED + MERGED (§88) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; `APP_VERSION` 2.58.30 без bump)** — единый Reviewer gate **Approved C0/H0 (R3) на 4-м цикле** (после D13-амендмента; Scanner отсутствует); binding Reviewed-Commit `e8646af` / Working-Tree-Hash `5f9ac819…` / Spec-Hash `63064ccb…`; **ADR-1026-19 (D1–D13) Accepted**; архив — `plans/archive/image-context-memory-round1026/` (6 файлов: spec + ADR + tasks + evidence + threat-failure-analysis + review-T-3640; SHA-256 до/после переноса идентичны; spec `63064ccb…` байт-в-байт). **Результат:** детерминированный helper `services/image_context_memory.py` (не второй pipeline; одна реализация — две точки вызова direct/tool); класс-закрывающий корроборационный шлюз идентичности **D13/G0–G3** (alias-матч = только кандидат; `resolved` — через G1 person-intent + G2 persona-рекорд `db.get_persona_names` + G3 image-object cross-check); ограниченный визуальный срез (image-капы строже A6; психологический denylist); **5-частная сборка** `build_final_prompt` с §37-обёрткой «данные ≠ инструкции»; reply-заметка «арт ≠ достоверный портрет» + честный запрос фото; env-only kill-switch `IMAGE_CONTEXT_MEMORY_ENABLED` (default ON); Δ DDL=0 / Δ каталога=0; канон 12; pytest **9303/0**, JS **47/47**; risk **R3** (findings C4-N1/C4-N2/N2 — non-blocking watch-items агрегатного gate эпика). **▶️ Следующая — A8 `telegram-reactions`** (§41/§44-механика; канонический ID/название/разделы — строка **A8** таблицы выше). **Вход A7 (зафиксирован T-3643):** (1) **контракты памяти уже существуют** — `get_user_context` (A6, §87; purpose-enum 6, envelope 5 полей, канон 12) и `image_context_memory` (A4, §88; `build_image_memory_context`); (2) **хук сборки image-prompt** `build_final_prompt` (A3, §85) с 5-частной memory-сборкой (A4, §88.3) — A7 не переписывает; (3) **kill-switches** — `UNIFIED_IMAGE_REQUEST_ENABLED` (A3), `IMAGE_CONTEXT_MEMORY_ENABLED` (A4), `MEMORY_LOOKUP_ENABLED` (A6), `TOOL_CHAIN_LIMITS_ENABLED`/`ARTICLE_TOOL_ENABLED` (A2); (4) **архив A4** `plans/archive/image-context-memory-round1026/` + **§88** `plans/ARCHITECTURE.md`; (5) **known boundaries** — C4-N1 (G1-остаток: alias = word-person-маркер, после G2; reply/sender-плумбинг вне scope A4), C4-N2 (over-block false-negative), N2 (cap-заметка); унаследованные F1–F7 (A5, §86.8) + L-A6-01…-06 (A6, §87.11); owner-gate прод-проб A3 `PENDING OWNER VERIFICATION` (внешний). **Откат вклада A4:** hot = `IMAGE_CONTEXT_MEMORY_ENABLED=OFF` (режим A3, байт-в-байт); cold = `git revert` к анкеру `e8646af`; DDL-откат не требуется (Δ DDL=0). Порядок Эпика 3 — без изменений: **A0 ✅ → A1 ✅ → A2 ✅ → A3 ✅ → A5 ✅ → A6 ✅ → A4 ✅ → A7 ✅ → A8 → A9 → A10** (следующая — **A8 `telegram-reactions`**, §41/§44-механика). **Остаются волны A7 ✅ → A8 → A9 → A10 до агрегатного Reviewer release gate Эпика 3; watch-items агрегатного gate: C4-N1/C4-N2/N2 (A4) + F1–F7 (A5) + L-A6-01…-06 (A6) + owner-gate A3 PENDING OWNER VERIFICATION.**

**✅ A7 handoff (T-3666 @PM; архивация 25.09.2026):** A7 `decision-making-round1026` — Wave 5 Эпика 3 **завершён** (T-3647…T-3666 ✅; **T-3665 = `DEFERRED_TO_EPIC`** — агрегатный релиз Эпика 3, НЕ per-фича/НЕ «done/VERIFIED»; **T-3666 handoff → A8 закрыт**). **Статус A7: COMPLETED + MERGED (§89) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; `APP_VERSION` 2.58.30 без bump)** — единый Reviewer gate **Approved на 2-м цикле C0/H0 (R3 сохранён; Scanner отсутствует)**; binding Reviewed-Commit `e8646af` / Working-Tree-Hash `4b3df140…` / Spec-Hash `950c5f80…`; **ADR-1026-20 (D1–D12) Accepted**; архив — `plans/archive/decision-making-round1026/` (6 файлов: spec + ADR + tasks + evidence + threat-failure-analysis + review-T-3663; SHA-256 до/после переноса идентичны 6/6; spec `950c5f80…` байт-в-байт; `deployment.md` отсутствует — деплой отложен, это корректно). **Результат:** `action` {reply/react/silent/tool} **отдельно** от `style`; программная 2-фазная политика §42–§45 (детерминированная, **без случайности**) + порядок §46 (`silent`/`react` → Вербализатор не запускается; `reply` → стиль+Вербализатор; `tool` → инструменты → итоговое действие); контекст §47 из существующих `bot_replies`/`reply_to`/маркеров (без нового хранилища); ровно **3 настройки §48** в `mod_direct` (Δ каталога +3/+1, F8-переиздание по ADR-1026-2 — 473/430/448/102/100/21); R17-safe `reason_code` (15 кодов); **без 3-го LLM-вызова** (AMEND `physical-two-call-pipeline`; 2 tool / 0 silent); канон **12**; Δ DDL=0. **▶️ Следующая — A8 `telegram-reactions`** (§41/§44-механика; backend/Telegram API; **P0**; зависит A7), затем **A9 `agentic-events-graph`** (§49/§51; P1), затем **A10 `agentic-verification`** (§52–§54; P0) — до агрегатного Reviewer release gate Эпика 3. **Вход A8 (зафиксирован T-3666):** (1) **контракт решения A7 уже поставляет `action=react`** — поля `action`/`target_message_id`/`reaction`/`reason_code` (wire-контракт `CoordinatorDecision`, AMEND A1 §83 / ADR-1026-20 D1); A8 — **потребитель**, правила выбора реакции (§44) **не переписывает**; (2) **точка переиспользования `react_moai`** — `services/smartmodule_utils.py:93–107` (best-effort/fail-silently, фиксированная 🗿), уже используется A7; A8 добавляет полную **§41-механику** (`setMessageReaction`: проверка доступности реакции в чате, разрешённая альтернатива/отказ, постановка на **правильное** сообщение — не путать `reply_to_id`/`message_id`/ID прошлого сообщения бота; «не превращать ошибку реакции в длинный текст»), новый механизм не строит; A7 `setMessageReaction` напрямую **не вызывал**; (3) **kill-switches** (env-only, default ON): `DIRECT_DECISION_MAKING_ENABLED` (A7; OFF → A1-baseline: политика не строится, `action` ∈ {tool,reply}), `DIRECT_COORDINATOR_ENABLED` (A1), `TOOL_CHAIN_LIMITS_ENABLED`/`ARTICLE_TOOL_ENABLED` (A2), `UNIFIED_IMAGE_REQUEST_ENABLED` (A3), `IMAGE_CONTEXT_MEMORY_ENABLED` (A4), `IMAGE_DAILY_LIMIT_ENABLED` (A5), `MEMORY_LOOKUP_ENABLED` (A6); (4) **архив A7** `plans/archive/decision-making-round1026/` + **§89** `plans/ARCHITECTURE.md`; (5) **граница §44** — правила (когда ставить реакцию) = A7, механика = A8 (§89.6/ADR D5); §49/§51 (события/ExecutionGraph) = A9; §104 `generate_image` — no-go. **Откат вклада A7:** hot = `DIRECT_DECISION_MAKING_ENABLED=OFF` (A1-baseline) и/или 3 тумблера §48 OFF; cold = `git revert` к анкеру `e8646af` + откат F8-артефактов; DDL-откат не требуется (Δ DDL=0). **Release policy EPIC_ONLY:** агрегатный bump 2.58.30 → **2.58.31** — на границе эпика; @DevOps не вызывался. **Watch-items агрегатного gate Эпика 3 (non-blocking):** F-9 (A1 boundary test), грубость эвристики `expects_tool_result`, устаревший code-указатель в `threat-failure-analysis.md`, алиас `bot_replied_recently`, `REASON_DISABLED` на объекте (не в OFF-строке лога) + унаследованные C4-N1/C4-N2/N2 (A4), F1–F7 (A5), L-A6-01…-06 (A6), owner-gate A3 `PENDING OWNER VERIFICATION` (внешний).

**✅ A8 handoff (T-3686 @PM; архивация 25.09.2026):** A8 `telegram-reactions-round1026` — Wave 5 (продолжение) Эпика 3 **завершён** (T-3667…T-3686 ✅; **T-3685 = `DEFERRED_TO_EPIC`** — агрегатный релиз Эпика 3, НЕ per-фича/НЕ «done/VERIFIED»; **T-3686 handoff → A9 закрыт**). **Статус A8: COMPLETED + MERGED (§90) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; `APP_VERSION` 2.58.30 без bump)** — единый Reviewer gate **Approved на 1-м цикле C0/H0 (R2; Scanner отсутствует)**; binding Reviewed-Commit `e8646af` / Working-Tree-Hash `ec57a951…` / Spec-Hash `6e5cfc80…`; **ADR-1026-21 (D1–D14) Accepted**; архив — `plans/archive/telegram-reactions-round1026/` (**5 файлов**: spec + adr-1026-21-reaction-mechanics + tasks + evidence + review-T-3683; SHA-256 до/после переноса идентичны, spec `6e5cfc80…` байт-в-байт; **`threat-failure-analysis.md` отсутствует — `NOT_APPLICABLE` при R2/D12**; `deployment.md` отсутствует — деплой отложен, это корректно). **Результат:** механика исполнения реакции §41 в **единственном примитиве** `react_moai` (`services/smartmodule_utils.py`) — официальный `setMessageReaction` (один call-site): реактивная доступность + детерминированная альтернатива/тихий отказ (**≤2 попытки**, без рандома), **закрытый enum 7 исходов** (`REACTION_OK/UNAVAILABLE/FORBIDDEN/MESSAGE_GONE/SERVICE/RATE_LIMITED/UNKNOWN`), no-text-on-error (ни один исход не вызывает `send_message`), корректная цель = trigger `target_message_id` (media-group — серверное «первое неудалённое»), 429 без повтора; контекстная карта **`reason_code→эмодзи`** ({🗿,😂,👍,🔥}; 🗿 — дефолт/fallback; `_reaction_for_reason` в `direct_chat_service.py`); проброс `reaction`/`reason_code` из решения A7 (`:1313–1315`); env-only киль-свитч **`REACTION_MECHANICS_ENABLED`** (default ON; OFF → legacy 🗿); канон **12**; **Δ DDL=0 / Δ каталога=0** (473/430/448/102/100/21; F8 не переиздаётся); pytest **9459/0** (+67), JS **47/47**. **▶️ Следующая — A9 `agentic-events-graph`** (§49, §51; **P1**; канонический ID/название/разделы — строка **A9** таблицы выше), затем **A10 `agentic-verification`** (§52–§54; P0) — до агрегатного Reviewer release gate Эпика 3. **Вход A9 (зафиксирован T-3686):** (1) **A9 добавляет события/ExecutionGraph, НЕ переписывая механику A8** — граница «A8 = механика реакции / A9 = события»; A8 поставляет исполнение реакции (`setMessageReaction`-механика, реактивную доступность/детерминированный fallback, корректную цель) и доставляемый `reaction` из решения A7; A9 добавляет `REACTION_SENT`/`MESSAGE_IGNORED` (+`DECISION_*`/`TOOL_*`/`IMAGE_*`) и узлы существующего ExecutionGraph (§49/§51) + Mini App §50; вторую аналитику не создавать (REUSE F6/S8, `plans/ARCHITECTURE.md` §65/§79). (2) **Контракт исполнителя A8 (потребить, не дублировать):** примитив `react_moai(bot, chat_id, message_id, *, reaction=None, reason_code=None)`; закрытый enum **7 исходов**; детерминированный fallback (≤2 попытки, только `unavailable` retryable, без рандома); карта `reason_code→эмодзи` ({🗿,😂,👍,🔥}); киль-свитч `REACTION_MECHANICS_ENABLED` (env-only, default ON; OFF → legacy 🗿); один call-site `set_message_reaction`. (3) **L-1 follow-up — ЯВНЫЙ ВХОД A9 (Low, OPEN из review T-3683):** `TelegramForbiddenError` **не** является подклассом `TelegramBadRequest`, поэтому реальный HTTP 403 классифицируется как `unknown` вместо `forbidden`; поведение безопасности идентично (тихий отказ, без fallback/текста/повтора). **Требуемый фикс:** добавить `except TelegramForbiddenError: return REACTION_FORBIDDEN` (с тем же R17-safe `_warn_reaction_failed`) перед generic-handler; unit-тест на `TelegramForbiddenError` → `forbidden`, 1 вызов, 0 текста. (4) **Архив A8** `plans/archive/telegram-reactions-round1026/` + **§90** `plans/ARCHITECTURE.md`; (5) **Watch-items агрегатного gate Эпика 3 (non-blocking):** L-1 (п. 3), A7-без-byte-снапшота (A7-код был незакоммичен — структурная инспекция + line-baseline ADR + зелёные A7-тесты) + унаследованные A7 F-9/R-1/R-2/R-3/`REASON_DISABLED` (§89.11), A4 C4-N1/C4-N2/N2 (§88.7), A5 F1–F7 (§86.8), A6 L-A6-01…-06 (§87.11), owner-gate A3 `PENDING OWNER VERIFICATION` (внешний); D-3 (feature-папка untracked — учтена в архиве); ADR hash re-pins (binding T-3683 не инвалидируют). **Откат вклада A8:** hot = `REACTION_MECHANICS_ENABLED=false` (legacy 🗿) и/или OFF A7-тумблеров REACTIONS/IMAGE_REACTIONS; cold = `git revert` к анкеру `e8646af` + агрегатный анкер Эпика 3; DDL-откат не требуется (Δ DDL=0). **Release policy EPIC_ONLY:** агрегатный bump 2.58.30 → **2.58.31** — на границе эпика; @DevOps не вызывался.

**✅ A9 handoff (T-3705 @PM; архивация 25.09.2026):** A9 `agentic-events-graph-round1026` — Wave 5 (продолжение) Эпика 3 **завершён** (T-3687…T-3705 ✅; **T-3704 = `DEFERRED_TO_EPIC`** — агрегатный релиз Эпика 3, НЕ per-фича/НЕ «done/VERIFIED»; **T-3705 handoff → A10 закрыт**). **Статус A9: COMPLETED + MERGED (§91) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment DEFERRED_TO_EPIC; `APP_VERSION` 2.58.30 без bump)** — единый Reviewer gate **Approved на 1-м цикле C0/H0 (R2; Scanner отсутствует)**; binding Reviewed-Commit `e8646af` / Working-Tree-Hash `84d6df00…` / Spec-Hash `3f703584…`; **ADR-1026-22 (D1–D14) Accepted**; архив — `plans/archive/agentic-events-graph-round1026/` (**5 файлов**: spec + adr-1026-22 + tasks + evidence + review-T-3702; SHA-256 до/после переноса идентичны, spec `3f703584…` байт-в-байт; **`threat-failure-analysis.md` отсутствует — `NOT_APPLICABLE` при R2/D1 (in-memory, Δ DDL=0)**; `deployment.md` отсутствует — деплой отложен, это корректно). **Результат:** эмиссия диагностических событий §49 (закрытый enum: 12 событий + 8 `ANTI_CLICHE_*` = 20) через fail-open `emit_agentic_event` (`services/agentic_events.py`) — R17-whitelist payload (id/enum/числа/имена инструментов/`reason_code`; без сырого досье/промптов/текстов), reuse `RunSnapshotStore` in-memory (Δ DDL=0); 9 реальных этапов ExecutionGraph §51 (Decision / Memory Lookup / RAG / Web Extraction / Factcheck / Image Prompt Preparation / Image Generation / Reaction / Text Generation) через существующий adapter, без второй аналитики; honest-`None`≠`$0` (no-fake-tokens) + `silent` → узел Decision без фиктивного Вербализатора; `MESSAGE_IGNORED` из silent-short-circuit A7; reuse `run_id=correlation_id`; аддитивное расширение **существующего** `GET /analytics/execution/latest` (нового endpoint нет; `analytics.py` diff пуст); display-only срез Mini App §50 в существующей вкладке «Аналитика» (без отдельной панели/дублирования параметров); env-only киль-свитч **`AGENTIC_EVENTS_ENABLED`** (default ON; OFF → 0 событий/узлов, паритет); **L-1 follow-up ✅ FIXED** (`TelegramForbiddenError` → `REACTION_FORBIDDEN`); канон **12**; **Δ DDL=0 / Δ каталога=0** (473/430/448/102/100/21; F8 не переиздаётся); pytest **9513/0** (+54), JS **47/47**. **A10 `agentic-verification` ✅ (T-3726…T-3729 @PM, 25.09.2026; см. блок «A10 handoff / aggregate-gate inputs» ниже)** (§52–§54; P0; зависит A0–A9; canonical `backlog:312`) — **ПОСЛЕДНЯЯ фича Эпика 3**, после неё — агрегатный Reviewer release gate (все фичи approved → агрегатное ревью релиза → доставка + @DevOps bump 2.58.30 → 2.58.31). **Вход A10 (зафиксирован T-3705):** (1) **контракт событий/эмиссии** — `services/agentic_events.py` (`emit_agentic_event`; fail-open; closed enum 20 событий; R17-whitelist); (2) **ExecutionGraph** — `services/execution_graph_source.py` (`RunSnapshotStore` in-memory; 9 этапов; honest-`None`; `tool`-kind) + `web/static/execution_graph.js`/`web/app.js`/`web/index.html` (display-only срез §50); (3) **киль-свитч** `AGENTIC_EVENTS_ENABLED` (env-only, default ON); (4) **L-1** закрыт (`services/smartmodule_utils.py` forbidden-классификация); (5) **имя тест-файла:** фактическое `tests/test_agentic_events_round1026.py` (в spec §9/T-3698 указано `test_agentic_events_graph_round1026.py` — расхождение имени, не состава; учесть в A10-доке); (6) **архив** `plans/archive/agentic-events-graph-round1026/` + **§91** `plans/ARCHITECTURE.md`; (7) **агрегатная приёмка Эпика 3** — A10 обязан покрыть §52 (**23 сценария**), §53 (критерии), §54 (результаты, 12 пунктов) по всем фичам A0–A9, поэтому **watch-items являются входом A10** (см. ниже). **Watch-items агрегатного gate (non-blocking, вход A10):** L-A9-3702-01 (evidence-stress-claim: заявлена конкурентность/bounded/`message_id=None` без явных тестов), L-A9-3702-02 (RAG/Factcheck-паттерны → реально достижимы **7 из 9** этапов), L-A9-3702-03 (top-level `started_at=null` у агентного прогона), L-A9-3702-04 (`_event` F0.3 через логгер `services.agentic_events` без префикса `[anticliche]`; молчит при OFF — внешний парсинг), тест-имя-дискрепансия (п. 5), BetterStack-флейк `tests/test_betterstack_handler.py` (среда), унаследованные D-3 staging / ADR hash re-pins (1026-20/21/22) / T-3667/T-3668 hygiene (архивный A8) / F-9/R-set (A7, §89.11) / C4-set (A4, §88.7) / F1–F7 (A5, §86.8) / L-A6-set (A6, §87.11) / owner-gate A3 `PENDING OWNER VERIFICATION`. **Откат вклада A9:** hot = `AGENTIC_EVENTS_ENABLED=false`; cold = `git revert` к анкеру `e8646af`; DDL-откат не требуется (Δ DDL=0). **Release policy EPIC_ONLY:** агрегатный bump 2.58.30 → **2.58.31** — на границе эпика; @DevOps не вызывался.

**✅ A10 handoff / aggregate-gate inputs (T-3726…T-3729 @PM; архивация 25.09.2026):** A10 `agentic-verification-round1026` — финальный read-only верификационный гейт Эпика 3 **завершён** (T-3706…T-3729 ✅; **T-3728 = `epic deployment not applicable`** — вердикт для **самой** A10, пер-фича деплоя/bump нет; **граница эпика: деплой остаётся требуемым и отложен**; **T-3729 handoff → агрегатный Reviewer release gate закрыт**). **Статус A10: COMPLETED + MERGED (§92) + ARCHIVED + ACCEPTED INTO PENDING EPIC 3 RELEASE (deployment `epic deployment not applicable`; `APP_VERSION` 2.58.30 без bump)** — единый Reviewer gate **Approved (R1; binding Reviewed-Commit `e8646af` / Working-Tree-Hash `2264399e…` / Spec-Hash `a50b7cb0…`)**; **ADR-1026-23 (D1–D10) Accepted** (ADR-Hash re-pin `37d8cef7…`; review-bound был `2fc5a010…`); архив — `plans/archive/agentic-verification-round1026/` (**5 файлов**: spec + adr-1026-23 + tasks + evidence + review-T-3726; SHA-256 до/после переноса идентичны; spec `a50b7cb0…` байт-в-байт; **`threat-failure-analysis.md` отсутствует — `NOT_APPLICABLE` при R1/D8**; `deployment.md` отсутствует — `deployment not applicable`, это корректно). **Приёмочный отчёт (D3):** `plans/reports/round1026_a10_acceptance.md` (**НЕ в архиве** — остаётся в `plans/reports/`). **Результат:** §52 **23/23 PASS multi-source** (+6 PENDING OWNER VERIFICATION — не закрыты), §53 **15/15 YES**, §54 **12/12 связаны**; watch-register CLOSE/PASS-THROUGH/PENDING без «тихого» закрытия; defect-rule D5 (4 residual зарегистрированы, не фиксятся); epic-wide pytest **9513/0**, JS **47/47**, F8 **473**, каталог **473/430/448/102/100/21**, канон **12**, Δ DDL=0 (SQLite v12). **▶️ QUEUE (Эпик 3): A0–A10 — ВСЕ DONE/APPROVED** → **СЛЕДУЮЩИЙ ШАГ — АГРЕГАТНЫЙ REVIEWER RELEASE GATE ЭПИКА 3** (агрегатное ревью релиза, **не** фича). **Вход агрегатного gate (ADR-1026-23 D7/D10):** (1) приёмочный отчёт A10; (2) watch-register; (3) чек-листы §52/§53/§54; покрыть полный эпик-diff **A2–A9+A10**, кросс-фичевые взаимодействия, интеграцию/регресс, **миграцию A5 `image_reservation`**, конфиг/наблюдаемость, совместимость, **R17/R18**, готовность отката (анкер `e8646af` + per-feature kill-switches), согласованность со всеми принятыми feature-spec; **binding** = релиз-коммит + детерминированный working-tree-hash + агрегатный spec-manifest-hash **incl ADR re-pins**. **После approval → доставка + @DevOps bump `2.58.30 → 2.58.31` + стейджинг D-3 (untracked архив-папки).** **Watch-регистр (для агрегатного gate):** Lows A10 **1–4** (Low-1 tasks-hash ✅ закрыт в архиве — см. `tasks.md` SHA-блок; Low-2 §54-9 pointer; Low-3 A7 F-4 в группе 6; Low-4 §2 строка 4 без test-node); **D-3 staging**; **ADR hash re-pins** (1026-20 `deb6257d…`, 1026-21, 1026-22 `3085143B…`, 1026-23 `37d8cef7…`); **T-3667/T-3668** hygiene; **F-9/R-set (A7)**; **C4-set (A4)**; **F1–F7 (A5)**; **L-A6-set (A6)**; **A9 L-01…-04 + дискрепансия имени тест-файла**; **owner-gate A3 (6 PENDING** сценариев 1/2/4/10/18/22**)**; **BetterStack-флейк** (среда, non-blocker); **doc-maintenance** (`backlog:29` §52-счёт 22→23 — **✅ исправлено @PM**; `:314` ведущая сводка/маркеры — **✅ нормализованы**, исторические dated-актуализации сохранены). **Superset Working-Tree-Hash всех 61 untracked (для агрегатного gate): `96c291c1d35da8e400ce69df1268742a8b7bccc3ed4410524de9b28a64bd37fe`.** Owner-гейты (6) — **PENDING OWNER VERIFICATION**, non-blocking, **никогда не закрыты**.
