...
| **F2** 🟦 | `design-tokens-liquidglass-v2-round1025` — **🟦 IN PROGRESS (Step 1 @PM, 21.09.2026): декомпозиция T-2529…T-2562 (34) — `plans/features/design-tokens-liquidglass-v2-round1025/tasks.md`; Step 2 @Architect — `spec.md` + ADR-1025-9 (T-2530)** — палитра §8 (SUPERSEDE OD4 `#161616`/`#14CBB6` + glass 10.20 + `--grad-d:#FF8A3D`/`--grad-speed:6s`; AMEND ADR-1020-9), Liquid Glass A/B/C §9 (`feDisplacementMap`, deny-list textarea/таблиц/логов/форм/редакторов промптов), фон 60–90 с (вторичные 90–120 с) §10 (без оранжевого, transform/opacity, пауза `document.hidden`), контраст (`.75rem` на `#090D17`)/reduced-motion, атомарный inventory-тест маркеров, Playwright §71 (10 вьюпортов)+скриншоты, `APP_VERSION` 2.58.5 | §8–§10, §70, §71, §117 п.7–8 | визуальная система (web/CSS) | **P0** | F1 | **T-2529…T-2562 (34)** |
...
2. **Волна 0.5 (каркас):** **F1 ✅ ARCHIVED + DEPLOYED** (`fe0f7bb`; IA/shell; потребитель инвентаря F8 и контракта сохранения F0) + **P0-фикс после F1 ✅ ARCHIVED + DEPLOYED** (`fea2daa`, hotfix2: render/media/avatars/APP_VERSION/matrix). **Дальше — F2 🟦 IN PROGRESS (T-2529…T-2562) ∥ F3.**
3. **Волна 1 (🟦 IN PROGRESS):** **F2** (токены/glass/фон; **T-2529…T-2562**, `plans/features/design-tokens-liquidglass-v2-round1025/tasks.md`) ∥ **F3** (селектор области) — после F1.
