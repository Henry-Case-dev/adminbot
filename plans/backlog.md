...
| **S7** 🟦 IN PROGRESS | `summary-logging-runid` — `run_id` + события §108, детали логов §109, фильтр в log viewer §110 (без ключей) | §108–§110 | backend/лог+UI | **P0** | **S5 ✅** (ядро `run_id`/событий FILTER/L1/L2/FORMAT/обложки независимо от S6; события `PUBLISH_RICH_*`/`PUBLISH_TEXT_*` §108 — **GATED вместе с S6/D4**, нарезка — Step 2 @Architect) | **🟦 IN PROGRESS — Step 1 @PM ✅ (24.09.2026): `plans/features/summary-logging-runid-round1026/tasks.md` (блоки 0/A–H, **T-3380…T-3409, 30**); baseline ожидаемо `59e5b12`/2.58.25, pytest 8854/0, JS 44/44, каталог 469/426/444/100/98/21, Δ DDL=0; deploy/bump 2.58.25→2.58.26 или N/A — Step 2 @Architect; события `PUBLISH_*` — GATED. ▶️ следующий — Step 2 @Architect (`spec.md` + ADR-1026-9) → сверка @PM → baseline @DevOps (T-3380) → @Builder B–F → G → H. S8 зависит от S7; S6 остаётся ⛔ BLOCKED (GATED D4).** |
