...
### 🌗 Хотфикс-6 (пакет Волны 1.5) — `hotfix6-webview-shell-heartbeat-round1025` (внеплановый, между F3 и F4) — ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 22.09.2026); код НЕ закоммичен → ⏳ deploy Шаг 9 @DevOps; ⏳ live-гейт владельца T-2617 открыт
...
### ✅ Хотфикс-7 (Волна 1.6, ПРИОРИТЕТНЫЙ по UPD) — `hotfix7-shell-glass-heartbeat-round1025` (внеплановый, выше F4) — ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 22.09.2026); Merge §59; deploy ⏳ Шаг 9 @DevOps; live T-2682 ⏳ владелец
...
| **F4** | `module-catalog-quickpanel-store-round1025` — **⏸ PAUSED (UPD «Срочный фикс текущего фронта» вышел приоритетнее; Step 1 @PM done, 22.09.2026):** `tasks.md` детализирован (блоки 0, A–H, 9; **T-2619…T-2657, 39**; дублей ID нет — максимум до hotfix6 T-2618); открытые вопросы (a)–(e)+(f) — @Architect; `spec.md`/ADR-1025-13 — Step 2 @Architect. Папка — `plans/features/module-catalog-quickpanel-store-round1025/` (`tasks.md`). **Возврат — после deploy HOTFIX7** (`hotfix7-shell-glass-heartbeat-round1025` — ✅ COMPLETED+MERGED+ARCHIVED, ⏳ deploy Шаг 9 @DevOps). **Суть:** каталог §31–§45 + панель избранного + канонический `ModuleConfigurationStore` §37–§42 (одна мутация → все представления, `scope_type+scope_id+module_id`, scoped keys, **откат при ошибке §41**), reuse F3/F0 (scopeEpoch/`persistItems`); Δ DDL=0, Δ каталога=0. | §31–§45, §72, §73, §79, §116 | UI+state (web) | **P0** | F2, F3 | T-2619…T-2657 (39) |
| **F5** | `module-workspace-tabs` — рабочее пространство модуля §46 + гибридная библиотека промптов §48 (один объект, два маршрута) + модели/подключения §49 + UI прямого чата §84 / Саммари §85 | §46–§49, §84, §85 | UI+IA (web) | **P0** | F4 | детализация при старте |
| **F6** | `memory-analytics-reorg` *(в ТЗ @Memory — `memory-access-reorg`)* — раздел «Память» §52–§59 + «Сводка»→«Аналитика» §21 + карта вызовов §22–§30 (adapter ExecutionGraph, 2 режима, mobile) | §4, §21–§30, §52–§59, §75, §76 | UI+adapter (web+api) | **P0** | F1, F4 | детализация при старте |
| **F7** | `permsoc-local-space` — локальное пространство §60–§67, мастер-тумблеры §61 (иерархия, сохранение дочерних, **серверная поддержка отключения**), блоки Славик/Костик/Оля/Мимикрия/Общие реакции/Расписания | §60–§67 | UI+backend | **P0** | F3, F4 | детализация при старте |
| **F8** | `parameter-registry-widget-map` — обязательный аудит §2 + сохранность конфигурации §3 + результаты §117(1–4): реестр параметров, карта «старый экран→параметр→новый экран→API», карта виджетов, бэкап, diff до/после | §1–§3, §117 | audit+data-safety (enabler) | **P0** | — (Wave 0) | детализация при старте |
| **F9** | `secrets-and-save-states` — **⚠️ UPD: сужена** — только **UI-слой секретов §50** (`{configured,last4}`, «Ключ установлен», маска ≠ значение input, отдельные замена/удаление) + **визуальная часть SaveBar §69/§78** (safe area, клавиатура, последнее поле); **persistence/409/412/state-machine/безопасный merge/версия — перенесены в F0 (Wave 0)**, здесь не дублируются | §50, §51, §69, §78 | UI+API | **P0** | **F0**, F4, F5 | детализация при старте |
...
4. **Волна 1.5 (✅ COMPLETED + MERGED + ARCHIVED, Шаг 8 @PM, 22.09.2026; Merge §58):** **`hotfix6-webview-shell-heartbeat-round1025`** — **A** Liquid Glass (преломление без `backdrop-filter: url()` + стекло панелей), **B** нижняя панель mobile (`contentSafeAreaInset.bottom`), **C** сердцебиение §15 (overflow + Canvas 2D/rAF; **§15 вынесено из F11**), **D** шапка/⛶/отдельная строка/резерв высоты (T-2581…T-2618). Архив — `plans/archive/hotfix6-webview-shell-heartbeat-round1025/` (`spec.md` + ADR-1025-12 + `tasks.md`). **⏳ Код НЕ закоммичен → deploy — Шаг 9 @DevOps; ⏳ live-гейт владельца T-2617 открыт.**
5. **Волна 1.6 (✅ COMPLETED + MERGED + ARCHIVED, Шаг 8 @PM, 22.09.2026; Merge §59; ВНЕПЛАНОВЫЙ ПРИОРИТЕТНЫЙ UPD, выше F4):** **HOTFIX7** `hotfix7-shell-glass-heartbeat-round1025` — layout+fullscreen, heartbeat premium, glass shell/liquid glass, shell-выравнивание, приёмка UPD 6, аудит ТЗ UPD 7 (T-2658…T-2694, 37). Архив — `plans/archive/hotfix7-shell-glass-heartbeat-round1025/` (`spec.md` + ADR-1025-13 + `tasks.md` + `evidence.md` + `review.md`); @Reviewer Approved, @Scanner **C0/H0/M0**. **⏳ deploy — Шаг 9 @DevOps (T-2690); ⏳ live-гейт владельца T-2682.** Вставлен между hotfix6 и F4 по UPD «Срочный фикс текущего фронта» (`plans/current_task.md` 6073–6154). **F4 → ⏸ PAUSED → возобновить после deploy.**
6. **Волна 2:** **F4** `module-catalog-quickpanel-store-round1025` (каталог+store) — после hotfix6 и **HOTFIX7**. **⏸ PAUSED — Step 1 @PM ✅ (22.09.2026):** `plans/features/module-catalog-quickpanel-store-round1025/tasks.md` (T-2619…T-2657, 39); далее **Step 2 @Architect** (`spec.md` + новый ADR — ⚠️ **ADR-1025-13 занят HOTFIX7**, брать следующий свободный **ADR-1025-14**). **Возврат — после deploy HOTFIX7 (Шаг 9 @DevOps); отдельного решения владельца не требует.**
7. **Волна 3:** **F5** (workspace+промпты) → **F6** (Память+Аналитика) → **F7** (PERMsoc) ∥ **F11** (Статус-витрина, после F2+F6; **без §15** — сердцебиение в hotfix6).
8. **Волна 4:** **F9** (**только секреты §50 + визуальный SaveBar**; серверный persistence/409/state-machine уже закрыты в F0) — после F4/F5.
9. **Волна 5:** **F10** (приёмка §71–§79) — после всех; затем **СТОП-ГЕЙТ Эпика 1** (приёмка владельцем) → старт Эпика 2.

> **Порядок фич (единый, по UPD):** **F0 → F1 → F2 → F3 → hotfix6 (Волна 1.5) → HOTFIX7 (Волна 1.6, приоритетный UPD) → F4 → F5 → F6 → F7 → F11 → F9 → F10**. Изменение относительно исходной декомпозиции: F0 добавлена в начало (Wave 0), F11 передвинута **перед** F9 (обе после F4/F5/F6/F7), F9 сужена; **hotfix6** вставлен между F3 и F4 по итогам живой приёмки владельца (блоки A–D), **§15 вынесено из F11 в hotfix6**; **HOTFIX7** вставлен между hotfix6 и F4 по новому приоритетному UPD «Срочный фикс текущего фронта» (§6073–6154), **F4 — ⏸ PAUSED**.
>
> **▶️ Текущий шаг (Шаг 8 @PM, 22.09.2026):** **`hotfix7 ✅ COMPLETED + MERGED + ARCHIVED` → `(deploy — Шаг 9 @DevOps, T-2690)` → `возобновить F4`** `module-catalog-quickpanel-store-round1025` (Step 2 @Architect `spec.md` + ADR-1025-14 → build → ревью → аудит) → далее по порядку. Живые гейты владельца (в т.ч. **T-2682**) закрываются параллельно и не блокируют старт F4 (UPD 7 п.7 «без human gate»).
