...
| **S4** ✅ | `summary-fact-package` — пакет фактов §96 (название/описание/хронология/факты/подтверждающие ID/фрагменты; без повторного сырого лога; не генерировать «доказательства» самостоятельно) — **✅ DONE / VERIFIED (автономный контур: @Reviewer Approved C0/H0; @Scanner C0/H0/M1-процесс(закрыт `review.md`)/L2/I2) + ARCHIVED (Шаг 8 @PM, T-3304, 23.09.2026; архив — `plans/archive/summary-fact-package-round1026/`).** `spec.md` (SC-01…SC-10) + **ADR-1026-6 Accepted (D1–D6)**; `tasks.md` — блоки 0/A–H, **T-3283…T-3307 (25)**; модуль `services/summary_fact_package.py` + 52 теста, интерфейс `build_fact_package(l1_result, payload_items, *, budget=None, correlation_id=None) -> FactPackageResult`; сборка из `L1Result` (fail-closed §95/§106), 0 LLM-вызовов, `description`/`chronology` — детерминированно (не LLM), `service{response_mode,cover_prompt}` транзитом, врезка — **GATED (не раньше S5/S6)**, pytest **8737/0** (+52), JS 43/43, Δ DDL=0, **Δ каталога=0** (468/426/443; F8 не переиздавался — N/A), `APP_VERSION` bump **2.58.23**; **deploy — ⏳ после T-3305 @DevOps**; live — ⏳ PENDING OWNER VERIFICATION. Follow-up L-R1026S4-1/-2/-3 + I-R1026S4-1/-2 → S5. | §96, §93–§95, §106 | backend/сборка | **P0** | S3 ✅ | **T-3283…T-3307 — `plans/archive/summary-fact-package-round1026/tasks.md`** |
| **S5** ▶️ NEXT | `summary-l2-writer-formatter` — L2 Писатель §97–§99 (структура статьи §98, структурированный вывод §99, серверный форматтер → Rich Message; H1 §101, абзацы/акценты §102, экранирование) — **▶️ NEXT** (зависимость **S4 ✅ DONE**; сюда же — **врезка L1→пакет фактов→L2**, слот/UI модели L2 §82 и закрытие follow-up S3 `L-R1026S3-*`/`R-R1026S3-*` + S4 `L-R1026S4-1/-2/-3`/`I-R1026S4-1/-2`); гейт S6/S10+D4 и live — без изменений | §97–§99, §101, §102 | backend/LLM+формат | **P0** | S4 ✅ | Step 1 @PM при старте |
...
**Порядок Эпика 2:** `S1 → S2 → S3 → S4 → S5 → [S6 ∥ S9 (после S5)] → S7 → S8 → S10`. **▶️ S1 ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §71) → S2 `summary-context-restore-round1026` ✅ COMPLETED + MERGED (§73) + ARCHIVED (Шаг 8 @PM, T-3249, 23.09.2026; `tasks.md` T-3224…T-3252, 29; deploy — ✅ задеплоено 2.58.21 (Шаг 9 @DevOps); live — `[ ]` PENDING OWNER VERIFICATION) → ✅ **S3 `summary-l1-clusterizer` — COMPLETED (автономный контур) + ARCHIVED (Шаг 8 @PM, T-3279, 23.09.2026; архив — `plans/archive/summary-l1-clusterizer-round1026/`; deploy — после T-3280; live — PENDING OWNER VERIFICATION) → ✅ **S4 `summary-fact-package` — DONE / VERIFIED (автономный контур) + ARCHIVED (Шаг 8 @PM, T-3304, 23.09.2026; архив — `plans/archive/summary-fact-package-round1026/`; deploy — ⏳ после T-3305; live — ⏳ PENDING OWNER VERIFICATION) → ▶️ **S5 `summary-l2-writer-formatter` — NEXT** (зависимость S4 ✅; врезка L1→пакет→L2 и слот/UI L2 — S5; гейт S6/S10+D4 и live — без изменений).**
