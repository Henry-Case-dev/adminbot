...
| **S7** 🟦 IN PROGRESS | `summary-logging-runid` — `run_id` + события §108, детали логов §109, фильтр в log viewer §110 (без ключей) | §108–§110 | backend/лог+UI | **P0** | **S5 ✅** (ядро `run_id`/событий FILTER/L1/L2/FORMAT/обложки независимо от S6; события `PUBLISH_RICH_*`/`PUBLISH_TEXT_*` §108 — **GATED вместе с S6/D4**, нарезка — Step 2 @Architect) | **🟦 IN PROGRESS — Step 1 @PM ✅ + Step 2 @Architect ✅ + сверка @PM ✅ (T-3382, 24.09.2026): `plans/features/summary-logging-runid-round1026/{tasks.md,spec.md,adr-1026-9-summary-runid-logging-logviewer.md}` (блоки 0/A–H, **T-3380…T-3409, 30**; REQ-S7-01…-13 → SC-01…SC-16 → ADR D1–D8). Решения ADR: `run_id` = существующий `correlation_id` (UUID4 hex, одна точка на прогон); события — reuse `FILTER_*`/`RESTORE_*`/`L1_*`/`L2_*`/`TEST_*` + новые `SUMMARY_*`/`FORMAT_*`/`COVER_*`, **`PUBLISH_*` — GATED (не реализуются)**; **Δ DDL=0** (ring-buffer+`llm_usage_events`), **Δ каталога=0 / F8 НЕ переиздаётся (N/A)**; §110 — клиентский фильтр-чип в `web/app.js` без diff `routes.py`; fail-closed `*_ERROR`+`SUMMARY_FAILED`; **deploy = ДА, bump 2.58.25 → 2.58.26** (hot-OFF не нужен). baseline ожидаемо `59e5b12`/2.58.25, pytest 8854/0, JS 44/44, каталог 469/426/444/100/98/21. **T-3380 ✅ (annotated-тег `pre-round1026-s7` → `f774ecc`); @Builder B–F ✅ + rework ×2 ✅ (T-3405); T-3403 ✅ Approved (итер.3, C0/H0/M0, единый Reviewer gate); 24.09.2026 — миграция управляющих документов: Scanner удалён, T-3404 → вторая линза Reviewer (focused change audit), binding пересчитывается.** ▶️ следующий — T-3404 (линза 2 + пересчёт binding) → T-3406 merge → T-3407 archive → T-3408 deploy (2.58.26) → T-3409 handoff. S8 зависит от S7; S6 остаётся ⛔ BLOCKED (GATED D4). Владельцу решать нечего; live — PENDING OWNER VERIFICATION.** |
| **S8** | `summary-analytics-adapter` — §111 adapter ExecutionGraph + узлы Filter/Clusterizer/Writer/Formatting/Publication (kind: algorithm/llm/format/publish; **GraphViewer не переписывать**), метрики §112 | §30, §111, §112 | web+api adapter | **P0** | S7, S6, F6 | Step 1 при старте Эпика 2 (после S7) |
...
**Примечание (Шаг 8 @PM, T-3377, 24.09.2026) — S9 ✅ DONE/VERIFIED + ARCHIVED, S7 🟦 IN PROGRESS (Step 1 @PM ✅ + Step 2 @Architect ✅ + сверка @PM ✅; build B–F + rework ×2 ✅; T-3403 ✅ Approved — единый Reviewer gate; 24.09.2026 — миграция Scanner→Reviewer, следующий — T-3404; `tasks.md` T-3380…T-3409, 30):** S9 закрыта автономным контуром (итерация 2: rework T-3375 закрыл **B-R1026S9-2** High «краш вкладки на error-путях» и **B-R1026S9-1** Medium «§112 Процент отсева»; @Reviewer Approved C0/H0 + @Scanner C0/H0/блокирующих 0/L5/I2), архив — `plans/archive/summary-testing-ui-round1026/`; deploy **VERIFIED 2.58.25** (`ac3f8fc`/`59e5b12`); live — PENDING OWNER VERIFICATION. **Проверка зависимостей S7** (`summary-logging-runid`, §108–§110): ядро `run_id`/события FILTER/L1/L2/FORMAT/обложки **не требует S6** → может стартовать (после S9); события `PUBLISH_RICH_*`/`PUBLISH_TEXT_*` §108 относятся к публикации → **GATED вместе с S6 (D4)**, окончательная нарезка — Step 2 @Architect. **S8 зависит от S7** (и S6/F6). **S6 остаётся ⛔ BLOCKED (GATED D4).**
