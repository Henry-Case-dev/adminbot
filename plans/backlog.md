...
### 🗺️ ЭПИК 2 (Раунд 10.26) — «Summary Hybrid Pipeline» — 10 фич (S1–S10; **✅ авто-приёмка Эпика 1 (F10 §70) закрыта 23.09.2026; live-приёмка владельца — [ ] PENDING OWNER VERIFICATION; старт независимых задач — по ADR-1025-24 D4; смена существующего публикационного пайплайна — только после live-приёмки Эпика 1**; **S1 `summary-filter-round1026` ✅ COMPLETED + MERGED + ARCHIVED (§71, 23.09.2026) → следующая S2 `summary-context-restore`**)
...
**Порядок Эпика 2:** `S1 → S2 → S3 → S4 → S5 → [S6 ∥ S9 (после S5)] → S7 → S8 → S10`. **▶️ S1 ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §71) → следующая — S2 `summary-context-restore` (§90–§92; блок 0 S2 — @DevOps тег, Step 2 @Architect).** Канон-цепочка **S3 → S4 → S5 → S6** атомарна (ADR-1013-3). **Δ DDL / новые параметры `summary.filter.*` и слоты моделей** — санкционировать в `spec.md` (параллельное хранилище §87 запрещено). **СТОП-ГЕЙТ:** старт Эпика 2 — только после приёмки Эпика 1 (§0/§79).
