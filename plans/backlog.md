...
| **S6** ⛔ BLOCKED | `summary-publish-integration` — публикация §100–§106: reuse `sendRichMessage` (§100) + настоящий H1, **`generate_image` НЕ трогать** (§104), fallback `sendMessage` (§105), коды ошибок §106 (в т.ч. L-R1026S5-4: `SUMMARY_GENERATION_FAILED`/`COVER_GENERATION_FAILED`/`RICH_MESSAGE_SEND_FAILED`/`TEXT_FALLBACK_FAILED`), сверка документации Telegram §103 | §100–§106, §103 | backend/публикация | **P0** | S5 ✅ | **⛔ BLOCKED — GATED (D4): смена существующего публикационного пайплайна только после live-приёмки Эпика 1 (ADR-1025-24 D4). Step 1 не стартует до гейта.** |
...
| **S10** | `summary-deploy` — прямой деплой §107 + проверка §114 + первый рабочий запуск §115 + результаты §117 | §107, §114, §115, §117 | deploy/ops (gate) | **P0** | S1…S9 (**S7 ✅; S6 ⛔**) | ⛔ **GATED (D4 — live-приёмка Эпика 1); Step 1 после S6/S9 и снятия гейта** |
