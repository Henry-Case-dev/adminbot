...
> Эталон для теста `test_system_prompt_byte_for_byte` (tests/test_summary_prompts.py, хелпер `_backlog_system_prompt`): содержимое кодового блока выше — строки 1518–1539 (1-индекс), слайс `lines[1517:1539]` (v4 — Epic 29, 22 строки). Фигурные скобки: **3 пары** (2 уникальных плейсхолдера: `{max_symbols}` ×1, `{username}` ×2) — тест `test_max_symbols_is_the_only_placeholder` проверяет НАБОР плейсхолдеров (D72). Хвостовые пробелы строк не сохраняются (D74).
...
## Epic 28: Качество памяти — векторы L3, репосты, алиасы, очистка ответа LLM — 2026-08-16 🆕 IN PROGRESS (Шаг 1: PM ✅ → Шаг 2: @Architect)
...
## Epic 29: UX-полировка — удаление команды, ack-вариации, промпт v4 — 2026-08-16 🆕 IN PROGRESS (Шаг 1: PM ✅ → Шаг 2: @Architect)
...
**Статус: Epic 29 — Шаг 1 (PM) ✅ (2026-08-16): требования R29-1…R29-6 и решения D81–D84 зафиксированы. T-221 (P0) → передан @Architect (Шаг 2: порядок delete→ack + Section 37, решение по нумерации пунктов промпта v4). T-222…T-226 → READY после Шага 2 (T-222 может стартовать параллельно по согласованию).**
**Date: 2026-08-16**
