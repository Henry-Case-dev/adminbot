...
**Аудит остатка ТЗ (UPD 7) — ✅ выполнен:** **`plans/reports/round1025_tz_remaining_audit.md`** (+ **§9 «Итоговый отчёт UPD-7»** — три обязательных блока) — что закрыто (F0/F1/F2/F3 + hotfix-media/2/3/4/5/6 + §15 + **HOTFIX7**), что осталось (F4 ⏸, F5–F11; Эпик 2 S1–S10; Эпик 3 A0–A10; внеплановый бэклог), рекомендуемый порядок. **Порядок продолжения (обновлён Шаг 8 @PM по HOTFIX8):** **HOTFIX7 ✅ → F4 ✅ COMPLETED + MERGED + ARCHIVED + DEPLOYED (2.58.9) → F5 ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 22.09.2026; Merge §61; deploy ✅ Шаг 9 @DevOps, `APP_VERSION` 2.58.10; live T-2742 ⏳) → HOTFIX8 ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 22.09.2026; Merge §62; deploy ⏳ Шаг 9 @DevOps; live T-2776 ⏳) → F6** `memory-analytics-reorg-round1025` → F7 → F11 → F9 → F10 → СТОП-ГЕЙТ Эпика 1 → Эпик 2 → Эпик 3. **Следующая фича — F6** `memory-analytics-reorg-round1025` (отдельного решения владельца не требует — UPD 7 п.7 «без human gate»).

### ✅ Хотфикс-8 (Волна 1.7, ПРИОРИТЕТНЫЙ по UPD2) — `hotfix8-shell-glass-aurora-round1025` (внеплановый, выше F6) — ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 22.09.2026; Merge §62); deploy ⏳ Шаг 9 @DevOps; live T-2776 ⏳ владелец
...
> **Порядок фич (единый, по UPD):** **F0 → F1 → F2 → F3 → hotfix6 (Волна 1.5) → HOTFIX7 (Волна 1.6, приоритетный UPD) → F4 → F5 → F6 → F7 → F11 → F9 → F10**. Изменение относительно исходной декомпозиции: F0 добавлена в начало (Wave 0), F11 передвинута **перед** F9 (обе после F4/F5/F6/F7), F9 сужена; **hotfix6** вставлен между F3 и F4 по итогам живой приёмки владельца (блоки A–D), **§15 вынесено из F11 в hotfix6**; **HOTFIX7** вставлен между hotfix6 и F4 по новому приоритетному UPD «Срочный фикс текущего фронта» (§6073–6154), **F4 — ✅ ARCHIVED (Шаг 8 @PM, 22.09.2026; Merge §60; deploy ⏳ Шаг 9 @DevOps, live T-2656 ⏳).**
>
> **▶️ Текущий шаг (Шаг 8 @PM, 22.09.2026):** **HOTFIX8 `hotfix8-shell-glass-aurora-round1025` (Волна 1.7, приоритетный UPD2) — ✅ COMPLETED + MERGED + ARCHIVED** (Merge §62; архив — `plans/archive/hotfix8-shell-glass-aurora-round1025/`; @Reviewer **Approved**, @Scanner **C0/H0/M0**). **Дальше: `deploy — Шаг 9 @DevOps` (T-2786 bump 2.58.11 → T-2787 deploy; код/тесты/отчёты НЕ закоммичены) → `Шаг 10 @Memory: метрики 10.25-HOTFIX8 + KG` (T-2789) → следующая фича — F6 `memory-analytics-reorg-round1025`.** Ранее закрыта **F5** `module-workspace-tabs-round1025` (deploy `63dddd3`/`0d3ea40`/`af137cd`, `APP_VERSION` 2.58.10). Живые гейты владельца (в т.ч. **T-2776** HOTFIX8, **T-2742**, **T-2682**, **T-2617**, **T-2656**) закрываются параллельно и не блокируют старт (UPD2 §10 «без human gate»).
