...
**Пометка Epic 59 (правка пользователя, D224):** суть ИЗМЕНИЛАСЬ — из интро удалено «с выходом в сеть internet», ведущие пробелы «Способ 1/2/3» раздела 6 убраны; fenced-блок ниже ОБНОВЛЁН (strip-теги нового канона == содержимому fenced-блока).

```
Гайд по фичам бота. Никаких слеш-команд, всё работает нативно прямо в диалоге.
...
6. Прямое обращение к Богу Машине
Способ 1 (словами через рот): Бот откликается на бот, ботик, ботяра и ботохуета. Как вызвать: просто напиши одно из этих слов в чат - бот ответит реплаем на твое сообщение. Робот, работа и ботва не в счет: они его не разбудят.
Способ 2 (реплай): бот отвечает если ответить (Reply) на его сообщение.
Способ 3 (тегнуть): Бот ответит на тег через "@".

7. Скачивание видео (стяни себе, пока не удалили)
- Как вызвать: напиши скачай, загрузи или стяни и кинь ссылку на видео в одном сообщении.
...
Нюансы: Между скачиваниями стоит кулдаун — сервак не бездонный.

8. Расшифровка голосовых и кружочков (для тех, кто не нажал play)
- Как вызвать: никак, тут авто-режим — просто отправь в чат голосовое или кружочек, бот сам пришлет текст реплаем.
Нюансы: Голосовые длиннее 10 минут бот не осилит.
```
...
## Epic 53: ALAN_REPLIES v2 + LLM 502 (direct_chat) + RESEARCH_HUMAN — 2026-08-23 ✅ IMPLEMENTED + REVIEWED (APPROVED) (пользовательский запрос, Шаг 6 @Memory, target v2.38.0, 2354 passed / 0 failed, DEPLOY PENDING @DevOps)

> **Цель:** Три пункта нового запроса пользователя: **(П1)** довести 5 новых тем
> ALAN_REPLIES (NixOS/Линукс, Продажа SSD, Витамины Life Extension, 5-секундные прогулки
> с гантелями, Уличный тренажёр + колени) до ПОЛНОЦЕННЫХ (сейчас по ~3 фразы — огрызки);
> фразу «разминался сегодня?...» (handlers/alan.py:43) заменить на пул издевательских
> токсичных ВОПРОСОВ; ALAN_REPLIES_ENABLED остаётся **false** на проде (НЕ включать).
> **(П2)** Инцидент 2026-08-23: direct_chat keyword-ответ → ReadTimeout (попытка 1, 30с)
> → 502 → 502 → «LLM server error 502 after 3 attempts» (services/llm_client.py:139/163/175-178,
> services/direct_chat_service.py:135) — рецидив инцидента Epic 47: апстрим apinet.cloud
> отдаёт устойчивые 502, ретраи не спасают. Расследовать + прагматичный фикс (D216).
> **(П3)** Создать `plans/RESEARCH_HUMAN.md` — человеческий документ из плана RESEARCH.md
> (ВСЕ рекомендации, особенно §6 6.1–6.11 + таблица P0/P1/P2 + чек-лист) простыми словами:
> что это / зачем / что даст / цена / приоритет + чекбоксы [ ] для выбора пользователем.
> RESEARCH.md НЕ трогать.
> **Исполнители:** @Architect (T-418), @Builder (T-419…T-421), @QA (T-422),
> @Docs (T-423/T-424), @DevOps (T-425/T-426). Без @Orchestrator.
> **Target:** v2.38.0 (прод сейчас v2.37.0 `56cccd6`). **Baseline:** 2302 теста (0 регрессий).

### Факты Шага 0 (что уже есть — проверка кода исполнителем PM)

- **ALAN_REPLIES (handlers/alan.py:29-87):** 37 фраз; блоки: Original 6 (строки 31-36),
  Тренировки (39-43), Лонгковид (46-49), Нейросети (52-55), Жим дьявола (58-61),
  NixOS/Линукс (64-66, 3 фразы), Продажа SSD (69-71, 3 фразы), Витамины Life Extension
  (74-76, 3 фразы), 5-сек прогулки (79-81, 3 фразы), Уличный тренажёр + колени (84-86,
  3 фразы). Таргет-фраза для замены — строка 43 (блок «Тренировки», «разминался сегодня?...»).
- **Гейт:** `handlers/alan.py:113` `if settings.ALAN_REPLIES_ENABLED and count % interval == 0:`
  — ТОЛЬКО reply-блок; счётчик и F7v2 silence greeting работают безусловно. Прод .env:
  `ALAN_REPLIES_ENABLED=false` (Epic 52, T-416). НЕ включать (требование пользователя).
- **Тесты-контракты tests/test_alan.py:** `test_topic_coverage` (113-139) требует темы:
  тренировк, лонгковид, нейросет, жим дьявола, никс, линукс, нейрокластер, планшет, колен,
  ssd|ссд, витамин|life extension, тренажёр|гантел; `test_no_trading_words` (142-157)
  word-boundary запрет фьючерс/биток/биткоин/рынок/трейдер/график/шорт/лонг/крипт;
  `test_pool_has_minimum_size` (106-110) ≥16.
- **LLM-стек:** `services/llm_client.py` — ретраи транзиентных (Epic 47, 56.3/56.4):
  TransportError + 408/425/429/5xx, backoff BASE*2**attempt кап 8с + jitter 2с, Retry-After,
  бюджет asyncio.timeout LLM_TOTAL_BUDGET=60с, LLM_TIMEOUT=30с per-request, LLM_MAX_RETRIES=2
  (3 попытки). 4xx-диагностика (57.4): тело ≤500 симв. 5xx НЕ диагностируются (тело не логируется).
- **config/settings.py:298-311:** LLM_BASE_URL=apinet.cloud/v1, LLM_MODEL_NAME=deepseek-v4-flash,
  LLM_API_KEY, LLM_TIMEOUT=30.0, LLM_MAX_RETRIES=2, LLM_RETRY_BACKOFF_BASE=1.0/CAP=8.0/JITTER=2.0,
  LLM_TOTAL_BUDGET=60.0. Фоллбэк-провайдера и circuit breaker НЕТ.
- **direct_chat degrade:** `services/direct_chat_service.py:134-138` — LLMError → WARNING +
  `random.choice(CHAT_ERROR_PHRASES)` (R50-8, VERBATIM — НЕ трогать). Отдельного «LLM лежит»-
  состояния нет: каждый триггер = 3 попытки × ~30с ≈ до 60с+ ожидания при лежащем апстриме.
- **RESEARCH.md:** 462 строки; §6 (161-411): 6.1 резюме, 6.2 триггеры, 6.3 контекст,
  6.4 память, 6.5 стиль, 6.6 троттлинг/бюджет, 6.7 InaccessibleMessage (подтверждение D214),
  6.8 интеграции, 6.9 анти-паттерны, 6.10 ограничения Bot API, 6.11 таблица приоритетов
  P0/P1/P2 + сводный чек-лист (397-411).

### Требования (Requirements — обязательный чек-лист)

| # | Требование |
|---|-----------|
| **R53-1** | **ALAN_REPLIES v2 (П1):** 5 новых тем (NixOS/Линукс, Продажа SSD, Витамины Life Extension, 5-сек прогулки с гантелями, Уличный тренажёр + колени) довести до ПОЛНОЦЕННЫХ — минимум 4-5 фраз на тему в стиле/качестве существующих (токсично-ироничный, абсурдный, «живой собеседник»). Старые темы (Original 6, Тренировки, Лонгковид, Нейросети, Жим дьявола) СОХРАНЯЮТСЯ без изменений — их требуют контракты test_topic_coverage, пользователь их не отменял. Фразу alan.py:43 («разминался сегодня?...») ЗАМЕНИТЬ на пул из 3-5 издевательских токсичных ВОПРОСОВ в стиле бота (например, про разминку/гантели/дыхалку). Тексты фраз — КАНОН @Architect (Section 62, VERBATIM), Builder переносит байт-в-байт. Новые фразы НЕ должны содержать трейдинг-слов (контракт test_no_trading_words, word-boundary). Гейт `ALAN_REPLIES_ENABLED` не трогать; на проде остаётся false. |
| **R53-2** | **LLM 502 (П2):** (1) РАССЛЕДОВАНИЕ: чтение llm_client.py/settings.py/direct_chat_service.py; анализ логов прода (окно инцидента 2026-08-23, последовательность ReadTimeout→502→502); проверка настроек LLM_BASE_URL/LLM_MODEL_NAME/таймаутов/ретраев; (2) ФИКС по D216 (circuit breaker + опциональный фоллбэк-провайдер + диагностика 502); (3) тесты: мок httpx (502-последовательности, таймауты, фоллбэк, circuit breaker), direct_chat degrade-ветка. Существующие каноны R50-4/R50-8 (CHAT_SYSTEM_PROMPT, CHAT_ERROR_PHRASES) НЕ трогать — новые фразы отдельным пулом. |
| **R53-3** | **RESEARCH_HUMAN.md (П3):** отдельный документ `plans/RESEARCH_HUMAN.md` (исходный plans/RESEARCH.md НЕ трогать). ВСЕ рекомендации RESEARCH.md, особенно §6 (6.1–6.11 + таблица P0/P1/P2 + чек-лист), простыми словами БЕЗ жаргона: каждая рекомендация = что это / зачем / что даст / цена/сложность / приоритет + чекбокс [ ] для выбора. Документ должен позволять пользователю легко выбрать что внедрять (→ будущие эпики). |
| **R53-4** | **Качество и деплой:** полный pytest 0 регрессий (база 2302); README v2.38.0 (иронично) + MEMORY; коммит на русском (conventional) + пуш **origin/master**; деплой: git pull, .env (ALAN_REPLIES_ENABLED=false ОСТАЁТСЯ — не менять; LLM_FALLBACK_* по D216), systemctl restart, journalctl 0 traceback, smoke. |

### PM Decisions (зафиксированы 2026-08-23, Шаг 1 PM)

| # | Решение |
|---|---------|
| **D215** | **Epic 53 — ALAN_REPLIES v2 (П1, P1), target v2.38.0, задачи T-419.** Старые темы (Original 6, Тренировки, Лонгковид, Нейросети, Жим дьявола) СОХРАНЯЮТСЯ — они требуются контрактами `test_topic_coverage` (тренировк/лонгковид/нейросет/жим дьявола) и не отменены пользователем. Интерпретация «новые темы полностью» = 5 новых тем должны быть ПОЛНОЦЕННЫМИ (≥4-5 фраз каждая), а не огрызками по 3 фразы. Фраза alan.py:43 — отдельный пул издевательских вопросов (3-5 шт., random.choice в общий ALAN_REPLIES; сохранение темы «гантел» для контракта). Каноны фраз VERBATIM — Section 62 (@Architect). Тест-контракты test_alan.py СОХРАНЯЮТСЯ и усиливаются: проверка полноты новых тем (≥3 фразы на каждую новую тему). Прод: `ALAN_REPLIES_ENABLED=false` остаётся — НЕ включать (деплой T-426 это проверяет). |
| **D216** | **Epic 53 — LLM 502 direct_chat (П2, P0), target v2.38.0, задачи T-418/T-420/T-421/T-422.** Прагматичный минимум — **circuit breaker (ОСНОВНОЙ механизм) + опциональный фоллбэк-провайдер (вторичный) + диагностический лог 502**. Обоснование: (а) фоллбэк-провайдер сам по себе не спасает — второго ключа/провайдера у пользователя может не быть, а если апстрим-класс (apinet.cloud) лежит целиком — фоллбэк бессмыслен; поэтому он ОПЦИОНАЛЬНЫЙ (env пуст = выключен, поведение не меняется); (б) circuit breaker решает корневую UX-проблему «ретраи не спасают»: после N подряд 5xx/таймаутов НЕ дёргать дохлый апстрим (сейчас каждый триггер = до 60с+ ожидания), а сразу отвечать человеческой фразой; (в) health-check НЕ делаем — дорогой (отдельный вызов перед каждым generate), ничего не гарантирует (502 может прийти через секунду после успешного чека), CB покрывает тот же кейс дешевле. Скоуп CB: direct_chat (НЕ глобально — не ломать summary/checkup/др. подсервисы; глобальность — открытый вопрос @Architect). Раздельные таймауты для чата — гипотеза расследования, НЕ блокер. |
| **D217** | **Epic 53 — RESEARCH_HUMAN.md (П3, P2), задача T-423.** Имя документа: `plans/RESEARCH_HUMAN.md`. Структура: вводная «зачем этот документ» → группы рекомендаций (по разделам RESEARCH.md, §6 в приоритете) → каждая рекомендация: заголовок простыми словами, блоки «Что это / Зачем / Что даст / Цена и сложность / Приоритет» + чекбокс [ ] в начале; в конце — сводная таблица выбора и чек-лист. Без жаргона (расшифровка терминов: throttle → «защита от флуда», token budget → «лимит длины контекста в токенах» и т.п.). RESEARCH.md НЕ трогать. |

### Открытые вопросы для @Architect (закрыть в Section 62)

1. **CB-порог и кулдаун:** сколько подряд 5xx/таймаутов открывает CB (кандидат: 3), длительность кулдауна (кандидат: 300-600с), half-open после кулдауна (одна пробная попытка) или полный сброс? Какие ошибки считаются «подряд» (5xx, таймауты, транспортные) — счётчик на уровне llm_client или direct_chat_service?
2. **Скоуп CB:** только direct_chat (PM-рекомендация) или общий слой в llm_client с per-caller счётчиками? Влияние на summary/checkup/factcheck — их degrade-фразы остаются как есть.
3. **Фоллбэк-провайдер:** env `LLM_FALLBACK_BASE_URL`/`LLM_FALLBACK_MODEL`/`LLM_FALLBACK_API_KEY` (пусто = выключен); когда срабатывает (после исчерпания ретраев primary? на первой 5xx?), 1 попытка без ретраев? Участвует ли фоллбэк-фейл в CB-счётчике? (кандидат PM: да — любая неудача фоллбэка инкрементит).
4. **Человеческие фразы CB:** новый пул `CHAT_LLM_DOWN_PHRASES` (3-5, VERBATIM в Section 62) или reuse CHAT_ERROR_PHRASES? (PM-рекомендация: НОВЫЙ пул — R50-8 неприкосновенен; тон — «человеческий», без «LLM server error»).
5. **Диагностика 502:** тело/длина в лог при 5xx (по образцу 57.4, ≤500 симв., R17 без секретов) — только при 502 или все 5xx?
6. **Таймауты для чата:** нужен ли отдельный CHAT_LLM_TIMEOUT < 30с (гипотеза: апстрим лежит → ReadTimeout 30с на попытку — долго)? Или текущих 30с + CB достаточно?
7. **Канон новых alan-фраз:** точные тексты 5 тем × ≥4-5 фраз + пул издевательских вопросов (замена строки 43) VERBATIM; гарантия отсутствия трейдинг-слов (word-boundary чистота).
8. **Тест полноты тем:** порог фраз на новую тему (кандидат: ≥3) и реализация (счёт по маркер-слову темы в пуле).

### Задачи

### T-418 (@Architect) — РАССЛЕДОВАНИЕ 502 + Section 62: дизайн CB/фоллбэк + каноны фраз (R53-1/R53-2, D215/D216)

**Приоритет:** P0. **Зависимости:** нет. **Оценка:** 0.75d.

- [x] T-418-A: РАССЛЕДОВАНИЕ: чтение `services/llm_client.py`, `config/settings.py` (298-311), `services/direct_chat_service.py` (134-138); анализ логов прода окна инцидента 2026-08-23 (последовательность ReadTimeout 30с → 502 → 502 → LLMError после 3 попыток; частота/устойчивость 502 apinet.cloud); проверка настроек (LLM_BASE_URL/LLM_MODEL_NAME/LLM_TIMEOUT=30/LLM_MAX_RETRIES=2/бюджет 60с) — что можно улучшить (вопрос 6)
- [x] T-418-B: `plans/ARCHITECTURE.md` Section 62: дизайн circuit breaker (вопросы 1-2), фоллбэк-провайдер (вопрос 3), КАНОН VERBATIM новых фраз: 5 тем × ≥4-5 фраз + пул издевательских вопросов (замена alan.py:43) + `CHAT_LLM_DOWN_PHRASES` (вопрос 4, 3-5 фраз), диагностика 502 (вопрос 5), таймауты (вопрос 6), тест-план (вопросы 7-8); закрыть открытые вопросы 1–8
- [x] T-418-C: Self-review + PM-аппрув; T-419…T-422 → READY

**DoD:** причина 502 задокументирована (Section 62.1 «Расследование»); CB/фоллбэк-дизайн + каноны VERBATIM; вопросы 1–8 закрыты; PM-аппрув.

### T-419 (@Builder) — ALAN_REPLIES v2: полноценные темы + пул вопросов (R53-1, D215)

**Приоритет:** P1. **Зависимости:** T-418 (каноны). **Оценка:** 0.5d.

- [x] T-419-A: `handlers/alan.py`: 5 новых тем расширить до ≥4-5 фраз каждая (байт-в-байт из Section 62); фразу строки 43 («разминался сегодня?...») УДАЛИТЬ, вставить пул издевательских токсичных вопросов (3-5 шт., VERBATIM); старые темы НЕ трогать; word-boundary чистота (нет трейдинг-слов)
- [x] T-419-B: `tests/test_alan.py`: контракты сохранить; добавить проверку полноты новых тем (≥3 фразы на тему по Section 62); test_pool_has_minimum_size (≥16) продолжает проходить (пул станет больше)

**DoD:** 5 тем по ≥4-5 фраз; фразы-вопросы вместо alan.py:43; старые темы нетронуты; тесты зелёные; `ALAN_REPLIES_ENABLED` код не менялся (прод false).

### T-420 (@Builder) — llm_client: диагностика 502 + circuit breaker + фоллбэк (R53-2, D216)

**Приоритет:** P0. **Зависимости:** T-418. **Оценка:** 1d.

- [x] T-420-A: диагностика 502 (Section 62, вопрос 5): тело ≤500 симв. + длина в лог при 5xx (по образцу 57.4, R17); инцидентный сигнал Betterstack
- [x] T-420-B: CircuitBreaker (по Section 62, вопросы 1-2): порог N подряд (3), кулдаун (300-600с), half-open пробная попытка; счётчик на 5xx/таймауты; `LLM_CB_ENABLED`/`LLM_CB_FAILURE_THRESHOLD`/`LLM_CB_COOLDOWN_SECONDS` в settings + .env.example (`_env_bool`/`_env_int`)
- [x] T-420-C: фоллбэк-провайдер (вопрос 3): `LLM_FALLBACK_BASE_URL`/`LLM_FALLBACK_MODEL`/`LLM_FALLBACK_API_KEY` (пусто = выключен); 1 попытка после исчерпания primary; фейл фоллбэка → CB-счётчик; конфиг в settings + .env.example

**DoD:** 502 логируется с телом; CB открывается после N подряд и блокирует вызовы (мгновенный ошибка-результат); фоллбэк работает только при заполненных env; пустые env = поведение как раньше.

### T-421 (@Builder) — direct_chat: degrade-ветка CB + человеческие фразы (R53-2, D216)

**Приоритет:** P0. **Зависимости:** T-420. **Оценка:** 0.5d.

- [x] T-421-A: `services/direct_chat_service.py::handle`: CB OPEN → БЕЗ вызова llm.generate — сразу `random.choice(CHAT_LLM_DOWN_PHRASES)` (новый пул, VERBATIM из Section 62) через `_reply`; WARNING-лог «CB open»; CHAT_ERROR_PHRASES (R50-8) остаются для обычных LLMError
- [x] T-421-B: счётчик CB: успешный ответ → сброс (closed); интеграция с фоллбэком из T-420 (фоллбэк-фейл инкрементит)

**DoD:** при OPEN: 0 вызовов LLM, человеческая фраза в чат; при обычной LLMError — старый пул R50-8; каноны R50-4/R50-8 не тронуты.

### T-422 (@Builder + @QA) — Тесты CB/фоллбэк/502 + прогон (R53-2/R53-4, D216)

**Приоритет:** P0. **Зависимости:** T-419…T-421. **Оценка:** 1d.

- [x] T-422-A (@Builder): юнит-тесты llm_client (мок httpx): последовательность 502×3 → LLMError; 502×2 + успех → ответ; таймаут → LLMTimeoutError; CB: N подряд → OPEN → следующий вызов без HTTP (мок-счётчик), кулдаун, half-open; фоллбэк: env задан → вызов fallback после primary-фейла; env пуст → fallback НЕ вызывается; диагностика 502 (лог содержит тело ≤500)
- [x] T-422-B (@Builder): тесты direct_chat: CB OPEN → llm.generate НЕ вызван + фраза из CHAT_LLM_DOWN_PHRASES; обычная LLMError → CHAT_ERROR_PHRASES (R50-8); успех → CB сброс
- [x] T-422-C (@QA): тесты alan (полнота тем, word-boundary чистота, пул вопросов); полный pytest — 0 регрессий (2354 passed / 0 failed, база 2302 +52); ревью APPROVED; `git diff --check` чист

**DoD:** APPROVED; 2302+ passed; каноны R50-4/R50-8 и существующие эталоны не тронуты.

### T-423 (@Docs) — RESEARCH_HUMAN.md (R53-3, D217)

**Приоритет:** P2. **Зависимости:** нет. **Оценка:** 0.5d.

- [x] T-423-A: `plans/RESEARCH_HUMAN.md` (новый): все рекомендации RESEARCH.md (особенно §6 6.1–6.11 + таблица P0/P1/P2 + чек-лист) простыми словами; каждая = [ ] чекбокс + «Что это / Зачем / Что даст / Цена и сложность / Приоритет»; расшифровка терминов без жаргона; сводная таблица выбора; RESEARCH.md НЕ трогать

**DoD:** документ читается без подготовки; все рекомендации §6 покрыты; чекбоксы для выбора; RESEARCH.md без изменений.

### T-424 (@Docs) — README v2.38.0 + MEMORY (R53-4)

**Приоритет:** P2. **Зависимости:** T-422/T-423. **Оценка:** 0.25d.

- [ ] T-424-A: README v2.38.0 (иронично: alan-темы v2, CB/фоллбэк, RESEARCH_HUMAN.md) + `plans/MEMORY.md` (Epic 53, v2.38.0) + .env.example (в T-420)

**DoD:** доки синхронизированы.

### T-425 (@DevOps) — Коммит + пуш master (R53-4)

**Приоритет:** P1. **Зависимости:** T-424. **Оценка:** 0.1d.

- [ ] T-425-A: коммит код+тесты одним коммитом на русском (conventional); пуш **origin/master** (ветка проекта — master, НЕ main)

**DoD:** коммит в origin/master.

### T-426 (@DevOps) — Деплой v2.38.0 (R53-4)

**Приоритет:** P1. **Зависимости:** T-425. **Оценка:** 0.25d.

- [ ] T-426-A: ssh git pull; `.env` (бэкап `.env.bak.epic53`): ПРОВЕРИТЬ `ALAN_REPLIES_ENABLED=false` ОСТАЁТСЯ (НЕ включать); LLM_FALLBACK_* НЕ ставить (или по решению пользователя в D216); `systemctl restart admin_bot`; status active + новый PID; `journalctl -n 50` 0 traceback; smoke: alan (false), direct_chat keyword-ответ (LLM или CB-фраза), славик, checkup

**DoD:** прод v2.38.0, active (running), 0 traceback, ALAN_REPLIES_ENABLED=false подтверждён, smoke пройден.

### env-переменные (Epic 53)

| Переменная | Дефолт | Прод .env (T-426) | Где |
|-----------|--------|-------------------|-----|
| `ALAN_REPLIES_ENABLED` | `true` | **`false` ОСТАЁТСЯ (не менять)** | уже в settings (154) — без изменений |
| `LLM_CB_ENABLED` | `true` | не ставим (default) | `config/settings.py` + `.env.example` |
| `LLM_CB_FAILURE_THRESHOLD` | `3` | не ставим (default) | `config/settings.py` + `.env.example` |
| `LLM_CB_COOLDOWN_SECONDS` | `300` | не ставим (default) | `config/settings.py` + `.env.example` |
| `LLM_FALLBACK_BASE_URL` | `""` (пусто = выключен) | не ставим | `config/settings.py` + `.env.example` |
| `LLM_FALLBACK_MODEL` | `""` | не ставим | `config/settings.py` + `.env.example` |
| `LLM_FALLBACK_API_KEY` | `""` | не ставим | `config/settings.py` + `.env.example` |

Все — `_env_bool`/`_env_int` с дефолтами (канон env).

### Риски (Epic 53)

1. **Контракты тестов test_alan.py** — `test_topic_coverage` требует И старые И новые темы: старые сохраняются (D215); `test_no_trading_words` word-boundary — новые фразы не должны содержать фьючерс/биток/рынок/трейдер/график/шорт/лонг/крипт (внимание к «рынок» при расширении темы SSD); `test_pool_has_minimum_size` ≥16 — пул только растёт.
2. **Каноны промптов VERBATIM** — CHAT_SYSTEM_PROMPT (R50-4), CHAT_ERROR_PHRASES (R50-8) НЕ трогать; новый пул CHAT_LLM_DOWN_PHRASES отдельно (D216, вопрос 4); тест-эталоны байт-в-байт.
3. **Невозможность локального LLM-smoke** — CB/фоллбэк/502 проверяются ТОЛЬКО мок-тестами (httpx-мок); живой апстрим недоступен с локальной машины; проде проверка — деплой-smoke (T-426). Реальный CB-переход смотреть в журнале прода после инцидента.
4. **CB блокирует легитимные ответы после восстановления апстрима** — half-open пробная попытка + ограниченный кулдаун (вопрос 1); митигация в Section 62.
5. **Фоллбэк без второго ключа мёртв** — поэтому CB основной механизм, фоллбэк опционален (D216); пустые env = ноль изменений поведения.
6. **ALAN_REPLIES_ENABLED случайно включат на проде** — явный шаг проверки в T-426 (НЕ менять .env); флаг остаётся false (требование пользователя).

### Файлы (планируемые)

`handlers/alan.py` (пул), `services/llm_client.py` (502-лог, CB, фоллбэк), `services/direct_chat_service.py` (degrade-CB), `services/smartmodule_phrases.py` (CHAT_LLM_DOWN_PHRASES), `config/settings.py` + `.env.example`; тесты: `tests/test_alan.py`, `tests/test_llm_client.py`, `tests/test_direct_chat.py` (+ возможный `tests/test_circuit_breaker.py`); `plans/RESEARCH_HUMAN.md` (новый), `plans/ARCHITECTURE.md` (Section 62), `README.md`, `plans/MEMORY.md`.

**НЕ трогать:** `plans/RESEARCH.md` (R53-3); каноны R50-4/R50-7/R50-8 и их эталоны; CHAT_SYSTEM_PROMPT; гейт-структуру `handlers/alan.py` (113) и F7v2 silence greeting; прод `.env` ALAN_REPLIES_ENABLED=false; порядок роутеров; миграций БД нет.
...
## Epic 55: /info — «Что нового» по последним апдейтам — 2026-08-23 🚧 IN PROGRESS (пользовательский запрос, Шаг 1 @PM, target v2.39.0)

> **Цель:** В /info добавить НЕДОСТАЮЩУЮ информацию по последним апдейтам бота
> (Epic 52 v2.37.0, Epic 53 v2.38.0, Epic 54 v2.38.1) — новый раздел «Что нового»
> в том же иронично-свойском стиле. Существующие 5 разделов справки НЕ менять
> (требование пользователя) — только append. Канон-цепочка обновляется СИНХРОННО
> во всех 5 местах, иначе падают байт-в-байт тесты `test_info_service.py`.
> **Исполнители:** @Architect (T-431), @Builder (T-432/T-433), @QA (T-433),
> @DevOps (T-434/T-435), @Docs (T-436). Без @Orchestrator.
> **Target:** v2.39.0 (minor — меняется код `DEFAULT_INFO_TEXT` + тесты, D220).
> **Baseline:** прод v2.38.1 (`148328a`, PID 1052789), 2354 теста, Epics 1–54 ALL CLOSED.

### Требования (Requirements — обязательный чек-лист)

| # | Требование |
|---|-----------|
| **R55-1** | **Новый раздел /info:** составить текст нового раздела «Что нового» (канон создаёт @Architect по фактуре ниже — пользователь дал фактуру, НЕ дословный текст). Стиль существующих разделов: `<b>N. Название (ироничное пояснение)</b>`, «- Как вызвать:», «Нюансы:» где уместно; без эмодзи, без маркдауна, без секретов (R17). Существующие 5 разделов — байт-в-байт НЕ менять, только append. |
| **R55-2** | **Канон-цепочка (5 мест) синхронно:** `services/info_service.py::DEFAULT_INFO_TEXT` (КАНОН) = `info_text.md` (живой файл на проде, правится через /edit_info) = `plans/ARCHITECTURE.md` Section 53.3 (python-блок + html-блок) = `plans/backlog.md` «Канон R44-1» (текст БЕЗ тегов). Append нового раздела во ВСЕ места одним коммитом. |
| **R55-3** | **Тесты:** маркеры новых фич в `test_covers_features`; байт-в-байт тесты (`test_byte_for_byte_with_architecture` / `test_byte_for_byte_with_html_block` / `test_canon_matches_backlog_r44_1_essence`) зелёные; полный pytest — 0 регрессий (2354 passed / 0 failed). |
| **R55-4** | **Деплой с прод-процедурой:** ДО pull — бэкап `info_text.md.bak.epic55` + `git checkout -- info_text.md` (админ мог править файл через /edit_info); `git pull --ff-only`; restart; `journalctl -n 50` 0 traceback; smoke /info (новый раздел виден, старые 5 на месте). |
| **R55-5** | **Документация:** README changelog v2.39.0 (иронично) + `plans/MEMORY.md` (Epic 55, v2.39.0). |

### PM Decisions (зафиксированы 2026-08-23, Шаг 1 PM)

| # | Решение |
|---|---------|
| **D220** | **Версия: v2.39.0 (minor), а не patch.** Меняется код (`DEFAULT_INFO_TEXT` в `services/info_service.py`) и тесты — ровно тот случай, под который D218 (Epic 54) зарезервировал minor-бамп: «семантика minor (v2.39.0) зарезервирована для релизов с изменениями кода». Нумерация: Epic 55, задачи T-431+ (после T-430). |
| **D221** | **Структура нового раздела:** append ПОСЛЕ раздела 5 (Checkup) — единым блоком «6. Что нового …» (точная формулировка — @Architect в стиле существующих заголовков). Фактура группируется по апдейтам: Epic 52 (v2.37.0), Epic 53 (v2.38.0), Epic 54 (v2.38.1) — по 1-2 ироничные строки на фичу. RESEARCH_HUMAN.md — внутренний план-документ, в /info его, вероятнее всего, НЕ упоминать (решение за @Architect). Ключи/значения env в текст НЕ попадают (R17). |
| **D222** | **Тест-стратегия:** байт-в-байт тесты НЕ переписываются — они остаются зелёными за счёт синхронного append канон-цепочки (хелперы `_arch_default_info`/`_arch_info_html_block` читают полные блоки Section 53.3, `_backlog_r44_1_text` — полный fenced-блок). Меняется только `test_covers_features` — добавить маркеры новых фич (список фиксирует @Architect в T-431). |

### Канон-цепочка (5 мест — обновлять СИНХРОННО, одним коммитом T-432)

| # | Место | Действие |
|---|-------|----------|
| 1 | `services/info_service.py::DEFAULT_INFO_TEXT` | КАНОН — append нового раздела байт-в-байт из Section 53.3 (python-блок) |
| 2 | `info_text.md` (репо) | append того же (UTF-8/LF/без хвостового `\n`) |
| 3 | `plans/ARCHITECTURE.md` Section 53.3 — python-блок | append (@Architect в T-431) |
| 4 | `plans/ARCHITECTURE.md` Section 53.3 — html-блок | append (@Architect в T-431) |
| 5 | `plans/backlog.md` «Канон R44-1» (fenced-блок БЕЗ тегов) | append plain-эталона (@Builder в T-432) |

### Фактура апдейтов для текста (из @Memory — канон текста создаёт @Architect)

- **Epic 52 (v2.37.0):** Алан — трейдинг убран, новые темы, выключен; common/work медиа выключено; Славик — одно действие; dead page — месть за удалённый репост; direct_chat — слова «бот»-семейства.
- **Epic 53 (v2.38.0):** Алан v2 — 5 тем × 5 фраз + вопросы-подколы, всё ещё выключен; фикс 502 — circuit breaker, бот больше не висит 60с, отвечает человечными фразами; RESEARCH_HUMAN.md (внутренний план).
- **Epic 54 (v2.38.1):** фоллбэк DeepSeek включён на проде.

### Открытые вопросы для @Architect (закрыть в T-431)

1. **Точная формулировка и нумерация:** «6. Что нового» — единый блок с тремя подблоками по версиям или плоский список; ироничные пояснения в заголовках подблоков.
2. **Что писать про выключенное:** Алан (ALAN_REPLIES_ENABLED=false) и common/work — формулировка «выключено» в стиле бота (не «баг», а «усыплено/отпущено»), чтобы /info не врал пользователю.
3. **Упоминание RESEARCH_HUMAN.md и DeepSeek-фоллбэка:** RESEARCH_HUMAN.md — вероятно НЕ упоминать (внутренний план); фоллбэк DeepSeek — упомянуть фактом «включён», БЕЗ ключей/URL (R17).
4. **Маркеры `test_covers_features`:** какие фразы нового раздела искать (не пересекаться с существующими маркерами: «Гайд по фичам», «фактчек», «чекап», «кулдаун», «Checkup», «youtu.be», «какой-то-сайт.ru»).

### Задачи

### T-431 (@Architect) — Канон-текст нового раздела + Section 53.3 (R55-1, D221/D222)

**Приоритет:** P0. **Зависимости:** нет. **Оценка:** 0.5d.

- [ ] T-431-A: составить канон-текст нового раздела «Что нового» по фактуре Epic 52/53/54 — html-версия (стиль существующих разделов, без эмодзи/маркдауна/секретов R17) + plain-эталон (БЕЗ тегов, для backlog «Канон R44-1»)
- [ ] T-431-B: вписать в `plans/ARCHITECTURE.md` Section 53.3 — ОБА блока (python + html), append, существующее НЕ трогать
- [ ] T-431-C: зафиксировать маркеры новых фич для `test_covers_features` (список дословно); закрыть открытые вопросы 1–4

**DoD:** канон зафиксирован байт-в-байт (html + plain); Section 53.3 дополнена (оба блока); маркеры перечислены дословно; существующие 5 разделов канона не тронуты.

### T-432 (@Builder) — Канон-цепочка: DEFAULT_INFO_TEXT + info_text.md + backlog R44-1 (R55-2)

**Приоритет:** P0. **Зависимости:** T-431. **Оценка:** 0.25d.

- [ ] T-432-A: `services/info_service.py::DEFAULT_INFO_TEXT` — append нового раздела байт-в-байт из Section 53.3 (python-блок)
- [ ] T-432-B: `info_text.md` — append того же (UTF-8/LF/без хвостового `\n`)
- [ ] T-432-C: `plans/backlog.md` «Канон R44-1» — append plain-эталона (без тегов)

**DoD:** все 5 мест канон-цепочки синхронны (дифф — только append); существующие 5 разделов байт-в-байт не изменены; HTML валиден.

### T-433 (@Builder + @QA) — Тесты: маркеры + байт-в-байт + полный прогон (R55-3, D222)

**Приоритет:** P0. **Зависимости:** T-432. **Оценка:** 0.25d.

- [ ] T-433-A (@Builder): `tests/test_info_service.py::test_covers_features` — добавить маркеры новых фич (список из T-431-C)
- [ ] T-433-B (@QA): байт-в-байт тесты зелёные (`test_byte_for_byte_with_architecture` / `test_byte_for_byte_with_html_block` / `test_canon_matches_backlog_r44_1_essence`); полный pytest — 0 регрессий (2354 passed / 0 failed); `git diff --check` чист

**DoD:** маркеры добавлены; 3 байт-в-байт теста зелёные; полный прогон 0 регрессий.

### T-434 (@DevOps) — Коммит + пуш (R55-4)

**Приоритет:** P1. **Зависимости:** T-433. **Оценка:** 0.1d.

- [ ] T-434-A: коммит код+канон+тесты+`info_text.md` одним коммитом на русском (conventional commits, D123-стиль); пуш **origin/master**

**DoD:** коммит создан и запушен в origin/master.

### T-435 (@DevOps) — Деплой v2.39.0 с прод-процедурой (R55-4)

**Приоритет:** P0. **Зависимости:** T-434. **Оценка:** 0.25d.

- [ ] T-435-A: ДО pull — бэкап `info_text.md.bak.epic55` + `git checkout -- info_text.md` (админ мог править файл через /edit_info)
- [ ] T-435-B: `git pull --ff-only`; `systemctl restart admin_bot` → active (running), новый PID; `journalctl -n 50` — 0 traceback
- [ ] T-435-C: smoke /info — новый раздел виден, существующие 5 разделов на месте

**DoD:** бэкап существует; pull ff; новый PID; 0 traceback; smoke /info показывает новый раздел.

### T-436 (@Docs) — README v2.39.0 + MEMORY (R55-5)

**Приоритет:** P1. **Зависимости:** T-435. **Оценка:** 0.25d.

- [ ] T-436-A: README changelog v2.39.0 (иронично: новый раздел /info) + `plans/MEMORY.md` (Epic 55, v2.39.0)

**DoD:** README/MEMORY актуализированы; Epic 55 помечен CLOSED в планах.

### Риски (Epic 55)

1. **Прод `info_text.md` отличается от репо** (админ правил через /edit_info) — прод-процедура обязательна: бэкап `.bak.epic55` + checkout ДО pull (T-435-A); правки админа сохраняются в бэкапе.
2. **Рассогласование канон-цепочки** — обновление всех 5 мест одним коммитом (T-432/T-434); контроль — 3 байт-в-байт теста (T-433-B).
3. **Случайное изменение существующих разделов** — требование пользователя «не менять»; контроль — дифф только append (T-432 DoD).
4. **R17 в тексте /info** — упоминание фоллбэка DeepSeek без ключей/URL/env-значений (D221).
5. **Тон формулировок про выключенные фичи** (Алан, common/work) — текст не должен врать: фичи «выключены», иронично по-свойски (вопрос 2, T-431).

### Файлы (планируемые)

`services/info_service.py` (DEFAULT_INFO_TEXT, append), `info_text.md` (append), `plans/ARCHITECTURE.md` (Section 53.3, оба блока, append), `plans/backlog.md` (этот эпик + «Канон R44-1», append), `tests/test_info_service.py` (маркеры), `README.md`, `plans/MEMORY.md` (T-436).

**НЕ трогать:** `handlers/info.py`, `/edit_info`, пулы `INFO_*`, прод `.env`, `.env.example`, миграций БД нет.

---

**Статус: Epic 55 — Шаг 1 (@PM) 🚧 (2026-08-23): PLANNING — T-431…T-436 занесены в backlog.md + board.md, target v2.39.0 (D220 — minor: меняется DEFAULT_INFO_TEXT + тесты; D221 — новый раздел append после раздела 5; D222 — байт-в-байт тесты живут за счёт синхронного append, меняются только маркеры). Далее — @Architect (T-431: канон-текст «Что нового» + Section 53.3 оба блока + маркеры). Без @Orchestrator.**
**Date: 2026-08-23**

---

## Epic 56: /info — правка пользователя (раздел 6 «Прямое обращение к Богу Машине») + `<code>`→`<blockquote>` — 2026-08-23 ✅ DONE & DEPLOYED (v2.40.0, коммит `1d9bf61`, прод PID 1054487, 2354 теста)

> **Цель:** (1) Зафиксировать правку пользователя, существующую ТОЛЬКО в локальной рабочей
> копии `info_text.md`: раздел 6 теперь «6. Прямое обращение к Богу Машине» (3 способа:
> слова-триггеры, реплай на бота, тег @). Локальная копия = источник истины по тексту;
> серверный `info_text.md` == HEAD `c7a6da5` (старого канона, правки НЕТ — факт сверки
> T-438-A); после коммита git-канон == источнику истины. (2) Теги `<code>` не видны в
> мобильном Telegram — заменить на цитирование `<blockquote>` (27 мест). ТЕКСТ НЕ МЕНЯТЬ —
> только теги. (3) Обязательная сверка ЧЕРЕЗ EXA: Telegram Bot API formatting options
> (blockquote в HTML parse mode — Bot API 7.0 (December 29, 2023), expandable — 7.7) + aiogram 3.29.1 передача
> `parse_mode`. (4) Деплой ещё раз. Канон-цепочка обновляется СИНХРОННО во всех 5 местах,
> иначе падают байт-в-байт тесты `test_info_service.py`.
> **Исполнители:** @Architect (T-437), @Builder (T-438), @Reviewer (T-439),
> @DevOps (T-440), @Docs (T-441). Без @Orchestrator.
> **Target:** v2.40.0 (minor — меняется `DEFAULT_INFO_TEXT` + тесты, D223).
> **Baseline:** прод v2.39.0 (`c7a6da5`, PID 1053785), 2354 теста, Epics 1–55 ALL CLOSED.

### Требования (Requirements — обязательный чек-лист)

| # | Требование |
|---|-----------|
| **R56-1** | **Пользовательская правка раздела 6 = источник истины:** правка существует ТОЛЬКО в локальной рабочей копии `info_text.md` — раздел 6 теперь «6. Прямое обращение к Богу Машине» (3 способа: слова-триггеры, реплай на бота, тег @). Локальная копия = источник истины по тексту; серверный `info_text.md` == HEAD `c7a6da5` (старого канона — факт сверки T-438-A); после коммита git-канон == источнику истины (T-438-A/T-439). |
| **R56-2** | **Замена тегов `<code>`→`<blockquote>` (27 мест):** теги `<code>` не видны в мобильном Telegram. Заменить ТОЛЬКО теги; сам текст байт-в-байт НЕ менять. |
| **R56-3** | **Обязательная сверка ЧЕРЕЗ EXA:** Telegram Bot API formatting options (blockquote в HTML parse mode — Bot API 7.0 (December 29, 2023), expandable blockquote — 7.7) + aiogram 3.29.1 передача `parse_mode`. Вердикт и решение по замене фиксирует @Architect (T-437). |
| **R56-4** | **Канон-цепочка (5 мест) синхронно:** `services/info_service.py::DEFAULT_INFO_TEXT` (КАНОН) = `info_text.md` = `plans/ARCHITECTURE.md` Section 53.3 (python-блок + html-блок) = `plans/backlog.md` «Канон R44-1» (текст БЕЗ тегов). Раздел 6 = пользовательская версия; `<code>`→`<blockquote>` во всех местах с тегами. |
| **R56-5** | **Тесты:** `test_html_tags_balanced` (счётчики `<code>`→`<blockquote>`); `test_no_unbalanced_special_chars` (strip-список); `test_covers_features` (маркеры — решить судьбу «100500%»/«пересаживается»/«ботяра» при новом разделе 6); полный pytest — 0 регрессий (2354 passed / 0 failed). |
| **R56-6** | **Деплой:** бэкап серверного `info_text.md.bak.epic56` ДО всего; коммит (код+канон+тесты+`info_text.md`, без mp4) + пуш; на проде `git pull` (git-версия теперь = пользовательская с blockquote; если серверный файл изменён локально — сверка/checkout по ситуации, правку НЕ терять); restart; `journalctl -n 50` 0 traceback; smoke /info. |
| **R56-7** | **Документация:** README changelog v2.40.0 (иронично) + `plans/MEMORY.md` (Epic 56, v2.40.0). |

### PM Decisions (зафиксированы 2026-08-23, Шаг 1 PM)

| # | Решение |
|---|---------|
| **D223** | **Версия: v2.40.0 (minor), а не patch.** Прецедент D220 (Epic 55): меняется код (`DEFAULT_INFO_TEXT` в `services/info_service.py`) и тесты. Нумерация: Epic 56, задачи T-437+ (после T-436). |
| **D224** | **Локальная рабочая копия `info_text.md` = источник истины по тексту раздела 6.** Правка пользователя существует ТОЛЬКО локально; серверный файл == HEAD `c7a6da5` (старого канона — сверка T-438-A). Коммит+пуш переносит источник истины в git (после коммита git-канон == источнику истины); сверка байт-в-байт ДО любых изменений (T-438-A). |
| **D225** | **Порядок деплоя инвертирован:** сначала канон (пользовательская версия раздела 6 + blockquote) в git → коммит+пуш → затем `git pull` на проде. Если серверный файл изменён локально (не через git) — сверка/checkout по ситуации, правку пользователя НЕ терять (R56-6). |

### Канон-цепочка (5 мест — обновлять СИНХРОННО, одним коммитом T-438)

| # | Место | Действие |
|---|-------|----------|
| 1 | `services/info_service.py::DEFAULT_INFO_TEXT` | КАНОН — раздел 6 = пользовательская версия; `<code>`→`<blockquote>` (27 мест) |
| 2 | `info_text.md` (репо) | то же байт-в-байт (UTF-8/LF/без хвостового `\n`) |
| 3 | `plans/ARCHITECTURE.md` Section 53.3 — python-блок | обновить (@Architect в T-437; или новая Section 63 — решение @Architect) |
| 4 | `plans/ARCHITECTURE.md` Section 53.3 — html-блок | обновить (@Architect в T-437) |
| 5 | `plans/backlog.md` «Канон R44-1» (fenced-блок БЕЗ тегов) | раздел 6 → plain пользовательской версии (@Builder в T-438) |

### Открытые вопросы для @Architect (закрыть в T-437)

1. **Вердикт по blockquote:** поддерживается ли `<blockquote>` в HTML parse mode (Bot API 7.0, December 29, 2023 — НЕ 7.1)? Что с expandable blockquote (Bot API 7.7+) — нужен ли, как передаётся? Совместимость со старыми клиентами Telegram (риск: не покажут/покажут плоско).
2. **aiogram 3.29.1 передача `parse_mode`:** как именно (send_message/reply, `ParseMode.HTML`), есть ли конфликт с `/edit_info`-валидацией превью и рендером /info.
3. **Куда вписать новый канон:** обновить Section 53.3 (прецедент Epic 55 T-431) или новая Section 63 — ВНИМАНИЕ: «Section 54» занята (Epic 45), следующая свободная — Section 63.
4. **Маркеры `test_covers_features`:** судьба «100500%»/«пересаживается»/«ботяра» при новом разделе 6 (остались ли в пользовательском тексте; заменить/удалить; новые маркеры для «Прямое обращение к Богу Машине»).
5. **Полный список правок тестов:** `test_html_tags_balanced` (счётчики), `test_no_unbalanced_special_chars` (strip-список), `test_covers_features` (маркеры) — перечислить дословно.

### Задачи

### T-437 (@Architect) — exa-ресёрч + вердикт + новый канон + правки тестов (R56-1…R56-3, D223/D224)

**Приоритет:** P0. **Зависимости:** нет. **Оценка:** 0.5d.

- [ ] T-437-A: exa-ресёрч: Telegram Bot API formatting options — blockquote в HTML parse mode (Bot API 7.0, December 29, 2023), expandable blockquote (7.7+); aiogram 3.29.1 передача `parse_mode`
- [ ] T-437-B: вердикт + решение по замене `<code>`→`<blockquote>` (27 мест), совместимость со старыми клиентами; что с expandable
- [ ] T-437-C: обновить `plans/ARCHITECTURE.md` Section 53.3 (ОБА блока; прецедент T-431) или новая Section 63 — новый канон: раздел 6 = пользовательская версия + blockquote; plain-эталон для «Канон R44-1»
- [ ] T-437-D: перечислить правки тестов дословно (test_html_tags_balanced / test_no_unbalanced_special_chars / test_covers_features); закрыть открытые вопросы 1–5

**DoD:** вердикт зафиксирован (blockquote в HTML mode / expandable / parse_mode в aiogram 3.29.1); решение по замене зафиксировано; Section 53.3 (или 63) дополнена обоими блоками с новым каноном; правки тестов перечислены дословно; вопросы 1–5 закрыты.

### T-438 (@Builder) — Серверная сверка + замена тегов + канон-цепочка + тесты (R56-1/R56-2/R56-4/R56-5)

**Приоритет:** P0. **Зависимости:** T-437. **Оценка:** 0.5d.

- [ ] T-438-A: вытянуть серверный `info_text.md` (SSH-доступ согласовать с @DevOps или по данным деплоя: `nik@198.46.175.136`, пароль у DevOps) → сверить с локальной копией байт-в-байт
- [ ] T-438-B: заменить ТОЛЬКО теги `<code>`→`<blockquote>` (27 мест); текст байт-в-байт НЕ трогать
- [ ] T-438-C: синхронизировать канон-цепочку 5 мест: `DEFAULT_INFO_TEXT`, `info_text.md`, ARCHITECTURE 53.3 (оба блока), backlog «Канон R44-1» (plain)
- [ ] T-438-D: обновить тесты: `test_html_tags_balanced` (счётчики b/code→blockquote), `test_no_unbalanced_special_chars` (strip-список), `test_covers_features` (маркеры по T-437-D)
- [ ] T-438-E: полный pytest — 0 регрессий (2354 passed / 0 failed); `git diff --check` чист

**DoD:** сервер↔локально сверено байт-в-байт; заменены ТОЛЬКО теги (27 мест), текст не тронут; 5 мест канон-цепочки синхронны; тесты обновлены и зелёные; полный прогон 0 регрессий.

### T-439 (@Reviewer) — Байт-в-байт сверка + дифф + ревью (R56-1/R56-4)

**Приоритет:** P0. **Зависимости:** T-438. **Оценка:** 0.25d.

- [ ] T-439-A: байт-в-байт сверка 5 мест канон-цепочки; дифф — ТОЛЬКО замены тегов (текст не тронут); полный прогон 0 регрессий; ревью APPROVED

**DoD:** APPROVED; 5 мест синхронны байт-в-байт; дифф содержит только замены тегов; 0 регрессий.

### T-440 (@DevOps) — Бэкап + коммит/пуш + деплой v2.40.0 (R56-6, D225)

**Приоритет:** P0. **Зависимости:** T-439. **Оценка:** 0.25d.

- [ ] T-440-A: бэкап серверного `info_text.md.bak.epic56` ДО всего
- [ ] T-440-B: коммит (код+канон+тесты+`info_text.md`, БЕЗ mp4) на русском (conventional commits, D123-стиль) + пуш **origin/master**
- [ ] T-440-C: деплой: git-версия теперь = пользовательская с blockquote — на проде `git pull` (если серверный файл изменён локально — сверка/checkout по ситуации, правку НЕ терять); `systemctl restart admin_bot` → active (running), новый PID; `journalctl -n 50` — 0 traceback
- [ ] T-440-D: smoke /info — раздел 6 «Прямое обращение к Богу Машине» виден, цитирование (blockquote) отображается, остальные разделы на месте

**DoD:** бэкап существует; коммит запушен (без mp4); pull выполнен; правка пользователя не потеряна; новый PID; 0 traceback; smoke /info OK.

### T-441 (@Docs) — README v2.40.0 + MEMORY (R56-7)

**Приоритет:** P1. **Зависимости:** T-440. **Оценка:** 0.25d.

- [ ] T-441-A: README changelog v2.40.0 (иронично: пользовательская правка + цитирование) + `plans/MEMORY.md` (Epic 56, v2.40.0)

**DoD:** README/MEMORY актуализированы; Epic 56 помечен CLOSED в планах.

### Риски (Epic 56)

1. **Затереть пользовательскую правку раздела 6** — источник истины = локальная копия (R56-1/D224; сервер == HEAD `c7a6da5`, правки на проде НЕТ); бэкап `.bak.epic56` + сверка ДО изменений (T-438-A/T-440-A).
2. **Порядок деплоя инвертирован (D225):** сначала канон в git (пользовательская версия + blockquote), потом `git pull` на проде; при локальных изменениях серверного файла — сверка/checkout по ситуации, правку НЕ терять.
3. **Blockquote-совместимость со старыми клиентами Telegram** — вердикт exa (T-437-B); expandable (7.7+) не использовать, если не нужен; деградация на старых клиентах — приемлема/задокументирована.
4. **Маркеры тестов** («100500%»/«пересаживается»/«ботяра») могут исчезнуть из нового раздела 6 — судьбу фиксирует @Architect (T-437-D).
5. **Рассогласование канон-цепочки** — обновление всех 5 мест одним коммитом (T-438-C/T-440-B); контроль — байт-в-байт тесты (T-439-A).

### Файлы (планируемые)

`services/info_service.py` (DEFAULT_INFO_TEXT — раздел 6 + blockquote), `info_text.md` (то же), `plans/ARCHITECTURE.md` (Section 53.3 или 63, оба блока), `plans/backlog.md` (этот эпик + «Канон R44-1»), `tests/test_info_service.py` (счётчики/strip/маркеры), `README.md`, `plans/MEMORY.md` (T-441).

**НЕ трогать:** `handlers/info.py`, `/edit_info`, пулы `INFO_*`, прод `.env`, `.env.example`, миграций БД нет. mp4/media в коммит не попадают.

---

**Статус: Epic 56 — ✅ DONE & DEPLOYED (2026-08-23): все T-437…T-441 DONE ([x]); коммит `1d9bf61` запушен в origin/master; деплой v2.40.0 (бэкап info_text.md.bak.epic56, pull ff c7a6da5..1d9bf61, PID 1054487, 0 traceback, smoke /info OK). Тесты 2354/0. Epics 1–56 ALL CLOSED. Далее — Epic 57. Без @Orchestrator.**
**Date: 2026-08-23**

---

## Epic 57: /info → rich text статья (заголовки H1/H2, цитаты → italic+подчёркивание+жирный) — 2026-08-23 🚧 IN PROGRESS (пользовательский запрос, Шаг 1 @PM, target v2.41.0)

> **Цель:** Переоформить /info в статью нового формата rich text: интро-строка
> «Гайд по фичам бота с выходом в сеть internet. Никаких слеш-команд, всё работает
> нативно прямо в диалоге.» → заголовок H1; 6 пронумерованных подзаголовков → H2;
> 27 цитат (`<blockquote>`) → italic + подчёркивание + жирный. Текст байт-в-байт
> НЕ менять — только разметка. Обязательный ресёрч (context7 + duckduckgo, fallback exa)
> по Telegram Bot API и aiogram: есть ли поддержка заголовков (rich text), в какой
> версии, как передаётся (`parse_mode`? новые entity-типы?), хватит ли aiogram 3.29.1
> (пин <4.0.0) или нужен апгрейд. Канон-цепочка (5 мест) обновляется СИНХРОННО,
> иначе падают байт-в-байт тесты `test_info_service.py`.
> **Исполнители:** @Architect (T-442), @Builder (T-443), @Reviewer (T-444),
> @DevOps (T-445), @Docs (T-446). Без @Orchestrator.
> **Target:** v2.41.0 (minor — меняется `DEFAULT_INFO_TEXT` + тесты, D226).
> **Baseline:** прод v2.40.0 (`1d9bf61`, PID 1054487), 2354 теста, Epics 1–56 ALL CLOSED.

### Требования (Requirements — обязательный чек-лист)

| # | Требование |
|---|-----------|
| **R57-1** | **Новая разметка /info:** интро-строка «Гайд по фичам бота с выходом в сеть internet. Никаких слеш-команд, всё работает нативно прямо в диалоге.» → H1; 6 пронумерованных подзаголовков → H2; 27 цитат (`<blockquote>`) → italic + подчёркивание + жирный. Текст байт-в-байт НЕ менять — только разметка. |
| **R57-2** | **Обязательный ресёрч (context7 + duckduckgo, fallback exa):** поддержка заголовков (rich text) в Telegram Bot API — есть ли H1–H4, синтаксис, с какой версии Bot API; как передаётся (`parse_mode` vs новые entity-типы); aiogram-совместимость (3.29.1, пин <4.0.0); нужен ли апгрейд и его риски для других сервисов. Вердикт фиксирует @Architect (T-442). |
| **R57-3** | **Канон-цепочка (5 мест) синхронно:** `services/info_service.py::DEFAULT_INFO_TEXT` (КАНОН) = `info_text.md` = `plans/ARCHITECTURE.md` Section 53.3 (python-блок + html-блок, + пометка Epic 57) = `plans/backlog.md` «Канон R44-1» (plain — суть без тегов НЕ меняется). Одним коммитом. |
| **R57-4** | **Тесты:** баланс тегов (счётчики H1/H2, blockquote→i/u/b), `test_no_unbalanced_special_chars` (strip-список), `test_covers_features` (маркеры — интро-строка остаётся в тексте как H1, маркер «Гайд по фичам» должен выжить); полный pytest — 0 регрессий (2354 passed / 0 failed). |
| **R57-5** | **Chunking и фоллбеки:** текущий /info ~3065 симв., новая разметка добавит теги — пересчитать итог < 4096 (лимит сообщения); plain-фоллбек и `/edit_info`-валидация превью не ломаются. |
| **R57-6** | **Деплой:** бэкап `info_text.md.bak.epic57` ДО pull (серверный == HEAD `1d9bf61` — сверить); коммит (код+канон+тесты+планы, БЕЗ untracked mp4 `media/common/danger/danger_boom_gif-03.mp4`) + пуш origin/master; `git pull --ff-only`; restart; `journalctl -n 50` 0 traceback; smoke /info (grep-сверки). |
| **R57-7** | **Документация:** README changelog v2.41.0 (иронично) + `plans/MEMORY.md` (Epic 57, v2.41.0). |

### PM Decisions (зафиксированы 2026-08-23, Шаг 1 PM)

| # | Решение |
|---|---------|
| **D226** | **Версия: v2.41.0 (minor), а не patch.** Прецедент D223 (Epic 56): меняется код (`DEFAULT_INFO_TEXT` в `services/info_service.py`) и тесты. Нумерация: Epic 57, задачи T-442+ (после T-441). |
| **D227** | **Формат rich text — вердикт @Architect (T-442):** `parse_mode` vs entities, теги H1/H2 — по ресёрчу; апгрейд aiogram ТОЛЬКО если ресёрч докажет необходимость (пин <4.0.0); риски апгрейда для других сервисов проверяет @Reviewer (T-444). |
| **D228** | **Интро-строка остаётся в тексте как H1** — маркер «Гайд по фичам» выживает (проверка в T-442-D); plain-канон «Канон R44-1» суть без тегов НЕ меняется. |

### Канон-цепочка (5 мест — обновлять СИНХРОННО, одним коммитом T-443)

| # | Место | Действие |
|---|-------|----------|
| 1 | `services/info_service.py::DEFAULT_INFO_TEXT` | КАНОН — новая разметка: интро→H1, 6 подзаголовков→H2, 27 blockquote→i+u+b |
| 2 | `info_text.md` (репо) | то же байт-в-байт (UTF-8/LF/без хвостового `\n`) |
| 3 | `plans/ARCHITECTURE.md` Section 53.3 — python-блок | обновить (@Architect в T-442, + пометка Epic 57) |
| 4 | `plans/ARCHITECTURE.md` Section 53.3 — html-блок | обновить (@Architect в T-442) |
| 5 | `plans/backlog.md` «Канон R44-1» (fenced-блок БЕЗ тегов) | суть не меняется (plain, без тегов) — проверить, что не нужен апдейт (@Builder в T-443) |

### Открытые вопросы для @Architect (закрыть в T-442)

1. **Rich text в Bot API:** есть ли поддержка заголовков H1–H4? Синтаксис (HTML parse mode — какие теги; или MarkdownV2)? С какой версии Bot API появилась? Деградация на старых клиентах (визуальная, не ломающая?).
2. **Способ передачи:** `parse_mode` (HTML) vs новые entity-типы; как в aiogram 3.29.1 (параметры send_message, `ParseMode`, поддержка новых тегов); конфликт с `/edit_info`-валидацией превью и рендером /info.
3. **aiogram-совместимость:** хватит ли aiogram 3.29.1 (пин <4.0.0) или нужен апгрейд (какая версия, что меняется); риски апгрейда для других сервисов бота.
4. **Chunking:** текущий /info ~3065 симв.; новая разметка добавит теги — пересчитать итог < 4096; plain-фоллбек и `/edit_info`-валидация не ломаются.
5. **Маркеры тестов:** интро-строка становится H1 — маркер «Гайд по фичам» выживает? Баланс тегов (счётчики H1/H2, blockquote→i/u/b), strip-список — полный перечень правок дословно.

### Задачи

### T-442 (@Architect) — ресёрч rich text + вердикт + новый канон Section 53.3 + правки тестов (R57-1…R57-5, D226/D227)

**Приоритет:** P0. **Зависимости:** нет. **Оценка:** 0.5d.

- [x] T-442-A: ресёрч context7 + duckduckgo (fallback exa): Telegram Bot API rich text — заголовки (H1–H4? синтаксис, версия Bot API), aiogram-совместимость (3.29.1, пин <4.0.0), нужен ли апгрейд + риски апгрейда для других сервисов
- [x] T-442-B: вердикт по формату: `parse_mode` (HTML) vs entities; решение по апгрейду (да/нет, в рамках <4.0.0)
- [x] T-442-C: новый канон разметки в `plans/ARCHITECTURE.md` Section 53.3 (ОБА блока, + пометка Epic 57)
- [x] T-442-D: перечень правок тестов дословно (баланс тегов, strip-список, маркеры — проверить, что маркер «Гайд по фичам» выживет); риски: chunking (пересчитать < 4096), plain-фоллбек, /edit_info-валидация; закрыть открытые вопросы 1–5

**DoD:** вердикт зафиксирован (заголовки/версия Bot API/parse_mode vs entities/aiogram 3.29.1 или апгрейд); Section 53.3 обновлена обоими блоками с пометкой Epic 57; правки тестов перечислены дословно; маркер «Гайд по фичам» подтверждён; chunking пересчитан (< 4096); вопросы 1–5 закрыты.

### T-443 (@Builder) — Разметка + канон-цепочка 5 мест + тесты (R57-1/R57-3/R57-4/R57-5)

**Приоритет:** P0. **Зависимости:** T-442. **Оценка:** 0.5d.

- [x] T-443-A: применить разметку: интро-строка → H1; 6 пронумерованных подзаголовков → H2; 27 `<blockquote>` → italic + подчёркивание + жирный; текст байт-в-байт НЕ менять
- [x] T-443-B: синхронизировать канон-цепочку 5 мест: `DEFAULT_INFO_TEXT`, `info_text.md`, ARCHITECTURE 53.3 (оба блока), backlog «Канон R44-1» (plain — суть без тегов не меняется)
- [x] T-443-C: обновить тесты по T-442-D (баланс тегов, strip-список, маркеры)
- [x] T-443-D: полный pytest — 0 регрессий (2354 passed / 0 failed); `git diff --check` чист

**DoD:** разметка применена (H1/H2, i+u+b), текст не тронут; 5 мест канон-цепочки синхронны; тесты зелёные; полный прогон 0 регрессий.

### T-444 (@Reviewer) — Байт-в-байт сверка + дифф + ревью (R57-1/R57-3)

**Приоритет:** P0. **Зависимости:** T-443. **Оценка:** 0.25d.

- [x] T-444-A: байт-в-байт сверка 5 мест канон-цепочки; дифф — ТОЛЬКО разметка (текст не тронут); риски aiogram-апгрейда для других сервисов (если вердикт T-442 — апгрейд); полный прогон 0 регрессий; ревью APPROVED

**DoD:** APPROVED; 5 мест синхронны байт-в-байт; дифф содержит только изменения разметки; 0 регрессий.

### T-445 (@DevOps) — Бэкап + коммит/пуш + деплой v2.41.0 (R57-6)

**Приоритет:** P0. **Зависимости:** T-444. **Оценка:** 0.25d.

- [ ] T-445-A: бэкап серверного `info_text.md.bak.epic57` ДО pull; сверка серверного файла == HEAD `1d9bf61`
- [ ] T-445-B: коммит (код+канон+тесты+планы, БЕЗ untracked mp4 `media/common/danger/danger_boom_gif-03.mp4`) на русском (conventional commits, D123-стиль) + пуш **origin/master**
- [ ] T-445-C: деплой: `git pull --ff-only`; `systemctl restart admin_bot` → active (running), новый PID; `journalctl -n 50` — 0 traceback
- [ ] T-445-D: smoke /info — grep-сверки: H1 (интро), H2 (6 подзаголовков), 27 цитат i+u+b

**DoD:** бэкап существует; коммит запушен (без mp4); pull ff; новый PID; 0 traceback; smoke /info OK.

### T-446 (@Docs) — README v2.41.0 + MEMORY (R57-7)

**Приоритет:** P1. **Зависимости:** T-445. **Оценка:** 0.25d.

- [x] T-446-A: README changelog v2.41.0 (иронично: rich text статья) + `plans/MEMORY.md` (Epic 57, v2.41.0)

**DoD:** README/MEMORY актуализированы; Epic 57 помечен CLOSED в планах.

### Риски (Epic 57)

1. **aiogram-апгрейд может затронуть другие сервисы** — решение по ресёрчу (T-442-B); риски проверяет @Reviewer (T-444-A).
2. **Контекст 4096:** новая разметка добавит теги (сейчас ~3065 симв.) — пересчитать итог < 4096 (T-442-D); при превышении — решение @Architect.
3. **Rich text может не рендериться на старых клиентах** — деградация визуальная, не ломающая (не блокер; зафиксировать в T-442-B).

### Файлы (планируемые)

`services/info_service.py` (DEFAULT_INFO_TEXT — новая разметка), `info_text.md` (то же), `plans/ARCHITECTURE.md` (Section 53.3, оба блока, пометка Epic 57), `plans/backlog.md` (этот эпик + проверка «Канон R44-1»), `tests/test_info_service.py` (баланс тегов/strip/маркеры), `README.md`, `plans/MEMORY.md` (T-446).

**НЕ трогать:** `handlers/info.py`, пулы `INFO_*`, прод `.env`, `.env.example`, миграций БД нет. Untracked `media/common/danger/danger_boom_gif-03.mp4` в коммит НЕ попадает.

---

**Статус: Epic 57 — Шаг 2 (@Architect) ✅ T-442 DONE (2026-08-23): ресёрч (context7 — Invalid API key; duckduckgo — anomaly; фактура exa): реальные H1–H4 существуют ТОЛЬКО в Rich Messages (Bot API 10.1, June 11, 2026 — `sendRichMessage`, блок `RichBlockSectionHeading`, rich-HTML `<h1>`–`<h6>`; Bot API 10.2, July 14, 2026 — блоки `blocks`); MessageEntity-типа «heading» НЕТ; aiogram 3.29.1 УЖЕ содержит sendRichMessage (с v3.29.0, commit 8c3423e) — апгрейд НЕ нужен (пин <4.0.0). Rich Messages для /info ОТВЕРГНУТЫ: на Telegram Web/Desktop/старых мобильных (и в группах/форвардах) — «This message is not supported», тело сообщения НЕДОСТУПНО — не «визуальная деградация», а блокер (риск 3 закрыт — оказался блокером). Решение: sendMessage + parse_mode="HTML" остаётся; интро→H1-эмуляция `<b><u>`, 6 подзаголовков→H2-эмуляция `<b>`, 27 blockquote→`<b><i><u>…</u></i></b>`; счётчики 34 b / 27 i / 28 u / 0 blockquote / 2 a, сущностей 91 (<100); 2964 симв. / 4679 байт < 4096 — одиночное сообщение, chunking не срабатывает; маркер «Гайд по фичам» выживает; «Канон R44-1» fenced-блок БЕЗ изменений (суть не менялась); Section 53.3 обновлена (оба блока + «Свойства канона» + «Дополнение Epic 57»); handlers/info.py НЕ трогаем. Далее — @Builder (T-443: разметка DEFAULT_INFO_TEXT + info_text.md + тесты T-442-D). Без @Orchestrator.**
...
## Epic 59: Документационно-исследовательский: память бота + замеры на проде + полная переписка README — 2026-08-24 🚧 IN PROGRESS (пользовательский запрос, Шаг 5: re-review, target v2.42.1)

> **Цель:** Три направления, изначально docs-only, но включена канон-синхронизация
> /info с правкой пользователя (D224) — код затронут точечно
> (`services/info_service.py::DEFAULT_INFO_TEXT`): (1) веб-ресёрч + дополнение
> `plans/RESEARCH_HUMAN.md` темой «память бота: как улучшить/оптимизировать»;
> (2) замеры памяти на проде (nik@198.46.175.136) + оценка роста при увеличении
> ретеншена долгосрочной памяти; (3) полная переписка README.md (без changelog,
> понятно не-программисту).
> **Исполнители:** @Researcher (T-452), @DevOps (T-453/T-456), @Docs (T-454/T-457),
> @Reviewer (T-455). Без @Orchestrator.
> **Target:** v2.42.1 (patch/chore, D232 — исходно docs-only, включена канон-синхронизация D224: один code-файл точечно).
> **Baseline:** прод v2.42.0 (`c455728`, PID 1063356), 2360 тестов, Epics 1–58 ALL CLOSED.

### Требования (Requirements — обязательный чек-лист)

| # | Требование |
|---|-----------|
| **R59-1** | **RESEARCH_HUMAN.md:** веб-ресёрч (duckduckgo → exa; context7 НЕ нужен) по долгосрочной памяти LLM-ботов / GraphRAG / RAG retention/decay/consolidation; дополнить НОВЫМ блоком (структура по D233) человеческим языком, БЕЗ дублей с Блоком 2 №10–20 (память/контекст там уже описаны — только НОВОЕ); УДАЛИТЬ «## Итоговая шпаргалка» (~646–703) и «## Что дальше» (~707–713) — лишнее дублирование, выбор [x] пользователь сделает позже. Нумерацию блоков сохранить. Приватность бота на проде ВЫКЛЮЧЕНА — рекомендаций включать/выключать её НЕ давать (статус-кво). `plans/RESEARCH.md` НЕ трогать. |
| **R59-2** | **Замеры на проде (ТОЛЬКО замеры и оценка, код НЕ трогаем):** SSH nik@198.46.175.136 (/var/www/admin_bot): `df -h`; `ls -la local_database.db*` (db + WAL/SHM); `du -sh media/ venv/ .git/`; `journalctl --disk-usage` (sudo, пароль НЕ логировать); размеры таблиц БД read-only (`python -c sqlite3 …`, БЕЗ изменения данных). ОЦЕНКА роста при увеличении ретеншена: `FULL_MEMORY_RETENTION_DAYS` 30→180/365, `ARCHIVE_MEMORY_RETENTION_DAYS` 90→180/365, «убрать лимит вовсе» (допущение: вечность — постоянный темп, линейная экстраполяция). Считать раздельно L2 (smart_messages), L3 (smart_archive + smart_archive_facts), GraphRAG (graph_facts/nodes/edges), суммировать; учесть пайплайн сжатия L2→L3 (при большем FULL-ретеншене сырьё лежит дольше, L3 наполняется позже). Возраст чата/темп сообщений — из MIN/MAX timestamp в БД. Отчёт с цифрами (в МБ). |
| **R59-3** | **Полная переписка README.md:** убрать ВСЕ changelog-блоки (v2.12.1…v2.42.0) и техводу. Суть из changelog ПЕРЕНЕСТИ в «Функции» ДО удаления: DirectChat, SmartCache, Checkup, YouTube/Web-саммари, /info (Rich Message + /edit_info), GraphRAG, Summary (LLM-или-ничего), Circuit Breaker + фолбэк DeepSeek и пр. Структура: Что это → Функции (все 20+) → Параметры/env (полная таблица из `config/settings.py` — 141 поле: группы, дефолты, на что влияют, где менять — прод `.env` на сервере / `.env.example` локально) → Как менять тексты → Telegram ID → Запуск/тесты → Мониторинг → Нюансы. Ироничный стиль, понятно не-программисту. |
| **R59-4** | **Ревью + финализация:** @Reviewer APPROVED (RESEARCH_HUMAN, README, арифметика DevOps-оценки, канон-синхронизация R59-5); полный pytest 0 регрессий (2360); коммит docs/plans на русском + пуш origin/master; деплой ТРЕБУЕТСЯ — код затронут точечно (канон-синхронизация D224): сверка серверного `info_text.md` == HEAD `c455728`, бэкап `info_text.md.bak.epic59` ДО pull, `git pull --ff-only`, grep-сверки, `systemctl restart admin_bot` ОБЯЗАТЕЛЕН, 0 traceback, новый PID (D234); MEMORY/board актуализированы, Epic 59 CLOSED. |
| **R59-5** | **Канон-синхронизация с правкой пользователя (D224):** `DEFAULT_INFO_TEXT` = `info_text.md` = ARCHITECTURE 53.3 (оба блока) = backlog R44-1 fenced (strip-теги); тесты не тронуты, 2360/0. |

### PM Decisions (зафиксированы 2026-08-24, Шаг 1 PM)

| # | Решение |
|---|---------|
| **D232** | **Версия: v2.42.1 (patch/chore).** Прецедент D218 (Epic 54: chore без кода/тестов, только конфиг и доки). Исходно docs-only; канон /info синхронизирован с правкой пользователя (D224) — один точечный code-файл (`services/info_service.py`). Нумерация: Epic 59, задачи T-452+ (после T-451). |
| **D233** | **Структура нового блока RESEARCH_HUMAN:** «Блок 8. Память: как улучшить и оптимизировать» ПОСЛЕ Блока 7 (не разрывать Блоки 3–7), нумерация рекомендаций продолжается с №52 (№1–51 заняты; Блок 7 — факты Telegram без номеров). Существующие Блоки 1–7 не трогать. Формат пункта — как в Блоках 1–6 (что/зачем/цена/приоритет); чекбоксы [ ] НЕ проставляем — выбор пользователь сделает позже. |
| **D234** | **Синхронизация сервера:** деплой ТРЕБУЕТСЯ (код затронут точечно — канон-синхронизация D224): сверка серверного `info_text.md` == HEAD `c455728`, бэкап `info_text.md.bak.epic59` ДО pull, `git pull --ff-only`, grep-сверки (H1 без «с выходом в сеть internet», «Способ 1/2/3» без ведущих пробелов, h1=1/h2=6/b=27/i=27/u=0/a=2), `systemctl restart admin_bot` ОБЯЗАТЕЛЕН, 0 traceback, новый PID. При грязном дереве — пропустить и зафиксировать причину. |

### Открытые вопросы и гэпы (зафиксированы Шаг 1 PM)

1. **Нумерация RESEARCH_HUMAN:** Блок 7 — ненумерованные факты Telegram; шпаргалка (удаляется) ссылалась на №1–51. Новый Блок 8 начинает с №52 без перенумерации существующего (D233). @Researcher проверяет, что после удаления шпаргалки/«Что дальше» битых ссылок на них не осталось.
2. **Семантика «убрать лимит вовсе»:** допущение «вечность» — линейная экстраполяция по текущему темпу; на практике = неограниченный рост БД/векторов. Код НЕ трогаем — это оценочный сценарий, НЕ план внедрения.
3. **Приватность:** на проде ВЫКЛЮЧЕНА; R59-1 запрещает рекомендации включать/выключать её — только констатация факта в новом блоке.
4. **Тесты-счётчики README:** PM проверил — тестов, читающих README.md, НЕТ; @Docs перепроверяет при переписке (T-454-D).

### Задачи

### T-452 (@Researcher) — веб-ресёрч + RESEARCH_HUMAN: новый Блок 8 + удаление шпаргалки (R59-1, D233) ✅ DONE

**Приоритет:** P0. **Зависимости:** нет. **Оценка:** 0.5d.

- [x] T-452-A: ресёрч duckduckgo → exa (context7 НЕ нужен): долгосрочная память LLM-ботов, GraphRAG, RAG retention/decay/consolidation, гигиена/сжатие памяти, TTL, дедупликация, веса/важность фактов — что применимо к текущему стеку (SQLite + sqlite-vec + GraphRAG + компрессия L2→L3)
- [x] T-452-B: «Блок 8. Память: как улучшить и оптимизировать» ПОСЛЕ Блока 7, пункты №52+, человеческим языком; НЕ дублировать Блок 2 №10–20; без рекомендаций по приватности (статус-кво)
- [x] T-452-C: удалить «## Итоговая шпаргалка» (~646–703) и «## Что дальше» (~707–713); хвостовой блок «> Связь с исходником» сохранить

**DoD:** все рекомендации с что/зачем/цена/приоритет; стиль человеческий (пересказ, без техножаргона) сохранён; Блоки 1–7 нетронуты, нумерация продолжена с №52; шпаргалка и «Что дальше» удалены; RESEARCH.md не тронут; чекбоксы не проставлены.

**Результат (Шаг 2 @Researcher, 2026-08-24):** ресёрч duckduckgo → exa выполнен; «Блок 8. Память: как улучшить и оптимизировать» добавлен после Блока 7 (пункты №52+, что/зачем/цена/приоритет, человеческим языком, без дублей с №10–20, без приватность-рекомендаций); «Итоговая шпаргалка» и «Что дальше» удалены, битых ссылок не осталось, хвостовой блок «> Связь с исходником» сохранён; RESEARCH.md не тронут; чекбоксы не проставлены.

### T-453 (@DevOps) — SSH-замеры памяти + оценка роста ретеншена 180/365/∞ (R59-2) ✅ DONE

**Приоритет:** P0. **Зависимости:** нет. **Оценка:** 0.25d.

- [x] T-453-A: SSH nik@198.46.175.136 (/var/www/admin_bot): `df -h`; `ls -la local_database.db*`; `du -sh media/ venv/ .git/`; `sudo journalctl --disk-usage` (пароль НЕ логировать)
- [x] T-453-B: размеры таблиц БД read-only (`python -c sqlite3`, dbstat/COUNT(*)+length-оценки — БЕЗ изменения данных): smart_messages, smart_archive, smart_archive_facts, graph_facts, nodes, edges, chat_history, channel_state, smart_cache и др.; возраст чата (MIN timestamp) и темп сообщений
- [x] T-453-C: оценка роста: FULL 30→180/365, ARCHIVE 90→180/365, «без лимита» (вечность); раздельно L2/L3/GraphRAG + сумма (МБ); учесть сжатие L2→L3; допущения явно
- [x] T-453-D: отчёт с цифрами (МБ) в статус задачи (board) — без паролей и секретов (R17)

**DoD:** цифры в отчёте; пароль не залогирован; прод-конфиг (.env/БД) НЕ изменён; оценка по всем трём сценариям, раздельно по слоям и суммарно.

**Результат (Шаг 2 @DevOps, 2026-08-24):** БД 14.9 MiB (+WAL 4.0 MiB, суммарно ~19.5 MiB), свободно 17 ГБ (диск 24G), media 96M, venv 166M, journalctl 1.4G. Темпы: L2 0.31 МБ/д (11923 строк за ~7.9 д, 213 Б/сообщение), L3 ~0.48–1.45 МБ/д (после старта компрессии; сейчас 0), GraphRAG ~1.0 МБ/д (79 фактов/д × 12.3 КБ). Сценарии +1 год: S0 692 МБ (4.0%), S1 597, S2/S3 478. Находка: векторы graph_facts_vec не чистятся при TTL — кандидат в бэклог. Допущения: линейная экстраполяция, L3 — оценка.

### T-454 (@Docs) — полная переписка README.md (R59-3) ✅ DONE

**Приоритет:** P0. **Зависимости:** нет. **Оценка:** 0.5d.

- [x] T-454-A: инвентаризация: суть из ВСЕХ changelog-блоков (v2.12.1…v2.42.0) → в «Функции» (DirectChat, SmartCache, Checkup, YouTube/Web-саммари, /info + /edit_info, GraphRAG, Summary, Circuit Breaker + фолбэк DeepSeek, и пр.) ДО удаления changelog
- [x] T-454-B: новая структура: Что это → Функции (все 20+) → Параметры/env → Как менять тексты → Telegram ID → Запуск/тесты → Мониторинг → Нюансы; иронично, понятно не-программисту
- [x] T-454-C: env-таблица ПОЛНАЯ: все 141 поле `config/settings.py` (имя, дефолт, на что влияет, где менять: прод `.env` на сервере /var/www/admin_bot/.env vs локальные `.env`/`.env.example`); секреты — только имена, без значений
- [x] T-454-D: перепроверить тесты-счётчики README (PM: таковых НЕТ) + полный pytest 0 регрессий (2360) + `git diff --check`

**DoD:** все функции описаны (вкл. перенесённые из changelog); changelog-блоков нет; env-таблица полная (141 параметр .env (139 полей settings.py + SENTRY_DSN/LOGTAIL_SOURCE_TOKEN из bot.py), поимённо vs settings.py); «где менять» понятно; 0 регрессий (2360); diff --check чист.

**Результат (Шаг 4 @Docs, 2026-08-24):** README переписан полностью (changelog-блоки удалены, суть перенесена в «Функции» ДО удаления); структура Что это → Функции (все 20+) → Параметры/env (141 поле) → Как менять тексты → Telegram ID → Запуск/тесты → Мониторинг → Нюансы; иронично, понятно не-программисту; тесты-счётчики README отсутствуют (перепроверено); полный pytest 2360 passed / 0 failed; `git diff --check` чист.

### T-455 (@Reviewer) — ревью RESEARCH_HUMAN + README + арифметика + 0 регрессий (R59-4) ✅ DONE

**Приоритет:** P0. **Зависимости:** T-452, T-453, T-454. **Оценка:** 0.25d.

- [x] T-455-A: RESEARCH_HUMAN — шпаргалка/«Что дальше» удалены; Блок 8 без дублей с №10–20; стиль человеческий; нумерация сохранена; RESEARCH.md не тронут
- [x] T-455-B: README — все функции описаны; env-таблица полная vs settings.py (поимённо); понятность не-программисту; ирония; changelog отсутствует
- [x] T-455-C: DevOps-оценка — арифметика (темп × retention), раздельно L2/L3/GraphRAG + сумма; допущения явные
- [x] T-455-D: полный pytest 0 регрессий (2360); APPROVED

**DoD:** APPROVED; все три артефакта отревьюены; 0 регрессий (2360); замечания закрыты или явно зафиксированы.

**Результат (Шаг 5 @Reviewer, 2026-08-24):** REJECTED — БЛОКЕР-1: план/факт рассинхрон (заявлено docs-only «без кода/деплоя/рестарта» при фактической канон-синхронизации D224 с точечной правкой кода; счётчик полей env 145 ≠ 141). Правки внесены @PM (настоящий шаг) — после правок APPROVED pending: re-review @Reviewer.

### T-456 (@DevOps) — коммит docs/plans + пуш + синхронизация сервера (R59-4, D234)

**Приоритет:** P1. **Зависимости:** T-455. **Оценка:** 0.25d.

- [ ] T-456-A: коммит на русском (conventional commits, `docs(epic59): …`) — 8 файлов: README.md, plans/RESEARCH_HUMAN.md, plans/backlog.md, plans/board.md, plans/MEMORY.md, info_text.md, services/info_service.py, plans/ARCHITECTURE.md; БЕЗ untracked `media/common/danger/danger_boom_gif-03.mp4`, БЕЗ .env/local_database.db; пуш **origin/master**
- [ ] T-456-B: синхронизация сервера (D234): сверка серверного `info_text.md` == HEAD `c455728` + чистое дерево → бэкап `info_text.md.bak.epic59` ДО pull → `git pull --ff-only` → grep-сверки (H1 без «с выходом в сеть internet»; «Способ 1/2/3» без ведущих пробелов; h1=1/h2=6/b=27/i=27/u=0/a=2) → `systemctl restart admin_bot` ОБЯЗАТЕЛЕН, 0 traceback, новый PID; при грязном дереве — пропустить с фиксацией причины

**DoD:** коммит запушен (8 файлов, без mp4/.env/db); сервер синхронизирован (grep-сверки, 0 traceback, новый PID) ИЛИ обоснованный отказ; рестарт ОБЯЗАТЕЛЕН и выполнен.

### T-457 (@Docs/PM) — MEMORY/board + финализация Epic 59 (R59-4)

**Приоритет:** P1. **Зависимости:** T-456. **Оценка:** 0.25d.

- [ ] T-457-A: MEMORY.md (Epic 59, v2.42.1, фактура замеров T-453), board.md — [x] по T-452…T-456, Epic 59 → CLOSED

**DoD:** доки актуализированы; Epic 59 помечен CLOSED (v2.42.1).

### Риски (Epic 59)

1. **Оценка роста «без лимита»** — условная (вечность); НЕ план внедрения; код не трогаем (R59-2).
2. **README-переписка** — риск потерять фактуру из changelog: сначала перенос в «Функции», потом удаление (T-454-A).
3. **journalctl --disk-usage** требует sudo для nik (известное ограничение MEMORY) — через sudo, пароль не логировать (T-453-A).
4. **sqlite3 read-only на проде** — только SELECT/размеры, write-операции запрещены; прод-конфиг не менять (T-453-B).

### Файлы (планируемые) — 8 шт.

`README.md` (полная переписка), `plans/RESEARCH_HUMAN.md` (Блок 8 + удаление шпаргалки/«Что дальше»), `plans/backlog.md`/`plans/board.md`/`plans/MEMORY.md` (план/статус/финализация), `info_text.md` + `services/info_service.py` + `plans/ARCHITECTURE.md` (канон-синхронизация /info с правкой пользователя, D224).

**НЕ трогать:** тесты (2360/0), `plans/RESEARCH.md`, `.env`/`.env.example`, `config/settings.py`, прод-конфиг и БД на сервере, весь прочий код (кроме точечного `services/info_service.py::DEFAULT_INFO_TEXT`).

---

**Статус: Epic 59 — Шаг 5: re-review (2026-08-24): Шаг 1 @PM ✅ (требования R59-1…R59-5, решения D232–D234, задачи T-452…T-457). Шаги 2–4 ✅ DONE: T-452 @Researcher (Блок 8 + удаление шпаргалки), T-453 @DevOps (замеры на проде, цифры в статусе задачи), T-454 @Docs (README переписан, 2360/0). Шаг 5 @Reviewer: REJECTED — БЛОКЕР-1 (заявлено docs-only «без кода/деплоя/рестарта» при фактической канон-синхронизации D224 с точечной правкой кода; 145 ≠ 141 поле env). Правки внесены @PM (настоящий шаг): Цель/D232/D234/R59-4/R59-5/T-456/Файлы — деплой ТРЕБУЕТСЯ, restart ОБЯЗАТЕЛЕН, 8 файлов. Далее — re-review @Reviewer (T-455). Без @Orchestrator.**
**Date: 2026-08-24**

---

## Архив: Epics 1–59 — ✅ ALL CLOSED & DEPLOYED (зафиксировано 2026-08-24, Шаг 1 Epic 60)

> Перенесено из `board.md` («In Progress» → «✅ Done») при архивации (PM, 2026-08-24, Шаг 1 Epic 60).
> Полные треки всех эпиков — в разделах выше (требования/решения/задачи) и в `plans/MEMORY.md` (Шаги 8).
> Актуальный прод: **v2.42.1 (`d555454`, PID 1064777), 2360 тестов, 0 регрессий.** Сводка последних закрытий:

| Epic | Версия | Коммит | Прод PID | Тесты | Суть |
|---|---|---|---|---|---|
| 59 | v2.42.1 | `d555454` | 1064777 | 2360 | RESEARCH_HUMAN Блок 8 + замеры на проде + README переписан + канон-синхронизация /info (D224) |
| 58 | v2.42.0 | `c455728` | 1063356 | 2360 | /info настоящий Rich Message (sendRichMessage, честные H1/H2, фолбек D231) |
| 57 | v2.41.0 | `ba91035` | 1062574 | 2354 | /info rich-эмуляция (H1/H2, цитаты курсив+подчёркивание+жирный) |
| 56 | v2.40.0 | `1d9bf61` | 1054487 | 2354 | /info правка пользователя («Бог Машине») + 27 `<code>`→`<blockquote>` |
| 55 | v2.39.0 | `c7a6da5` | 1053785 | 2354 | /info раздел «Что нового» |
| 54 | v2.38.1 | `148328a` | 1052789 | 2354 | Фоллбэк-провайдер DeepSeek включён (прод .env) |
| 53 | v2.38.0 | `a8f82b1` | 1052443 | 2354 | ALAN_REPLIES v2 + Circuit Breaker 502 + RESEARCH_HUMAN.md |
| 52 | v2.37.0 | `56cccd6` | 1051710 | 2302 | Запрос пользователя (ALAN/common/slavik/direct_chat «бот»+deleted-post) |
| 48–51 | v2.36.0 | `b394e1e` | 1018603 | 2205 | degraded-откат + чекап 400 + DirectChat + Intelligent Caching (user_version=2) |
| 47 | v2.35.1 | `6d0cba0` | 1013533 | 2099 | Resilience: LLM-ретраи, memorize-повтор, summary retry-once |
| 45–46 | v2.35.0 | `eef5939` | 995355 | 2070 | Betterstack SQL API + GraphRAG v2 (origin/TTL, Extractor, гибридный RAG) |
| 44 | v2.34.1 | `5f17e21` | 992020 | 1981 | Новый /info-текст + фикс delete + Telemetry |
| 42–43 | v2.34.0 | `cb339d6` | 990054 | 1976 | Checkup + /info live-редактор |
| 41 | v2.33.1 | `eaa84c5` | 986288 | 1796 | YouTube hardening (ru-first, ретраи, токсичные фразы) |

Epics 1–40 — см. исторические разделы выше и `board.md` («✅ Done»). Прод v2.42.1 (`d555454`, PID 1064777) — точка отсчёта Epic 60.
...
> троттлинг/кэш, per-chat замок генерации, реакция 🗿, кулдаун-молчание, стилевые якоря,
> команды /clear /persona /tone /forget, стриминг саммари, индикатор «печатает…», temperature-
> пресеты, настроение, «забудь»+карточки фактов; (2) память — дедуп при записи, страховка от
> отравления, бэкап+экспорт, кэш эмбеддингов, бегущий конспект, токены tiktoken, веса 0..1,
> слияние эпизодов, time-decay, квота, TTL+LRU, сжатие векторов, «золотые вопросы», MMR,
> профили, конфликты, пересмотр, бюджеты контекста; (3) чекап — метрики здоровья (память,
> диск, всё что можно) в том же токсичном стиле; (4) правила-каноны 20/49/63.
> **Исполнители:** @Architect (T-458), @Researcher (T-459), @Builder (T-460…T-492, T-499),
> @Reviewer (T-494), @Docs (T-493/T-495/T-498), @DevOps (T-496/T-497). Без @Orchestrator.
> **Target:** v2.43.0 (minor, D235).
> **Baseline:** прод v2.42.1 (`d555454`, PID 1064777), 2360 тестов, Epics 1–59 ALL CLOSED & DEPLOYED.

### Требования (R60-1…R60-35 — обязательный чек-лист)

| # | Требование (пункт RESEARCH_HUMAN) | Приоритет | Комментарий пользователя |
|---|-----------------------------------|-----------|--------------------------|
| **R60-1** | **п.13 — Персистентность:** контекст и счётчики троттлинга/кэша — в БД с TTL вместо in-memory (переживают рестарт). Сейчас in-memory: CooldownTracker (smartmodule_utils) и token bucket `DirectChatService._state` (dict); SmartCache уже SQLite (аддитивная таблица). Миграция `user_version` 2→3 идемпотентная (прецеденты Epic 46 T-360, Epic 50 58.7). Флудер не «перезаряжается» рестартом. | **P0** | — |
| **R60-2** | **п.36 — Per-chat замок генерации:** одно обращение — одно раздумье, следующее ждёт (asyncio-блокировка per-chat вокруг генерации direct_chat); ответы в правильном порядке. Прецедент Epic 35 (alan_greeting per-chat lock). | **P0** | — |
| **R60-3** | **п.52 — Дедуп фактов при записи:** перед записью сравнение с ближайшими соседями по смыслу → добавить / обновить / пропустить (прецедент Mem0: add/update/delete/no-op). | **P1** | — |
| **R60-4** | **п.15 — Страховка от отравления памяти:** логи сжатия «что во что», правило «свежий факт побеждает старый», сомнительное → «не подтверждено». | **P1** | — |
| **R60-5** | **п.57 — Бэкап БД раз в день + текстовый экспорт фактов** (читаемый глазами, для ручной правки). | **P1** | — |
| **R60-6** | **п.60 — Кэш эмбеддингов:** таблица «текст → вектор», проверка перед вызовом модели; уважать TTL фактов. | **P1** | — |
| **R60-7** | **п.58+п.43 — Метрики здоровья памяти + расширенный чекап:** в рамках того же чекапа (в том же сообщении / нескольких подряд, в том же токсичном стиле) бот оценивает свою память, размер памяти/оставшееся место на диске и все возможные метрики здоровья. CHECKUP_SYSTEM_PROMPT (R42-6) VERBATIM — метрики подаются ДАННЫМИ в контекст, промпт не менять; пулы R42-2/3/4/5 не трогать. | **P1** | п.43: «Да, вот это максимально нужная вещь. Сейчас уже есть чекап, который прекрасно работает, бот уже может лезть в логи. Было бы круто если бы он мог в рамках этого же чекапа (прям в этом же сообщении \ нескольких сообщениях подряд и в том же токсичном стиле как сейчас) оценивать свою память, размер памяти\оставшееся место на диске и все возможные метрики здоровья какие только можно придумать.»; п.58: «Как в пункте 43, это должно быть также одной из метрик которую бот смотрит при команде "чекап".» |
| **R60-8** | **п.11 — Бегущий конспект:** суммаризация при ~80% заполнения, последние 20–30 сообщений дословно, лениво в фоне, хранить в БД с TTL (не в памяти процесса). | **P1** | — |
| **R60-9** | **п.10 — Лимиты в токенах (tiktoken)** вместо символов; саммари должна также ориентироваться на время (6 часов — как сейчас настроено). | **P1** | «Да, но с оговоркой, саммари должна также ориентироваться на время (6 часов как сейчас настроено)» |
| **R60-10** | **п.33+п.37 — Пустой ответ модели → молчание + реакция 🗿** (вместо 👀), без текстовой заглушки и без ошибки «текст сообщения пуст»; для ВСЕХ LLM-фич. | **P1** | п.33: «Да, круто, но заменить 👀 на эмодзи истукана 🗿»; п.37: «Непонятно чем отличается от 33, но тоже истукан вместо глаз» |
| **R60-11** | **п.38 — Бот отредактировал свой ответ → обновить запись в цепочке** (память цепочки не врёт). На правку человеком своего триггера — не переотвечать (не закладываться). | **P1** | — |
| **R60-12** | **п.25 — Молчать после N кулдаунов подряд** (репутация человека); дефолт N=5 (конфиг). | **P2** | «по дефолту выставить 5 кулдаунов» |
| **R60-13** | **п.28 — Стилевые якоря:** показывать модели 2–3 её недавних ответа явной секцией («вот как ты отвечал недавно, держи тон»). | **P2** | — |
| **R60-14** | **п.9 — Команды внутри диалога:** /clear, /persona, /tone, /forget. | **P2** | — |
| **R60-15** | **п.31 — Стриминг ответа (send + editMessageText) ТОЛЬКО для саммари** — в качестве теста. | **P1** | «Звучит интересно, надо сделать только для саммари в качестве теста» |
| **R60-16** | **п.30 — Индикатор «печатает…» (sendChatAction) для ВСЕХ фич с ИИ (весь смарт-модуль):** появляется с момента отправки контекста в ИИ и до момента отправки сообщения; таймаут → сброс; БЕЗ искусственной паузы (пауза и так есть). | **P2** | «Крутая фича, но искусственную паузу не добавлять, так как пауза все равно имеется так или иначе. индикатор должен появляться с момента отправки контекста в ИИ и до момента отправки сообщения. Если таймаут то индикатор сбрасывается. Индикаторы должны быть у всех фичей связанных с ИИ (весь смарт модуль)» |
| **R60-17** | **п.32 — temperature в конфиг с пресетами** точный / сбалансированный / болтливый. | **P2** | — |
| **R60-18** | **п.34 — Ловить настроение собеседника и подстраивать тон;** системный промпт НЕ менять (инъекция в user-контекст). | **P2** | «да, но при этом системный промпт не должен изменяться» |
| **R60-19** | **п.14 — Явное «забудь/сбрось» + защищённые факты-карточки + возможность забывать определённые факты** (запомненные неправильно) **или перезаписать факт**. | **P2** | «Да и + было бы хорошо иметь возможность забывать определенные факты которые были запомнены неправильно, или перезаписать факт» |
| **R60-20** | **п.53 — Веса значимости фактов 0..1** (колонка); влияние на порядок выдачи и на TTL (важные живут дольше). | **P2** | — |
| **R60-21** | **п.54 — Слияние повторяющихся эпизодов в общие факты** (фоновая задача раз в N дней; слияние с проверкой «ничего не потерялось»). | **P2** | — |
| **R60-22** | **п.55 — Time-decay связей графа:** вес × коэффициент давности при чтении; свежие связи предпочтительнее. | **P2** | — |
| **R60-23** | **п.56 — Квота памяти на человека:** лимит фактов на человека; при упоре в квоту новый факт вытесняет менее значимый. | **P2** | — |
| **R60-24** | **п.59 — Автоочистка по использованию: TTL + LRU в связке с таймером** (использование продлевает жизнь, неиспользуемое — кандидат на досрочный выброс). | **P2** | «Да, но в связке с таймером» |
| **R60-25** | **п.61 — Сжатие векторов float16/int8** (sqlite-vec); делать ПОСЛЕ R60-6 (п.60), чтобы backfill был дешёвым. | **P2** | — |
| **R60-26** | **п.62 — «Золотые вопросы»:** проверочный список вопросов с ожидаемыми ответами; прогон после каждого релиза, касающегося памяти. | **P2** | — |
| **R60-27** | **п.16 — MMR:** разнообразие при подборе фактов (похожесть + разнообразие). | **P2** | — |
| **R60-28** | **п.17 — Профили пользователей:** карточки фактов о людях в графе + **метод удобного просмотра и редактирования карточек человека**. | **P2** | «Нужно также разработать метод чтобы можно было удобно просматривать эти карточки человека и редактировать» |
| **R60-29** | **п.18 — Факты протухают, свежий побеждает при конфликте** (срок годности уже есть — добавить разрешение конфликтов, не сломать). | **P2** | — |
| **R60-30** | **п.19 — Периодический пересмотр фактов:** склейка дублей, выброс устаревшего (не только «по сообщению»). | **P2** | — |
| **R60-31** | **п.12 — Бюджеты контекста:** system ~5%, история ~30%, ветка реплая ~20%, факты ~15%, ответ 15–25%, запас 10%. | **P2** | — |
| **R60-32** | **п.20 — ПРАВИЛО (канон):** мусор и фразы-ошибки не пишутся в память (уже соблюдается — зафиксировать, НЕ сломать; тесты-стражи). | **P0 (правило)** | — |
| **R60-33** | **п.49 — ПРАВИЛО:** списки слов только в конфиге. ⚠ Вынести keyword-регекс «бот\|ботик\|ботяра…» (`handlers/direct_chat.py:39-41`, `_BOTWORD_RE`, Epic 52) в конфиг. | **P1 (правило)** | — |
| **R60-34** | **п.63 — ПРАВИЛО:** не переезжать на Mem0/Zep/Letta — остаёмся на своей схеме (L2/L3/GraphRAG уже «взрослая»); подсматривать только приёмы №52–55. | **P1 (правило)** | — |
| **R60-35** | **п.8 — Дедуп+кэш одинаковых текстов подряд,** ключ «чат+человек+текст» («бот бот бот…» → один ответ/кэш). **В КОНЕЦ списка приоритетов** (D237). | **P1 (конец очереди)** | «можно реализовать, будет полезно для direct chat например, но у нас уже есть барьер в виде тротлинг таймеров. В конец списка приоритетов» |

### PM Decisions (зафиксированы 2026-08-24, Шаг 1 PM)

| # | Решение |
|---|---------|
| **D235** | **Версия: v2.43.0 (minor).** Прецеденты D230 (Epic 58) / D226 (Epic 57) — новые фичи. Нумерация: Epic 60, R60-1…R60-35, D235–D244, задачи T-458…T-499 (после T-457), Sections ARCHITECTURE **63–67** (59–62 заняты: 59=Epic 51 кэш, 60=план релиза v2.36.0, 61=Epic 52, 62=Epic 53). Один релиз в конце эпика (деплой T-497); P0-фазу A не деплоить отдельно без решения пользователя. ⚙ Дизайн фаз — Sections 63–67 (@Architect, D245–D249; порядок деплоя — 67.6). |
| **D236** | **Объединение пунктов:** п.33+п.37 → одна фича «реакция 🗿 на пустой ответ» (R60-10); п.43+п.58 → «метрики здоровья в чекап» (R60-7). Остальные 35 пунктов — по требованию на пункт; правила п.20/п.49/п.63 — отдельные требования-правила (R60-32/33/34). ⚙ Дизайн: 🗿 — Section 65.1; метрики — Section 64.5. |
| **D237** | **п.8 — в КОНЕЦ очереди** (T-499): комментарий пользователя «у нас уже есть барьер в виде тротлинг таймеров». Не смешивать с R60-1 (там — персистентность существующих счётчиков/кэша URL; здесь — дедуп одинаковых текстов с ключом «чат+человек+текст»). ⚙ Дизайн — Section 67.4 (slug `direct_dedup` на smart_cache, T-499, ПОСЛЕДНИМ). |
| **D238** | **Пользовательские дефолты (дословно из комментариев):** п.25 N=5 кулдаунов; п.31 стриминг только саммари; п.30 без искусственной паузы, индикатор для всего смарт-модуля, сброс при таймауте; п.33/37 эмодзи 🗿 (не 👀); п.34 системный промпт неизменен; п.14 забывать/перезаписывать отдельные факты; п.17 просмотр+редактирование карточек. ⚙ Реализация: N=5+стачка — 65.3; стриминг — 65.6; typing — 65.7; 🗿 — 65.1; настроение — 65.9; забудь/карточки — 65.10/66.9; таймаут замка — 63.2. |
| **D239** | **Персистентность (R60-1):** SQLite-бэкинг CooldownTracker + DirectChat token bucket + кэшей с TTL; миграция `user_version` 2→3 (идемпотентная, на остановленном боте — БД root:developers 0644, только sudo — факт из MEMORY Epic 45/46). SmartCache уже SQLite (user_version не поднимается) — проверить, нужно ли расширение. ⚙ Дизайн — Sections 63.1 (таблица throttle_state, атомарный UPSERT — T-459 тема 9, стена time.time, fail-open, рубильник) / 63.3 (миграция v3); ответ на вопрос 2 — 63.0; CB НЕ переносим; `_bot_replies` → таблица bot_replies (TTL+LRU, T-459 тема 8). |
| **D240** | **Чекап (R60-7):** CHECKUP_SYSTEM_PROMPT (R42-6) VERBATIM НЕ меняется; метрики памяти/диска подаются ДАННЫМИ (SQL-счётчики + `df`-фактура + журналы) в контекст чекапа; пулы R42-2/3/4/5 не трогать; токсичный стиль сохраняется (пользователь: «в том же токсичном стиле как сейчас»). ⚙ Дизайн — Section 64.5 (`services/memory_health.py`, data-секция `<memory_health>` в ТОМ ЖЕ сообщении); ответ на вопрос 7 — 63.0. |
| **D241** | **Ресёрч-темник T-459** (context7 → duckduckgo → exa): tiktoken русско-английские тексты (кодировка для deepseek-v4-flash), sendChatAction aiogram (typing, продление, BusinessConnection НЕ нужен), стриминг editMessageText (лимиты правок/flood), MMR, time-decay, дедуп эмбеддингов, сжатие векторов sqlite-vec (float16/int8), LRU+TTL, persistent cooldown/circuit-паттерны. Вердикты → @Architect (T-458, Sections 63–67). ⚙ Вердикты ПОЛУЧЕНЫ (plans/research_epic60.md, 2026-08-24) и вшиты в Sections 63–67: o200k_base (64.7), ChatActionSender.typing interval=5.0 (65.7), стриминг 1.0/3.0 сек + not-modified/retry_after (65.6), int8+float-реранк + rebuild (66.6), дедуп-канон ≥0.95 noop / 0.85–0.95 supersede / <0.85 add (64.1), MMR λ=0.6 fetch_k=20 (66.8), time-decay half_life=60 от last_confirmed_at (66.3), атомарный UPSERT (63.1), TTL+LRU lazy-sweep (66.5). Осталось открытым до реализации: сигнатура set_messageReaction (65.1, Q8 — T-459 тему не покрыл). |
| **D242** | **RESEARCH_HUMAN.md:** незакоммиченные правки (галочки `[х]` + комментарии пользователя, +39/−20) ВХОДЯТ в коммит Epic 60 (T-496); текст пользователя не редактировать, коммитить как есть; прод не затронут (файл только в репо). ⚙ Учтено в 67.6 (коммит T-496). |
| **D243** | **Канон R60-32 (п.20):** поведение уже в коде (токсичные ответы не сохраняются — Epic 46); фиксируется как правило в Section 67 + тесты-стражи БЕЗ изменения поведения. ⚙ Зафиксировано — Section 67.1 (стражи по пулам фраз). |
| **D244** | **Деплой (T-497):** SSH nik@198.46.175.136 /var/www/admin_bot; бэкапы `.bak.epic60` (БД/`info_text.md`/`.env` при правках); `git pull --ff-only`; при необходимости `nano .env` (без указания — НЕ менять); миграции (если есть) на остановленном боте; `systemctl restart admin_bot`; `systemctl status admin_bot`; 0 traceback; smoke (direct_chat, чекап с метриками, саммари). ⚙ Порядок деплоя — Section 67.6; миграция v3 на остановленном боте — 63.3; бэкап-директория — 64.3. |

### Каноны (НЕ трогать — 0 регрессий, 2360 тестов)

R50-4 CHAT_SYSTEM_PROMPT VERBATIM; R42-6 CHECKUP_SYSTEM_PROMPT VERBATIM (токсичный); R50-7/8 пулы кулдауна/ошибок DirectChat; R42-2/3/4/5 CHECKUP-пулы; R11 SUMMARY prompt; R46-2/R46-4 FACT_EXTRACT/GraphRAG промпты; UX-фразы R13 байт-в-байт; D224 канон-цепочка /info (5 мест: `DEFAULT_INFO_TEXT` = `info_text.md` = ARCHITECTURE 53.3 = backlog R44-1 fenced); media-папка; R17 секреты; прод `.env` без указания; бэкапы `.bak.epicNN`.

### Открытые вопросы (зафиксированы Шаг 1 PM; закрывает @Architect/@Researcher)

1. **tiktoken:** какая кодировка для deepseek-v4-flash (o200k_base vs cl100k_base), точность на кириллице, цена подсчёта — T-459.
2. **R60-1:** какие именно счётчики персистентны — все CooldownTracker (summary/factcheck/search/youtube/web/checkup/direct_chat/info) + DirectChat-бакет + MediaGroupCaptionBuffer? — T-458 (Section 63).
3. **R60-15 стриминг:** лимиты editMessageText (интервал правок, flood) для саммари-чанков — T-459.
4. **R60-16:** typing живёт ~5с — как продлевать (повторный sendChatAction по таймеру) и когда сбрасывать при таймауте LLM — T-459.
5. **R60-28 профили:** entity_type для карточек людей — CHECK ('user','topic','event','fact') уже содержит 'user'; нужна ли отдельная таблица/миграция — T-458 (Section 66).
6. **R60-25:** формат хранения сжатых векторов (sqlite-vec float16/binary quant) — T-459 (после T-465).
7. **R60-7:** граница «в том же сообщении vs несколько сообщений подряд» для чекап-отчёта с метриками — T-458 (Section 64).
8. **R60-10:** setMessageReaction в aiogram 3.29.1 (пин <4.0.0), падение реакции при удалённом сообщении — T-459.
9. **R60-33:** куда выносить `_BOTWORD_RE` — канон в word_lists.py или env-строка (regex в .env опасен) — T-458 (Section 67).
10. **R60-31 бюджеты:** применяются ли к direct_chat только или ко всем пайплайнам (context partitioning Epic 50 уже есть) — T-458.

### Задачи (T-458…T-499)

| # | Исполнитель | Приоритет | Зависимости | Суть (пункт → требование) | DoD |
|---|---|---|---|---|---|
| T-458 | @Architect | P0 | T-459 (параллельно) | Дизайн Sections 63 (Фаза A), 64 (Фаза B), 65 (Фаза C), 66 (Фаза D), 67 (Фаза E) в ARCHITECTURE.md; каноны-тексты дословно; закрыть вопросы 1–10 | Sections 63–67; каноны зафиксированы; вопросы закрыты; тест-правки перечислены дословно; diff --check чист  **DONE** |
| T-459 | @Researcher | P0 | нет | Ресёрч context7 → duckduckgo → exa по 9 темам (D241); вердикты → T-458 | Вердикты по 9 темам; источники с датами  **DONE** |
| T-460 | @Builder | P0 | T-458 | п.13/R60-1: персистентный троттлинг/кэш в БД с TTL; миграция user_version 2→3 | Счётчики не сбрасываются рестартом; тесты; 0 регрессий  **DONE** |
| T-461 | @Builder | P0 | T-458 | п.36/R60-2: per-chat asyncio-замок генерации direct_chat | Конкурентные тесты (прецедент Epic 35); порядок ответов  **DONE** |
| T-462 | @Builder | P1 | T-458 | п.52/R60-3: дедуп фактов при записи (add/update/skip) | Дубли не пишутся; тесты  **DONE** |
| T-463 | @Builder | P1 | T-458 | п.15/R60-4: логи сжатия «что во что» + свежий побеждает + «не подтверждено» | Журнал сжатия; тесты  **DONE** |
| T-464 | @Builder | P1 | T-458 | п.57/R60-5: бэкап БД раз в день + текстовый экспорт фактов | Бэкап-джоб + экспорт; тесты  **DONE** |
| T-465 | @Builder | P1 | T-458 | п.60/R60-6: кэш эмбеддингов (текст → вектор) | Повторный текст не дёргает API; TTL уважен; тесты  **DONE** |
| T-466 | @Builder | P1 | T-458 | п.58+43/R60-7: метрики здоровья памяти + расширенный чекап (память/диск/счётчики), токсичный стиль | Чекап отвечает метриками; канон/пулы не тронуты; тесты  **DONE** |
| T-467 | @Builder | P1 | T-458 | п.11/R60-8: бегущий конспект (80%, 20–30 дословно, лениво, БД+TTL) | Конспект в БД; тесты  **DONE** |
| T-468 | @Builder | P1 | T-459, T-458 | п.10/R60-9: лимиты в токенах tiktoken; саммари по времени (6 ч) сохранён | Токены вместо символов; 6-часовой таймер; тесты  **DONE** |
| T-469 | @Builder | P1 | T-458 | п.33+37/R60-10: пустой ответ → молчание + 🗿 (без заглушки/ошибки) во всех LLM-фичах | Реакция 🗿; заглушки нет; тесты  **DONE** |
| T-470 | @Builder | P1 | T-458 | п.38/R60-11: правка своего ответа → обновить цепочку | Цепочка правдива после правок; тесты  **DONE** |
| T-471 | @Builder | P2 | T-458 | п.25/R60-12: молчание после N кулдаунов, дефолт 5 | После 5 кулдаунов — молчание; тесты  **DONE** |
| T-472 | @Builder | P2 | T-458 | п.28/R60-13: стилевые якоря (2–3 недавних ответа секцией) | Секция в контексте; тесты  **DONE** |
| T-473 | @Builder | P2 | T-458 | п.9/R60-14: /clear /persona /tone /forget | 4 команды; тесты  **DONE** |
| T-474 | @Builder | P1 | T-459, T-458 | п.31/R60-15: стриминг ТОЛЬКО саммари (send+editMessageText) | Саммари «растёт»; тесты  **DONE** |
| T-475 | @Builder | P2 | T-459, T-458 | п.30/R60-16: typing-индикатор для всех LLM-фич; без паузы; таймаут → сброс | Индикатор во всех фичах; сброс при таймауте; тесты  **DONE** |
| T-476 | @Builder | P2 | T-458 | п.32/R60-17: temperature-пресеты точный/сбалансированный/болтливый | 3 пресета + дефолт; тесты  **DONE** |
| T-477 | @Builder | P2 | T-458 | п.34/R60-18: настроение → тон; системный промпт НЕ менять | Промпт verbatim; тесты  **DONE** |
| T-478 | @Builder | P2 | T-458 | п.14/R60-19: «забудь/сбрось» + карточки + забыть/перезаписать факт | Карточки + управление фактами; тесты  **DONE** |
| T-479 | @Builder | P2 | T-462 | п.53/R60-20: веса значимости 0..1 (выдача + TTL) | Веса в БД и выдаче; тесты  **DONE** |
| T-480 | @Builder | P2 | T-462 | п.54/R60-21: слияние эпизодов в общие факты | Слияние в фоне; тесты  **DONE** |
| T-481 | @Builder | P2 | T-458 | п.55/R60-22: time-decay связей графа | Коэффициент давности; тесты  **DONE** |
| T-482 | @Builder | P2 | T-479 | п.56/R60-23: квота памяти на человека + вытеснение | Квота; тесты  **DONE** |
| T-483 | @Builder | P2 | T-479 | п.59/R60-24: автоочистка TTL+LRU в связке с таймером | Использование продлевает жизнь; тесты  **DONE** |
| T-484 | @Builder | P2 | T-465 | п.61/R60-25: сжатие векторов float16/int8 (после п.60) | Сжатие + backfill; тесты  **DONE** |
| T-485 | @Builder | P2 | нет | п.62/R60-26: «золотые вопросы» + процедура прогона | Список + процедура  **DONE** |
| T-486 | @Builder | P2 | T-458 | п.16/R60-27: MMR при подборе фактов | MMR в RAG; тесты  **DONE** |
| T-487 | @Builder | P2 | T-462 | п.17/R60-28: профили-карточки + просмотр/редактирование | Карточки + просмотр/правка; тесты  **DONE** |
| T-488 | @Builder | P2 | T-462 | п.18/R60-29: протухание, свежий побеждает при конфликте | Конфликт → свежий; тесты  **DONE** |
| T-489 | @Builder | P2 | T-462 | п.19/R60-30: периодический пересмотр (склейка/выброс) | Джоб пересмотра; тесты  **DONE** |
| T-490 | @Builder | P2 | T-458 | п.12/R60-31: бюджеты контекста (5/30/20/15/15–25/10%) | Бюджеты в сборке; тесты  **DONE** |
| T-491 | @Builder | P0-правило | T-458 | п.20/R60-32: канон «мусор не пишем» + стражи | Канон в Section 67; стражи; 0 регрессий  **DONE** |
| T-492 | @Builder | P1-правило | T-458 | п.49/R60-33: `_BOTWORD_RE` (direct_chat.py:39-41) → конфиг | Паттерн в конфиге; тесты  **DONE** |
...
1. **Объём:** 35 требований / 42 задачи — фазы идут последовательно (A → E); риск растянуть релиз — приоритеты фаз жёсткие (P0-фаза A первой), деплой один (D235).
2. **Миграция user_version 2→3** — на остановленном боте, sudo (БД root:developers 0644), идемпотентность обязательна (прецеденты Epic 45/46, 50).
3. **Канон-касания:** R60-13/18/31 меняют СБОРКУ контекста, а не промпты — риск незаметной порчи VERBATIM-эталонов (55.7.1–55.7.5, R50-4, R11); каждый таск обязан прогнать эталон-тесты.
4. **R60-15 стриминг:** Telegram-лимиты правок/flood — вердикт T-459; при блокере — деградировать до обычной отправки чанков (существующее поведение).
5. **R60-16 индикатор:** «вечно печатает» при зависшем LLM — обязательный сброс по таймауту (требование пользователя).
6. **R60-10:** пустой ответ раньше шёл в UX-заглушки (R13/пулы) — теперь реакция; проверить все 7 LLM-пайплайнов, R13-эталоны не сломать.
7. **R60-33:** вынос regex в конфиг — риск рассинхрона word-boundary-семантики (тесты 61.5 обязательны).
8. **RESEARCH_HUMAN.md:** коммитить как есть (D242) — не «улучшать» текст пользователя.
9. **R60-28 профили:** профиль-карточки людей — чувствительные данные в графе; не публиковать в /info, только локальный просмотр/редактирование (R17).

### Файлы (планируемые)

Код: `services/database.py` (миграция v3, новые таблицы), `services/direct_chat_service.py` + `handlers/direct_chat.py`, `services/smartmodule_utils.py` (CooldownTracker), `services/summary_memory.py`, `services/smart_cache.py`, `services/llm_client.py`, `services/summary_generator.py`, `services/checkup_service.py` + `handlers/checkup.py`, `services/system_logs_fetcher.py` (df/метрики), `config/settings.py` + `.env.example`, `word_lists.py` (R60-33), новые сервисы по Sections 63–67; тесты (+стражи канонов); `requirements.txt` (+tiktoken по вердикту T-459). Доки: `README.md` (T-495), `plans/ARCHITECTURE.md` (Sections 63–67), `plans/RESEARCH_HUMAN.md` (коммит как есть, D242), `plans/backlog.md`/`plans/board.md`/`plans/MEMORY.md`.

**НЕ трогать:** media-папка, `plans/RESEARCH.md`, каноны (см. выше), прод `.env` без указания, mp4 untracked (`danger_boom_gif-03.mp4` — в коммит НЕ включать), прод-БД (только миграция по Sections).
...
## Epic 61: Хотфикс: чекап-метрики + tiktoken на проде — 2026-08-24 ✅ DEPLOYED & CLOSED (v2.43.1, коммит 352afa1, прод PID 1072251, tiktoken 0.14.0, 2617 тестов; пользовательский запрос, пост-деплой дефекты v2.43.0)

> **Цель:** Устранить два прода-дефекта, выявленных по логам пользователя сразу после деплоя
> v2.43.0 (`9a47567`, PID 1071436):
> **Дефект 1 — чекап без метрик памяти:** `[checkup] input truncated | chars=20506 -> 12000`.
> Причина: `services/checkup_service.py:57-65` — секция метрик `<memory_health>` аппендится
> ПОСЛЕ логов, затем единый потолок `CHECKUP_MAX_INPUT_SYMBOLS=12000` режет хвост → метрики
> ВСЕГДА теряются при длинных логах (journalctl-фолбэк отдаёт ~20.5к символов). Нарушает
> R60-7 (пп.43+58: метрики должны быть в чекапе). Попутно: строка 59 делает
> `escape_xml_text(metrics)`, а строка 66 повторно `escape_xml_text(body)` — двойное
> экранирование метрик.
> **Дефект 2 — tiktoken не установлен на проде:** WARNING «tiktoken unavailable — chars×0.3
> fallback» (`services/token_counter.py:43`). Причина: `tiktoken>=0.7.0` есть в
> requirements.txt, но venv сервера не обновлялся при деплое (git pull не ставит пакеты).
> **Требование пользователя по сути:** метрики здоровья памяти ДОЛЖНЫ присутствовать в чекапе;
> токены должны считаться tiktoken'ом.
> **Исполнители:** @Architect (T-500), @Builder (T-501), @DevOps (T-502), @Reviewer (T-503),
> @Docs/@PM (T-504). Без @Orchestrator.
> **Target:** v2.43.1 (patch, hotfix).
> **Baseline:** прод v2.43.0 (`9a47567`, PID 1071436), 2611 тестов, Epics 1–60 ALL CLOSED & DEPLOYED.

### Требования (R61-1…R61-3 — обязательный чек-лист)

| # | Требование | Приоритет | Комментарий пользователя |
|---|-----------|-----------|--------------------------|
| **R61-1** | **Метрики памяти гарантированно в чекапе:** порядок сборки user-контента чекапа — сначала собрать секцию `<memory_health>`, зарезервировать её длину, ЛОГИ обрезать до `CHECKUP_MAX_INPUT_SYMBOLS − len(секция метрик)` → метрики выживают всегда (логи 20к+ и короткие); снять двойное экранирование (escape ровно один раз — на финальной обёртке); лог трекации показывает реальные значения. CHECKUP_SYSTEM_PROMPT (R42-6) VERBATIM, пулы R42-2/3/4/5 не трогать. | **P0** | Метрики здоровья памяти ДОЛЖНЫ присутствовать в чекапе (нарушение R60-7). |
| **R61-2** | **tiktoken реально работает на проде:** venv обновлён (`pip install -r requirements.txt` или точечно tiktoken), `import tiktoken` + `get_encoding('o200k_base')` проходят; WARNING «chars×0.3 fallback» исчезает; лимиты считаются токенами (R60-9). | **P0** | Токены должны считаться tiktoken'ом, а не fallback'ом. |
| **R61-3** | **Деплой v2.43.1 + верификация:** пуш фикса → pull ff на проде → restart → новый PID, journalctl 0 traceback, WARNING token_counter исчез, чекап отвечает метриками; полный pytest 0 регрессий (база 2611); README/MEMORY/board актуализированы кратко. | **P0/P1** | Хотфикс-релиз v2.43.1. |

### PM Decisions (зафиксированы 2026-08-24, Шаг 1 PM)

| # | Решение |
|---|---------|
| **D250** | **Версия: v2.43.1 (patch, hotfix).** Прецеденты: v2.31.1 (Epic 34), v2.31.2 (Epic 35) — прода-хотфиксы. Нумерация: Epic 61, R61-1…R61-3, D250+, задачи T-500…T-504 (после T-499). Дизайн — минимальная правка Section 64.5 ARCHITECTURE.md (не новые секции): порядок «метрики сначала + резерв длины», escape один раз на финальной обёртке. Каноны НЕ трогать: R42-6 CHECKUP_SYSTEM_PROMPT VERBATIM, пулы R42-2/3/4/5, R50-4, R11, D224. Один релиз в конце (деплой T-502, включая обновление venv — git pull не ставит пакеты, корень Дефекта 2). ⚙ Дизайн — Section 64.5 (правка хотфикса): новый порядок сборки payload (метрики сначала → потолок метрик min(len,2000) → бюджет логов = MAX_INPUT − len(секции) → scrub/обрезка логов с честным WARNING → ОДИН escape на обёртке) + тест-план (а)-(е) зафиксированы в plans/ARCHITECTURE.md (T-500). |

### Задачи (T-500…T-504)

| # | Исполнитель | Приоритет | Зависимости | Суть | DoD |
|---|---|---|---|---|---|
| T-500 ✅ DONE | @Architect | P0 | нет | Минимальная правка дизайна Section 64.5 (plans/ARCHITECTURE.md): порядок сборки user-контента чекапа — сначала собрать секцию метрик, зарезервировать её длину, ЛОГИ обрезать до (CHECKUP_MAX_INPUT_SYMBOLS − len(секция метрик)), метрики гарантированно выживают; снять двойное экранирование (escape только один раз — на финальной обёртке); лог трекации правдивый. Не менять R42-6/R42-2 и канон-пулы | Section 64.5 обновлена; каноны зафиксированы; diff --check чист |
| T-501 ✅ DONE | @Builder | P0 | T-500 | Реализовать фикс в services/checkup_service.py + тесты: (а) логи 20к+ → метрики присутствуют в финальном payload в пределах лимита; (б) логи короткие → без обрезки, метрики на месте; (в) metrics_enabled=false / db=None → ровно старое поведение; (г) collect_metrics падает → старое поведение + WARNING; (д) отсутствие двойного экранирования (амперсанды/угловые скобки в метриках экранируются ровно один раз); (е) лог truncation показывает реальные значения | Все 6 сценариев покрыты тестами; полный pytest 0 регрессий (2611 база) |
| T-502 ✅ DONE | @DevOps | P0 | T-501, T-503 | На проде: venv/bin/pip install -r requirements.txt (или точечно tiktoken), верификация `venv/bin/python -c "import tiktoken; tiktoken.get_encoding('o200k_base')"`; после пуша фикса — pull ff, рестарт, проверка: 0 traceback, WARNING token_counter исчез, новый PID | Прод v2.43.1, новый PID, 0 traceback, tiktoken работает |
| T-503 ✅ DONE | @Reviewer | P0 | T-501 | Ревью фикса + полный прогон | APPROVED; 0 регрессий; diff --check чист |
| T-504 ✅ DONE | @Docs/@PM | P1 | T-502 | README/MEMORY/board актуализация после деплоя (кратко) | Доки актуальны; Epic 61 CLOSED |

### Риски (Epic 61)

1. **Резерв длины:** если секция метрик сама разрастётся — бюджет логов может стать слишком малым; ограничить размер секции метрик разумным потолком (дизайн T-500).
2. **venv на проде:** pip install требует остановки/осторожности (root:developers права, прецедент Epic 45/46 миграций); ставить до или сразу после рестарта.
3. **Канон-касания:** правка сборки контента чекапа — риск незаметной порчи VERBATIM-эталонов; эталон-тесты обязательны.
4. **Escape-семантика:** снятие двойного экранирования меняет байты payload — убедиться, что существующие тесты на escape обновлены осознанно, а не «подогнаны».

### Файлы (планируемые)

Код: `services/checkup_service.py` (порядок сборки + escape + лог truncation). Доки: `plans/ARCHITECTURE.md` (Section 64.5), `README.md`/`plans/MEMORY.md` (кратко), `plans/backlog.md`/`plans/board.md`. Прод: venv (pip install), рестарт admin_bot.

**НЕ трогать:** каноны (см. выше), прод `.env` без указания, media-папка, mp4 untracked, прод-БД.

---

**Статус: Epic 61 — ✅ DEPLOYED & CLOSED (2026-08-24).** Задачи **T-500…T-504 → ALL DONE.** Коммит `352afa1` запушен (`e6bdbf2..352afa1` origin/master); деплой успешен: pip установил tiktoken 0.14.0 в venv прода (верификация o200k_base OK), pull ff `9a47567..352afa1`, рестарт OK — прод **PID 1072251** (был 1071436), journalctl **0 traceback / 0 WARNING «tiktoken unavailable» / 0 error**. Тесты 2617 passed / 0 failed, ревью APPROVED. R61-1…R61-3 выполнены. База была: прод v2.43.0 (`9a47567`, PID 1071436), 2611 тестов. Epics 1–61 ALL CLOSED & DEPLOYED. Без @Orchestrator.**
**Date: 2026-08-24**
...
> Это чисто конфигурационное изменение (patch, v2.43.2 chore) — код не меняется:
> `services/llm_client.py` провайдер-агностик (OpenAI-compatible API), просто меняются
> `LLM_BASE_URL` / `LLM_MODEL_NAME` / `LLM_API_KEY`.
> **Исполнители:** @DevOps (T-505…T-510), @Reviewer (T-508-B). Без @Orchestrator.
> **Target:** v2.43.2 (patch, chore).
> **Baseline:** прод v2.43.1 (`352afa1`, PID 1072251), 2617 тестов (0 регрессий).

### Факты Шага 0 (инвентаризация конфигурации)

- **Текущие значения:** `LLM_API_KEY=sk-…` (apinet.cloud, старый ключ — не публикуется, R17),
  `LLM_BASE_URL=https://apinet.cloud/v1`, `LLM_MODEL_NAME=deepseek-v4-flash`.
- **Целевые значения (заданы пользователем):** `LLM_API_KEY=sk-or-v1-…` (OpenRouter, ключ задан
  пользователем — не публикуется, R17), `LLM_BASE_URL=https://openrouter.ai/api/v1`,
  `LLM_MODEL_NAME=stealth/ox-alpha`.
- **`services/llm_client.py`** — OpenAI-compatible (`POST {base_url}/chat/completions` + `embeddings`),
  использует `settings.LLM_BASE_URL` / `LLM_MODEL_NAME` / `LLM_API_KEY` — **никаких изменений кода не требуется**.
...
| # | Требование |
|---|-----------|
| **R62-1** | **Конфиг .env → OpenRouter:** `LLM_API_KEY=sk-or-v1-…`, `LLM_BASE_URL=https://openrouter.ai/api/v1`, `LLM_MODEL_NAME=stealth/ox-alpha` в прод `.env` (бэкап `.env.bak.epic62` ДО правки; ключ не публиковать в планах/логах, R17). Первичные `LLM_API_KEY`/`LLM_BASE_URL`/`LLM_MODEL_NAME` (apinet.cloud) — заменяются. |
| **R62-2** | **`.env.example` + `config/settings.py`:** `LLM_BASE_URL` default → `https://openrouter.ai/api/v1`, `LLM_MODEL_NAME` default → `stealth/ox-alpha`; `LLM_API_KEY` остаётся placeholder `your_key_here`; комментарии/канон обновлены (байт-в-байт, без утечки ключа). |
| **R62-3** | **Код не меняется:** `services/llm_client.py` провайдер-агностик (OpenAI-compatible API); circuit breaker (Epic 53) и ретраи (Epic 47) работают через llm_client без изменений. |
| **R62-4** | **Embeddings НЕ меняются:** `EMBEDDING_MODEL_NAME=gemini-embedding-001` через apinet.cloud/Vertex — отдельный путь (R46-5), не затрагивается сменой чат-провайдера. |
| **R62-5** | **Тесты + верификация:** полный pytest — **0 регрессий (база 2617)**; `git diff --check` чист (исключить `.env`); smoke на проде: 0 traceback, бот отвечает (LLM-путь живой через OpenRouter); `curl` с сервера к `https://openrouter.ai/api/v1/chat/completions` с Bearer-ключом → 200 (опционально, T-416-аналог). |
| **R62-6** | **Деплой:** бэкап `.env.bak.epic62`; git pull ff; правка `.env` на проде (T-506); `systemctl restart admin_bot` → active (running), новый PID; `journalctl -n 50` — 0 traceback. |

### PM Decisions (зафиксированы 2026-08-24, Шаг 1 @PM)

| # | Решение |
|---|---------|
| **D251** | **Epic 62 — переключение LLM-провайдера na OpenRouter (P0), target v2.43.2 (chore, patch).** Обоснование patch: код/tests не меняются — только конфиг (`.env` + `.env.example` + `settings.py`-дефолты). Задачи **T-505…T-510**. Без @Orchestrator. |

### Задачи

### T-505 (@DevOps) — Конфиг: .env → OpenRouter (R62-1, D251)
...
### T-506 (@DevOps) — .env.example + config/settings.py: дефолты и канон (R62-2, D251)
...
### T-507 (@Reviewer) — git diff --check + ревью конфига (R62-5)
...
### T-508 (@QA) — Тесты: 0 регрессий (R62-5)
...
### T-509 (@DevOps) — Коммит + пуш конфига (R62-5)
...
### T-510 (@DevOps) — Деплой на прод (R62-1/R62-6, D251)
...
### env-переменные (Epic 62)

| Переменная | Было (apinet.cloud) | Стало (OpenRouter) | Где |
|-----------|---------------------|---------------------|-----|
| `LLM_API_KEY` | `sk-…` (старый, apinet.cloud) | `sk-or-v1-…` (OpenRouter, ключ задан пользователем) | прод `.env` (бэкап `.env.bak.epic62`); локальный `.env` |
| `LLM_BASE_URL` | `https://apinet.cloud/v1` | `https://openrouter.ai/api/v1` | `.env` (прод+локал), `.env.example`, `config/settings.py` (default) |
| `LLM_MODEL_NAME` | `deepseek-v4-flash` | `stealth/ox-alpha` | `.env` (прод+локал), `.env.example`, `config/settings.py` (default) |

### Риски (Epic 62)

1. **Утечка ключа OpenRouter** — `sk-or-v1-…` пишется ТОЛЬКО в прод `.env`; в `backlog.md`/`board.md`/`ARCHITECTURE.md` — только «ключ задан пользователем» (R17); `.env` в `.gitignore` — не попадает в коммит.
2. **Embeddings ломаются** — `EMBEDDING_MODEL_NAME=gemini-embedding-001` НЕ меняется; если OpenRouter не поддерживает `/v1/embeddings`, embeddings остаются на apinet.cloud (отдельный путь, R46-5) — `EMBEDDING_MODEL_NAME` и `LLM_FALLBACK_*` (Epic 53) не затрагиваются.
3. **circuit breaker / ретраи (Epic 53/47)** — работают через `llm_client._post` с `settings.LLM_*`; новые defaults в settings.py → старая логика без изменений; CB-фильтр в `direct_chat_service` — тоже без изменений.
4. **0 регрессий (база 2617)** — config-only change: tests/test_settings_helpers.py проверяет настройки; smoke на проде после деплоя.

### Файлы

**`config/settings.py`** (LLM_BASE_URL/LLM_MODEL_NAME defaults → OpenRouter), **`.env`** (прод, правка через бэкап), **`.env.example`** (LLM_BASE_URL/LLM_MODEL_NAME → OpenRouter, комментарий), **`plans/backlog.md`** (этот эпик), **`plans/board.md`** (In Progress). `services/llm_client.py` — НЕ трогать (провайдер-агностик).
...
> **Цель:** Блокер Epic 69 снят — пользователь предоставил TELEGRAM_API_ID и TELEGRAM_API_HASH.
> Добить активацию Epic 66 (Cobalt Downloader + Local Bot API) на проде
> nik@198.46.175.136 (/var/www/admin_bot).
>
> **⚠️ ПОРЯДОК КРИТИЧЕН (наследие Epic 69):** `DOWNLOAD_ENABLED=True` СТРОГО ПОСЛЕДНИМ ШАГОМ,
> только после рабочего telegram-bot-api (import-time сессия через TelegramAPIServer.from_base
> — иначе бот ляжет при старте). Ключи — ТОЛЬКО в прод .env (секреты НЕ в git).
>
> **Шаги:** ключи в прод .env → `docker compose up -d telegram-bot-api` → проверить логин
> (FDCA логин в логах контейнера) и `curl :8081` → и только затем DOWNLOAD_ENABLED=True →
> restart admin_bot → smoke «скачай <ссылка>». Коммитит @DevOps в конце цикла. Без @Orchestrator.

### Требования и решения
- **R70-1** — TELEGRAM_API_ID/TELEGRAM_API_HASH вписаны в прод `.env` (бэкап `.bak.epic70`); секреты НЕ попадают в git/планы/коммиты.
- **R70-2** — telegram-bot-api поднят (`docker compose up -d telegram-bot-api`), логин подтверждён (FDCA логин в логах контейнера), `curl :8081` отвечает.
- **R70-3** — `DOWNLOAD_ENABLED=True` в прод `.env` СТРОГО последним шагом после рабочего telegram-bot-api; рестарт admin_bot; smoke «скачай <ссылка>» на проде.

### Задачи
- [ ] T-547 (@DevOps, P0): прод .env — вписать TELEGRAM_API_ID/TELEGRAM_API_HASH (бэкап `.env.bak.epic70`; секреты НЕ в git); `docker compose up -d telegram-bot-api` (+ cobalt, если не поднят); проверка: FDCA логин в логах контейнера, `curl :8081` отвечает. **DoD:** telegram-bot-api жив и залогинен
- [ ] T-548 (@DevOps, P0, ←T-547): `DOWNLOAD_ENABLED=True` в прод .env СТРОГО ПОСЛЕДНИМ ШАГОМ (после рабочего telegram-bot-api); `systemctl restart admin_bot`; status/journalctl 0 traceback; smoke на проде: «скачай <ссылка>» → видео-реплай (выбор качества). **DoD:** скачивание работает на проде

---

## Epic 71: Обновление /info — новые фичи в справке — 2026-08-26 🆕 Шаг 1 (PM ✅) — P0

> **Цель:** Добавить в /info информацию о новых фичах: скачивание видео через «скачай <ссылка>»
> и авторасшифровка голосовых и кружочков — СОХРАНЯЯ существующий стиль справки
> (ироничные разделы `<h2>`, триггеры жирным курсивом).
>
> **⚠️ КРИТИЧНО (research @Memory, D224):** канон-цепочка 5 мест синхронно одним коммитом:
> 1. `DEFAULT_INFO_TEXT` в `services/info_service.py` (КАНОН)
> 2. файл `info_text.md`
> 3. `plans/ARCHITECTURE.md` Section 53.3 (ОБА блока)
> 4. plain-версия в backlog («Канон R44-1», fenced-блок БЕЗ тегов)
> 5. счётчики тегов в `tests/test_info_service.py` И `tests/test_info_handlers.py`
> Дисциплина байт-в-байт во всех местах.
>
> **Примечание для команды:** пример пользователя «- /summary по Х» — иллюстрация ФОРМАТА
> кратких строк, НЕ требование слеш-команд (интро канона гласит «никаких слеш-команд»);
> существующий стиль справки сохраняем по явному указанию пользователя. Без @Orchestrator.

### Требования и решения
- **R71-1** — Стиль справки сохранён: ироничные разделы `<h2>`, триггеры жирным курсивом, интро «никаких слеш-команд»; НЕТ новых слеш-команд в тексте.
- **R71-2** — Новое содержание: раздел про скачивание видео (триггер «скачай <ссылка>») + раздел про авторасшифровку голосовых и кружочков; формат кратких строк как в существующих разделах.
- **R71-3** — Канон-цепочка 5 мест синхронно одним коммитом (D224): DEFAULT_INFO_TEXT = info_text.md = ARCHITECTURE 53.3 (оба блока) = backlog «Канон R44-1» (plain) = счётчики тегов в обоих тест-файлах; полный pytest 0 регрессий.

### Задачи
- [ ] 👤 T-549 (@Architect, P0): архитектурный эталон нового текста /info ДО реализации — html-версия (стиль существующих разделов: `<h2>` иронично, триггеры жирным курсивом, без слеш-команд) + plain-эталон для backlog «Канон R44-1»; Section 53.3 (оба блока); тест-план с новыми счётчиками тегов. **DoD:** эталон зафиксирован в ARCHITECTURE.md, READY FOR BUILDER
- [ ] T-550 (@Builder, P0, ←T-549): реализация 5 мест канона синхронно одним коммитом: `DEFAULT_INFO_TEXT` (services/info_service.py), `info_text.md`, ARCHITECTURE 53.3 (оба блока), backlog «Канон R44-1» (plain), счётчики тегов в `tests/test_info_service.py` и `tests/test_info_handlers.py`; полный pytest 0 регрессий. **DoD:** strip-теги всех мест совпадают байт-в-байт, тесты зелёные
- [ ] T-551 (@Reviewer, P0, ←T-550): ревью Epic 71 — байт-в-байт по всем 5 местам канона, стиль справки сохранён (ироничные `<h2>`, триггеры жирным курсивом, никаких слеш-команд), счётчики тегов корректны, diff --check чист. **DoD:** APPROVED
...
## Epic 73: Кулдаун скачивания (30m→5m + touch-after-probe) + диагностика «ошибка после выбора качества» — 2026-08-26 🆕 Шаг 1 (PM ✅) — P0

> **Цель:** Два трека после v2.47.1 (research @Memory, цикл 2026-08-26b). Target: **v2.47.2**.
>
> **A. Кулдаун/конкурентность скачивания (P0, код):**
> Факты research: кулдаун dual-layer per-(chat,user) через `make_cooldown("video_download")`
> (handlers/video_download.py:57/:76, persistent в throttle_state); глобальный asyncio.Lock
> БЕЗ очереди — мгновенный отказ «занят» (`DownloadBusyError`, :123–125, BUSY-попап show_alert :274);
> пропускная способность под локом ~11–120 загрузок/час; DOWNLOAD_COOLDOWN=0 кодом корректно
> обработан, но открывает флуд параллельных probe. **НАЙДЕННЫЙ БАГ:** `cooldown_touch` вызывается
> ДО probe (:177) → неудачная попытка сжигает юзеру весь кулдаун.
>
> **Ожидаемое направление решения (@Architect формулирует финал):**
> 1. `DOWNLOAD_COOLDOWN` дефолт/прод **~5m вместо 30m**;
> 2. перенести `cooldown_touch` ПОСЛЕ успешного probe (fail не сжигает кулдаун);
> 3. очередь НЕ строить (избыточна при текущей нагрузке);
> 4. BUSY-попап остаётся защитой от конкуренции.
>
> **B. Диагностика пользовательского бага «ошибка после выбора качества» (P0, серверная):**
> По коду НЕ воспроизводим: probe вызывается ровно один раз ДО показа кнопок
> (:182 одиночная ссылка, :201 multi через `_safe_probe`); колбэк `vd:<height>` yt-dlp не зовёт
> вообще (download = чистый Cobalt). Ошибка из лога пользователя имеет dt/pid старого процесса
> v2.47.0 (pid 1141251, ДО фикса прокси Epic 72). **Кодовых правок скорее всего НЕ требуется.**
> Требуется серверная диагностика @DevOps: свежие логи journalctl, маркер
> `[videodl] probe | proxy=set` (video_downloader.py:100–101), полный сквозной smoke «скачай <yt>».
>
> Порядок: @Architect (решение ДО реализации) → @Builder → @Reviewer; @DevOps — диагностика
> параллельно, деплой в конце. **НИ ОДНОЙ задачи на @Orchestrator.**

### Архитектурное решение
- [ ] 👤 T-559 (@Architect, P0): архитектурное решение по кулдауну/touch ДО реализации — зафиксировать: новое значение DOWNLOAD_COOLDOWN (дефолт settings.py:677 + .env.example:456 + README:517), точку вызова cooldown_touch (после успешного probe, до входа в лок/скачивание — точную строку определить по handlers/video_download.py), отказ от очереди (обоснование), сохранение BUSY-попапа; обновить Section 70 ARCHITECTURE.md (~14414 D264); тест-план (fail-probe не жжёт кулдаун, success жжёт, 0=disabled без флуда probe — решение). **DoD:** решение зафиксировано в ARCHITECTURE.md, READY FOR BUILDER

### Реализация
- [ ] T-560 (@Builder, P0, ←T-559): реализация touch-after-probe — перенести `cooldown_touch(_cooldown, chat_id, user_id)` из :177 на позицию ПОСЛЕ успешного probe (по решению T-559); DOWNLOAD_COOLDOWN дефолт 30m→5m синхронно в `config/settings.py`, `.env.example`, README.md; тесты: (1) неудачный probe → кулдаун НЕ затронут, юзер может ретраить сразу; (2) успешный probe → кулдаун активен; (3) BUSY-отказ не трогает кулдаун; (4) дефолт 5m в settings; полный pytest 0 регрессий. **DoD:** fail не сжигает кулдаун, тесты зелёные

### Серверная диагностика (параллельно, кода не касается)
- [ ] T-561 (@DevOps, P0): диагностика бага «ошибка после выбора качества» НА СЕРВЕРЕ nik@198.46.175.136 — journalctl за период после деплоя v2.47.1: WARNING 'probe failed' с pid != 1141251 (старый процесс v2.47.0); маркер `[videodl] probe | proxy=set` (подтверждение активного прокси в probe); риск-режим 'probe timeout' при медленном прокси (таймаут 20с); вердикт: воспроизводится ли баг на v2.47.1; полный сквозной smoke «скачай <yt-ссылка>» на проде. Кодовых правок НЕ делать — только диагноз + отчёт. **DoD:** вердикт по багу (воспроизводим/не воспроизводим) с логами, smoke зелёный

### Ревью
- [ ] T-562 (@Reviewer, P0, ←T-560): ревью Epic 73 — cooldown_touch строго после probe (не раньше, не дважды); BUSY/флуд-поведение не деградировало; канон Section 70 = коду; дефолты синхронны в 3 местах (settings/.env.example/README); существующие кулдауны других хендлеров не затронуты; diff --check чист; полный pytest APPROVED. **DoD:** APPROVED
...
> **Цель:** Устранить инфраструктурную причину прод-бага Epic 75: устойчивый мягкий бан
> googlevideo старого exit IP xray **195.181.173.207/.208** (AS60068) → заменить outbound
> в `/usr/local/etc/xray/config.json` на новую VLESS Reality gRPC ноду пользователя.
> Чисто серверная ops-задача (@DevOps), код НЕ трогается, git НЕ затрагивается.
>
> **Контекст (research @Memory 2026-08-26e):** бинарь `/usr/local/bin/xray` v26.3.27,
> systemd `xray.service` (drop-in restart.conf); inbound http 127.0.0.1:10808 с Basic Auth
> через settings.accounts (эмпирика: поле users молча игнорируется — при ротации НЕ ТРОГАТЬ);
> креды дублируются в `/root/.xray_local_creds`. Оба потребителя (.env
> YOUTUBE_TRANSCRIPT_PROXY_URL и docker-compose COBALT_HTTP_PROXY) НЕ меняются —
> endpoint прокси остаётся прежним, меняется только outbound.
>
> **Безопасность (R17):** все значения новой ноды (uuid, sni, publicKey, shortId,
> serviceName) существуют ТОЛЬКО на сервере — в git/репо/логи отчёта не попадают.

### Ротация outbound + верификация
- [ ] T-572 (@DevOps, P0): ротация исходящего IP xray на проде nik@198.46.175.136. Шаблон outbound (значения заполнять ТОЛЬКО на сервере, R17): protocol vless → settings.vnext[{address, port:443, users[{id:<uuid>, encryption:none, flow:"", level:0}]}] → streamSettings{network:"grpc", security:"reality", realitySettings{serverName:<sni>, fingerprint:"firefox", publicKey:<pbk>, shortId:<sid>}, grpcSettings{serviceName:<svc>, multiMode:false}}; flow ПУСТОЙ (vision/tcp only), mux выключен; pbk в v26.x = publicKey (если `-test` отвергнет — поле password, сверить эмпирически). Шаги: (1) бэкап `/usr/local/etc/xray/config.json` (+ `.env`, `.bak.rotation`); (2) правка outbound ТОЛЬКО на сервере (inbound/accounts НЕ трогать); (3) валидация `xray run -test -c /usr/local/etc/xray/config.json`; (4) `systemctl restart xray` + journalctl чистый; (5) верификация по чеклисту Epic 40: ipify через прокси = НОВЫЙ IP ≠ 195.181.173.x и ≠ 198.46.175.136; негатив-тест без Basic Auth → HTTP 407; smoke YouTube-движка (`/tmp/epic39_verify.py` ≥3/4 через прокси); smoke cobalt tunnel с ненулевым файлом (POST → tunnel → размер >0). Rollback: восстановить бэкап конфига + restart xray.service. Отчёт PM: новый IP (или маска), результаты чеклиста. **DoD:** новый exit IP подтверждён ipify, `-test` чистый, 407 негатив зелёный, YouTube-smoke ≥3/4, cobalt tunnel файл ненулевого размера, xray.service active, секреты не всплыли
- [ ] T-573 (@DevOps, P0): ротация outbound xray на ноду Нью-Йорк — продолжение Epic 76 после итога T-572 (новая нода ams.298qu… вышла через ТУ ЖЕ забаненную googlevideo подсеть 195.181.173.0/24). VLESS Reality gRPC от того же провайдера; значения ТОЛЬКО на сервере R17 (uuid/sni/pbk/sid/serviceName не в git/логи). **ОБЯЗАТЕЛЬНЫЙ пре-чек ДО правки конфига:** `curl -x http://<creds>@127.0.0.1:10808 https://api.ipify.org` через ноду → exit IP должен быть ВНЕ 195.181.173.0/24; если внутри подсети — зафиксировать в отчёте и НЕ тратить время на smoke (прецедент MEMORY 2026-08-26). Шаги при прохождении пре-чека: бэкап `config.json.bak.rotation-2608` (уже существует) → замена outbound → `xray run -test` → restart xray.service. **Гейт успеха:** cobalt tunnel файл >0 байт (POST → tunnel → размер >0); доп: ipify подтверждает новый IP, негатив-тест 407. **Rollback:** `/usr/local/etc/xray/config.json.bak.rotation-2608` + restart xray.service. Отчёт PM: exit IP (маска), вердикт пре-чека, результат гейта
...
> **Цель:** YouTube-скачивание сломано структурно: современные DASH-форматы YouTube под
> SABR/PO-Token enforcement отдают пустой стрим ДАЖЕ с чистым IP (легаси работает).
> Research-вердикт: cobalt 11.7.1 — последняя версия, проект спит, проблему не решает
> (статический poToken). yt-dlp уже решил системно через PO Token Provider Framework
> (плагин `bgutil-ytdlp-pot-provider`, GVS-токен per-video) — yt-dlp УЖЕ в проекте.
>
> **Решение:** YouTube-ссылки качать ЛОКАЛЬНЫМ yt-dlp (прокси из существующего
> `build_ytdlp_base_opts()` + POT-плагин), Cobalt оставить для остальных платформ
> (VK/Rutube/TikTok и т.д.). Инфраструктура: ffmpeg на сервере (merge видео+аудио
> для 1440p/2160p), docker-контейнер `brainicism/bgutil-ytdlp-pot-provider` (:4416)
> в compose, pip-плагин в venv, фолбэк/роутинг в `tools/video_downloader.py`.
>
> **Гейт-флаг:** YTDLP_FOR_YOUTUBE (рубильник роутинга). Порядок: @Architect (дизайн ДО
> реализации) → @Builder → @Reviewer → @DevOps (инфра + сквозной smoke). Без @Orchestrator.

### Архитектура и дизайн
- [ ] 👤 T-574 (@Architect, P0): дизайн роутинга ДО реализации — детект YouTube URL (youtube.com/youtu.be/music.youtube + shorts; без ложных срабатываний на прочие платформы); yt-dlp download path с format selection по высоте (`bv*[height<=H]+ba/b`, merge → mp4); интеграция с существующим флоу лока/cleanup/отправки (глобальный asyncio.Lock, try...finally удаление файла, FSInputFile supports_streaming=True, Inline-выбор качества переиспользуется); гейт-флаг YTDLP_FOR_YOUTUBE (settings.py + .env.example); поведение при сбое yt-dlp (токсичная фраза из пула ошибок, кулдаун не сожжён — прецедент touch-after-probe Epic 73); канон Section 78 ARCHITECTURE.md; тест-план. **DoD:** Section 78 зафиксирована, контракты (детект URL, формат-селекшн, точки интеграции) определены

### Реализация
- [ ] T-575 (@Builder, P0, ←T-574): реализация + тесты — ветка yt-dlp в `tools/video_downloader.py` по решению T-574 (YtDlpYouTubeDownloader или эквивалент: base opts из build_ytdlp_base_opts() + POT через bgutil endpoint :4416, format selection по выбранной высоте, выходной файл → общий cleanup/отправка); детект YouTube URL в handlers/video_download.py до выбора провайдера; гейт YTDLP_FOR_YOUTUBE=False → ровно старое cobalt-поведение (0 изменений); АТОМАРНО тесты (детект URL позитив/негатив, гейт off → cobalt, гейт on → yt-dlp path, format string по высоте, сбой → фраза+кулдаун цел, cleanup в finally, регресс cobalt-сценариев Epics 66–75); полный pytest 0 регрессий. **DoD:** все сценарии покрыты, тесты зелёные

### Ревью
- [ ] T-576 (@Reviewer, P0, ←T-575): ревью Epic 77 — детект URL не ловит чужие платформы; гейт off = байт-в-байт старое поведение; лок семантика не сломана (yt-dlp под тем же asyncio.Lock или обоснованное исключение); cleanup гарантирован; секретов нет; канон Section 78 = коду; diff --check чист; полный pytest APPROVED. **DoD:** APPROVED

### Инфраструктура и деплой
- [ ] T-577 (@DevOps, P0, ←T-574/T-576): инфраструктура на проде nik@198.46.175.136 — (1) `apt install ffmpeg` (+ проверка `ffmpeg -version`); (2) docker-compose.yml: сервис `bgutil-ytdlp-pot-provider` (образ brainicism/bgutil-ytdlp-pot-provider, порт :4416) + `docker compose up -d` на проде, healthcheck зелёный; (3) venv: `pip install -U yt-dlp` + плагин `bgutil-ytdlp-pot-provider` (pip), верификация `yt-dlp --version` + загрузки плагина в verbose; (4) прод .env: `YTDLP_FOR_YOUTUBE=True` (бэкап `.bak.epic77`; секреты НЕ в git); (5) коммит цикла на русском (conventional commits) + пуш origin/master; git pull, restart admin_bot, journalctl 0 traceback; (6) сквозной smoke «скачай <yt>» 1080p И 2160p — **главный гейт: ненулевой файл** (2160p дополнительно доказывает ffmpeg merge bv+ba). Rollback: YTDLP_FOR_YOUTUBE=False + restart. Отчёт PM: размеры файлов, версии, вердикт гейта. **DoD:** ffmpeg установлен, bgutil-контейнер жив, плагин в venv, прод v2.48.0, оба smokes зелёные с ненулевыми файлами

---

## Epic 78: Хотфикс v2.48.x — транскрибация кружков/голосовых сломана на проде с локальным Bot API (FileNotFoundError «video_notes/file_0.mp4») — 2026-08-26 🆕 Шаг 1 (PM ✅) — P0

> **Цель:** Починить транскрибацию кружков (video_note) и, вероятно, голосовых (voice)
> на проде с включённым локальным Bot API: хендлер транскрипции падает
> **FileNotFoundError «video_notes/file_0.mp4»**.
>
> **Root cause (факты от DevOps):**
> - Локальный сервер Telegram Bot API возвращает **ОТНОСИТЕЛЬНЫЙ** `file_path`
>   (например `video_notes/file_0.mp4`);
> - aiogram при `is_local=True` читает исходник с диска **относительно cwd бота**,
>   а файл фактически лежит в
>   `/var/www/admin_bot/docker/telegram-bot-api/<bot_id>:<bot_token>/video_notes/file_0.mp4`;
> - итого путь не резолвится → FileNotFoundError → расшифровки нет.
>
> **Направление решения (@Architect фиксирует финал):**
> - новая настройка `TELEGRAM_API_FILES_DIR` (дефолт `docker/telegram-bot-api`);
> - хелпер резолва пути: при локальном режиме и ОТНОСИТЕЛЬНОМ file_path →
>   `<TELEGRAM_API_FILES_DIR>/<bot_id>:<bot_token>/<file_path>`;
> - ветка копирования файла в tmp вместо `bot.download` для резолвнутого дискового пути;
> - фолбэк на штатный `bot.download` для облачного режима (и если резолв не удался).
>
> Порядок: @Architect (дизайн ДО реализации) → @Builder → @Reviewer → @DevOps
> (деплой + проверка кружка/голосового на проде). Без @Orchestrator.

### Архитектурное решение
- [ ] 👤 T-578 (@Architect, P0): краткий дизайн резолва пути ДО реализации — настройка `TELEGRAM_API_FILES_DIR` в settings.py + .env.example (дефолт `docker/telegram-bot-api`); хелпер резолва `<dir>/<bot_id>:<token>/<file_path>` при локальном режиме и относительном file_path (критерий «относительный», обработка существования/не-найденного); ветка копирования в tmp вместо `bot.download` (сохранить существующий cleanup try...finally и лимиты Epic 67); фолбэк `bot.download` для облачного режима; канон Section 79 ARCHITECTURE.md (новая секция); тест-план (относительный path + локальный режим → диск; абсолютный path; облачный режим → bot.download; резолв не удался → fallback). **DoD:** Section 79 зафиксирована, контракты определены, READY FOR BUILDER

### Реализация
- [ ] T-579 (@Builder, P0, ←T-578): реализация по решению T-578 — настройка + хелпер резолва + ветка копирования в tmp в handlers/voice_transcription.py (или вынесенный helper-модуль по дизайну); фолбэк `bot.download` для облачного режима без изменений существующего поведения; АТОМАРНО тесты tests/test_voice_transcription.py: относительный path при is_local → файл из TELEGRAM_API_FILES_DIR скопирован в tmp и расшифрован; абсолютный path → без ре-resolve; облачный режим → bot.download как раньше; резолв мимо диска → graceful fallback/ошибка; cleanup tmp в finally сохранён; полный pytest 0 регрессий. **DoD:** все сценарии покрыты, тесты зелёные

### Ревью
- [ ] T-580 (@Reviewer, P0, ←T-579): ревью Epic 78 — резолв только при is_local И относительном path (облачный режим байт-в-байт старое поведение); bot_token не логируется при сборке пути (`<bot_id>:<token>` — секрет НЕ всплывает в логах/ошибках); cleanup tmp гарантирован; канон Section 79 = коду; diff --check чист; полный pytest APPROVED. **DoD:** APPROVED
...
## Epic 85: TMA Admin Dashboard & Dynamic RBAC — 2026-08-30 ✅ DONE & DEPLOYED (v2.51.0, коммиты 86b3d3a / c975912 / 96d567d, 3135 тестов passed / 0 failed, прод active)

> ✅ **DONE (30.08.2026):** все задачи T-611…T-647 закрыты; ревью **APPROVE WITH MINOR** (N1–N3 →
> пост-деплойный бэклог T-648…T-655 ниже); коммиты `86b3d3a` (feat v2.51.0), `c975912` (хотфикс
> jsonb-типов), `96d567d` (фикс дедлока logtail/uvicorn при старте) запушены в master. Деплой на прод
> успешен: бот active (running), PostgreSQL healthy, миграция 234 параметра, polkit
> `50-adminbot-control.rules`, `TimeoutStopSec=30`, smoke OK, TypeError устранён (0 за 5 мин).
> Тесты: **3135 passed / 0 failed**.
...
> 2. `media/srv_cookies.txt` не нужен (в git не попадает); файлы `plans/` коммитятся в git.
> 3. **aiogram >=3.31** (последняя стабильная 3.x) — обязательное обновление.
> 4. **GraphRAG остаётся на SQLite — ВНЕ скоупа Epic 85**; миграция GraphRAG → PostgreSQL = **Epic 86 (future/planned)**.
> 5. **Маскировка ключей подтверждена:** роли без права `keys` видят configured/last4 — в `/api/config` и в LLM-карточках `/api/status`.
>
> **Дельта №2 (30.08.2026, Шаг 4c @PM) — 4 требования человека:**
> 1. **Полная миграция:** ВСЕ регулируемые параметры из env → админка (246 полей `Settings` по категориям prompts/models/keys/limits/flags/reactions/content; infra ~15 НЕ переносится) с сохранением текущих значений сервера (сид ДО первого старта бота) — канон 84.12.
> 2. **«Как это работает»:** rich-текст `info_text.md` → БД (ключ `info.how_it_works`), публичный просмотр (любая роль), редактирование ТОЛЬКО admin (`edit_info`) — канон 84.13.
> 3. **RBAC v2 гранулярный:** кастомные роли + чекбоксы вплоть до отдельных параметров и ключей (permissions-объект, неймспейс section./param./key./action., матчинг exact→секция→wildcard), правка ЛЮБОЙ роли включая системные — канон 84.14.
> 4. **Control:** кнопки restart/stop/start в «📊 Статус», только admin+moderator (`control.*`); polkit-правило без sudo; флаг-файл `.adminbot_keep_stopped`; pre-existing фикс SIGTERM (graceful shutdown ≤10с) — канон 84.15.
>
> **Цель:** Telegram Mini App Admin Dashboard с динамическим RBAC. Отказ от .env для логики бота
> (в .env остаются ТОЛЬКО инфраструктурные/секретные: `API_TOKEN`, `POSTGRES_DSN`, `WEB_PORT`, `DB_PATH`,
> `TELEGRAM_API_ID/HASH`, `SENTRY_DSN`, `LOGTAIL_SOURCE_TOKEN`, cobalt-прокси). PostgreSQL 16
> (`postgres:16-alpine`, `max_connections=50`, `shared_buffers=128MB`, volume) — таблицы `bot_settings`
> (key PK, value JSONB, category, updated_at), `bot_roles` (role_name PK, permissions JSONB, is_custom),
> `bot_admins` (telegram_id BIGINT PK, role_name FK, added_by, created_at). **КРИТИЧНЫЙ СИД:**
> 5885953495→admin, 1313107079→moderator, 134812796→moderator; роли admin/moderator/user.
> FastAPI в одном event loop с Aiogram 3.x + общий `ConfigCache` (загрузка при старте, hot-reload без
> рестарта). TMA-auth: валидация initData через `aiogram.utils.web_app.safe_parse_webapp_init_data`
> (секрет `API_TOKEN`), заголовок `X-Telegram-Init-Data`, свежесть auth_date ≤ 24 ч. REST:
> GET/POST `/api/config`, `/api/admins` (+`/remove`), `/api/roles`, `/api/me`, `/api/health`.
> Статика `/web`. Фронтенд zero-build: Vue 3 + Tailwind CDN, MatDash-стиль (тёмная тема, сайдбар, хэдер,
> grid-карточки `#2b2b40` rounded-xl shadow-lg, фиолетово-синие градиенты `#8b5cf6`/`#3b82f6`),
> **КРИТИЧНО** анимированный градиентный фон (`background-size: 400% 400%`, `animation: gradient 15s
> ease infinite`) + glassmorphism-карточки. 6 вкладок: Промпты, Модели и Провайдеры, API Ключи,
> Лимиты и Модули, Управление доступом (юзеры + конструктор ролей с чекбоксами), **📊 Статус**
> (health dashboard, **всегда видима** — RBAC-исключение; метрики бот/сервер/LLM, Chart.js-график
> аптайма по `uptime_events`, лог-вьюер ERROR/WARN + копирование). Динамический RBAC
> (`requires_permission`, скрытие вкладок по правам из `/api/me`). REST-дельта: `GET /api/status`,
> `GET /api/status/logs` — публичны для валидного TMA-юзера (без `requires_permission`). Домен
> `https://embody-grafted-ritalin.ngrok-free.dev`.
>
> **Канон:** `plans/ARCHITECTURE.md` Section 84 + **84.11 Status/Health Dashboard** + **84.12–84.16 (Дельта №2)** (DESIGN ✅, @Architect дельты 30.08.2026; фазы 84.9: 0–5, DoD 84.10 + 84.16.2 пп.11–16).
> Порядок: @Builder → @Reviewer → @DevOps (деплой). **Без @Orchestrator.**

### Фаза 0 — docker-compose + конфиг
- [x] T-611 (@Builder, P0): `docker-compose.yml` — сервис postgres по канону 84.8: image `postgres:16-alpine`, container_name `adminbot-postgres`, `command: postgres -c max_connections=50 -c shared_buffers=128MB`, environment `POSTGRES_DB/USER/PASSWORD` (с дефолтами и `${POSTGRES_PASSWORD:?required}`), volume `adminbot_pgdata:/var/lib/postgresql/data` (объявить в блоке volumes), порт СТРОГО `127.0.0.1:5432:5432`, healthcheck `pg_isready` (10s/5s/5 retries), `restart: unless-stopped`. `.env.example`: `POSTGRES_DSN=postgresql://adminbot:<pass>@127.0.0.1:5432/adminbot` + `WEB_PORT=8000` (с комментариями по конвенции). **DoD:** `docker compose config` валиден; порт только на 127.0.0.1; существующие 3 сервиса не затронуты; volume объявлен

### Фаза 1 — БД, ConfigCache, миграционный скрипт
- [x] T-612 (@Builder, P0, ←T-611): `services/pg_db.py` — asyncpg-коннектор (DSN из `POSTGRES_DSN`, пул), идемпотентный DDL при старте (`CREATE TABLE IF NOT EXISTS` 3 таблицы + индексы `idx_bot_settings_category`, `idx_bot_admins_role` — дословно по 84.3), сид `INSERT … ON CONFLICT DO NOTHING`: `bot_roles` admin `["prompts","models","keys","limits","access"]` / moderator `["limits"]` / user `[]` (is_custom=false); `bot_admins` **КРИТИЧНО** (5885953495→admin, 1313107079→moderator, 134812796→moderator, added_by NULL); стартовый сид `bot_settings` (категории prompts/models/limits/flags по 84.3; категория keys — НЕ хардкодить секреты: заполняется T-614 или POST /api/config). **DoD:** DDL идемпотентен (повторный запуск без ошибок); КРИТИЧНЫЕ telegram_id засеяны; юнит-тесты на DDL/сид
- [x] T-613 (@Builder, P0, ←T-611): `services/config_cache.py` — ConfigCache (контракт 84.4): `__init__(dsn)`, `async init()` (retry-loop + WARNING при недоступности PG — бот не блокируется, R6), `get(key, default)` (sync-чтение in-memory словаря), `async set(key, value, category)` (upsert PG + in-memory под asyncio.Lock), `async get_permissions(telegram_id)` (bot_admins JOIN bot_roles → permissions), `async reload()`; in-memory кэш ролей/админов, обновляется при POST /api/admins|/api/roles. **DoD:** юнит-тесты get/set/upsert/reload/permissions; PG недоступен → init завершается с WARNING и бот жив (R6)
- [x] T-614 (@Builder, P1, ←T-612/T-613): `scripts/migrate_env_to_pg.py` — однократный идемпотентный экспорт текущих значений .env/settings.py в `bot_settings` (категории prompts/models/keys/limits/flags по 84.3; пустое значение = «уровень каскада выключен»); флаг `--dry-run`. **DoD:** повторный запуск не дублирует записи (upsert); dry-run печатает план без записи; секреты НЕ логируются

### Фаза 3 (фундамент) — FastAPI + один event loop
- [x] T-615 (@Builder, P0, ←T-613): переработка `main()` в `bot.py` (канон 84.4): один `asyncio.run(main())` → `cache = ConfigCache(...); await cache.init()` → `await on_startup(cache)` (существующая инициализация + инъекция кэша в сервисы) → `app = create_app(cache)` (FastAPI-фабрика, `app.state.cache = cache` — общий объект aiogram+FastAPI) → `server = uvicorn.Server(uvicorn.Config(app, host="127.0.0.1", port=WEB_PORT, loop="auto"))` → `polling = asyncio.create_task(dp.start_polling(bot))` → `await server.serve()`; `finally: polling.cancel(); await on_shutdown()`. Старый запуск (чистый polling) заменяется, не дублируется. **DoD:** бот отвечает в Telegram при поднятом uvicorn (тест с реальным диспетчером); shutdown → cancel polling + on_shutdown ровно один раз; `loop="auto"` (Windows-dev работает, R3); запрещён `uvicorn.run()` внутри корутины (R2)

### Фаза 2 — горячие точки бота на ConfigCache
- [x] T-619 (@Builder, P0, ←T-613): миграция .env-параметров на ConfigCache в горячих точках (правило R1 — каждый параметр переводится независимо, `settings` остаётся источником дефолтов): промпты (сборка контекста factcheck/direct_chat/summary/checkup), лимиты символов, cooldown-ы (троттлы создаются/перечитываются на запрос), API-ключи (LLMClient берёт ключ из кэша на вызов), флаги модулей (проверка в обработчике). **DoD:** параметр есть в PG → бот использует значение из кэша; параметра нет → дефолт из settings; POST /api/config → следующее обращение бота уже с новым значением (тест hot-reload); без PG поведение = прежнее (дефолты)

### Фаза 3 — TMA-auth и REST
- [x] T-616 (@Builder, P0, ←T-615): `web/api/deps.py` — `get_tma_user(request, x_telegram_init_data: str = Header(...))` через `aiogram.utils.web_app.safe_parse_webapp_init_data(token=settings.API_TOKEN, init_data=…)` (ValueError → 401); свежесть `now − auth_date ≤ 86400` (иначе 401); права юзера кладутся в `request.state` из `cache.get_permissions()`; `requires_permission(perm)` — фабрика FastAPI-dependency (нет права → 403). **DoD:** юнит-тест на известном HMAC-векторе из https://docs.telegram-mini-apps.com/platform/init-data (84.10.1); тесты свежести и 401/403
- [x] T-617 (@Builder, P0, ←T-615/T-616): REST-эндпоинты (матрица 84.5): `GET /api/health` (без auth) → `{"status":"ok"}`; `GET /api/me` (валидный initData) → `{telegram_id, role_name, permissions[], is_custom}`, нет роли → 403 `{"detail":"no role","permissions":[]}`; `GET /api/config` (любая роль) → `[{key, value, category, updated_at}]`, категория `keys` без права `keys` → маскировка `{"configured":true,"last4":…}` (R5); `POST /api/config` (право по категории: prompts/models/keys→свои; limits,flags→`limits`) → `{updated:[keys]}`; `GET/POST /api/admins` + `POST /api/admins/remove` (право `access`; несуществующая роль → 422; удаление последнего admin → 409); `GET/POST /api/roles` (право `access`; upsert кастомной is_custom=true; правка системной → 409). Коды ошибок: 400/401/403/404/409/422/500. **DoD:** тесты на каждый маршрут + полную матрицу ошибок; маскировка keys не пропускает секреты
- [x] T-618 (@Builder, P1, ←T-615): статика: `app.mount("/web", StaticFiles(directory="web", html=True))` + `GET /` → RedirectResponse(`/web/`). **DoD:** `/web/` отдаёт index.html; `/` → 307 на `/web/`

### Фаза 4 — фронтенд (zero-build)
- [x] T-620 (@Builder, P0, ←T-618): `web/index.html` + `web/app.js` + `web/style.css` — каркас MatDash (тёмная тема, сайдбар слева, хэдер, main-grid `repeat(auto-fill, minmax(320px, 1fr))`); **КРИТИЧНО CSS-канон 84.7**: body `linear-gradient(-45deg, #1a1033, #312e81, #4c1d95, #0f172a)`, `background-size: 400% 400%`, `animation: gradient 15s ease infinite` + `@keyframes gradient` (background-position 0%↔100%), `prefers-reduced-motion` отключает анимацию; карточки `rgba(43,43,64,0.55)` + `backdrop-filter: blur(20px) saturate(140%)` (+`-webkit-`) + `border: 1px solid rgba(255,255,255,0.08)` + `border-radius: 1rem` + `box-shadow: 0 8px 30px rgba(0,0,0,.35)`; активные кнопки `linear-gradient(135deg, #8b5cf6, #3b82f6)`; Vue 3 global build (unpkg) + Tailwind Play CDN + `tailwind.config` (#2b2b40, #8b5cf6, #3b82f6); `Telegram.WebApp.ready()`/`expand()`/`setHeaderColor`. **DoD:** страница открывается по `/web/`, фон анимирован, карточки glassmorphism (визуальная проверка человеком при смоуке T-625)
- [x] T-621 (@Builder, P0, ←T-617/T-620): `app.js` (Options API) — `data` {activeTab, me, config, admins, roles}; `mounted` → `GET /api/me`; fetch-обёртка добавляет `X-Telegram-Init-Data = Telegram.WebApp.initData` ко всем запросам; computed-список вкладок по `permissions` (RBAC-скрытие сайдбара); 5 вкладок: 🧠 Промпты (textarea-карточки), ⚙️ Модели и Провайдеры, 🔑 API Ключи (маскированные input), 🚦 Лимиты и Модули (числа + toggle-флаги), 👥 Управление доступом (таблица админов + форма добавления + конструктор ролей чекбоксами). **DoD:** вкладки скрываются/появляются по permissions из /api/me; без права `access` вкладка «Управление доступом» не видна; 401/403 от API обработан (UI не падает)

### Фаза 5 — тесты, ревью, доки, деплой
- [x] T-622 (@Builder, P0, ←T-617/T-619/T-621): тестовый пакет: `tests/conftest.py` — ConfigCache-стаб (R7); HMAC-вектор (84.10.1); `requires_permission` + матрица 401/403 (84.10.2); сид-набор с КРИТИЧНЫМИ telegram_id (84.10.2); ConfigCache upsert/маскировка; `/api/me` по трём ролям (84.10.3); hot-reload параметра без рестарта; дополнение `tests/test_docker_compose.py` под postgres-сервис (84.10.4); полный pytest 0 регрессий (база ~2630). **DoD:** все пункты DoD 84.10.1–84.10.4 покрыты; полный прогон зелёный; `git diff --check` чист
- [x] T-623 (@Reviewer, P0, ←T-622): ревью Epic 85 — Section 84 + **84.11** = коду (DDL/сид/контракты ConfigCache/deps/REST-матрица/CSS-канон; log_ring sanitize, status_service, uptime_events heartbeat, публичность `/api/status*` для любой роли, маскировка LLM-карточек); КРИТИЧНЫЕ telegram_id в сиде; секреты не логируются и не уходят в GET /api/config без права `keys` (R5/R17); uvicorn только через `server.serve()` — один event loop (R2); PG недоступен → бот жив (R6); RBAC-скрытие вкладок (+ «📊 Статус» всегда видима); `diff --check` чист; полный pytest APPROVED. **DoD:** APPROVED
- [x] T-624 (@Builder, P1, ←T-623): документация — README.md (секция дашборда, ироничный тон, changelog v2.51.0), MEMORY.md (bump v2.51.0), ARCHITECTURE.md Section 84 → статус IMPLEMENTED после деплоя (при отклонениях от канона — зафиксировать правкой секции). **DoD:** доки актуальны; версия v2.51.0
- [x] T-625 (@DevOps, P0, ←T-623): деплой на прод nik@198.46.175.136:/var/www/admin_bot: `docker compose up -d postgres` (healthcheck зелёный); venv: `pip install asyncpg psutil`; прод `.env` +`POSTGRES_DSN`/`WEB_PORT=8000` (бэкап `.env.bak.epic85`; секреты НЕ в коммите); запуск `scripts/migrate_env_to_pg.py` (заполнить keys/prompts/limits из прод .env); коммит+пуш (conventional, русский); git pull; `systemctl restart admin_bot`; journalctl 0 traceback; smoke: `curl https://embody-grafted-ritalin.ngrok-free.dev/health` → `{"status":"ok"}`, `GET /api/status` (валидный initData) отдаёт сводку, `/web/` открывается, TMA-кнопка «Админка» в BotFather ведёт на дашборд (проверка человеком), под admin-ролью видны 6 вкладок (включая 📊 Статус), под moderator/user — только разрешённые + 📊 Статус (RBAC-исключение). Rollback: `docker compose stop postgres` + revert + restart. Отчёт PM: HEAD, PID, версия, smoke status. **DoD:** прод v2.51.0, postgres healthy, health/status/web отвечают, 0 traceback

### Фаза 0.5 (дельта) — aiogram >=3.31
- [x] T-626 (@Builder, P0): обновление aiogram: `pip install -U "aiogram>=3.31,<4"` + сверка `requirements.txt` (`aiogram>=3.31.0,<4.0.0`), полный pytest (база ~2630) + фиксы несовместимостей 3.29→3.31 (канон 84.2/84.11.6 п.3). **DoD:** `pip show aiogram` ≥3.31; полный pytest зелёный (0 регрессий); найденные несовместимости исправлены и зафиксированы коммитом

### Фаза 1 (дельта) — uptime_events
- [x] T-627 (@Builder, P0, ←T-612): `services/pg_db.py` — 4-я таблица по канону 84.11.3: `CREATE TABLE IF NOT EXISTS uptime_events (id BIGSERIAL PRIMARY KEY, ts TIMESTAMPTZ NOT NULL DEFAULT now(), status TEXT NOT NULL DEFAULT 'up')` + `CREATE INDEX IF NOT EXISTS idx_uptime_events_ts ON uptime_events (ts)`. **DoD:** DDL идемпотентен (повторный старт без ошибок); юнит-тест на создание таблицы/индекса

### Фаза 3 (дельта) — log_ring + status_service + эндпоинты
- [x] T-628 (@Builder, P0, ←T-615/T-626): `services/log_ring.py` (канон 84.11.1) — `LogRingHandler(logging.Handler)` на root-logger + `collections.deque(maxlen=LOG_RING_MAX_ENTRIES)` (дефолт 1000, env); запись `{ts, level, logger, message, exc_text}` (exc_text через `traceback.format_exception`, усечение 4000 симв., без исключения — null); Filter отбрасывает логгеры `logtail*`; `errors_total` = накопленное число записей level ≥ ERROR; `sanitize(text)` в `emit` ДО записи в буфер (R17): литеральные секреты (`API_TOKEN`, `llm_api_key`, `groq_api_key`, `openrouter_api_key`, `tavily_api_key`, `exa_api_key`, пароль из `POSTGRES_DSN`) → `"***"`; `Authorization: Bearer <токен>` → `Bearer ***`; префиксы `sk-…`/`gsk_…`/`or-…`/`tvly-…` → первые 4 символа + `***`; URI-креды `://user:pass@` → `://***@`. Подключение в `bot.py` рядом с `basicConfig`. **DoD:** юнит-тесты sanitize (ни один известный секрет не попадает в буфер); errors_total инкрементится; logtail-записей в буфере нет

- [x] T-629 (@Builder, P0, ←T-613/T-615/T-628): `services/status_service.py` — singleton `status` (канон 84.11.2): **бот** (`started_at = time.monotonic()` первой строкой `main()`, uptime, state `starting|polling|polling_error` — задача polling `done()` с exception, mode `polling` + `local_api` из `settings.DOWNLOAD_ENABLED`, `version` — константа `APP_VERSION` в `config/settings.py` (синхронизировать с changelog MEMORY.md), `errors_total` из log_ring); **сервер** через `psutil` (cpu_percent, virtual_memory, disk_usage("/"), getloadavg → null на Windows-деве, Process(os.getpid()) → rss_mb/threads/cpu); **LLM-реестр** из ConfigCache (категории models/keys): deepseek (`llm_base_url`+`llm_model_name`+фоллбэк `LLM_FALLBACK_*`), groq, openrouter — поля `provider, model, key:{configured,last4}` (**без права `keys` — только configured/last4**, решение 5), `last_latency_ms`, `health:{ok,http_status,latency_ms,checked_at}`; `record_llm(provider, latency_ms, error)` вызывается из `LLMClient._post` и транскриберов groq/openrouter (status_service НЕ импортирует llm_client — циклов нет); health-check `GET {base}/models` (таймаут 5с, результат кэшируется 60с, выполняется ТОЛЬКО по запросу /api/status). **DoD:** юнит-тесты метрик, маскировки LLM-карточек (роль с/без `keys`), кэша health-check

- [x] T-630 (@Builder, P0, ←T-612/T-615/T-627): heartbeat аптайма (канон 84.11.3) — APScheduler `AsyncIOScheduler` в том же event loop: джоб `interval=60s` → `INSERT INTO uptime_events DEFAULT VALUES` (ретраи до 3; при устойчивой ошибке PG — WARNING, бот жив, R6); джоб раз в час → `DELETE FROM uptime_events WHERE ts < now() - interval '72 hours'` (ретенция env `UPTIME_EVENTS_RETENTION_HOURS`, дефолт 72). **DoD:** тесты heartbeat-вставки и автоочистки (фейк-тайм); PG down → бот жив и логирует WARNING

- [x] T-631 (@Builder, P0, ←T-616/T-629/T-630): эндпоинты (канон 84.11.4): `GET /api/status` → `{bot:{uptime_seconds,state,mode,version,errors_total}, server:{cpu_percent,memory,disk,loadavg,process}, llm:[{provider,model,key,last_latency_ms,health}], uptime:{buckets,since,until}}` (агрегация в 5-минутные бакеты за 24ч, ≤288 точек, `date_trunc`/GROUP BY; «down» не пишется — разрыв на графике и есть downtime); `GET /api/status/logs?level=&limit=` → `{count, logs:[{ts,level,logger,message,exc_text}]}` от новых к старым (level ALL/DEBUG/INFO/WARNING/ERROR/CRITICAL регистронезависимо; limit 1–1000, дефолт 200). Оба — ТОЛЬКО `get_tma_user` (401 без/невалидного initData), **БЕЗ `requires_permission`** (RBAC-исключение, доступ любой роли включая `[]`); секреты замаскированы уже в буфере — отдаём как есть. **DoD:** тесты 401; доступ роли с пустыми правами и юзера без роли; фильтры level/limit; JSON-схемы по канону

### Фаза 4 (дельта) — вкладка «📊 Статус»
- [x] T-632 (@Builder, P0, ←T-620/T-621/T-631): фронтенд (канон 84.11.5) — 6-я вкладка «📊 Статус» **ВСЕГДА в сайдбаре** (исключение из computed-фильтрации вкладок по permissions; даже при 403 «no role» вкладка видна, контент — заглушка 403): grid-карточки **Бот** (uptime humanized, badge state, mode, version, errors_total), **Сервер** (CPU/RAM/disk — progress-бары, loadavg, PID/rss), **LLM** (карточка на провайдера: provider+model, badge ключа configured/`••••last4`, last_latency_ms, health ok/fail); **график аптайма Chart.js v4 UMD** — `https://cdn.jsdelivr.net/npm/chart.js@4` (глобал `Chart`), линейный график по бакетам, `spanGaps:false` (разрыв = downtime), подпись оси X — время; **лог-вьюер:** фильтр по уровню (select), отдельные секции ERROR и WARN с количеством (подсвечены), моноширинный список, клик раскрывает `exc_text`, кнопки «Копировать» (запись) и «Копировать всё» — `navigator.clipboard.writeText`; автообновление /api/status poll каждые 30с; логи — по открытию вкладки/кнопке «Обновить» (не поллить). **DoD:** вкладка видна любой роли; график без данных не падает; копирование работает (HTTPS); визуальная проверка человеком при смоуке T-625

### Фаза 5 (дельта) — тесты Status + зависимости
- [x] T-633 (@Builder, P0, ←T-622/T-631/T-632): тесты Status-модуля: `/api/status` и `/api/status/logs` — публичность (роль `[]`, юзер без роли, 401 без initData), фильтры level/limit, порядок от новых к старым; `sanitize` (все классы секретов замаскированы, в буфер ничего не протекло); `uptime_events` (heartbeat-вставка, автоочистка 72ч, агрегация бакетов 5 мин/24ч); кэш health-check 60с; psutil/getloadavg на Windows → null без падения; полный pytest 0 регрессий; `git diff --check` чист. **DoD:** покрыты пункты 84.11.1–84.11.5; полный прогон зелёный

- [x] T-634 (@Builder, P1, ←T-626/T-629): зависимости и конфиг: `requirements.txt` += `psutil>=5.9` (риск R9), сверка `aiogram>=3.31,<4` (с T-626); `.env.example` += `LOG_RING_MAX_ENTRIES=1000`, `UPTIME_EVENTS_RETENTION_HOURS=72` (с комментариями по конвенции); при необходимости — дополнение `tests/test_docker_compose.py` (если не покрыто T-622). **DoD:** свежий venv собирается через `pip install -r requirements.txt`; `docker compose config` валиден; env-переменные задокументированы

### Дельта №2 (Шаг 4c @PM, 4 требования человека) — задачи T-635…T-647

- [x] T-635 (@Builder, P0, ←T-612/T-613; **КРИТИЧНО ДО T-616/T-617**): RBAC v2 — дельта к T-612 (канон 84.14.1–84.14.3): DDL `permissions JSONB NOT NULL DEFAULT '{}'` (вместо `'[]'`), структура `{sections, params, keys, actions, wildcard}`; единый неймспейс идентификаторов (`section.<id>`, `param.<category>.<key>`, `key.<category>.<key>`, `action.<id>`; в массивах — голые id); сид ролей v2: admin `{"wildcard":true}`, moderator `{"sections":["limits"],"actions":["control.restart","control.stop","control.start"]}`, user `{}` (КРИТИЧНЫЕ telegram_id БЕЗ изменений); `requires_permission(perm)` — матчинг по 84.14.2: wildcard → exact → секция-родитель покрывает вложенные (`param.<cat>.*`/`key.*` покрыты секцией; `action.*` секциями НЕ покрывается) → 403; `POST /api/roles` — правка ЛЮБОЙ роли включая системные (старый 409 для is_custom=false отменяется); guard: удаление последней wildcard-роли → 409, неизвестный id права → 422 (валидация против каталога T-636). **DoD:** юнит-тесты матчинга (exact/секция/wildcard/отказ), сид v2, guard 409
- [x] T-636 (@Builder, P0, ←T-612/T-613): `services/param_catalog.py` — единый каталог-реестр ВСЕХ 246 параметров `Settings` (84.12.1): `{settings-поле → (category: prompts|models|keys|limits|flags|reactions|content, title_ru, type: str|int|float|bool|duration|json|secret, secret: bool)}`; `infra`-исключения (~15: api_token, postgres_dsn, telegram_*, sentry_dsn, logtail_source_token, web_port, db_path и т.д.) помечены `category=None` (не мигрируются, не попадают в дерево прав); секретные группы (ключи LLM/поиск/транскриб, прокси youtube/cobalt, cookies-путь) → `secret:true`. ЕДИНЫЙ источник для `migrate_env_to_pg.py` (T-637), `/api/roles/tree` (T-640) и фронта — дублирование запрещено. **DoD:** юнит-тест полноты (каждое поле Settings покрыто, count==246) и секретности (все keys + прокси → secret)
- [x] T-637 (@Builder, P1, ←T-612/T-613/T-636): расширение `scripts/migrate_env_to_pg.py` (дельта к T-614, канон 84.12.2/84.12.3): полный экспорт ВСЕХ 246 полей минус infra-исключения по каталогу T-636; эффективное значение = settings-дефолт + env-переопределение (логика `Settings`); флаги `--env-file` (дефолт `.env`), `--dry-run` (план без записи; для secret — только `configured`), `--force` (`DO UPDATE SET value, updated_at`; перезаписи логируются с updated_by), `--only-category cats`, `--exclude prefix,...`; идемпотентный upsert `ON CONFLICT (key) DO NOTHING` (повторный запуск = no-op); R17: секреты НИКОГДА не печатаются; итоговый отчёт — счётчики по категориям. Belt-and-suspenders (84.12.3): `ConfigCache.init()` на пустой `bot_settings` самозасевает дефолты категорий prompts/models/limits/flags/reactions (без секретов). **DoD:** повторный запуск без --force не дублирует; dry-run без записи; --force перезаписывает; only-category/exclude фильтруют; тесты по DoD 84.16.2 п.11
- [x] T-638 (@Builder, P0, ←T-613/T-615/T-617): «Как это работает» backend (канон 84.13.2–84.13.4): ключ `info.how_it_works` (категория `content`, JSONB `{html, updated_at, updated_by}`); сид при первом старте: ключа нет → прочитать `info_text.md` → записать в PG (`updated_by=ADMIN_USER_ID`, `updated_at=now()`); после сида источник истины — БД/ConfigCache, файл — legacy-фолбек (PG down, R6), `DEFAULT_INFO_TEXT` — только для самого первого запуска; `InfoService.get_text()` читает ConfigCache → файловый кэш → DEFAULT_INFO_TEXT; `save_text()` — единственная точка записи (файл + in-memory кэш + PG) — `/edit_info` и веб-POST сходятся в ней; эндпоинты: `GET /api/info` (валидный initData, любая роль, БЕЗ requires_permission — RBAC-исключение как Статус) → `{key, html, updated_at, updated_by}`, `POST /api/info` (`requires_permission('edit_info')`; лимит 32768, пусто → 422; `updated_by` из initData). **DoD:** сид из файла; `/info` читает из кэша; PG down → файл (тест); матрица 401/403/422/200 (DoD 84.16.2 п.12)
- [x] T-639 (@Builder, P0, ←T-638/T-621): фронт вкладки «Как это работает» (канон 84.13.5): вкладка ВИДНА ВСЕМ (исключение из computed-фильтрации, вместе с «📊 Статус»); рендер rich-HTML через `v-html` в glass-карточке после `DOMPurify.sanitize(html)` (CDN `https://cdn.jsdelivr.net/npm/dompurify@3`; CDN недоступен → деградация на сырой рендер + WARNING в консоль, страница не падает); режим редактирования ТОЛЬКО при праве `edit_info`/wildcard: textarea + «Предпросмотр» (тот же v-html после DOMPurify) + «Сохранить» → POST /api/info → обновить state + тост; остальным — read-only. **DoD:** вкладка видна всем (включая роль `[]`); редактирование только admin; битая/вредоносная разметка нейтрализована DOMPurify; 401/403/422 обработаны в UI
- [x] T-640 (@Builder, P0, ←T-635/T-636/T-617/T-621): конструктор ролей v2 (канон 84.14.4/84.14.5): `GET /api/roles/tree` (право `access`) → дерево из param_catalog `{sections:[{id,title,params:[{key,title,type,secret}],keys:[{key,title}]}], actions:[{id,title}]}`; фронт — чекбокс-дерево (секции → параметры → ключи → действия; чек секции отмечает дочерние, частичный чек = indeterminate); создание кастомных ролей И редактирование существующих (включая системные) через `POST /api/roles` upsert; правило видимости вкладок: вкладка видна при ≥1 праве внутри неё (покрытие по 84.14.2), действия внутри — по своим правам (кнопка «Редактировать» в «Как это работает» — `edit_info`, control-кнопки — отдельно). **DoD:** дерево рендерится из /api/roles/tree; upsert роли применяется сразу; 409 (последняя wildcard) и 422 показаны пользователю; RBAC-скрытие вкладок работает по новой схеме
- [x] T-641 (@Builder, P0, ←T-615/T-616/T-642): `services/control_service.py` + `POST /api/control/restart|stop|start` (канон 84.15.2/84.15.4): права `control.restart|control.stop|control.start`; автоопределение режима (env `CONTROL_MODE`): Linux + `/run/systemd/system` + `shutil.which('systemctl')` → `systemd`, иначе `dev`; дебаунс ≥30с между любыми control-вызовами (in-memory) → 429; аудит `[control] action=… by=<telegram_id> mode=…` в ring-buffer + journal; немедленный 202 → `asyncio.sleep(2.0)` → выполнение: restart = отвязанный subprocess `systemctl restart admin_bot` (`start_new_session=True`, не ждать) → SIGTERM-handler T-642 делает graceful shutdown; stop = флаг-файл `.adminbot_keep_stopped` (вне git) + `systemctl stop admin_bot`; start = `systemctl start` + удаление флага; проверка флага при старте бота → мгновенный exit 0 (защита от Restart=always); dev: restart/stop = graceful exit (exit 0 + WARNING «перезапустите вручную»), start = 409. **DoD:** тесты с моками subprocess: 202+отложенное (фейк-тайм), дебаунс 429, dev 409/exit, флаг-файл (DoD 84.16.2 п.14)
- [x] T-642 (@Builder, P0, **независим**): graceful shutdown — закрывает pre-existing «SIGTERM ~90с» (канон 84.15.5): `bot.py`/`webapp.py` — `loop.add_signal_handler(SIGTERM, …)` (и SIGINT) → `asyncio.Event` → `polling.cancel()` → `server.should_exit = True` → `await on_shutdown()` (закрытие PG-пула, APScheduler, http-клиентов, smart_cache) → exit 0 ≤10с; `TimeoutStopSec=30` в юните admin_bot исполняет @DevOps (T-643). **DoD:** SIGTERM → on_shutdown ровно один раз ≤10с (тест); полный pytest 0 регрессий (DoD 84.16.2 п.15)
- [x] T-643 (@DevOps, P0, ←T-641): polkit-правило + верификация (канон 84.15.3): создать `/etc/polkit-1/rules.d/50-adminbot-control.rules` (user `nik`, action `org.freedesktop.systemd1.manage-units`, unit `admin_bot.service`, verbs `restart|stop|start`, Result.YES); `systemctl cat admin_bot` — подтвердить/задокументировать `Restart=always`; добавить `TimeoutStopSec=30` в юнит + `systemctl daemon-reload`; верификация из-под `nik` БЕЗ sudo: `systemctl stop admin_bot` → inactive, `systemctl start admin_bot` → active (PID новый, 0 traceback); негатив-тест другого юнита → отказ. Фолбек при неработе polkit: sudoers-строка NOPASSWD ТОЛЬКО для трёх команд systemctl admin_bot. **DoD:** stop/start без sudo работает; другой юнит — отказ; юнит-конфиг (Restart=always/TimeoutStopSec) задокументирован
- [x] T-644 (@Builder, P0, ←T-641/T-632): фронт control-кнопок в карточке «Бот» вкладки «📊 Статус» (канон 84.15.4, дельта 84.11.5): три кнопки Restart/Stop/Start, видимость по правам `control.*` (по сиду admin+moderator; user — не видит); модальное подтверждение `Telegram.WebApp.showConfirm`; после 202 — кнопки disabled на 30с (дебаунс-зеркало); после restart/stop — баннер «бот перезапускается/остановлен, соединение может прерваться»; 429/409 обработаны в UI; аудит доступен всем в /api/status/logs. **DoD:** под moderator кнопки видны и работают; под user — скрыты; confirm/дебаунс/баннер работают; 409 dev-start показан
- [x] T-645 (@Builder, P0, ←T-635/T-637/T-638/T-641/T-644): тесты дельты №2 (DoD 84.16.2 пп.11–15): RBAC v2 (матчинг exact/секция/wildcard/отказ, сид v2, guard последней wildcard → 409, `/api/roles/tree` из каталога); миграция (dry-run/force/идемпотентность/only-category/exclude, секреты не печатаются, отчёт по категориям); `/api/info` (сид, PG-down фолбек на файл, 401/403/422/200, updated_by из initData); control (202+отложенное, дебаунс 429, dev 409/exit, флаг-файл, моки subprocess/systemctl); graceful shutdown ≤10с; маскировка per-key (значение видит ТОЛЬКО роль с правом на конкретный ключ — R5/84.12.4); полный pytest 0 регрессий; `git diff --check` чист
- [x] T-646 (@Reviewer, P0, ←T-645): ревью-дельта к T-623 — Section 84.12–84.16 = коду: RBAC v2 (неймспейс, матчинг, guard последней wildcard, правка системных ролей), param_catalog (полнота 246, секретность), полная миграция (R17, идемпотентность), info.how_it_works (сид/фолбек/edit_info), control_service (202/дебаунс/флаг-файл/dev-режим), SIGTERM-handler; `sanitize()` расширен (proxy/cookies/postgres_dsn); маскировка per-key; КРИТИЧНЫЕ telegram_id в сиде; один event loop (R2); PG down → бот жив (R6); `diff --check` чист; полный pytest APPROVED. **DoD:** APPROVED
- [x] T-647 (@DevOps, P0, ←T-646/T-643): деплой-дельта к T-625 (канон 84.12.3, Фаза 0): postgres healthy → `migrate_env_to_pg.py --dry-run` (сверка отчёта) → `migrate_env_to_pg.py` (сид ДО первого старта бота с фичей) → `systemctl restart admin_bot`; polkit уже развёрнут (T-643); smoke: бот работает идентично (значения в PG == бывшим .env); `SELECT count(*) FROM bot_settings` сходится с отчётом миграции; `GET /api/info` публичен; `POST /api/control/restart` → 202 → новый PID, 0 traceback; кнопки control видны admin+moderator, НЕ user; «Как это работает» видна всем; journalctl 0 traceback. Отчёт PM: HEAD, PID, версия, smoke status. **DoD:** прод v2.51.0 + дельта №2 живая, миграция выполнена ДО старта, 0 traceback

**Файлы для изменения:** `docker-compose.yml` (T-611), `.env.example` (T-611/T-634), `services/pg_db.py` (T-612/T-627), `services/config_cache.py` (T-613), `scripts/migrate_env_to_pg.py` (T-614), `bot.py` (T-615/T-628/T-630), `web/api/` (deps, routes) (T-616/T-617/T-631), `web/` (index.html/app.js/style.css) (T-618/T-620/T-621/T-632), горячие точки сервисов/хендлеров (промпты/лимиты/cooldowns/ключи/флаги) (T-619), `services/log_ring.py` (T-628), `services/status_service.py` + `config/settings.py` (APP_VERSION) (T-629), `services/llm_client.py` + транскриберы (record_llm) (T-629), `services/scheduler.py`/`bot.py` (heartbeat APScheduler) (T-630), `requirements.txt` (T-626/T-634), `tests/` + `tests/test_docker_compose.py` + `tests/conftest.py` (T-622) + `tests/test_status*.py`/`tests/test_log_ring.py` (T-633), `README.md`/`MEMORY.md` (T-624)

**Файлы дельты №2:** `services/param_catalog.py` (T-636), `services/pg_db.py` — DDL v2 + сид v2 (T-635), `web/api/deps.py` + routes (T-635/T-638/T-640/T-641), `services/info_service.py` (T-638), `services/control_service.py` (T-641), `bot.py`/`webapp.py` — SIGTERM-handler + проверка флаг-файла (T-642/T-641), `web/app.js`/`web/index.html` (T-639/T-640/T-644), `scripts/migrate_env_to_pg.py` (T-637), `tests/` — `test_rbac_v2*.py`, `test_info_api.py`, `test_control*.py`, `test_migrate*.py` (T-645), `/etc/polkit-1/rules.d/50-adminbot-control.rules` + systemd-юнит admin_bot (T-643)

---

## Epic 85: Пост-деплойный бэклог — 2026-08-30 🆕 (финал Epic 85 @PM, статус pending)

> **Источник:** ревью Epic 85 (APPROVE WITH MINOR: N1–N3) + миноры ревью хотфикса `c975912` + известный
> дефект после деплоя v2.51.0. Все задачи — новые, исполнители @Builder/@Reviewer (верификация на проде —
> @DevOps). Без @Orchestrator. Отклонения от канона Section 84 фиксировать правкой секции.
>
> ⚠️ **Открытый вопрос человеку (блокер):** ngrok-туннель не активен — внешний домен
> `embody-grafted-ritalin.ngrok-free.dev` отдаёт 404; админка слушает `127.0.0.1:8000` на сервере.
> Требуется запустить ngrok на сервере (или настроить другой туннель) — до этого TMA-кнопка BotFather
> ведёт на 404, дашборд доступен только локально.
> **→ Снят follow-up задачей T-657 (@DevOps, P1, In Progress) — см. ниже.**

### Миноры ревью (N1–N3)
- [ ] T-648 (@Builder, P2): N1 — POST /api/config неатомарен (частичное применение при сбое в середине списка) → атомарное применение (валидация всего списка до записи / транзакция)
- [ ] T-649 (@Builder, P2): N2 — POST /api/info двойная запись `updated_by` → единая точка записи
- [ ] T-650 (@Builder, P3): N3 — docstring `can_edit_param` поправить под фактические права

### Миноры ревью хотфикса (c975912)
- [ ] T-651 (@Builder, P2): унифицировать `_cast_to_type` / `_coerce_catalog_value` / `_coerce_value` (один механизм кастинга значений каталога)
- [ ] T-652 (@Builder, P3): `hot.get` на cache-miss не должен кастовать default (возвращать как есть)
- [ ] T-653 (@Builder, P3): guard `alan` на мусорные значения из БД (валидация + фолбек на дефолт)
- [ ] T-654 (@Builder, P3): документировать NaN/inf в контрактах значений (каталог + API)

### Известный дефект (пост-деплой)
- [ ] T-655 (@Builder, P2, верификация @DevOps): graceful shutdown по SIGTERM не работает — aiogram перехватывает сигнал, uvicorn не получает `should_exit` → процесс убивается SIGKILL по `TimeoutStopSec=30`; нужен честный graceful stop (сигнал в uvicorn до/вместо перехвата aiogram; тест ≤10с)

### Новые follow-up задачи от человека (30.08.2026)
- [ ] T-656 (@Builder, P1, Ready): **«In-Memory State Dump»** — скрытая команда `/debug_config` (+ опционально эндпоинт `GET /api/debug/config`) в боте: возвращает ТЕКУЩИЕ значения переменных из ОПЕРАТИВНОЙ ПАМЯТИ работающего инстанса (из ConfigCache в RAM), НЕ из БД; только для админов (`telegram_id 5885953495`); секреты маскируются; показывает тип значения и источник (кэш vs settings-фолбек). Цель: проверка цепочки «Mini App → БД → инвалидация кэша → память» (изменил в Mini App → `/debug_config` → значение обновилось). Критерии: команда доступна только админу, маскировка секретов, тесты. **Зависимости:** независима; после реализации — деплой через @DevOps (коммит в master на русском + push + git pull + `systemctl restart admin_bot` + проверка `/debug_config` на проде).
- [ ] T-657 (@DevOps, P1, In Progress): **ngrok как постоянная systemd-служба на сервере 198.46.175.136** — apt-репо ngrok + install, `ngrok config add-authtoken`, юнит `/etc/systemd/system/ngrok.service` (ExecStart=`/usr/bin/ngrok http --domain=embody-grafted-ritalin.ngrok-free.dev 8000`, Restart=always, User=nik), daemon-reload, enable --now, status — служба active; HTTPS-домен должен отвечать (проверка `/api/health` через туннель). Снимает открытый вопрос/блокер ngrok-туннеля.

#### Волна 2 (T-658…T-661) — 30.08.2026
- [ ] T-658 (@Builder, P1, Ready, ←T-656): **Редизайн /debug_config** — (1) без параметра: список строк `KEY = value` (env-стиль имени, секреты masked, без лишней воды), недавно изменённые ключи — в начале списка (сортировка по `updated_at` из кэша); (2) вызов по параметру: `/debug_config SEARCH_MAX_SYMBOLS` — env-имя, case-insensitive, фолбек на pg-ключ; то же для `GET /api/debug/config?key=<key>`. Доступ и маскировка не меняются (как в T-656). **Критерии:** список без воды (только KEY = value), сортировка по updated_at, параметр-лукап работает при любом регистре и по pg-ключу, GET-эквивалент telegram-вызова, тесты. **Деплой:** после закрытия T-658/T-661 — @DevOps (коммит на русском + push + git pull + рестарт).
- [ ] T-659 (@DevOps, P1, In Progress, ←T-657, решение @Architect): **Убрать ngrok interstitial** («You are about to visit...») в Telegram Mini App — по решению Архитектора: вероятно бесплатный DuckDNS-поддомен + Let's Encrypt + nginx-прокси на VPS (стабильный HTTPS без предупреждений) ЛИБО другое бесплатное решение (подтвердить у Архитектора). После реализации — обновление URL Mini App в BotFather (шаги для человека). **Критерии:** Mini App открывается без interstitial, валидный сертификат, инструкция для человека по BotFather оформлена.
- [ ] T-660 (@DevOps, P1, In Progress): **Инцидент «админка UI без данных, только 2 вкладки, спиннеры»** — расследование на сервере: git pull (креды доступны DevOps), снять uvicorn access-логи (журнал: `GET /api/me` и др. — статусы реальных клиентов 200/401/403), проверить версию app.js на сервере и через туннель, при необходимости рестарт. **Критерии:** root cause + фикс (код-фикс при необходимости), логи подтверждены.
- [ ] T-661 (@Builder, P1, Ready, ←T-660): **Обход кэша WebView** — версионирование статики (`?v=`), Cache-Control заголовки для index.html/app.js (StaticFiles headers) — чтобы клиенты получали актуальный JS. **Критерии:** `?v=` в HTML на статике, Cache-Control на index.html/app.js, после деплоя клиенты получают новый JS без ручного сброса кэша; тесты. **Деплой:** после закрытия T-658/T-661 — @DevOps (коммит на русском + push + git pull + рестарт).

#### Волна 3 — Follow-up из расследования скам-инцидента (30.08.2026): T-662…T-666

> **Источник:** расследование инцидента 30.08.2026 — бот написал юзеру конференции в ЛИЧКУ
> рекламно-скам-сообщение «ПРОБЕЙ и РАЗЫГРАЙ…» с кнопками («Поиск Контактов - ВОЙД»,
> «Устроить ПРАНК-ЗВОНОК», «Взорвать SMS-BOOM»). Внутренние пути исключены (такого
> функционала в боте НЕТ, команды/контент не найдены) → вероятная утечка токена
> (репозиторий публичный, GitHub secret scanning). Без @Orchestrator.

- [ ] T-662 (@DevOps, P1, In Progress): **Хардненинг секретов** — chmod 600 `.env*` на сервере; очистка PAT из remote URL (переход на SSH deploy key — см. T-665); проверка git-истории (репо публичный): `git log -p` / GitHub scanning на `API_TOKEN`/PAT — 0 хитов ИЛИ список мест утечки в отчёт. **Критерии:** `.env*` → `-rw-------`; `git remote -v` без токенов; вердикт по git-истории (чисто/утечка).
- [ ] T-663 (@Builder, P1, Ready): **Админ-гейт + аудит-логи для /mimic, /deadpage, /alangreet** — сейчас DM-ветки доступны ЛЮБОМУ юзеру (handlers/admin_commands.py:72 — «DM: anyone can test mimic transform»; T-048/T-049 фиксируют «DM: любой пользователь»); добавить гейт `ADMIN_USER_ID` (или RBAC-право, фолбек при деградации как /debug_config T-656) для DM-веток /mimic (единым подходом — /deadpage, /alangreet); аудит-логи с `user_id, chat_id, command, result` БЕЗ секретов (R17); тесты (DM-админ, DM-не-админ, группа-админ, аудит-лог есть + без секретов). **Критерии:** не-админ в DM получает отказ; аудит-логи с user_id без секретов; полный pytest 0 регрессий.
- [ ] T-664 (@DevOps, P1, Ready): **Логирование запросов локального Bot API** — контейнер `adminbot-telegram-bot-api` (docker-compose.yml; локальный Bot API-сервер показывал ПУСТЫЕ логи — вызовы sendMessage не видны); включить лог запросов (SDK-флаг/конфиг контейнера или инструкция в docker-compose/README/ARCHITECTURE для этого контейнера). **Критерии:** `docker logs adminbot-telegram-bot-api` показывает исходящие вызовы бота (sendMessage и др.) — ЛИБО задокументированный конфиг-флаг и инструкция его включения в compose/README (R17: токены в логах НЕ допускаются).
- [ ] T-665 (человек + @DevOps, P1, ⏸ ждёт человека): **Ротация секретов** — (человек) BotFather → /revoke токена бота; GitHub: отозвать PAT, перейти на SSH deploy key; репозиторий → private; отозвать подозрительные сессии BotFather и активные сессии (Telegram/BotFather); (DevOps) после ротации — обновить `.env` на сервере (API_TOKEN, LLM/Exa/Tavily/Groq/OpenRouter-ключи, proxy, POSTGRES_PASSWORD по согласованию), бэкап `.env.bak.rotation`, рестарт, smoke. **Критерии:** getMe с новым токеном; PAT отозван (deploy по SSH работает); репо private; старые ключи невалидны; бот отвечает, 0 traceback.
- [ ] T-666 (@Builder, P2, Ready, ←T-665): **Верификация username отправителя скама** — получить у человека точный username из сообщения скама; если `v1vv2as_bot` (двойная v = реальный username бота, подтверждено getMe) → утечка токена ПОДТВЕРЖДЕНА → приоритет T-662/T-665, вердикт в отчёт; если иной (`v1w2as_bot`, `v1vv2as_b0t` и т.п.) → фейк-бот (копия имени/аватара), токен НЕ при чём — вердикт + снятие приоритета ротации по усмотрению человека. **Критерии:** в отчёт PM — точный username + вердикт (утечка / фейк) + рекомендации.

---

## Epic 86: GraphRAG → PostgreSQL — future/planned (вне Epic 85)

> **Статус:** future/planned (решение человека от 30.08.2026). **Суть:** миграция GraphRAG-хранилища с SQLite (`services/database.py`) на PostgreSQL 16 (asyncpg): таблицы графа знаний, векторы, индексы. Вынесена из Epic 85 — Epic 85 в PG добавляет ТОЛЬКО таблицы 84.3 + `uptime_events` (84.11.3). Задач пока нет; планируется отдельным эпиком после Epic 85 (нужен DESIGN @Architect и аппрув человека).
