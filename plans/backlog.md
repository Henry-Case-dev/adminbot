...
### 🖼️🧭 Хотфикс-4 после hotfix3 — `hotfix4-cover-nav-shell-round1025` (внеплановый, между hotfix3 и F2) — 🟦 PLANNING (Step 1 @PM, 21.09.2026)
...
**Статус:** 🟦 **Step 1 @PM — декомпозиция.** `tasks.md` — `plans/features/hotfix4-cover-nav-shell-round1025/tasks.md`; `spec.md`/ADR — Step 2 @Architect **при необходимости** (PM не создаёт). **Задачи:** **T-2507…T-2528 (22)**; преемственность — максимум до хотфикса **T-2506**, дублей нет. **Порядок: точка отката (T-2507) → A ∥ C → B → тесты версии → регресс → ревью → деплой → live-гейт → архивация.**
...
- **Регресс/деплой** (T-2522…T-2528): обновление F1-тестов/матрицы; bump `APP_VERSION` (**2.58.3** → далее; меняются `web/app.js`/`web/index.html`/`web/static/app.css`/`web/static/telegram-init.js`); pytest/JS=0 регрессий; @Scanner Critical 0/High 0; деплой + **⏳ live-гейт владельца** (обложка с **настроенным** стилем/заголовком; панель в пределах экрана; Статус+Справка первыми; регрессии F0/F1/hotfix/hotfix2/hotfix3 = 0); архивация (Шаг 8 @PM).
...
| **F2** | `design-tokens-liquidglass-v2` — палитра §8 (SUPERSEDE OD4/#FF8A3D), Liquid Glass A/B/C §9 (`feDisplacementMap`), фон 60–90/90–120 с §10, контраст/reduced-motion | §8–§10, §70 | визуальная система (web/CSS) | **P0** | F1 | детализация при старте |
| **F3** | `global-scope-selector` — постоянный селектор области §5 (Глобально/Чат/ЛС, поиск, **источник значения**, «Вернуть глобальное значение», отмена устаревших запросов, черновик не переносится) | §5, §14, §42, §43, §74 | UI+state (web) | **P0** | F1 | детализация при старте |
| **F4** | `module-catalog-quickpanel-store` — каталог §31–§45 + панель избранного + канонический `ModuleConfigurationStore` §37–§42 (одна мутация → все представления, `scope_type+scope_id+module_id`, scoped keys) | §31–§45, §72, §73 | UI+state (web) | **P0** | F2, F3 | детализация при старте |
...
| **F11** | `status-showcase-dashboard` — **[добавлено @PM, адаптация: §11–§20 не покрыты в F1–F10 по существу]** витрина «Статус» §11–§20: 12-кол. сетка §12, Hero §13, метрики §14, живое сердцебиение §15 (Canvas 2D+rAF), граф §16, сон/бейджи §17, мониторинг интеллекта §18, «Новые факты» §19, логи §20 + превью карты §21 | §11–§21, §70, §77 | UI (web) | **P0** | F1, F2, F6 | детализация при старте |
...
2. **Волна 0.5 (каркас):** **F1 ✅ ARCHIVED + DEPLOYED** (`fe0f7bb`; IA/shell; потребитель инвентаря F8 и контракта сохранения F0) + **P0-фикс после F1 ✅ ARCHIVED + DEPLOYED** (`fea2daa`, hotfix2: render/media/avatars/APP_VERSION/matrix). **Дальше — F2 (следующая).**
3. **Волна 1:** **F2** (токены/glass/фон) ∥ **F3** (селектор области) — после F1.
4. **Волна 2:** **F4** (каталог+store) — после F2/F3.
5. **Волна 3:** **F5** (workspace+промпты) → **F6** (Память+Аналитика) → **F7** (PERMsoc) ∥ **F11** (Статус-витрина, после F2+F6).
6. **Волна 4:** **F9** (**только секреты §50 + визуальный SaveBar**; серверный persistence/409/state-machine уже закрыты в F0) — после F4/F5.
7. **Волна 5:** **F10** (приёмка §71–§79) — после всех; затем **СТОП-ГЕЙТ Эпика 1** (приёмка владельцем) → старт Эпика 2.

> **Порядок фич (единый, по UPD):** **F0 → F1 → F2 → F3 → F4 → F5 → F6 → F7 → F11 → F9 → F10**. Изменение относительно исходной декомпозиции: F0 добавлена в начало (Wave 0), F11 передвинута **перед** F9 (обе после F4/F5/F6/F7), F9 сужена.
