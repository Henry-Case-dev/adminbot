...
### 🔥 Хотфикс-3 после hotfix2 — `hotfix3-summary-stt-anticliche-round1025` (внеплановый, между F1/hotfix2 и F2) — 🟦 Step 1 @PM (декомпозиция)
...
**Контуры работ (детали — `plans/features/hotfix3-summary-stt-anticliche-round1025/tasks.md`):**
...
- **Регресс/ревью/деплой/верификация** (T-2502…T-2506): pytest **8003/0 (+ новые)**, JS **21/21**, matrix **0**, `database is locked` = **0**; @Scanner Critical 0/High 0; деплой + live-гейт (обложка+rich уходят; видео 28 МБ транскрибируется; ручные фразы сохраняются с честным отчётом; таймауты логируются); архивация (Шаг 8 @PM).
