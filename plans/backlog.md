...
| **S9** ▶️ NEXT | `summary-testing-ui` — тестирование из Mini App §113 (чат/окно/«Проверить пайплайн», **без публикации**, без изменений памяти, без `generate_image` без подтверждения) | §113 | UI+api | **P0** | **S5 ✅** (зависимость от S6 не подтверждена — см. примечание ниже) | **▶️ NEXT — Step 0 @Memory → Step 1 @PM; независима от S6** (§113: тестовый прогон **не публикует**; порядок `S5 → [S6 ∥ S9] → S7 → S8 → S10`) |
