...
- [ ] T-172-E: Деплой: SSH git pull 198.46.175.136:/var/www/admin_bot → systemctl restart; **prod .env НЕ менять** (`DANGER_WORDS` пустой → дефолты)
- [ ] T-172-F: Smoke: «укрыться в убежище» → ответ из danger-пула; «бункер» → нет ответа; «хлопок» → ответ; «летит» → нет ответа; «ракетная атака» → ответ
- [ ] T-172-G: Better Stack логи чистые; прод v2.21.0
...
**Статус: Epic 23 (точная настройка danger-словаря) — IMPLEMENTED ✅ + APPROVED (деплой pending). PM-решения D55–D58 зафиксированы и реализованы 2026-08-16. T-169..T-171 DONE + APPROVED (ревью 2 раунда, 672 теста PASS). T-172: доки (A/B/C) и коммит+пуш (D) DONE ✅; деплой (E/F/G) PENDING — @DevOps. Target: v2.21.0.**
**Date: 2026-08-16**
