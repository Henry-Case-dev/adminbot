...
### Системный промпт (R11 — захардкодить ДОСЛОВНО; v3 — Epic 28 (T-217), 2026-08-16; плейсхолдер `{max_symbols}` подставляется в рантайме)
...
> Эталон для теста `test_system_prompt_byte_for_byte` (tests/test_summary_prompts.py, хелпер `_backlog_system_prompt`): содержимое кодового блока выше — строки 1518–1540 (1-индекс), слайс `lines[1517:1540]` (v3 — Epic 28, 23 строки). Фигурные скобки: **3 пары** (2 уникальных плейсхолдера: `{max_symbols}` ×1, `{username}` ×2) — тест `test_max_symbols_is_the_only_placeholder` проверяет НАБОР плейсхолдеров (D72). Хвостовые пробелы строк не сохраняются (D74).
