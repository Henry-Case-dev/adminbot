...
5. **Волна 2:** **F4** `module-catalog-quickpanel-store-round1025` (каталог+store) — после hotfix6 (и F2/F3).
6. **Волна 3:** **F5** (workspace+промпты) → **F6** (Память+Аналитика) → **F7** (PERMsoc) ∥ **F11** (Статус-витрина, после F2+F6; **без §15** — сердцебиение в hotfix6).
7. **Волна 4:** **F9** (**только секреты §50 + визуальный SaveBar**; серверный persistence/409/state-machine уже закрыты в F0) — после F4/F5.
8. **Волна 5:** **F10** (приёмка §71–§79) — после всех; затем **СТОП-ГЕЙТ Эпика 1** (приёмка владельцем) → старт Эпика 2.

> **Порядок фич (единый, по UPD):** **F0 → F1 → F2 → F3 → hotfix6 (Волна 1.5) → F4 → F5 → F6 → F7 → F11 → F9 → F10**. Изменение относительно исходной декомпозиции: F0 добавлена в начало (Wave 0), F11 передвинута **перед** F9 (обе после F4/F5/F6/F7), F9 сужена; **hotfix6** вставлен между F3 и F4 по итогам живой приёмки владельца (блоки A–D), **§15 вынесено из F11 в hotfix6**.
