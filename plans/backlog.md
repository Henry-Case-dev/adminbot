...
| **S7** ▶️ NEXT | `summary-logging-runid` — `run_id` + события §108, детали логов §109, фильтр в log viewer §110 (без ключей) | §108–§110 | backend/лог+UI | **P0** | **S5 ✅** (ядро `run_id`/событий FILTER/L1/L2/FORMAT/обложки независимо от S6; события `PUBLISH_RICH_*`/`PUBLISH_TEXT_*` §108 — **GATED вместе с S6/D4**, нарезка — Step 2 @Architect) | **▶️ NEXT (eligible) — Step 1 после S9 (Шаг 8 @PM, T-3377, 24.09.2026; S8 зависит от S7). S6 остаётся ⛔ BLOCKED (GATED D4).** |
...
**Примечание (Шаг 8 @PM, T-3377, 24.09.2026) — S9 ✅ DONE/VERIFIED + ARCHIVED, S7 ▶️ NEXT:** S9 закрыта автономным контуром (итерация 2: rework T-3375 закрыл **B-R1026S9-2** High «краш вкладки на error-путях» и **B-R1026S9-1** Medium «§112 Процент отсева»; @Reviewer Approved C0/H0 + @Scanner C0/H0/блокирующих 0/L5/I2), архив — `plans/archive/summary-testing-ui-round1026/`; deploy — после T-3378; live — PENDING OWNER VERIFICATION. **Проверка зависимостей S7** (`summary-logging-runid`, §108–§110): ядро `run_id`/события FILTER/L1/L2/FORMAT/обложки **не требует S6** → может стартовать (после S9); события `PUBLISH_RICH_*`/`PUBLISH_TEXT_*` §108 относятся к публикации → **GATED вместе с S6 (D4)**, окончательная нарезка — Step 2 @Architect. **S8 зависит от S7** (и S6/F6). **S6 остаётся ⛔ BLOCKED (GATED D4).**
