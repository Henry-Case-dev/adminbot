...
| **S5** 🟦 IN PROGRESS | `summary-l2-writer-formatter` — L2 Писатель §97–§99 (отдельная модель/провайдер/системный промпт; качественная структурированная статья; без сленга/беглой речи/бессвязного потока/мемов; умеренная ирония; не выдумывать цитаты/не приписывать реплики; структура статьи §98, структурированный вывод §99, серверный форматтер → Rich Message; H1 §101, абзацы/акценты §102, экранирование; plain-fallback §105; fail-closed §106) — **🟦 IN PROGRESS (Step 1 @PM ✅ 23.09.2026; `tasks.md` — блоки 0/A–H, T-3308…T-3344)** (зависимость **S4 ✅ DONE/DEPLOYED 2.58.23**; сюда же — **врезка L1→пакет фактов→L2** (ровно 2 вызова, AMEND ADR-1022-4 effective S5), слот/UI модели L2 §82 и закрытие follow-up S3 `L-R1026S3-*`/`R-R1026S3-*` + S4 `L-R1026S4-1/-2/-3`/`I-R1026S4-1/-2`); **гейт S6/S10+D4: врезка за kill-switch `flags.summary_hybrid_l2_enabled` (default OFF) + байт-в-байт OFF-путь — S5 верифицируем без владельца**; live — без изменений | §97–§99, §101, §102 | backend/LLM+формат | **P0** | S4 ✅ | **T-3308…T-3344 — `plans/features/summary-l2-writer-formatter-round1026/tasks.md`** |
...
**Порядок Эпика 2:** `S1 → S2 → S3 → S4 → S5 → [S6 ∥ S9 (после S5)] → S7 → S8 → S10`. **▶️ S1 ✅ COMPLETED + MERGED + ARCHIVED (Шаг 8 @PM, 23.09.2026; Merge §71) → S2 `summary-context-restore-round1026` ✅ COMPLETED + MERGED (§73) + ARCHIVED (Шаг 8 @PM, T-3249, 23.09.2026; `tasks.md` T-3224…T-3252, 29; deploy — ✅ задеплоено 2.58.21 (Шаг 9 @DevOps); live — `[ ]` PENDING OWNER VERIFICATION) → ✅ **S3 `summary-l1-clusterizer` — COMPLETED (автономный контур) + ARCHIVED (Шаг 8 @PM, T-3279, 23.09.2026; архив — `plans/archive/summary-l1-clusterizer-round1026/`; deploy — после T-3280; live — PENDING OWNER VERIFICATION) → ✅ **S4 `summary-fact-package` — DONE / VERIFIED (автономный контур) + ARCHIVED (Шаг 8 @PM, T-3304, 23.09.2026; архив — `plans/archive/summary-fact-package-round1026/`; deploy — **VERIFIED 2.58.23** (`08219ab`/`7722d66`); врезка GATED (S5/S6); live — ⏳ PENDING OWNER VERIFICATION) → 🟦 **S5 `summary-l2-writer-formatter` — IN PROGRESS** (Step 1 @PM ✅ 23.09.2026, `tasks.md` T-3308…T-3344; зависимость S4 ✅; врезка L1→пакет→L2 и слот/UI L2 — S5; гейт S6/S10+D4 и live — без изменений).**
