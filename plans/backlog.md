...
**Канон R44-1 (VERBATIM, байт-в-байт — слайс-эталон для тестов; HTML-теги расставит @Architect в Section 53, суть текста менять нельзя; Epic 56 T-437: раздел 6 — пользовательская версия, `<code>`→`<blockquote>`):**
...
## Epic 56: /info — правка пользователя (раздел 6 «Прямое обращение к Богу Машине») + `<code>`→`<blockquote>` — 2026-08-23 🚧 IN PROGRESS (пользовательский запрос, Шаг 1 @PM, target v2.40.0)
...
**Статус: Epic 56 — Шаг 1 (@PM) 🚧 (2026-08-23): PLANNING — T-437…T-441 занесены в backlog.md + board.md, target v2.40.0 (D223 — minor: меняется DEFAULT_INFO_TEXT + тесты, прецедент D220; D224 — локальная копия = источник истины (сервер == HEAD `c7a6da5`); D225 — порядок деплоя инвертирован). Далее — @Architect (T-437: exa-ресёрч blockquote/parse_mode + вердикт + новый канон Section 53.3/63 + правки тестов). Без @Orchestrator.**
**Date: 2026-08-23**