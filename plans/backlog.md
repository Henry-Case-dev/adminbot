...
6. Что нового (свежие апдейты, о которых ты не спрашивал)
Ниже — только последние обновления: старые фичи живут в разделах 1–5.

v2.37.0 — Алан, Славик и немного дисциплины
- Алан: трейдинг-фразы снесены, вместо них новые темы для трепа — NixOS и Линукс, нейрокластер, планшет, продажа SSD на фоне кризиса цен, витамины Life Extension 100500%, 5-секундные прогулки с гантелями, уличный тренажер и реванш за колени. Написали, но не показываем: фича выключена, Алан пока молчит.
- common/work медиа выключено: бот больше не напоминает про работу. И правильно.
- Славик призван к порядку: одно сообщение = одно действие. Медиа теперь заменяет «пошел нахуй», на вход в чат — только «ДОЛБОЕБ ВЕРНУЛСЯ» и ничего лишнего.
- Нюансы: если Славик удалил свой репост мертвой страницы — бот удалит и свою копию, а не хватит прав — пристыдит его за это.
- direct_chat: бот откликается на бот, ботик, ботяра и ботохуета. Как вызвать: просто напиши одно из этих слов в чат — бот ответит реплаем на твое сообщение. Робот, работа и ботва не в счет: они его не разбудят.

v2.38.0 — Алан версия 2 и предохранитель
- Алан v2: пять тем по пять фраз плюс пул издевательских вопросов-подколов. Все еще выключен, но уже готов причинять добро.
- Починили зависание: если нейросеть-провайдер лежит, бот больше не висит по минуте и не ноет «я сломался». Теперь он отвечает человечной фразой из заготовленных и молчит 5 минут. Внутри это называется circuit breaker, но тебе знать не обязательно.

v2.38.1 — запасной аэродром
- У бота появился запасной канал к нейросети: если основной прилег — он незаметно пересаживается на прямой API DeepSeek с той же моделью. Ты ничего не заметишь, кроме того, что бот продолжает отвечать.
```
