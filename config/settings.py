...
class Settings:
    ...
    # ── Раунд 10.26 (A1, ADR-1026-14 D6/D7): env-only ClassVar kill-switch
    # Координатора инструментов (программный слой решения о действии ВНУТРИ
    # существующего Синтезатора direct-чата). default ON, Δ каталога = 0 (в
    # param_catalog НЕ входит; новых PG-ключей/Settings нет). Резолв per-call
    # (не кэшируется). OFF → точный legacy-путь direct-чата байт-в-байт
    ...
    #     `size`/`response_format`); OFF → прежнее тело (byte-identical,
    #     откат). ВНИМАНИЕ: область действия — ВСЯ генерация изображений
    #     (обложка саммари, tool `generate_image`, пре-гейт прямого чата), а
    #     не только обложка (L1 review iter1).
    ...
    #     (Эти два — действительно только про обложку саммари.)
    IMAGE_MODEL_COMPAT_ENABLED: ClassVar[bool] = _env_bool(
        "IMAGE_MODEL_COMPAT_ENABLED", True)
    ...
    # ── Эпик 2 / S5 round1026 (ADR-1026-7 D3/D5) — AMEND ADR-1026-12 D2 (S10):
    # режим генерации Саммари гибридным L2-путём (L1 → пакет → L2). env-only
    # ClassVar (Δ каталога = 0, F8 не переиздаётся). Default ON (§107/ADR-1026-12
    # D2): после деплоя новый пайплайн — основной во всех чатах БЕЗ ручного
    # действия; `_run` резолвит `hybrid=True` и уходит в `_run_hybrid_l2` (ровно
    # 2 LLM-вызова). Резолв-цепочка `per-chat → hot → env/default` сохранена
    # байт-в-байт и работает как АВАРИЙНЫЙ kill-switch: явный `false` (env +
    # рестарт либо non-catalog `flags.summary_hybrid_l2_enabled=false`)
    # возвращает прежний `_generate_two_call`. «Ручная активация» (ON-действие/
    # UI-селектор Legacy↔Hybrid) не требуется и запрещена (§107).
    SUMMARY_HYBRID_L2_ENABLED: ClassVar[bool] = _env_bool(
        "SUMMARY_HYBRID_L2_ENABLED", True)
    # ── Эпик 2 / S9 round1026 (ADR-1026-8 D1/D8): флаг тест-контура «Тестирование»
    # (§113) — env-only ClassVar (НЕ dataclass-поле → вне каталога, Δ каталога=0).
    # Default ON: это тумблер доступности UI/API (dry-run 0/0/0, только global
    # admin), а НЕ гейт публикации. OFF — hot-OFF `SUMMARY_TEST_UI_ENABLED=false`
    ...
    NOSTALGIA_YEAR_BACK_DAYS_WINDOW: int = _env_int(
        "NOSTALGIA_YEAR_BACK_DAYS_WINDOW", 10)
