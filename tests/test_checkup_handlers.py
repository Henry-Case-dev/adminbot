...
class TestCheckupTriggers:
    ...
    @pytest.mark.parametrize(
        "text",
        [
            # bare-исключение «чекап» (без префикса) — как раньше
            "чекап", "ЧеКаП!!", "сделай чекап", "ЧЕКАП",
            # префикс + канонический триггер (раунд 10.15, F6)
            "Бот, ты в порядке?", "Бот, чекни здоровье", "Бот, живой?",
            "бот, ТЫ В ПОРЯДКЕ", "Бот: чекни здоровье",
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # F6-U1 (раунд 10.15): legacy-фразы сняты; bare-триггеры без
            # префикса больше не срабатывают.
            "живой собака", "пульс бота", "как сервак",
            "ты в порядке?", "чекни здоровье", "Бот, живой",
        ],
    )
