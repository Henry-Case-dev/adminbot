...
class TestCatalogFrozen:
    ...
    def test_counters_unchanged(self):
        assert len(pc.REGISTRY) == BASELINE["counts"]["REGISTRY"] == 459
        assert len(pc.GROUPS) == BASELINE["counts"]["GROUPS"] == 98
        assert len(pc._TAB_BY_GROUP) == BASELINE["counts"]["_TAB_BY_GROUP"] == 96
