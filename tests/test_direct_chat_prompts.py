...
# Эталон слепка прод-канона раунда 9 (бывший CHAT_SYSTEM_PROMPT ДО раунда
# 10.20: полное содержимое до анти-галлюцинаций/persona fallback) — байт-в-байт.
_PREV_CHAT_R2020_SYSTEM_PROMPT_REFERENCE = """КАК ЧИТАТЬ КОНТЕКСТ:
...
# Эталон нового канона (раунд 10.20, БЛОК 5.2/5.3): канон раунда 9 +
# АНТИ-ГАЛЛЮЦИНАЦИИ (цифры из JSON, разделение собеседник/другие) +
# ЕСЛИ ПОЙМАЛИ НА НЕСТЫКОВКЕ (persona fallback, «изящный слив») — байт-в-байт.
...
class TestChatSystemPromptCanon:
    ...
    def test_prev_chat_r2020_snapshot_matches_round9_canon(self):
        """Раунд 10.20: PREV_CHAT_R2020 == прод-канон раунда 9 (байт-в-байт)
        ДО добавления анти-галлюцинаций/persona fallback."""
        assert (PREV_CHAT_R2020_SYSTEM_PROMPT
                == _PREV_CHAT_R2020_SYSTEM_PROMPT_REFERENCE)
        # слепок — реально ДО новых блоков
        assert "АНТИ-ГАЛЛЮЦИНАЦИИ" not in PREV_CHAT_R2020_SYSTEM_PROMPT
        assert "ЕСЛИ ПОЙМАЛИ НА НЕСТЫКОВКЕ" not in PREV_CHAT_R2020_SYSTEM_PROMPT
    ...
    def test_new_differs_from_prev_and_legacy(self):
        ...
        assert CHAT_SYSTEM_PROMPT != PREV_CHAT_R2020_SYSTEM_PROMPT
        assert PREV_CHAT_R2020_SYSTEM_PROMPT != PREV_R9_CHAT_SYSTEM_PROMPT
    ...
    def test_no_format_placeholders(self):
        ...
        assert re.findall(r"\{(\w+)\}", PREV_CHAT_R2020_SYSTEM_PROMPT) == []
    ...
    def test_no_trailing_newline(self):
        ...
        assert not PREV_CHAT_R2020_SYSTEM_PROMPT.endswith("\n")

    # ── Раунд 10.20 (БЛОК 5.2/5.3): анти-галлюцинации + persona fallback ──

    def test_anti_hallucination_guard_present(self):
        canon = CHAT_SYSTEM_PROMPT
        assert "АНТИ-ГАЛЛЮЦИНАЦИИ:" in canon
        assert ("Цифру называй ТОЛЬКО из результата инструмента и разделяй: "
                "сколько раз это писал собеседник, а сколько - остальные."
                in canon)
        assert ("КАТЕГОРИЧЕСКИ запрещено придумывать количество упоминаний, "
                "даты и события. Если точных данных нет - не выдумывай их."
                in canon)
        assert ("Прямо разделяй, что писал собеседник, а что писали другие: "
                "не приписывай чужие слова тому, кто спросил." in canon)

    def test_persona_fallback_block_present(self):
        canon = CHAT_SYSTEM_PROMPT
        assert "ЕСЛИ ПОЙМАЛИ НА НЕСТЫКОВКЕ:" in canon
        assert ("Если собеседник ловит тебя на нестыковке в фактах или цифрах, "
                "а в памяти нет точного подтверждения - не выдумывай новые "
                "цифры." in canon)
        assert "оставайся в рамках фактов." in canon
        # блок идёт ДО главного ограничения (после инструментов)
        assert (canon.index("АНТИ-ГАЛЛЮЦИНАЦИИ:")
                < canon.index("ГЛАВНОЕ ОГРАНИЧЕНИЕ"))
        assert (canon.index("ЕСЛИ ПОЙМАЛИ НА НЕСТЫКОВКЕ:")
                < canon.index("ГЛАВНОЕ ОГРАНИЧЕНИЕ"))

    def test_round1020_new_lines_have_no_fancy_quotes_or_emdash(self):
        """Дисциплина типографики: новые блоки раунда 10.20 — без ёлочек и
        без длинных тире (прецедент R5/R9)."""
        for marker in ("АНТИ-ГАЛЛЮЦИНАЦИИ:", "ЕСЛИ ПОЙМАЛИ НА НЕСТЫКОВКЕ:"):
            segment = (CHAT_SYSTEM_PROMPT.split(marker)[1]
