"""SelfdevWordFilter — ловит «саморазвитие»-семью (Epic 30, R30-1, D85/D92).

Паттерн — клон DangerWordFilter (filters/danger_word.py):
  - кириллические word-boundary: (?<![а-яё])...(?![а-яё])
  - re.IGNORECASE
  - ветка фраз (SELFDEV_PHRASES) ПРОВЕРЯЕТСЯ ПЕРВОЙ (специфичнее)
  - text И caption

D92 (гейт репостов): `forward_origin is not None` → False — на репосты
бот про саморазвитие не проповедует, это личное.
