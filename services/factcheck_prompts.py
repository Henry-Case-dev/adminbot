...
# 10.23 (F2, ADR-1023-2 §3.3): жёсткое правило обязательного веб-поиска для
# тейков о реальном мире/новостях/датах/политике/общеизвестных фактах.
WEB_SEARCH_INSTRUCTION_BLOCK = (
    "ВЕБ-ПОИСК (СТРОГО):\n"
    "Если проверяемый тейк касается реального мира, новостей, дат, политики "
    "или общеизвестных фактов — ты ОБЯЗАН вызвать веб-поиск, а не опираться "
    "только на память чата. Без подтверждения из сети такие утверждения не "
    "считай проверенными."
)

# Слепок канона Аналитика после F1 (правило маркировки, БЕЗ правила поиска) —
# непосредственный прежний канон для ступени F2 (ADR-1013-3).
...
# 10.23 (F2, ADR-1023-2): + правило обязательного веб-поиска.
