...
GROUPS: tuple[GroupSpec, ...] = (
    ...
    GroupSpec("limits_worker", "limits", "Фоновые воркеры",
              ...
              "размазка тиков.", 28),
    # limits (29-30; раунд 10.19, F3/ADR-1019-3 D2): раздел «Бюджеты» —
    # бюджеты ключа чата и лимит контекста вынесены из «Прямого чата»,
    # чтобы их было видно (`limits_chat` тянул десятки контекстных ключей).
    GroupSpec("limits_chat_key", "limits", "Бюджет ключа чата",
              "Суточный лимит общего ключа для чата (интеллект): вызовы и "
              "слова (кусочки текста). 0 — чату запрещено тратить общий "
              "ключ, −1 — безлимит.",
              29),
    GroupSpec("limits_chat_context", "limits", "Лимит контекста (чат)",
              "Сколько памяти бот видит в ответе (единицы — слова, кусочки "
              "текста: фон разговора, ветка, общий бюджет). 0 — не задано "
              "(глобальный предохранитель), −1 — безлимит.", 30),
    # limits (31; раунд 10.24, F7/ADR-1024-3 D1): потолок динамических клише
    ...
    GroupSpec("flags_module_images", "flags", "Модуль: Генерация изображений",
              "Главный тумблер модуля генерации изображений (вкладка "
              "«Генерация изображений» раздела «Модули»).", 20),
    # flags (21; раунд 10.24, F21/ADR-1024-22 D1): master-рубильник бюджетов.
...
_KEYS: list[tuple] = [
    ...
    # ── Раунд 10.13 (F4, ADR-1013-1 §2.1): ключи выделенных LLM Интеллекта.
    # Пусто → рантайм-фоллбэк на keys.llm_api_key (F3, generate_worker).
    # R17: маска {configured,last4}, сырой секрет наружу не уходит. ──
    ("INTEL_HISTORY_API_KEY", "Ключ нейросети для Вех и Лора", "str", True,
     "keys_llm",
     "Ключ отдельной нейросети для исторической памяти (Вехи/Лор). Пусто — используется ключ основной нейросети."),
    ("INTEL_BG_API_KEY", "Ключ нейросети для фоновых проверок", "str", True,
     "keys_llm",
     "Ключ отдельной нейросети для фоновых проверок (оценка важности). Пусто — используется ключ основной нейросети."),
    # ── Раунд 10.14 (F8, UPD п.3, ADR-1013-1 §2.1): ключ LLM саморефлексии
    # (Экстрактор сути). Пусто → рантайм-фоллбэк на keys.llm_api_key.
    # R17: маска {configured,last4}, сырой секрет наружу не уходит. ──
...
_MODELS: list[tuple] = [
    ...
    # Два независимых подключения — Вехи/Лор (history) и фоновые проверки
    # (bg). Пустые поля → основная модель (models.llm_*). Группа —
    # models_extra_providers (вкладка «LLM Провайдеры»). ──
    ("INTEL_HISTORY_BASE_URL", "Адрес нейросети для Вех и Лора", "str",
     "models_extra_providers",
     "Адрес сервера отдельной нейросети для исторической памяти (Вехи/Лор). Пусто — используется основная нейросеть."),
    ("INTEL_HISTORY_MODEL_NAME", "Модель для Вех и Лора", "str",
     "models_extra_providers",
     "Название отдельной модели для исторической памяти. Пусто — берётся основная модель."),
    ("INTEL_HISTORY_DISPLAY_NAME", "Название модели для Вех и Лора", "str",
     "models_extra_providers",
     "Как называть эту нейросеть в админке. Пусто — покажется адрес сервера."),
    ("INTEL_BG_BASE_URL", "Адрес нейросети для фоновых проверок", "str",
     "models_extra_providers",
     "Адрес сервера отдельной нейросети для фоновых проверок (оценка важности). Пусто — используется основная нейросеть."),
    ("INTEL_BG_MODEL_NAME", "Модель для фоновых проверок", "str",
     "models_extra_providers",
     "Название отдельной модели для фоновых проверок. Пусто — берётся основная модель."),
    ("INTEL_BG_DISPLAY_NAME", "Название модели для фоновых проверок", "str",
     "models_extra_providers",
     "Как называть нейросеть фоновых проверок в админке. Пусто — покажется адрес сервера."),
    # ── Раунд 10.14 (F8, UPD п.3, ADR-1013-1 §2.1): третье выделенное
    # подключение — LLM для саморефлексии (Экстрактор сути). Пустые поля →
    # основная модель (models.llm_*). Группа — models_extra_providers. ──
    ("INTEL_REFLECTION_BASE_URL", "Адрес нейросети саморефлексии", "str",
     "models_extra_providers",
     "Адрес сервера нейросети для экстрактора сути (саморефлексия). Пусто — используется основная нейросеть."),
    ("INTEL_REFLECTION_MODEL_NAME", "Модель саморефлексии", "str",
     "models_extra_providers",
...
_FLAGS: list[tuple] = [
    ...
    # ── Раунд 10.14 (F1 anti-echo-self-reply, ADR-1014-2 D8): самоосознание
    # бота — ON по умолчанию (UPD п.2). OFF → ровно прежнее поведение.
    ("BOT_SELF_AWARENESS_ENABLED", "Самоосознание бота", "flags_memory",
     "Бот отдельно и критически помнит суть своих ответов. Выключено — бот не запоминает собственные слова и не предупреждается об эхо."),
    # ── Раунд 10.14 (F2 persona-storage-core, ADR-1014-1): личность бота.
    # ON по умолчанию (UPD п.2). OFF → промпт/триггер/воркер без изменений.
    ("PERSONA_ENABLED", "Личность бота (Persona)", "flags_memory",
     "Имя, биография, характер и динамические черты бота. Выключено — поведение прежнее, персональные настройки не влияют."),
    ...
    ("BELIEF_DECAY_ENABLED", "Охлаждение убеждений (сон)", "flags_memory",
     ...
     "в архив, откуда их можно «воскресить». Выключено (дефолт) — убеждения "
     "остаются вечными, поведение без изменений."),
    # ── F3 (cognition-deep-sleep, spec §7/§8): «глубокий сон» ───────────────
    ("DEEP_SLEEP_ENABLED", "Глубокий сон: мета-синтез (парадигмы)",
     "flags_memory",
     "Рубильник ежедневного «глубокого сна»: после обычного сна воркер ищет "
     "связи между свежими событиями и старой историей и сохраняет выводы "
     "(парадигмы) как фоновый слой. Выключено (дефолт) — 0 влияния."),
    # ── F8 (cognition-irony-dossier-round1013, spec §8): иронический фильтр ─
    ("IRONY_FILTER_ENABLED", "Ирония: фильтр досье (мемы vs факты)",
     "flags_relations",
     "Воркер досье отделяет шутки и абсурдные титулы от реальных фактов "
     "и подаёт их в контекст двумя блоками: [Факты] и "
    ...
    ("IMAGE_GENERATION_MODULE_ENABLED", "Модуль генерации изображений включён",
     ...
     "работают как раньше."),
    # ── Раунд 10.24 (F21, ADR-1024-22 §D1/D2): master-рубильник бюджетов ──
    ("BUDGETS_ENABLED", "Главный тумблер бюджетов", "flags_module_budgets",
     "Один рубильник на все бюджеты — глобально или для выбранного чата. "
     "При снятии лимиты не ограничивают и контекст не режется, но "
     "статистика расходов продолжает вестись. Дефолт — ВКЛ."),
]
...
_LIMITS: list[tuple] = [
    ...
    # F3 (10.19, ADR-1019-8 D2/D4): срок хранения импортированной истории.
    # Sentinel семейства retention (0 = вечно), НЕ бюджета.
    ("IMPORT_HISTORY_RETENTION_DAYS", "Хранение импорта (дней)", "int", "limits_memory",
     "Сколько дней хранить импортированную историю переписки (сырьё для "
     "поиска по памяти). 0 — вечно: удаление импорта КАТЕГОРИЧЕСКИ запрещено "
     "(значение `0` из данных сида целевого чата). Больше 0 — хранить N дней; перед удалением — "
     "архив и бэкап. Отрицательное значение не имеет смысла → глобальный "
     "дефолт (180)."),
    ...
    ("MEMORY_BACKUP_KEEP", "Ротация бэкапов (файлов)", "int", "limits_memory",
     "НЕ действует: по политике владельца (UPD3 №5) хранится ровно 1 бэкап БД. Значение игнорируется (см. services/disk_retention.py)."),
    ...
    # F3 (10.19, ADR-1019-3 D2): группа limits_chat → limits_chat_context.
    # F4 (10.19, ADR-1019-4 D5): sentinel: -1 = безлимит (потолок
    # безопасности), 0 = не задано → глобальный дефолт. Единицы в UI —
    # «слов (кусочков текста)» (round-10.9 jargon-гейт запрещает слово
    # «токен» в пользовательских текстах; технически это токены).
    # Простыми словами (почему было 869): фон собирался с code-дефолтом 1000,
    # из которых «запас безопасности» ×1.15 оставлял 869 — это была не
    # настройка владельца, а жёсткий дефолт в коде; теперь дефолт 5000.
    ("CHAT_GLOBAL_CONTEXT_MAX_TOKENS", "Контекст: фон разговора, слов (кусочков текста)", "int", "limits_chat_context",
     "Сколько памяти бот видит в одном ответе (фон: свежий хвост + "
     "конспект). 0 — не задано (глобальный дефолт); −1 — безлимит "
     "(ограничен только потолком безопасности). Больше — помнит шире, но "
     "дороже."),
    ("CHAT_THREAD_MAX_TOKENS", "Контекст: ветка ответов, слов (кусочков текста)", "int", "limits_chat_context",
     "Размер ветки (reply-цепочки), которую бот видит в ответе. 0 — не задано "
     "(глобальный дефолт); −1 — безлимит (в пределах потолка безопасности). "
     "Больше — точнее, но дороже."),
    ...
    ("GRAPH_FACT_WEIGHT_BOT", "Стартовый вес слов бота о себе", "float", "limits_graph",
     "Сколько весит собственное высказывание бота. Ниже пользовательского — бот легко передумает, если человек поправит."),
    ...
    # F3 (10.19, ADR-1019-3 D2): группа limits_chat_budgets → limits_chat_context
    # (в limits_chat_budgets остаются только доли CHAT_BUDGET_*_RATIO).
    ("CHAT_CONTEXT_BUDGET_TOKENS", "Контекст: общий бюджет, слов (кусочков текста)", "int", "limits_chat_context",
     "Суммарный потолок текста на ответ в прямом чате (все блоки вместе). "
     "0 — не задано (глобальный дефолт); −1 — безлимит (агрегатное усечение "
     "не применяется, потолки отдельных блоков действуют). Должен быть не "
     "меньше суммы потолков блоков — иначе они режутся дважды. Больше — "
     "полнее, но дороже."),
    ...
    # ── F1 (cognition-4d-memory, spec §3.2/§7): порог устаревания памяти ────
    ("RAG_STALE_AFTER_DAYS", "Память: срок жизни факта, дней — пометка устаревания", "int", "limits_memory",
     "Факт памяти старше порога получает пометку «возможно устарело» "
     "(180 — примерно 6 месяцев). Ровно порог — ещё не устарел."),
    # ── F2 (cognition-belief-decay, spec §7): охлаждение/воскрешение ────────
    ("BELIEF_INACTIVITY_DAYS", "Убеждения: срок без подкрепления, дней", "int", "limits_memory",
     "Сколько дней убеждение может жить без новых подтверждающих фактов, "
     "прежде чем начнёт терять вес (180 — примерно 6 месяцев)."),
    ("BELIEF_DECAY_PER_MONTH", "Убеждения: потеря веса за месяц", "float", "limits_memory",
     "На сколько падает вес убеждения за каждый просроченный месяц "
     "без подкрепления (0.1)."),
    ("BELIEF_ARCHIVE_THRESHOLD", "Убеждения: порог архива (вес)", "float", "limits_memory",
     "Вес ниже этого порога отправляет убеждение в архив — обычный ответ "
     "его больше не видит (0.3)."),
    ("BELIEF_RESONANCE_PENALTY", "Убеждения: пенальти архива в поиске", "float", "limits_memory",
     "Насколько понижается сходство архивного убеждения при векторном "
     "поиске, чтобы активные шли впереди (0.3)."),
    ("BELIEF_RESONANCE_THRESHOLD", "Убеждения: порог воскрешения (сходство)", "float", "limits_memory",
     "Если косинусное сходство диалога с архивным убеждением не ниже порога, "
     "убеждение возвращается в активные (0.78)."),
    # ── F3 (cognition-deep-sleep, spec §7): лимиты стоимости RAG ────────────
    ("DEEP_SLEEP_TOP_K", "Глубокий сон: кандидатов из памяти", "int", "limits_memory",
     "Сколько найденных памятью фактов глубокий сон берёт на разбор за раз "
     "(20; ограничивает стоимость поиска по всей базе)."),
    ("DEEP_SLEEP_MAX_PARADIGMS", "Глубокий сон: парадигм за прогон", "int", "limits_memory",
     "Потолок новых мета-фактов (парадигм) за один прогон глубокого сна (3)."),
    ("DEEP_SLEEP_TOKENS_PER_DAY", "Глубокий сон: слов в сутки (денежный)", "int", "limits_memory",
     "Суточный запас текста на глубокий сон (40000). Больше — глубже "
     "обобщения, больше расход."),
    # ── Раунд 10 (multi-chat-rbac-byok, F-7 §5.2): бюджет глобального ключа ──
    # F3 (10.19, ADR-1019-3 D2): группа limits_chat → limits_chat_key
    # (раздел «Бюджеты»); описания — человекочитаемые (UPD3 п.2).
    ("CHAT_GLOBAL_KEY_BUDGET_TOKENS", "Лимит слов общего ключа в сутки (чат)",
     "int", "limits_chat_key",
     "Сколько слов (кусочков текста) бот может потратить на ответы этому "
     "чату за сутки, пока у чата нет своего ключа. 0 — общий ключ чату "
     "запрещён; −1 — безлимит (не считать). Исчерпан — бот ответит заглушкой."),
    ("CHAT_GLOBAL_KEY_BUDGET_REQUESTS", "Лимит вызовов общего ключа в сутки (чат)",
     "int", "limits_chat_key",
     "Сколько ответов (вызовов нейросети) на общем ключе доступно этому чату "
     "за сутки. 0 — общий ключ чату запрещён; −1 — безлимит. По умолчанию 100; "
     "исчерпан — бот честно скажет, что лимит.\n"
     "Своими ключами чат не ограничен — лимит только на общий ключ."),
    ...
    # ── Раунд 10.24 (F7, ADR-1024-3 D1): регулируемый лимит динамических
    # клише. Хардкод 20 снят; дефолт 200, защитный потолок 1000 (clamp в
    # `anticliche_cache.max_patterns()`).
    ("ANTICLICHE_MAX_PATTERNS", "Анти-клише: лимит паттернов", "int",
     "limits_anticliche",
...
_MEMORY: list[tuple] = [
    ...
    ("DREAM_TOKENS_PER_DAY", "Сон: слов в сутки (денежный)", "int",
     ...
     "advanced"),
    # ── F3 (cognition-deep-sleep, spec §7): расписание «глубокого сна» ──────
    # dotted-ключи memory.deep_sleep_* (тот же прецедент memory.dream_*).
    ("DEEP_SLEEP_TRIGGER", "Глубокий сон: триггер запуска", "str",
     "memory_dream",
     "Когда запускать глубокий сон: «после сна» (сразу после успешного "
     "обычного сна) или «своё время» (в указанный час local).",
     "basic"),

    ("DEEP_SLEEP_HOUR", "Глубокий сон: час запуска (local)", "int",
     "memory_dream",
     "Час запуска, если выбран триггер «своё время» (7). Часовой пояс — "
     "общий для фоновых воркеров.",
     "advanced"),
...
_SELECT_WIDGET_PRESETS: dict[str, dict] = {
    ...
    # F3 (cognition-deep-sleep, spec §7): триггер глубокого сна — select.
    "DEEP_SLEEP_TRIGGER": {
        "widget": "select",
...
# F3 (cognition-deep-sleep, spec §7): PG-ключ отличается от settings-имени —
# явный pg_id (единственная запись; дефолт `limits.deep_sleep_max_paradigms`
# из имени поля не совпадает с каноническим `..._per_run`).
_PG_ID_OVERRIDES: dict[str, str] = {
    "DEEP_SLEEP_MAX_PARADIGMS": "limits.deep_sleep_max_paradigms_per_run",
}
for _name, _pg_id in _PG_ID_OVERRIDES.items():
    _spec = REGISTRY.get(_name)
    if _spec is not None:
...
TAB_RULES: tuple[tuple[str, tuple[tuple[str, object], ...]], ...] = (
    ...
    # F3 (10.19, ADR-1019-3 D1): «Бюджеты» — оба контура + лимит контекста.
    # F21 (10.24, ADR-1024-22 D1): +master-группа флагов на ТОЙ ЖЕ вкладке
    # (новых вкладок/карточек нет — tma-menu-freeze не затрагивается).
    (TAB_MOD_BUDGETS, (
    ...
    (TAB_PROMPTS, (
        ...
        # Раунд 10.24 (F7/ADR-1024-3 D1): лимит анти-клише — рядом с карточкой
        # монитора (data-block="anticliche-monitor") на вкладке «Промпты».
        (CATEGORY_LIMITS, frozenset({"limits_anticliche"})),
    )),
