...
_LIMITS: list[tuple] = [
    ...
    ("SMARTMODULE_CONCURRENCY_PER_CHAT", "Параллельных LLM-запросов на чат (smart module)", "int", "limits_chat",
    ...
    ("GRAPH_PURGE_PROTECT_WEIGHT", "Вес-гейт защиты от purge", "float", "limits_graph",
     "Факты с весом не ниже порога не удаляются TTL-чисткой. Больше — агрессивнее чистка."),
    ("GRAPH_PURGE_PROTECT_DAYS", "Свежесть подтверждения для защиты от purge, дней", "int", "limits_graph",
     "Факт, подтверждённый дедупом недавнее N дней назад, не удаляется TTL-чисткой."),
